...
public class ParkObjValue extends ObjValue {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5176256196698682646L;
	...
	public ObjValue getNode(String domain, String node) {
		...
		if (domain != null) {
			if (node != null) {
				...
				Object updatetime = this
						.getObj(ParkMeta.getYGSJ(domainnodekey));
	...
	public ObjValue getParkInfo() {
		 ...
		 * 
		 * for(Iterator iter=obj.keySet().iterator();iter.hasNext();){ String
		 * curkey = (String)iter.next();
		 * if(curkey.indexOf(ParkMeta.getYSX())!=-1
		 * &&obj.get(curkey).equals(ParkMeta.getSXXT()))
		 * hbkeylist.add(curkey.substring
		 * (0,curkey.indexOf(ParkMeta.getYSX())));//creattime }
		 * 
		 * for(String hbkey:hbkeylist) obj.removeNodeWidely(hbkey);
		 */
	...
	public List<String[]> getParkInfoExp(long exp) {
		...
		for (Iterator iter = this.keySet().iterator(); iter.hasNext();) {
			...
			if (curkey.indexOf(ParkMeta.getYCSJ()) != -1) {
				String domainnodekey = curkey.substring(0,
						curkey.indexOf(ParkMeta.getYCSJ()));
				String[] keyarr = getDomainNode(domainnodekey);
				if (keyarr != null && keyarr.length == 2) {
					String propvalue = this.getString(ParkMeta
							.getYSX(domainnodekey));
					// System.out.println("propvalue:"+propvalue);
					if (propvalue == null
							|| !propvalue.equals(ParkMeta.getSXXT()))
						if (System.currentTimeMillis()
								- (Long) this.get(curkey) >= exp)
	...
	public ObjValue removeNode(String domain, String node) {
		...
		if (domain != null) {
			if (node != null) {
				...
				Object updatetime = this
						.remove(ParkMeta.getYGSJ(domainnodekey));
	...
	public static void main(String[] args) {
		...
		ParkObjValue.checkGrammar("d", "a.b");
