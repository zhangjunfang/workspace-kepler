...
public class CoolHashMap extends LinkedHashMap implements Map, CoolHashBase {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -375529270497178984L;
	...
	private class DataMapEntry implements Map.Entry, Comparable<Map.Entry> {
		...
		public boolean equals(Object o) {
			if (o instanceof Map.Entry) {
				...
				if ((entry.getKey() == null ? k == null : entry.getKey()
						.equals(k))
						&& (entry.getValue() == null ? v == null : entry
								.getValue().equals(v)))
					return true;
			}
			return false;
	...
	public final class CoolKeySet<E> extends LinkedHashSet<E> implements Set<E> {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 7981655138122162802L;
	...
	public <T> Collection<T> values(Class<T> classType) {
		...
		for (Iterator<byte[]> i = getValues().iterator(); i.hasNext();)
			vls.add(Target.getTargetObject(i.next(), classType));
		return vls;
	}
	...
	public <T> T[] getValues(Class<T> classType) {
		...
		int j = 0;
		for (Iterator<byte[]> i = getValues().iterator(); i.hasNext();) {
			vls[j++] = Target.getTargetObject(i.next(), classType);
		}
		return vls;
	}
	...
	public boolean containsValue(Object value) {
		return super.containsValue(Target.getTargetBytes(value));
	}
		...
		if (o instanceof CoolHashMap) {
			...
			if (size() == odm.size()) {
				...
				for (int i = 0; i < meset.length; i++) {
					...
					if (!(me.getKey().equals(odmme.getKey()) && Arrays.equals(
							(byte[]) me.getValue(), (byte[]) odmme.getValue())))
						return false;
				}
				return true;
			}
	...
	public Object get(Object key) {
		...
		return vbs != null ? Target.getTargetObject(vbs) : vbs;
	}
	...
	public <T> T get(String key, Class<T> classType) {
		return Target.getTargetObject((byte[]) getValue(key), classType);
	}
	...
	private Object putValue(Object key, Object value, boolean t) {
		if (key != null && value != null) {
			value = t ? Target.getTargetBytes(value) : value;
			value = (!t || chex.checkKeyValue(key, value)) ? super.put(key,
					value) : null;
		} else
			LogUtil.fail("[NullPointerException]", "[put]",
					"put failed, key or value cant be null!");
		return value;
	}
