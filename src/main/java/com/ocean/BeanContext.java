...
public class BeanContext extends ServiceContext {
	...
	// get all config msg
	public static ParkLocal getPark(String host, int port, String sn,
			String[][] servers) {
		return DelegateConsole.bind(ParkLocal.class, new ParkProxy(host, port,
				servers, sn));
	}
	...
	public static void startPark(String host, int port, String sn,
			String[][] servers) {
		try {
			// DelegateConsole.bind(Park.class, new ParkService(host, port,
			// servers, sn));
			startService(host, port, sn, new ParkService(host, port, servers,
					sn));
			// BeanService.putBean(host, true, port, sn, new ParkService(host,
			// port, servers, sn));//input serverconfiglist string[][]
			if (Boolean.valueOf(ConfigContext.getConfig("PARK", "STARTWEBAPP",
					null, "false")))
				startInetServer();
	...
	static void startWorker(String host, int port, String sn, MigrantWorker mwk) {
		/*
		 * try{ //System.setProperty("responseTimeout", 1000+"");
		 * startService(host, port, sn, new WorkerService(mwk));
		 * }catch(RemoteException e){ LogUtil.info("[BeanContext]",
		 * "[startWorker]", e); }
		 */
	...
	static void startWorker(String host, int port, String sn,
			MigrantWorker mwk, boolean issup) {
		try {
			ParkActive pa = DelegateConsole.bind(Worker.class,
					new WorkerService(mwk));// Worker.class
			if (issup)
	...
	static void startFttpWorker(String host, int port, String sn,
			MigrantWorker mwk) {
		try {
			ParkActive pa = (ParkActive) DelegateConsole.bind(new Class[] {
					Worker.class, FttpWorker.class },
	...
	public static void startFttpServer(String host, int port) {
		new FttpMigrantWorker().waitWorking(host, port,
				FttpMigrantWorker.FTTPSN);
	}
	...
	static WorkerLocal getWorkerLocal(String host, int port, String sn) {
		return (WorkerLocal) DelegateConsole.bind(new Class[] {
				WorkerLocal.class, CtorLocal.class }, new WorkerServiceProxy(
				host, port, sn));
	}

	static WorkerLocal getWorkerLocal(String domainnodekey) {
		return DelegateConsole.bind(WorkerLocal.class, new WorkerParkProxy(
				domainnodekey));
	}

	static WorkerLocal getWorkerLocal() {
		return (WorkerLocal) DelegateConsole.bind(
				new Class[] { WorkerLocal.class }, new WorkerLocalProxy());
	}

	static WorkerLocal getFttpLocal(String host, int port, String sn) {
		return (WorkerLocal) DelegateConsole.bind(new Class[] {
				WorkerLocal.class, CtorLocal.class, FttpLocal.class },
				new FttpWorkerProxy(host, port, sn));
	}

	static Workman getWorkman(String host, int port, String sn) {
		return DelegateConsole.bind(Workman.class, new WorkerServiceProxy(host,
				port, sn));
	}
	...
	public static void startCacheFacade(String host, int port) {
		try {
			startService(host, port, ConfigContext.getCacheFacadeService(),
					DelegateConsole.bind(Cache.class,
							new CacheFacade(ConfigContext.getCacheService(),
	...
	public static Cache getCacheFacade(String host, int port) {
		return getService(Cache.class, host, port,
				ConfigContext.getCacheFacadeService());
	}
	...
	public static CacheLocal getCache(String host, int port) {
		return DelegateConsole.bind(CacheLocal.class,
				new CacheProxy(host, port));
	...
	public static StartResult<Integer> tryStart(FileAdapter fa,
			String... params) {
	...
	public static StartResult<Integer> tryStart(Map env, FileAdapter fa,
			String... params) {
		ProcessBuilder pb = new ProcessBuilder(params);
		pb.redirectErrorStream(true);
		if (env != null)
	...
	public static void killOnExit(final StartResult<Integer>[] srarr,
			final String info) {
	...
	public static boolean getLinux() {
		return System.getProperty("os.name").contains("Linux");
	}

	public static void startCoolHashServer(String parkhost, int parkport,
			int worknum) {
		startCoolHashServer("com.ocean.CoolHashWorker", parkhost, parkport,
				worknum, "fourinone.jar" + File.pathSeparator, "log", null);
	}

	public static void startCoolHashServer(String parkhost, int parkport,
			int worknum, String jarclasspath, String logdir, String langCode) {
		startCoolHashServer("com.ocean.CoolHashWorker", parkhost, parkport,
				worknum, jarclasspath, logdir, langCode);
	}

	public static void startCoolHashServer(String parkhost, int parkport,
			int worknum, String langCode) {
		startCoolHashServer("com.ocean.CoolHashWorker", parkhost, parkport,
				worknum, "fourinone.jar" + File.pathSeparator, "log", langCode);
	}

	static void startCoolHashServerLocal(String parkhost, int parkport,
			int worknum) {
		startCoolHashServer("com.ocean.DumpWorker", parkhost, parkport,
				worknum, "fourinone.jar" + File.pathSeparator, "log", null);
	}

	static void startCoolHashServerLocal(String parkhost, int parkport,
			int worknum, String jarclasspath, String logdir) {
		startCoolHashServer("com.ocean.DumpWorker", parkhost, parkport,
				worknum, jarclasspath, logdir, null);
	}

	private static void startCoolHashServer(String service, String parkhost,
			int parkport, int worknum, String jarclasspath, String logdir,
			String langCode) {
		try {
			final StartResult[] starts = new StartResult[worknum + 1];
			System.out.println("Start ParkService and waiting 4 seconds...");
			starts[0] = BeanContext.tryStart("java", "-cp", jarclasspath,
					"com.ocean.BeanContext", parkhost, parkport + "");
			starts[0].print(logdir + "/park.log");
			Thread.sleep(4000);
			System.out
					.println("Start CoolHashService and waiting 5 seconds...");
			int port = parkport + 5;
			for (int i = 1; i < starts.length; i++) {
				if (langCode != null)
					starts[i] = BeanContext.tryStart("java", "-Dfile.encoding="
							+ langCode, "-cp", jarclasspath, service, parkhost,
							parkport + "", parkhost, (port++) + "");
				else
					starts[i] = BeanContext.tryStart("java", "-cp",
							jarclasspath, service, parkhost, parkport + "",
							parkhost, (port++) + "");
