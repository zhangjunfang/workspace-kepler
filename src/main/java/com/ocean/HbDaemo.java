...
public class HbDaemo {
	...
	final static long pt = Long.parseLong(ConfigContext.getConfig("PARK",
			"HEARTBEAT", null, "500"));
	final static long dt = Long.parseLong(ConfigContext.getConfig("PARK",
			"MAXDELAY", null, "0"));
	...
	public static void runPutTask(Park pk, ParkLeader pl, String domain,
			String node, String sessionid) {
		if (putTask == null) {
	...
	public static void runClearTask(ParkService ps) {
		if (clrTask == null) {
			Double cpd = new Double(ConfigContext.getConfig("PARK",
					"CLEARPERIOD", null, "12"));
			if (cpd > 0) {
				LogUtil.fine("", "", "Run ClearTask");
				Double exp = new Double(ConfigContext.getConfig("PARK",
						"EXPIRATION", null, "24"));
...
class PutHbTask extends TimerTask {
	...
	PutHbTask(Park pk, ParkLeader pl, String domain, String node,
			String sessionid) {
		this.pk = pk;
		this.pl = pl;
		this.domain = domain;
		...
		for (String domainnodekey : hbarr) {
			...
			if (t > HbDaemo.gt) {
				...
				if (HbDaemo.dt > 0 && t / HbDaemo.gt < 2)
					LogUtil.warn("[Park]", "[Slow]", domainnodekey
				...
				if (t > HbDaemo.gt + HbDaemo.dt) {
					if (HbDaemo.dt > 0)
						LogUtil.warn(
								...
								"[Dead]",
								domainnodekey
										+ " has exceeded max delaytime and is regarded as dead by park.");
...
class ClearTask extends TimerTask {
	...
	public void run() {
		...
		if (expkeys.size() > 0) {
			LogUtil.fine("", "",
					"Get some expiration data and save for backup...");
			// intofile and date for name
		}

		for (String[] keyarr : expkeys) {
			LogUtil.fine("[Clear]", "[Expiration]",
					ParkObjValue.getDomainnodekey(keyarr[0], keyarr[1]));
			ps.delete(keyarr[0], keyarr[1]);
		}
	}
}