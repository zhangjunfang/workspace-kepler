...
import com.ocean.CoolKeyResult;
...
public class RunClient {
	...
	public static void findDemo(String[] args) {
		...
		try {
			...
			CoolHashResult hr = chc.find("user.*.name",
					ValueFilter.contains("8888"), true);// 查询名字包含88888的所有用户（like%88888%）
			...
			start = System.currentTimeMillis();
			CoolKeyResult kr = chc.findKey("user.*.age", ValueFilter.less(20l),
					true);// 查询年龄小于20岁的用户key
	...
	public static void pointDemo(String[] args) {
		...
		try {
			...
			System.out.println(chc.getPoint("order.0.buyer", ".order.0",
					".price"));// 指向user.0.order.0再指向order.0.price
	...
	public static void findPointDemo(String[] args) {
		...
		try {
			...
			CoolKeyResult kr = chc.findKey("user.*.order.*",
					ValueFilter.less(10l), true, ".price");
			CoolKeySet ks = kr.nextBatchKey(50);
			System.out.println("find time taken in MS--"
					+ (System.currentTimeMillis() - start));
			System.out.println("findKey:" + ks);
			...
			CoolHashResult hr = chc.find("order.*.buyer",
					ValueFilter.endsWith("8888name"), true, ".name");
			CoolHashMap chmb = hr.nextBatch(50);
			System.out.println("find time taken in MS--"
					+ (System.currentTimeMillis() - start));
			System.out.println("find:" + chmb);
	...
	public static void andOrDemo(String[] args) {
		...
		try {
			...
			chc.put(hm);// 写入100条测试数据

			// 查询名字以88name结束的person
			CoolKeyResult kn = chc.findKey("person.*",
					ValueFilter.endsWith("88name"), true);
			CoolKeySet ksn = kn.nextBatchKey(50);
			System.out.println("findKey:" + ksn);
			System.out.println("findKey total:" + ksn.size());

			// 查询年龄大于60的person
			CoolKeyResult ka = chc.findKey("person.*", ValueFilter.greater(60),
					true);
			...
			System.out.println("and:"
					+ ksn.getSuperKeys(1).and(ksa.getSuperKeys(1)));// 取并集
			System.out.println("or:"
					+ ksn.getSuperKeys(1).or(ksa.getSuperKeys(1)));// 取或集
			System.out.println("except:"
					+ ksa.getSuperKeys(1).except(ksn.getSuperKeys(1)));// 取差集
			System.out.println("time taken in MS--"
					+ (System.currentTimeMillis() - start));
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public static void sortDemo(String[] args) {
		...
		try {
			...
			System.out.println("sort time taken in MS--"
					+ (System.currentTimeMillis() - start));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
