 ...
 * 
 *****************************************/
public class AuthTask extends TaskAdapter {
	private final static Logger logger = LoggerFactory.getLogger(AuthTask.class);
	/** 鉴权信息同步状态		*/
			...
			if(hours.equals(clearTime)){
				...
				if(dayStr.length == 2){
					if(dayStr == null || dayStr.length != 2){
						logger.error("--Auth--增量更新静态鉴权车辆异常!!! 同步标记异常,请重启同步服务! ==>" + auth);
					}
//				当天删除次数为0
...
//				logger.info("当前时间"+ lastUtc + ", 可视化时间:"+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(lastUtc)));
			...
			while (rs.next()) {
				try {
					String str = rs.getString("AUTH_CODE");
					CharBuffer cb = CharBuffer.wrap(str);
					ByteBuffer bbf = cs.encode(cb);
					byte[] bs= 	new byte[bbf.limit()];
					System.arraycopy(bbf.array(), 0, bs, 0, bbf.limit());
					map.put(rs.getBytes("COMMADDR"),bs); 
					index++;
				} catch (redis.clients.jedis.exceptions.JedisConnectionException ex) {
					logger.error("--Auth--增量同步鉴权信息 - 连接redis异常:" + ex.getMessage(), ex);
					map.clear();
					break;
				} catch (Exception e) {
					logger.error("--Auth--增量同步鉴权信息 - 写入redis异常:" + e.getMessage(), e);
					error++;
					continue;
				}
			}
...
//				1。 全量同步鉴权信息
				syncType = "--All--全量";
				if(!firstSync){
					jedis.del("KCTX.SECURE".getBytes());
				}
				jedis.hmset("KCTX.SECURE".getBytes(), map);
...
//				处理完成就更新标记
			...
			logger.info("--Auth--{}同步鉴权信息结束,处理数据:[{}]条, 正常处理:[{}]条, 异常:[{}]条 , {} oracle查询耗时:[{}]ms, redis写入耗时:[{}]ms, 总耗时[{}]ms", syncType, (index + error), index, error, syncAll, s1 - start, end - s1, end -start);
		} catch (Exception e) {
			if(jedis != null){
				this.redis.returnBrokenResource(jedis);
			}
			logger.error("--Auth--增量同步鉴权信息异常:" + e.getMessage(), e);
		} finally {
			if(jedis != null){
			...
			try {
				...
				logger.error("--Auth--增量同步鉴权信息关闭资源异常:" + e.getMessage(), e);
			}
			authUtc = start - 60000;
		}
	}
