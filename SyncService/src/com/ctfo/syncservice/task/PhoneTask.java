 ...
 * 
 *****************************************/ 
public class PhoneTask extends TaskAdapter {
	private final static Logger logger = LoggerFactory.getLogger(PhoneTask.class);
	/** 日期状态		*/
			...
			if(hours.equals(clearTime)){
				...
				if(dayStr.length == 2){	
					if(dayStr == null || dayStr.length != 2){
						logger.error("--phone--增量同步手机号异常!!! 同步标记异常,请重启同步服务! ==>" + status);
					}
	//				当天删除次数为0
...
//				logger.info("当前时间"+ lastUtc + ", 可视化时间:"+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(lastUtc)));
			...
			while (rs.next()) {
				try {
					...
					map.put(bsKey, rs.getBytes("VAL")); 
					index++;
				} catch (redis.clients.jedis.exceptions.JedisConnectionException ex) {
					logger.error("--phone--同步手机号 - 连接redis异常:" + ex.getMessage(), ex);
					map.clear();
					break;
				} catch (Exception e) {
					logger.error("--phone--同步手机号 - 写入redis异常:" + e.getMessage(), e);
					error++;
					continue;
				}
			}
...
//				1。 全量同步手机号
				syncType = "--All--全量";
				if(!firstSync){
					jedis.del("KCTX.PLATE2SIM".getBytes());
				}
				jedis.hmset("KCTX.PLATE2SIM".getBytes(), map);
...
//				处理完成就更新标记
			...
			logger.info("--phone--{}同步手机号结束,处理数据:[{}]条, 正常处理:[{}]条, 异常:[{}]条 , {} oracle查询耗时:[{}]ms, redis写入耗时:[{}]ms, 总耗时[{}]ms", syncType, (index + error), index, error, syncAll, s1 - start, end - s1, end -start);
		} catch (Exception e) {
			if(jedis != null){
				this.redis.returnBrokenResource(jedis);
			}
			logger.error("--phone--同步手机号异常:" + e.getMessage(), e);
		} finally {
			if(jedis != null){
			...
			try {
				...
				logger.error("--phone--同步手机号关闭资源异常:" + e.getMessage(), e);
			}
			lastUtc = start;
		}
	}
