...
public class PhoneTaskTest {
	...
	public PhoneTaskTest() throws Exception{ 
		...
		for (TaskConfiger tc : ConfigLoader.TASK_LIST) {
			if(tc.getName().equals("phone")){
				Class<?> taskClass = Class.forName(tc.getImpClass());
				task = (PhoneTask) taskClass.newInstance();
