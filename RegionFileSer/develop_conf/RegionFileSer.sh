...
if [ "$LBS_HOME" = "" ]; then
	LBS_HOME="/home/kcpt/supp_app/region"
fi
...
case "$1" in
	start)
		if [ `whoami` = "kcpt" ]; then
			${JAVACMD} -Xmx512m -cp $CLASSPATH ${SERVICE_CLASS} -f ${DST_CONF} -start &
		fi
		;;
	stop)
		if [ `whoami` = "kcpt" ]; then
