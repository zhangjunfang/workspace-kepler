...
JAVACMD="$JAVA_HOME/bin/java"
CLASS_HOME=""
SERVICE_CLASS="com.ctfo.storage.dispatch.core.DispatchMain"
...
unset CLASSPATH
for i in ${LBS_HOME}/dispatch/lib/*.jar ; do
  if [ "$CLASSPATH" != "" ]; then
...
LOGBACK=${LBS_HOME}/dispatch/conf/logback.xml
...
JVM_LOG_DIR="${LBS_HOME}/dispatch/logs/"
...
unset CLASSPATH
for i in ${LBS_HOME}/init/lib/*.jar ; do
  if [ "$CLASSPATH" != "" ]; then
    CLASSPATH=${CLASSPATH}:$i
  else
    CLASSPATH=$i
...
LOGBACK=${LBS_HOME}/init/conf/logback.xml
...
JVM_LOG_DIR="${LBS_HOME}/init/logs/"
JVM_LOG="${JVM_LOG_DIR}jvm-${DATE_STRING}.log"
