...
//		}
	...
	public void messageReceived(IoSession session, Object message) throws Exception {
		try {
			//logger.info("接收数据:"+message.toString());
...
//				sendSubScription(session);
				...
				index = 0;
				//logger.info("应答容器数量：[{}]",ResponseListen.getQueue().size());
			}
			index++;
	 ...
	 * 发送分中心订阅
	 */
	public void sendCenterSubScription(IoSession session) {
...
//		主分类类型	WORD	重复订阅个数
	 ...
	 * 发送分类订阅
	 */
	public void sendTypeSubScription(IoSession session) {
		StringBuffer login = new StringBuffer();
		StringBuffer subScription = new StringBuffer();
		login.append(msgSource);		// 消息来源
		login.append(msgDestination); 	// 消息目的地
...
//		主分类类型	WORD	重复订阅个数
		...
		body.append("02");		// 0删除订阅；1覆盖订阅；2增量订阅
		body.append("12");		// 为0表示删除或订阅所有消息
		body.append("16111612161E161D162316241629162A162B162D16311632163316371643164F16511652");		// 重复订阅个数
		login.append(body); 			// 消息体
	    String loginString = login.toString();
		String verifyCode = Tools.getCheckCode(loginString);
		subScription.append("5b");			// 消息开头	
...
//		centerBody.append("00");		//	WORD	主分类类型		重复订阅个数（如果订阅所有就为空）
		...
		body.append("12");				// BYTE	订阅ID个数 为0表示删除或订阅所有消息 TODO
		body.append("16111612161E161D162316241629162A162B162D16311632163316371643164F16511652");// WORD	重复订阅个数
	    login.append(body); 			// 消息体
	    String loginString = login.toString();
		String verifyCode = Tools.getCheckCode(loginString);
		subScription.append("5b");			// 消息开头	
