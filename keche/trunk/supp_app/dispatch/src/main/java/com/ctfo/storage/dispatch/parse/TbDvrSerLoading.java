...
public class TbDvrSerLoading extends Thread{
	private static Logger log = LoggerFactory.getLogger(TbDvr3GLoading.class);
	
	private static ArrayBlockingQueue<TbDvrSer> queue = new ArrayBlockingQueue<TbDvrSer>(50000);
	...
	private static List<TbDvrSer> list = new ArrayList<TbDvrSer>();
	
	public TbDvrSerLoading(){
...
//				TbDvr3G o = queue.poll();//获取并移除此队列的头，如果此队列为空，则返回 null。 		
	...
	private void process(TbDvrSer o) {
		...
		if(size == batchSize){			
			...
			mySqlService.tbDvrSerSave(list);
			processSize += list.size();
			log.info("【TB_DVRSER】 1000条批量提交:{}, 耗时:{}ms, 共提交总数:{}"  , list.size(), (System.currentTimeMillis() - start), processSize);
			list.clear();
			size = 0;
	...
	static void updateList(MySqlService mySqlService) {
		long start = System.currentTimeMillis();
		List<TbDvrSer> listCopy = new ArrayList<TbDvrSer>();
		listCopy.addAll(list);
		list.clear();
		mySqlService.tbDvrSerSave(listCopy);
		processSize += listCopy.size();
		log.info("【TB_DVRSER】 自动批量提交:{}, 耗时:{}ms, 共提交总数:{}"  , listCopy.size(), (System.currentTimeMillis() - start), processSize);
		size = 0;
		lastTime = System.currentTimeMillis();
	 ...
	 * @param data
	 */
	public static boolean offer(TbDvrSer data){
	 ...
	 * @return
	 */
	public static boolean add(TbDvrSer data){
	 ...
	 * @return list  
	 */
	public static List<TbDvrSer> getList() {
		return list;
	}
	 ...
	 * @param list
	 */
	public static void setList(List<TbDvrSer> list) {
		TbDvrSerLoading.list = list;
	}
	...
	public static void setLastTime(long lastTime) {
		TbDvrSerLoading.lastTime = lastTime;
	}
