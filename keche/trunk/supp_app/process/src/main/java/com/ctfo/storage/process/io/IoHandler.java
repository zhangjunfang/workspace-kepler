...
import com.ctfo.storage.process.parse.TransferProcess;
import com.ctfo.storage.process.util.MD5;
...
//				sendSubScription(session);
			...
			if (message instanceof String) {
				transferProcess.put(message.toString());
			}
		} catch (Exception e) {
...
//		centerBody.append("00");		//	WORD	主分类类型		重复订阅个数（如果订阅所有就为空）
		...
		body.append("04");				// BYTE	订阅ID个数 为0表示删除或订阅所有消息
		body.append("120512061209120b");// WORD	重复订阅个数
...
//		login.append("000dbce9");
		...
		ResponseListen.setSession(session);//应答监听线程
