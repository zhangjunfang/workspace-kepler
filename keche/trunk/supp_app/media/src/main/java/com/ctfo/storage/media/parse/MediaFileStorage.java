...
public class MediaFileStorage extends Thread{
	private static Logger log = LoggerFactory.getLogger(MediaFileStorage.class);
	/**	多媒体文件队列	*/
	private static ArrayBlockingQueue<MediaFile> queue = new ArrayBlockingQueue<MediaFile>(50000);
	/**	MongoDB接口	*/
	private MongoService mongoService;
	
	public MediaFileStorage(){
...
//				Object o = queue.poll();//获取并移除此队列的头，如果此队列为空，则返回 null。 
	...
	private void process(MediaFile mediaFile) {
		...
		mongoService.save(mediaFile, "pics", "fs");
	 ...
	 * @return
	 */
	public void put(MediaFile mediaFile){
		try {
			queue.put(mediaFile);
