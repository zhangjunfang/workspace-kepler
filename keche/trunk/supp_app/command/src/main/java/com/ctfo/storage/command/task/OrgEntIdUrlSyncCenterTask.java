...
public class OrgEntIdUrlSyncCenterTask extends TaskAdapter {

	/** 日志 */
	private static Logger log = LoggerFactory.getLogger(OrgEntIdUrlSyncCenterTask.class);

	public OrgEntIdUrlSyncCenterTask() {
		setName("OrgEntIdUrlSyncCenterTask");
	}
	...
	@Override
	public void init() {
		this.setName("OrgEntIdUrlSyncCenterTask");
		log.info("主中心组织id同步程序启动初始化！");
		this.execute();
	}
	...
	public void execute() {
		...
		try {
			...
			stat = conn.prepareStatement(config.get("sql_allCenterOrgEntId"));
			rs = stat.executeQuery();

			List<TbOrganization> list = new ArrayList<TbOrganization>(); // 查询所有主中心组织id
			while (rs.next()) {
			...
			log.info("--OrgEntIdUrlSyncCenterTask--主中心组织id同步任务执行完成, ---同步数据:[{}]条, 正常处理:[{}]条, ---总耗时:[{}]秒 ", list.size(), list.size(), (System.currentTimeMillis() - start) / 1000.0);
			// System.out.println("--OrgEntIdUrlSyncCenterTask--主中心组织id同步任务执行完成, ---同步数据:[{" + list.size() + "}]条, 正常处理:[{" + list.size() + "}]条, ---总耗时:[{" + (System.currentTimeMillis() - start) / 1000.0 + "}]秒 ");
	...
	private void getOrgParentId(TbOrganization org, StringBuffer buff, List<TbOrganization> list) {
		try {
			for (TbOrganization tbOrg : list) {
				if (tbOrg.getEntId().equals(org.getParentId())) {
	...
	private void updateMySqlOrgEntIdUrl(List<TbOrganization> list) {
		...
		try {
			...
			stat = conn.prepareStatement(config.get("sql_updateCenterEntIdUrl"));
			for (TbOrganization org : list) {
			...
			stat.executeBatch();
			conn.commit();
			// System.out.println("更新主中心组织表:[{" + list.size() + "}]条, 正常处理:[{" + list.size() + "}]条, ---总耗时:[{" + (System.currentTimeMillis() - start) / 1000.0 + "}]秒 ");
			log.info("更新主中心组织表:[{}]条, 正常处理:[{}]条, ---总耗时:[{}]秒 ", list.size(), list.size(), (System.currentTimeMillis() - start) / 1000.0);
		} catch (SQLException e) {
			log.error("主中心组织id更新异常:" + e.getMessage(), e);
		} finally {
			try {
				if (null != stat) {
					stat.close();
