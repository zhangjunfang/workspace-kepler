 ...
 *
 */
public class OrgTreeCacheSyncTask extends TaskAdapter {
	private static Logger log = LoggerFactory.getLogger(OrgTreeCacheSyncTask.class);
	private RedisService redisService = new RedisService();

	
	
	public OrgTreeCacheSyncTask(){
		setName("OrgTreeCacheSyncTask");
	}
	...
	public void execute() {
		...
		try {
			...
			stat = conn.prepareStatement(config.get("sql_initAllParentId"));
			result = stat.executeQuery();
			while(result.next()){
				StringBuffer buffKey = new StringBuffer();
				buffKey.append("HS_ENTID").append(result.getString("ENT_ID")).append("_").append(result.getString("CENTER_CODE"));
			...
			stat = conn.prepareStatement(config.get("sql_allOrgTree"));
			result = stat.executeQuery();
			while(result.next()){
				...
				sysOrgTreeModel.setCenterCode(result.getString("CENTER_CODE"));
			...
			log.info("--OrgTreeCacheSyncTask--分中心组织树缓存同步任务执行完成, ---同步数据:[{}]条, 正常处理:[{}]条, 异常：[{}]条, ---总耗时:[{}]秒 ", map.size(),map.size(),(orgTreeKeylist.size()-map.size()), (System.currentTimeMillis() - start)/1000.0);
