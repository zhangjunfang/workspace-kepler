...
public class DateUtil {
	...
	/**
	 * 按yyyy-MM-dd格式对指定日期进行格式化 方法的参数和返回值
	 * 
	 * @param date
	 * @return
	 */
	...
	/**
	 * 按“年 月 日 小时 分钟 秒”的格式对字符串进行日期转换， 以非数字字符对字符串进行分隔，如不足6个数字，则后边进行补0。 可匹配以下任意格式：2011-3-28，2011-03-28，2011－03－28，2011年03月28日， 2011-3-28 14:29:8，2011-03-28 14:29:08，2011－03－28 14：29：08，2011年03月28日14时29分8秒， 2011-3-28 14:2，2011-03-28 14:02，2011－03－28 14：02，2011年03月28日14时2分，等 方法的参数和返回值
	 * 
	 * @param dateStr
	 * @return
	 * @throws ParseException
	...
	/**
	 * 返回两个日期之间的天数，只计日期，不计时分秒， 如2011-02-04 00:00:00与2011-02-05 23:59:59相差1天 方法的参数和返回值
	 * 
	 * @param dateStart
	 * @param dateEnd
	 * @return
	...
	/**
	 * 返回指定日期后指定天数的日期，参数为负值时返回当前日期前的日期 方法的参数和返回值
	 * 
	 * @param date
	 * @param rd
	 * @return
	...
	/**
	 * 返回当前日期后指定天数的日期，参数为负值时返回当前日期前的日期 方法的参数和返回值
	 * 
	 * @param rd
	 * @return
	 */
	...
	/**
	 * 返回当前日期是周几，分别返回日,一,二,三,四,五,六 方法的参数和返回值
	 * 
	 * @return
	 */
	...
	/**
	 * 返回指定日期是周几，分别返回日,一,二,三,四,五,六 方法的参数和返回值
	 * 
	 * @param d
	 * @return
	 */
	...
	public static String getMonth(Date d) {
		...
		String month = (tempDate.get(Calendar.MONTH) + 1) < 10 ? "0" + (tempDate.get(Calendar.MONTH) + 1) : "" + (tempDate.get(Calendar.MONTH) + 1);
		return month;
	}
	...
	/**
	 * 从本月向前推i个月 显示当时月的一号 如今天5月08日，订单创建日期起始时间为3月1日 方法的参数和返回值 为"yyyy-MM-dd"字符串
	 * 
	 * @param date
	 * @param i
	 * @return
	...
	public static String getPreMonthLastDay() {
		...
		calendar.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), MaxDay, 00, 00, 00); // 按你的要求设置时间
		String s = df.format(calendar.getTime());
		return s;
	}
	 ...
	 * @return
	 */
	public static Long stringTimeToUtcTime(String stringTime) {
		// 将本地当前时间转换成UTC国际标准时间的毫秒形式
		if (stringTime != null) {
			Date date = null;
			try {
				DateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				date = format.parse(stringTime.replace("/", "-"));
			} catch (Exception e) {
	...
	public static void main(String[] args) {
		System.out.println(stringTimeToUtcTime(getPreMonthLastDay()));
	}
}