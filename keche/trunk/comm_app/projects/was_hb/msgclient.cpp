...
void MsgClient::HandleUpData( const char *data, int len )
{
	...
	if ( ! SendRC4Data( user._fd, data, len ) ) {
		OUT_ERROR( user._ip.c_str(), user._port, user._user_id.c_str() , "fd %d Send data failed" , user._fd->_fd );
		_dataqueue.WriteCache( WAS_CLIENT_ID, ( void * ) data, len );
		return;
	}

	OUT_SEND3( user._ip.c_str(), user._port, user._user_id.c_str() , "msg send: %s" , data );
}
...
void MsgClient::on_data_arrived( socket_t *sock, const void* data, int len )
{
	...
	if ( head == "LACK" ) {
		...
		//NOOP_ACK
		OUT_WARNING( ip, port, "MsgClient", "NOOP_ACK" ) ;
		// 更新用户最后一次时间
		user._last_active_time = time( 0 );
	} else if ( head == "CAITS" ) {
...
void MsgClient::HandleDgetpMsg( string &line )
{
	...
	if ( type == "0" ) { // 终端参数设制
		//读取参数
		dval.msgid = 0x8104;
	} else if ( type == "30" ) { // 历史数据查询命令

		dval.msgid = 0x8f12;

		QueryHistoryDataBody body; // 历史数据查询结构体

		it = kvmap.find( "311" );  // 历史数据查询类型
		if ( it == kvmap.end() ) {
			OUT_ERROR( NULL, 0, "MsgClient" , "DGET get query type error" );
			return;
		}
		body.type = atoi( it->second.c_str() ) - 1;

		it = kvmap.find( "312" ); // 历史数据开始时间
		if ( it == kvmap.end() ) {
			OUT_ERROR( NULL, 0, "MsgClient" , "DGET get start time error" );
			return;
		}
		convert2bcd( it->second, body.starttime );

		it = kvmap.find( "313" ); // 历史数据结束时间
		if ( it == kvmap.end() ) {
			OUT_ERROR( NULL, 0, "MsgClient" , "DGET get end time error" );
			return;
		}
		...
		it = kvmap.find( "314" );
		if ( it != kvmap.end() ) {
			body.num = htons( atoi( it->second.c_str() ) );
		} else {
			body.num = 0;
		}
		dval.buffer.writeBlock( &body, sizeof(body) ) ;
	}
	else if (type == "54"){ //怠速参数获取
		dval.msgid  = 0x8f18;
	}
...
void MsgClient::HandleDsetpMsg( string &line )
{
	...
	} else if ( type == "30" ) { //历史数据查询
		p = p_kv_map.find( "311" ); //数据类型
		if ( p != p_kv_map.end() ) {
	...
	case 24: // 报警确认应答
		{
			...
			if ( alamid == -1 ) {
				...
				dval.msgid = 0x8f16; // 人工确认报警
				dval.buffer.writeInt32( alamid ); // 处理扩展报警处理
			}
		}
		break;
	default:
		...
		const string &seq )
{
	...
	for ( int i = 0 ; i < count ; ++ i ) {
		...
		string cache_key = car_id + "_" + ustodecstr( val.msgid ) + "_" + ustodecstr( downseq );
