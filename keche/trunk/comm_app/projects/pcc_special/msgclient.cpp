...
void MsgClient::HandleUserData( const User &user, const char *data, int len )
{
	...
	unsigned int id = 0 ;
	if ( strncmp ( seqid.c_str(), SEQ_HEAD, SEQ_HEAD_LEN ) == 0 ) {
		id = atoi( (const char *)(seqid.c_str() + SEQ_HEAD_LEN) ) ;
	} else {
		id = _pEnv->GetSequeue() ;
	}

	char key[256] = {0} ;
	_pEnv->GetCacheKey( id, key ) ;
	...
	if (cmd != "L_PROV") {
		...
		if (head == "CAITS" && cmd == "U_REPT") {
			msg_buf = _convert->convert_urept(key, ome, phone, val, msg_len, msgid, method);
			// 添加发送统计数据,只统计上报数据的处理
			...
			// 处理通用应答数据
			msg_buf = _convert->convert_comm(key, phone.c_str(), val, msg_len, msgid);
		} else {
			return;
		}
		...
		channels.clear();
		if (!_pEnv->getChannels(macid, channels)) {
			_convert->free_buffer(msg_buf);
			OUT_WARNING(user._ip.c_str(), user._port, user._user_name.c_str(), "macid %s not found channel", macid.c_str());
			return;
		}
		...
		for (ssit = channels.begin(); ssit != channels.end(); ++ssit) {
			fields.clear();
			if (Utils::splitStr(*ssit, fields, ':') < 3) {
				continue;
			}
			...
			msgheader->vehicle_color = fields[1][0] - '0';
			memset(msgheader->vehicle_no, 0x00, sizeof(msgheader->vehicle_no));
			fields[2].copy(msgheader->vehicle_no, sizeof(msgheader->vehicle_no));
			...
			if (method == METHOD_REG) {
				...
				if (fields.size() > 3 && !fields[3].empty()) {
					// 厂家编号
					fields[3].copy(req->producer_id, sizeof(req->producer_id));
				}
				if (fields.size() > 4 && !fields[4].empty()) {
					// 终端型号
					fields[4].copy(req->terminal_model_type, sizeof(req->terminal_model_type));
				}
				if (fields.size() > 5 && !fields[5].empty()) {
					// 终端编号
					fields[5].copy(req->terminal_id, sizeof(req->terminal_id));
				}
				// 电话号码
				phone.copy(req->terminal_simcode, sizeof(req->terminal_simcode));
			}

			// 根据对应省发送给对应省处理
			_pEnv->GetPasClient()->HandleClientData(fields[0].c_str(), msg_buf,	msg_len); //发往PAS
		}
		...
		string areacode ;
		// 将监控扩展监管内部协议转为８０９下发
		char *msg_buf = _convert->convert_lprov( key, seqid, val , msg_len, areacode ) ;
		if ( msg_buf != NULL ) {
			// 直接递交发送给PAS处理
			_pEnv->GetPasClient()->HandleClientData( areacode.c_str(), msg_buf, msg_len ) ;
			_convert->free_buffer( msg_buf ) ;
		}
	}
}
