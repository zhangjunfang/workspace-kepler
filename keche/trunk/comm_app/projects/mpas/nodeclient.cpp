...
bool CNodeClient::Init( ISystemEnv *pEnv )
{
	...
	//msg_dev_name=eth0
	int nvalue = 0 ;
	if ( pEnv->GetInteger( "mpas_node_enable" , nvalue ) ) {
		_enable = ( nvalue == 1 ) ;
	}

	// 取得节点ID号
	if ( ! pEnv->GetInteger( "mpas_node_id" , nvalue ) ) {
		printf( "get mppas node id failed\n" ) ;
		return false ;
	}
	_nodeid = nvalue ;
	...
	if ( ! pEnv->GetString( "mpas_node_server" , buf ) ) {
		printf( "get mpas node server failed\n" ) ;
		return false ;
	}
	_client_user._ip = buf ;

	// 取得节点服务器的端口
	if ( ! pEnv->GetInteger( "mpas_node_port" , nvalue ) ) {
	...
	// 取得设备的名称
	if ( !pEnv->GetString( "mpas_dev_name" , buf ) ) {
		printf( "get msg dev name failed\n" ) ;
		return false ;
	}
	// 取得本机的IP的地址
	_send_ip = netutil::addrToString( netutil::getLocalAddr( buf ) ) ;

	// 取得当前服务的端口
	if ( ! pEnv->GetInteger( "mpas_listen_port" , nvalue ) ) {
	...
	case NODE_USERNAME_RSP:
		{
			...
			if ( mlen == (uint32_t)sizeof(NodeUserNameRsp) ) {
				...
				OUT_ERROR( ip, port, "NODE" , "get login user error" ) ;
			}
		}
		break ;
			...
			if ( mlen > (uint32_t)sizeof(NodeGetMsgRsp) ) {
				...
				if ( mlen == sizeof(NodeGetMsgRsp) + num * sizeof(AddrInfo) ) {
					...
					for ( short i = 0; i < num; ++ i ) {
						...
						OUT_INFO( ip, port, "NODE" , "get msg server ip %s port %d" , ip, port ) ;
						// 变更服务器
					...
					OUT_ERROR( ip, port, "NODE" , "get msg server length error" ) ;
				}
			} else {
				// 如果获取服务器失败则需要进一步发送请求直到成功
				GetServerMsg() ;
				OUT_ERROR( ip, port, "NODE" , "get msg server error" ) ;
			}
		}
		break ;
	...
	case NODE_MSGCHG_REQ:
		{
			...
			if ( ! SendDataEx( sock, buf.getBuffer(), buf.getLength() ) ) {
				OUT_ERROR( ip, port, "NODE" , "fd %d Send msg chg resp failed", sock->_fd ) ;
			}
		}
		break ;
	}
...
{
	AddrInfo addr ;
	safe_memncpy( addr.ip, _send_ip.c_str(), sizeof(addr.ip) ) ;
	addr.port = _send_port ;

	MsgData *msg = _pBuilder->BuildLoginReq( _nodeid , FD_NODE_PIPE , addr ) ;
...
void CNodeClient::SendLinkTest( void )
{
	...
	if ( ! SendDataEx( _client_user._fd, buf.getBuffer(), buf.getLength() ) ) {
		OUT_ERROR( NULL, 0, NULL, "SendLinkTest failed" ) ;
...
void CNodeClient::UserName( void )
{
	...
	if ( ! SendDataEx( _client_user._fd, buf.getBuffer(), buf.getLength() ) ) {
		OUT_ERROR( NULL, 0,NULL, "Send UserName failed" ) ;
		_pWaitQueue->DelQueue( msg->seq, _client_user._fd, true ) ;
		return ;
	}
}
...
void CNodeClient::GetServerMsg( void )
{
	...
	if ( ! SendDataEx( _client_user._fd, buf.getBuffer(), buf.getLength() ) ) {
		OUT_ERROR( NULL, 0,NULL, "Send Server Msg failed" ) ;
		_pWaitQueue->DelQueue( msg->seq, _client_user._fd, true ) ;
		return ;
	}
}
