...
bool CUserLoader::loadSubscribe(const string &code, const string &path, Macid2Channel &macid2Channel)
{
	...
	while(getline(ifs, macid)) {
		if(macid.empty() || macid[0] == '#') {
			continue;
		}
...
bool CUserLoader::getSubscribe(list<string> &macids)
{
	...
	for(it = _macid2channel.begin(); it != _macid2channel.end(); ++it) {
		...
		for(ssIt = it->second.begin(); ssIt != it->second.end(); ++ssIt) {
			channels.insert(*ssIt); //去重保存省管平台通道名称
		}
	}

	for(ssIt = channels.begin(); ssIt != channels.end(); ++ssIt) {
		macids.push_back(*ssIt); //平台消息也需要订阅
	}
