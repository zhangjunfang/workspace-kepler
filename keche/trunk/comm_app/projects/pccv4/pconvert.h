...
public:
	...
	// 处理通用应答
	char * convert_lplat_r( const string &key, const string &phone, const string &val, int &len, unsigned int &msgid ) ;
	// 转换监管协议
	char * convert_lprov( const string &key, const string &seqid, const string &val , int &len, string &areacode ) ;
	// 转换静态数据
	char * convert_lplat( const string &key, const string &seqid, const string &val , int &len, string &areacode );
	// 释放缓存
