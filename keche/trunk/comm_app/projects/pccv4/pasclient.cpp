	...
	_macid2seqid(true), _filecache(this), _srvCaller(srvCaller), _statinfo("PasClient")
{
	...
	_smsNotifyMail = "yuanchunqing@ctfo.com";
...
bool PasClient::SendDataToUser( const string &area_code, const char *data, int len)
{
	...
	if( user._user_id.empty() ){
		if ( area_code.length() != 6 ) {  // 430000  省域编码规则为六位
			OUT_ERROR( user._ip.c_str() , user._port , buf , "SendDataToUser len %d failed user not exist" , len ) ;
			return false;
		}

		// 如果没有地市级省管平台，就直接将数据送到省管平台
		sprintf( buf, "%s%s0000", PAS_USER_TAG, area_code.substr(0,2).c_str() ) ;
		user = _online_user.GetUserByUserId( buf ) ;
		if ( user._user_id.empty() ) {
			OUT_ERROR( user._ip.c_str() , user._port , buf , "SendDataToUser len %d failed user not exist" , len ) ;
			return false;
		}
		...
		case DOWN_CTRL_MSG_TAKE_PHOTO_REQ:  // 处理拍照
			{
				...
				char szKey[256]={0};
				sprintf( szKey, "%s_%d" , macidDst.c_str(), UP_CTRL_MSG_TAKE_PHOTO_ACK ) ;
