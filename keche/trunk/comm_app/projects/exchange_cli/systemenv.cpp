...
{
	_amqclient   = new AmqClient;
	_msgclient   = new MsgClient ;
	_exchclient  = new ExchClient ;
...
bool CSystemEnv::Init( const char *file , const char *logpath , const char *userfile , const char *logname )
{
	...
	// 初始化was的对象
	if (!_exchclient->Init(this)) {
		printf("CSystemEnv::Init exchange client failed\n");
		return false;
	}
	...
	if (!_amqclient->Init(this)) {
		printf("CSystemEnv::Init amq client failed\n");
		return false;
	}
