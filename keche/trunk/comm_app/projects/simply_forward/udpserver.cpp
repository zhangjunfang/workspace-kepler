...
UdpServer::UdpServer(ISystemEnv *pEnv) {
	_pEnv = pEnv;
	_thread_num = 1;
	_max_timeout = 180;
}
...
bool UdpServer::Init(int port, const vector<string> &addrs) {
	_listen_port = port;
	_listen_ip = "0.0.0.0";
	_dstAddrs = addrs;

	int nvalue = 0;
	if (_pEnv->GetInteger("udp_svr_thread", nvalue)) {
		_thread_num = nvalue;
	}
...
bool UdpServer::Start() {
	return StartUDP(_listen_port, _listen_ip.c_str(), _thread_num, _max_timeout);
}
...
void UdpServer::NoopWork() {
}

void UdpServer::on_data_arrived(socket_t *sock, const void *data, int len)
{
	...
	if(user._user_id.empty()) {
		...
		for(it = _dstAddrs.begin(); it != _dstAddrs.end(); ++it) {
			...
			_pEnv->GetUdpClient()->AddChannel(this, userid, dstAddr.c_str(), dstPort);
		}
	...
	if (user._ext_ptr != NULL) {
		...
		for (it = userids->begin(); it != userids->end(); ++it) {
			_pEnv->GetUdpClient()->HandleData(*it, data, len);
		}
	}
}

void UdpServer::on_new_connection(socket_t *sock, const char* ip, int port)
{
}

void UdpServer::on_dis_connection(socket_t *sock)
{
...
bool UdpServer::HandleData(const string &userid, const void *data, int len)
{
	string svrUserid;
	size_t pos;
...
void UdpServer::HandleOfflineUsers() {
...
bool UdpServer::ChkChannel(const string &userid) {
	string svrUserid;
	size_t pos;
