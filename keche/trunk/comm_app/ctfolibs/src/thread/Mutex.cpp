...
#define THRIFT_NO_CONTENTION_PROFILING
#endif
...
Mutex::~Mutex() { if (impl_ != NULL ) delete impl_; }
...
RWMutex::RWMutex() : impl_(new RWMutex::impl()) {}

RWMutex::~RWMutex() { if (impl_ != NULL ) delete impl_; }

void RWMutex::acquireRead() const { impl_->acquireRead(); }

void RWMutex::acquireWrite() const { impl_->acquireWrite(); }

bool RWMutex::attemptRead() const { return impl_->attemptRead(); }

bool RWMutex::attemptWrite() const { return impl_->attemptWrite(); }

void RWMutex::release() const { impl_->release(); }
