...
import com.ctfo.syn.kcpt_oracle2mysql.ConnectMemcachePool;

/**
 * <p>Title: SynData</p>
 * <p>Description: SynData</p>
...
public class HttpThread implements Runnable{
	...
	private final static Log log = LogFactory.getLog(HttpThread.class);

	private long memLastUpdateTime = 0;// 同步数据到Memcached中最后一次的UTC时间
	 ...
	 * @return
	 */
	private int getSynData() {
		long updateTime = memLastUpdateTime;
		...
		try {
			...
			// 同步數據到MemCacahed
			addMemCached(list);
			memLastUpdateTime = System.currentTimeMillis();// 更新缓存最后一次的修改时间
			int count = list.size();
			return count;
	...
	private void addMemCached(List<VehicleInfo> info) {
		...
		try {
			for (VehicleInfo v : info) {
				...
				ConnectMemcachePool.getSqlMap(SynPool.getinstance().getSql("httpMemAddr")).set("PCC_" + v.getCommaddr(), 0, v);
			}// End for
