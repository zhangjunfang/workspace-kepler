            ...
            try
            {
                ...
                if (((e.RowIndex <= -1) || (e.ColumnIndex <= -1)) && ((e.RowIndex == -1) && this.SpanRows.ContainsKey(e.ColumnIndex)))
                {
                    ...
                    using (SolidBrush sb = new SolidBrush(ColorTranslator.FromHtml("#e1f0fc")))
                    {
                        ...
                        graphics.FillRectangle(sb, new Rectangle(left - 2, y, right - left + 3, (bottom - y)));
                    }
                    using (Pen blackPen = new Pen(Color.FromArgb(221, 221, 221))) //(163, 192, 232)))//边框颜色
                    {
                        e.Graphics.DrawRectangle(blackPen, new Rectangle(left - 3, y + (bottom - y) / 2, right - left + 3, (bottom - y) / 2 - 1));//合计子列的边框
                        graphics.DrawLine(blackPen, right, y, right, (bottom - y) / 2);//合并右边的竖线
                        graphics.DrawLine(blackPen, left - 2, 0, right, 0);//合并上边的横线
                    }
                    //graphics.FillRectangle(new SolidBrush(this._mergecolumnheaderbackcolor), left, y, right - left, (bottom - y) / 2);
                    //graphics.DrawLine(new Pen(base.GridColor), left - 2, (y + bottom) / 2, right, (y + bottom) / 2);
