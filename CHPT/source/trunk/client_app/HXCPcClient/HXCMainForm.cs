...
namespace HXCPcClient
{
    ...
    public partial class HXCMainForm : FormExhyh
    {
        ...
        private void tmrInfo_Tick(object sender, EventArgs e)
        {
            ...
            {
                GlobalStaticObj.GlobalLogService.WriteLog("【时间更新】", ex);
            }
        }
        #endregion
            ...
            if (PUCName.Length > 0)
            {
                ...
                if (ctls.Length > 0)
                {
                    ...
                    if (Arr[1].Length > 0)
                    {
                        ...
                        if (this.panel_Content.Controls.ContainsKey(this.currentMenu))
                        {
                            this.panel_Content.Controls.Find(this.currentMenu, false)[0].Visible = true;
                            this.panel_Content.Controls.Find(this.currentMenu, false)[0].BringToFront();
                            ctls = this.panel_LeftMenu.Controls.Find(Arr[1], false);
                            string menu = this.currentMenu;
                            ...
                            childMemuState(this.currentMenu);
        ...
        private void tSMenuItem_Softupdate_Click(object sender, EventArgs e)
        {
            ...
            //检测升级
            string strUpData = AutoUpdate.CheckUpdate.ClientUpDataExe("HXCPcClient.exe", currVersion, "2");
            if (strUpData != "")
            {
        ...
        private void HXCMainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                GlobalStaticObj.GlobalLogService.WriteLog("系统退出!");
            }
            catch (Exception ex)
            {
                GlobalStaticObj.GlobalLogService.WriteLog(ex);
        ...
        private void lvlVoiceSwitch_Click(object sender, EventArgs e)
        {
            keybd_event(VK_VOLUME_MUTE, MapVirtualKey(VK_VOLUME_MUTE, 0), KEYEVENTF_EXTENDEDKEY, 0);
            keybd_event(VK_VOLUME_MUTE, MapVirtualKey(VK_VOLUME_MUTE, 0), KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP, 0);
            //SendMessage(this.Handle, WM_APPCOMMAND, 0x200eb0, APPCOMMAND_VOLUME_MUTE * 0x10000);//只设置本窗体静音
        }
