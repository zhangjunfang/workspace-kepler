...
namespace HXCPcClient.UCForm.CustomerService.Feedback
{
    ...
    public partial class UCFeedbackAddOrEdit : UCBase
    {
        ...
        private Boolean CheckValue()    //必填字段非空验证
        {
            ...
            {
                MessageBoxEx.Show("反馈人不能为空!", "提示", MessageBoxButtons.OK, MessageBoxIcon.None);
                txt_cb_title.Focus();
                return false;
            }
            ...
            if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(cbo_cb_Callback_mode.SelectedValue)))
            {
                MessageBoxEx.Show("反馈形式不能为空!", "提示", MessageBoxButtons.OK, MessageBoxIcon.None);
                cbo_cb_Callback_mode.Focus();
                return false;
            }
            if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(cbo_cb_Callback_type.SelectedValue)))
            {
        ...
        private void SetHandleInfo() //设置经办人信息
        {
            if (!String.IsNullOrEmpty(HandleId))
            {
                ...
                if (custInfo != null && custInfo.DefaultView.Count != 0)
                {
                    ...
                    txt_handle_name.Tag = HandleId;
                }
                return;
            }
            if (String.IsNullOrEmpty(HandleId))
            {
                var custInfo = DBHelper.GetTable("查询经办人信息", "sys_user", "*", "user_name = '" + txt_handle_name.Text + "'", "", "");
                if (custInfo != null && custInfo.DefaultView.Count != 0)
                {
                    txt_handle_name.Text = CommonCtrl.IsNullToString(custInfo.DefaultView[0]["user_name"]);
                    txt_handle_name.Tag = CommonCtrl.IsNullToString(custInfo.DefaultView[0]["user_id"]);
                }
            }
        }
        #endregion
