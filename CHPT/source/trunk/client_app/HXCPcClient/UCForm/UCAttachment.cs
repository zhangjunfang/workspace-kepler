    ...
    public partial class UCAttachment : UserControl
    {
        ...
        public bool CheckAttachment()
        {
            ...
            foreach (DataGridViewRow dgvr in dgvAttachment.Rows)
            {
                ...
                {
                    MessageBoxEx.Show("附件中有重复，请检查附件！");
                    isCheck = false;
                    break;
                }
                listPath.Add(path);
        ...
        public List<SQLObj> AttachmentSql
        {
            get
            {
                ...
                foreach (DataGridViewRow dgvr in dgvAttachment.Rows)
                {
                    ...
                    string fileName = Path.GetFileName(CommonCtrl.IsNullToString(dgvr.Cells["colAttPath"].Value));
        ...
        private void dgvAttachment_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvAttachment.ReadOnly)
            {
                if (dgvAttachment.Columns[e.ColumnIndex].Name == colAttPath.Name)
                {
                    DownFile();
                }
            }
