...
namespace HXCPcClient.UCForm.DataManage.WorkingTimeFile
{
    public partial class UCWorkingTimeManager : UCBase
    {
        ...
        private void UCWorkingTimeManager_Load(object sender, EventArgs e)
        {
            ...
            base.SetContentMenuScrip(dgvWorkList);
        }
        ...
        {
            if (this.dgvWorkList.Rows.Count == 0)
            {
                return;
            }
            try
            {
                string fileName = "工时档案" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, ExcelHandler.HandleDataTableForExcel(dgvWorkList.GetBoundData(), dgvWorkList));
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("工时档案" + ex.Message, "client");
        ...
        private void gvProList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用   
            {
                ...
                UCWorkingTimeView UCWorkView = new UCWorkingTimeView(suppId, this);
