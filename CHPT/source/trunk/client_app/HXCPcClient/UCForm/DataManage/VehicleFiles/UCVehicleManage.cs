...
namespace HXCPcClient.UCForm.DataManage.VehicleFiles
{
    ...
    public partial class UCVehicleManage : UCBase
    {
        ...
        private void SetQuick()
        {
            ...
            //设置车牌号速查
            txtCarNO.SetBindTable("v_vehicle", "license_plate", "license_plate");
        ...
        void txtCarNO_DataBacked(DataRow dr)
        {
            try
            {
                ...
                txtYardCode.Caption = CommonCtrl.IsNullToString(dr["turner"]);
                txtCustomNO.Text = CommonCtrl.IsNullToString(dr["cust_name"]);
                txtCustomNO.Tag = CommonCtrl.IsNullToString(dr["cust_id"]);               
            }
            catch (Exception ex)
        ...
        #region 打印事件
        void UCVehicleManage_PrintEvent(object sender, EventArgs e)
        {
            businessPrint.Print(dgvData.GetBoundData());
        }
        #endregion
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "车辆档案" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, dgvData);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【车辆档案】" + ex.Message, "server");
        ...
        public void BindPageData()
        {
            try
            {
                if (this.myLock)
                {
                    ...
                    {
                        strWhere += string.Format(" and  a.v_model = '{0}'", txtCarType.Tag.ToString());
                    }
                    ...
                    if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(txtYardCode.Caption.Trim())))//车场编码
                    {
                        strWhere += string.Format(" and  a.turner like '%{0}%'", txtYardCode.Caption.Trim());
                    }
                    ...
                    // string strFileds = @"a.license_plate,a.vin,e.dic_name as v_brand,d.vm_name as v_model ,a.carbuy_date,a.turner,b.cust_name,a.data_source,
                    //c.user_name,a.create_time, case a.status when '1'  then '启用'  when '0' then  '停用' else '' end  as status,a.remark,a.v_id";
                    // string strTables = @" tb_vehicle a left join tb_customer b on a.cust_id=b.cust_id 
                    // left join sys_user c on a.create_by=c.user_id left join tb_vehicle_models d on a.v_model=d.vm_id 
                    // left join sys_dictionaries e on a.v_brand=e.dic_id";
        ...
        {
            string strFiles = @" a.license_plate,a.vin,e.dic_name as v_brand,d.vm_name as v_model ,a.carbuy_date,a.turner,b.cust_name,a.data_source,
                c.user_name,a.create_time, case a.status when '1'  then '启用'  when '0' then  '停用' else '' end  as status,a.remark,a.v_id";
            string strTables = @" tb_vehicle a left join tb_customer b on a.cust_id=b.cust_id 
                left join sys_user c on a.create_by=c.user_id left join tb_vehicle_models d on a.v_model=d.vm_id 
                left join sys_dictionaries e on a.v_brand=e.dic_id";
            DataTable dt = DBHelper.GetTableByPage("分页查询车辆档案", strTables, strFiles, obj.ToString(), "", " order by a.create_time desc", page.PageIndex, page.PageSize, out this.recordCount);
        ...
        private void dgvData_HeadCheckChanged()
        {
            ...
            foreach (DataGridViewRow row in dgvData.Rows)
            {
                object check = row.Cells[colCheck.Name].EditedFormattedValue;
