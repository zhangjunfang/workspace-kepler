...
namespace HXCPcClient.UCForm.DataManage.VehicleFiles
{
    ...
    public partial class UCVehicleAddOrEdit : UCBase
    {
        ...
        {       
            //设置车型速查
            txtCarType.SetBindTable("tb_vehicle_models", "vm_name", "vm_name");
            ...
            //设置客户编码速查
            txtCustomNO.SetBindTable("tb_customer", "cust_name", "cust_name");
        ...
        void txtCarType_DataBacked(DataRow dr)
        {
            ...
            txtCarType.Tag = CommonCtrl.IsNullToString(dr["vm_id"]);
        }
        /// <summary>
        /// 客户编码速查连带信息
        /// </summary>
        ...
        {
            txtCustomNO.Text = CommonCtrl.IsNullToString(dr["cust_name"]);
        ...
        private Boolean CheckControlValue()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(CommonCtrl.IsNullToString(cobCarBrand.SelectedValue)))
                {
                    Validator.SetError(errorProvider1, cobCarBrand, "车辆品牌不能为空!");
                    return false;
                }               
                if (string.IsNullOrEmpty(CommonCtrl.IsNullToString(cobFactory.SelectedValue)))
                {
                    Validator.SetError(errorProvider1, cobFactory, "生产厂家不能为空!");
                    return false;
                }
                if (string.IsNullOrEmpty(txtVIN.Caption.Trim()))
                {
                    Validator.SetError(errorProvider1, txtVIN, "VIN不能为空!");
                    return false;
                }
            }
            catch (Exception ex)
        ...
        private void SaveVehicleData(List<SQLObj> listSql)
        {
            ...
            dicParam.Add("turner", new ParamObj("turner", txtYardCode.Caption.Trim(), SysDbType.VarChar, 20));//车场编号
            ...
            dicParam.Add("v_model", new ParamObj("v_model",!string.IsNullOrEmpty(CommonCtrl.IsNullToString(txtCarType.Tag))?txtCarType.Tag.ToString():null, SysDbType.VarChar, 40));//车型
            ...
            if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
            {
                ...
                obj.sqlString = @"insert into tb_vehicle (data_source,license_plate,v_brand,v_factory,vin,turner,v_color,v_model,v_class,cust_id,carbuy_date
                ...
                ,shift_num,shift_type,gps_factory,gps_model,gps_num,dvr_factory,dvr_model,dvr_num,enable_flag,status,v_id,create_by,create_time)
                 values (@data_source,@license_plate,@v_brand,@v_factory,@vin,@turner,@v_color,@v_model,@v_class,@cust_id,@carbuy_date
                ,@remark,@v_long,@v_wide,@v_high,@axle_count,@axle_distance,@wheel_distance,@total_quality,@approved_quality,@engine_factory,@engine_model,@engine_num,@discharge
                ,@fuel_type,@air_factory,@air_model,@air_num,@axle_factory,@axle_model,@axle_num,@lowdisc_factory,@wheel_count,@wheel_factory,@wheel_model,@shift_factory,@shift_model
                ,@shift_num,@shift_type,@gps_factory,@gps_model,@gps_num,@dvr_factory,@dvr_model,@dvr_num,@enable_flag,@status,@v_id,@create_by,@create_time);";
            }
            else if (wStatus == WindowStatus.Edit)
            {
                ...
                obj.sqlString = @"update tb_vehicle set data_source=@data_source,license_plate=@license_plate,v_brand=@v_brand,v_factory=@v_factory,vin=@vin,turner=@turner
        ...
        private void BindData()
        {
            ...
            txtYardCode.Caption = CommonCtrl.IsNullToString(dr["turner"]);//车场编号
        ...
        private void dgvUser_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                DataTable dut = DBHelper.GetTable("获取联系人(司机)", "tb_contacts", string.Format("*,{0} phone, {1} tel", EncryptByDB.GetDesFieldValue("cont_phone"), EncryptByDB.GetDesFieldValue("cont_tel")), "cont_id='" + contact.contID + "'", "", " order by cont_id desc");
                if (dut.Rows.Count > 0)
                {
                    ...
                    dgvUser.Rows[e.RowIndex].Cells["cont_phone"].Value = CommonCtrl.IsNullToString(dpr["phone"]);
                    dgvUser.Rows[e.RowIndex].Cells["cont_tel"].Value = CommonCtrl.IsNullToString(dpr["tel"]);
        ...
        {
            int introw = dgvUser.CurrentRow.Index;
            if (introw < 0)
            {
                return;
            }
            ...
            {
                MessageBoxEx.Show("至少保留一条驾驶员员信息！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            string strDirId = CommonCtrl.IsNullToString(dgvUser.CurrentRow.Cells["dir_v_id"].Value);
            string strcontName = CommonCtrl.IsNullToString(dgvUser.CurrentRow.Cells["cont_name"].Value);
            if (!string.IsNullOrEmpty(strDirId))
            {
                ...
                obj.cmdType = CommandType.Text;
                Dictionary<string, ParamObj> dicParam = new Dictionary<string, ParamObj>();
                dicParam.Add("dir_v_id", new ParamObj("dir_v_id", strDirId, SysDbType.VarChar, 40));
                obj.sqlString = @"delete from tr_driver_vehicle where dir_v_id=@dir_v_id";
                obj.Param = dicParam;
                listSql.Add(obj);
                DBHelper.BatchExeSQLMultiByTrans("删除驾驶员信息", listSql);
                dgvUser.Rows.RemoveAt(introw);
            }
            else
            {
                if (introw != dgvUser.Rows.Count - 1)
                {
                    dgvUser.Rows.RemoveAt(introw);
                }
            }
          
            listUser.Remove(strcontName);
        }
        #endregion
