...
namespace HXCPcClient.UCForm.DataManage.WareHouse
{
    public partial class UCWareHouseAddOrEdit : UCBase
    {
        ...
        private void SetBtnStatus()
        {
            ...
            {
                SetDataEditBtn();
                if (tb_warehouse_Model.status == ((int)DataSources.EnumStatus.Start).ToString())
                {
        ...
        private void SetQuick()
        {
            ...
            txtChooseWhHead.SetBindTable("sys_user", "user_name");
            txtChooseWhHead.GetDataSourced += new ServiceStationClient.ComponentUI.TextBox.TextChooser.GetDataSourceHandler(txtChooseWhHead_GetDataSourced);
            txtChooseWhHead.DataBacked += new ServiceStationClient.ComponentUI.TextBox.TextChooser.DataBackHandler(txtChooseWhHead_DataBacked);
        }

        void txtChooseWhHead_DataBacked(DataRow dr)
        {
            txtChooseWhHead.Text = dr["user_name"].ToString();
            txtChooseWhHead.Tag = dr["user_id"].ToString();
        }

        void txtChooseWhHead_GetDataSourced(ServiceStationClient.ComponentUI.TextBox.TextChooser tc, string sqlString)
        {
            sqlString = string.Format("select * from sys_user where user_name like '%{0}%' and enable_flag=1 and status=1", txtChooseWhHead.Text);
        ...
        void UCWareHouseAddOrEdit_StatusEvent(object sender, EventArgs e)
        {
            SetStatus();
        }

        void UCWareHouseAddOrEdit_DeleteEvent(object sender, EventArgs e)
        {
            Delete();
        }
        ...
        private bool StatusSql()
        {
            ...
            return DBHelper.Submit_AddOrEdit(btnStatus.Caption + "仓库", "tb_warehouse", "wh_id", tb_warehouse_Model.wh_id, status);
        }
        ...
        private void Delete()
        {
            if (MessageBoxEx.Show("是否要删除当前数据？", "提示", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                if (DBHelper.Submit_AddOrEdit("删除供仓库", "tb_warehouse", "wh_id", tb_warehouse_Model.wh_id, updateField))
                {
