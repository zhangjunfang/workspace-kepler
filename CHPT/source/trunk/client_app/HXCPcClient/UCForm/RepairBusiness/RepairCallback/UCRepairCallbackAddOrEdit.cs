    ...
    public partial class UCRepairCallbackAddOrEdit : UCBase
    {
        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            try
            {
                ...
                dicParam.Add("driver_name", new ParamObj("driver_name", txtDriver.Caption.Trim(), SysDbType.NVarChar, 20));//报修人
                if (!string.IsNullOrEmpty(txtDriverPhone.Caption.Trim()))//报修人手机
                {
                ...
                dicParam.Add("turner", new ParamObj("turner", !string.IsNullOrEmpty(CommonCtrl.IsNullToString(txtturner.Caption)) ? txtturner.Caption.ToString() : null, SysDbType.VarChar, 40));//颜色
                dicParam.Add("customer_code", new ParamObj("customer_code", txtCustomNO.Text, SysDbType.VarChar, 40));//客户编码
                dicParam.Add("customer_name", new ParamObj("customer_name", txtCarNO.Text.Trim(), SysDbType.NVarChar, 100));//客户名称
                dicParam.Add("customer_id", new ParamObj("customer_id", txtCustomNO.Tag.ToString(), SysDbType.VarChar, 40));//客户Id                
                ...
                dicParam.Add("repair_describe", new ParamObj("repair_describe", txtRepairDesc.Caption.Trim(), SysDbType.NVarChar, 200));//返修原因描述
                dicParam.Add("dispose_opinion", new ParamObj("dispose_opinion", txtOpinion.Caption.Trim(), SysDbType.NVarChar, 200));//故障查询及处理意见
                dicParam.Add("dispose_result", new ParamObj("dispose_result", txtResult.Caption.Trim(), SysDbType.NVarChar, 200));//处理结果
                if (status == DataSources.EnumAuditStatus.SUBMIT)//提交操作时生成单号
                {
                    dicParam.Add("repair_no", new ParamObj("repair_no",!string.IsNullOrEmpty(labReserveNoS.Text.Trim())?labReserveNoS.Text.Trim(): CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.RepairCallback), SysDbType.VarChar, 40));//单据编号 
                    //单据状态
                ...
                if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
                {
                    ...
                    ,turner,customer_code,customer_name,customer_id,linkman,linkman_mobile,responsible_opid,responsible_name,org_id,enable_flag,document_status,repairer_name
                    ,repairer_id,mileage,repair_describe,dispose_opinion,dispose_result,repair_id,create_by,create_name,create_time,repair_no,import_status)
                ...
                else if (wStatus == WindowStatus.Edit)
                {
                    ...
                    ,turner=@turner,customer_code=@customer_code,customer_name=@customer_name,customer_id=@customer_id,linkman=@linkman,linkman_mobile=@linkman_mobile,responsible_opid=@responsible_opid,responsible_name=@responsible_name,org_id=@org_id,enable_flag=@enable_flag,document_status=@document_status,repairer_name=@repairer_name
                    ,repairer_id=@repairer_id,mileage=@mileage,repair_describe=@repair_describe,dispose_opinion=@dispose_opinion,dispose_result=@dispose_result,import_status=@import_status,update_by=@update_by,update_name=@update_name,update_time=@update_time where repair_id=@repair_id";
                }
                obj.Param = dicParam;
                listSql.Add(obj);
            }
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    for (int i = 0; i <= intProws; i++)
                    {
                        if (string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["item_no"].Value)))
                        {
                            ...
                            //会员工时费
                            dgvproject.Rows[i].Cells["member_price"].Value = Convert.ToString(Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strQuotaPrice)?frmHours.strQuotaPrice:"0.00") * (Convert.ToDecimal(strPzk) / 100));
                            if (dgvproject.Rows[i].Cells["member_price"].Value.ToString() != "0")
                            {
                                //折扣额
                                dgvproject.Rows[i].Cells["member_sum_money"].Value = Convert.ToString(Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strQuotaPrice)?frmHours.strQuotaPrice:"0.00") - Convert.ToDecimal(dgvproject.Rows[i].Cells["member_price"].Value));
                            }
                            else
                            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    for (int i = 0; i <= intProws; i++)
                    {
                        if (string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["item_no"].Value)))
                        {
                            ...
                            dgvproject.Rows[i].Cells["whours_id"].Value = frmHours.strWhours_id;
                            dgvproject.Rows[i].Cells["man_hour_quantity"].Value = frmHours.strWhoursNum;
                            dgvproject.Rows[i].Cells["sum_money_goods"].Value = Convert.ToString(Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strWhoursNum)?frmHours.strWhoursNum:"0.0") * Convert.ToDecimal(!string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["member_price"].Value)) ? CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["member_price"].Value) : "0"));
                            dgvproject.Rows[i].Cells["three_warranty"].Value = "否";
                            dgvproject.Rows[i].Cells["data_source"].Value = frmHours.strData_source;
                            dgvproject.Rows[i].Cells["three_warranty"].ReadOnly = frmHours.strData_source == Convert.ToInt32(DataSources.EnumDataSources.SELFBUILD).ToString() ? true : false;
                            dgvproject.Rows.Add(1);
                ...
                if (intCurrentIndex >= 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(strItemNo))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            //会员工时费
                            dgvproject.Rows[intCurrentIndex].Cells["member_price"].Value = Convert.ToString(Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strQuotaPrice)?frmHours.strQuotaPrice:"0.00") * (Convert.ToDecimal(strPzk) / 100));
                            if (dgvproject.Rows[intCurrentIndex].Cells["member_price"].Value.ToString() != "0")
                            {
                                //折扣额
                                dgvproject.Rows[intCurrentIndex].Cells["member_sum_money"].Value = Convert.ToString(Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strQuotaPrice)?frmHours.strQuotaPrice:"0.00") - Convert.ToDecimal(dgvproject.Rows[intCurrentIndex].Cells["member_price"].Value));
                            }
                            else
                            {
                    ...
                    if (!string.IsNullOrEmpty(strPcode))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            if (dpt.Rows.Count > 0)
                            {
                                ...
                                string strUMoney = dgvMaterials.Rows[intMrows].Cells["unit_price"].Value != null ? dgvMaterials.Rows[intMrows].Cells["unit_price"].Value.ToString() : "0";
                                //会员单价
