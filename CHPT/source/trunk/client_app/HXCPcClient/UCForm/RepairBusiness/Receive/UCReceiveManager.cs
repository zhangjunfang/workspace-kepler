...
namespace HXCPcClient.UCForm.RepairBusiness.Receive
{
    ...
    public partial class UCReceiveManager : UCBase
    {
        ...
        public UCReceiveManager()
        {
            ...
            base.PrintEvent += new ClickHandler(UCReceiveManager_PrintEvent);
            base.SetEvent += new ClickHandler(UCReceiveManager_SetEvent);
            SetQuick();
            base.SetContentMenuScrip(dgvRData);
            #region 预览、打印设置
            string printObject = "tb_maintain_info";
            string printTitle = "维修接待单";
        ...
        #region 设置事件
        void UCReceiveManager_SetEvent(object sender, EventArgs e)
        {
        ...
        #region 打印事件
        void UCReceiveManager_PrintEvent(object sender, EventArgs e)
        {
        ...
        private void UpdateMaintainInfo(List<SQLObj> listSql, string strReservId,string strSource, string status,string strMaintain_no="")
        {
            ...
            if (!string.IsNullOrEmpty(strReservId) && !string.IsNullOrEmpty(strSource))
            {
                ...
                else if (strSource == "2")//返修单
                {
                    ...
                    if (status == "2")
                    {
                        ...
                        dicParam.Add("import_status", new ParamObj("import_status", "2", SysDbType.VarChar, 40));//锁定
                        obj.sqlString = "update tb_maintain_back_repair set import_status=@import_status ,update_by=@update_by,update_name=@update_name,update_time=@update_time where repair_id=@repair_id";
                    }
                    else if (status == "1")
                    {
                        //删除时，前置单据中的维修编号置空、前置状体置为开放0
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "维修接待单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, dgvRData);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【维修接待单】" + ex.Message, "server");
