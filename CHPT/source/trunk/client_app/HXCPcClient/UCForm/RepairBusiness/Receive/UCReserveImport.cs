...
namespace HXCPcClient.UCForm.RepairBusiness.Receive
{
    ...
    /// AddTime：2014.10.9
    /// </summary>
    public partial class UCReserveImport : FormChooser
    {
        #region 属性设置
        /// <summary>
        /// enable_flag 1未删除
        ...
        /// 新的维修单号
        /// </summary>
        string strNewMaintainNo = string.Empty;

        public UCReceiveAddOrEdit uc;
        #endregion

        #region 初始化
        public UCReserveImport()
        {
            InitializeComponent();
            ...
            dgvRData.Columns["reservation_no"].ReadOnly = true;
            dgvRData.Columns["reservation_date"].ReadOnly = true;
            dgvRData.Columns["customer_code"].ReadOnly = true;
            dgvRData.Columns["customer_name"].ReadOnly = true;
            dgvRData.Columns["reservation_mobile"].ReadOnly = true;
            dgvRData.Columns["reservation_man"].ReadOnly = true;
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            txtRepPersonPhone.Caption = string.Empty;
            dtpReserveSTime.Value = DateTime.Now.AddMonths(-1);
            dtpReserveETime.Value = DateTime.Now;
            txtCarNO.Focus();
        }
        ...
        private void BindData()
        {
            try
            {
                ...
                #endregion
                string strFiles = " reservation_no,reservation_date,customer_code,customer_name,reservation_mobile,reservation_man,vehicle_vin,vehicle_no,reserv_id";
                strWhere = string.Format(" enable_flag='1' and (document_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.AUDIT).ToString() + "' or document_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.SUBMIT).ToString() + "') and import_status='" + Convert.ToInt32(DataSources.EnumImportStaus.OPEN).ToString() + "'");//enable_flag 1未删除
        ...
        private void dgvRData_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (fieldNmae.Equals("reservation_date"))
            {
        ...
        private void dgvRData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                uc.strBefore_orderId = dgvRData.Rows[e.RowIndex].Cells["reserv_id"].Value.ToString();  //导入单据的Id值
                uc.strOrdersSouse = "1";
                uc.BindReserveData();
                this.Close();
            }
        ...
        private void dgvRData_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                uc.strBefore_orderId = dgvRData.CurrentRow.Cells["reserv_id"].Value.ToString();  //导入单据的Id值
                uc.strOrdersSouse = "1";
                uc.BindReserveData();
                this.Close();
            }
