...
namespace HXCPcClient.UCForm.RepairBusiness.RepairBalance
{
    ...
    public partial class UCRepairBalanceAddOrEdit : UCBase
    {
        ...
        public UCRepairBalanceAddOrEdit()
        {
            ...
            CommonFuncCall.BindBalanceWayByItem(cobBalanceType, ""); ;//结算方式
            CommonFuncCall.BindAccount(cobSetAccount, "");//结算帐户
            SetTopbuttonShow();
        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            try
            {
                ...
                dicParam.Add("driver_name", new ParamObj("driver_name", txtDriver.Caption.Trim(), SysDbType.NVarChar, 20));//司机
                ...
                dicParam.Add("turner", new ParamObj("turner", !string.IsNullOrEmpty(CommonCtrl.IsNullToString(txtturner.Caption.Trim())) ? txtturner.Caption.Trim() : null, SysDbType.VarChar, 40));//车厂编码
                ...
                dicParam.Add("remark", new ParamObj("remark", txtRemark.Caption.Trim(), SysDbType.NVarChar, 200));//备注
                dicParam.Add("favorable_reason", new ParamObj("favorable_reason", txtPreferReason.Caption.Trim(), SysDbType.NVarChar, 100));//优惠原因
                ...
                {
                    dicParam.Add("maintain_no", new ParamObj("maintain_no",!string.IsNullOrEmpty(labReserveNoS.Text.Trim())?labReserveNoS.Text.Trim():CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.Repair), SysDbType.VarChar, 40));//维修单号               
                    //单据状态
                    ...
                    if (!string.IsNullOrEmpty(strBefore_orderId))
                    {
                        ...
                        dicParam.Add("Import_status", new ParamObj("Import_status", Convert.ToInt32(DataSources.EnumImportStaus.LOCK).ToString(), SysDbType.VarChar, 1));  
                    }
                    else
                    {
                        ...
                        dicParam.Add("before_orderId", new ParamObj("before_orderId", labReserveNoS.Text.Trim(), SysDbType.VarChar, 40));
                    }
                }
                else
                {
                ...
                if ((wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy) && string.IsNullOrEmpty(strBefore_orderId))
                {
                    ...
                    obj.sqlString = @"insert into tb_maintain_info (vehicle_no,vehicle_vin,engine_no,vehicle_model,vehicle_brand,driver_name,driver_mobile,turner,travel_mileage
                 ,oil_into_factory,maintain_mileage,completion_time,maintain_payment,maintain_type,remark,favorable_reason,customer_code,customer_id,customer_name,linkman,link_man_mobile,responsible_opid
                 ,responsible_name,org_id,enable_flag,info_status,maintain_no,maintain_id,create_by,create_name,create_time,orders_source,Import_status,before_orderId)
                 values (@vehicle_no,@vehicle_vin,@engine_no,@vehicle_model,@vehicle_brand,@driver_name,@driver_mobile,@turner,@travel_mileage
                 ,@oil_into_factory,@maintain_mileage,@completion_time,@maintain_payment,@maintain_type,@remark,@favorable_reason,@customer_code,@customer_id,@customer_name,@linkman,@link_man_mobile,@responsible_opid
                 ,@responsible_name,@org_id,@enable_flag,@info_status,@maintain_no,@maintain_id,@create_by,@create_name,@create_time,@orders_source,@Import_status,@before_orderId);";
                }
                else if (wStatus == WindowStatus.Edit || !string.IsNullOrEmpty(strBefore_orderId))
                {
                    ...
                    obj.sqlString = @"update tb_maintain_info set vehicle_no=@vehicle_no,vehicle_vin=@vehicle_vin,engine_no=@engine_no,vehicle_model=@vehicle_model,vehicle_brand=@vehicle_brand,driver_name=@driver_name,driver_mobile=@driver_mobile,turner=@turner,travel_mileage=@travel_mileage
                 ,oil_into_factory=@oil_into_factory,maintain_mileage=@maintain_mileage,completion_time=@completion_time,maintain_payment=@maintain_payment,maintain_type=@maintain_type,remark=@remark,favorable_reason=@favorable_reason,customer_code=@customer_code,customer_id=@customer_id,customer_name=@customer_name,linkman=@linkman,link_man_mobile=@link_man_mobile,responsible_opid=@responsible_opid
                 ,responsible_name=@responsible_name,org_id=@org_id,enable_flag=@enable_flag,info_status=@info_status,maintain_no=@maintain_no,update_by=@update_by,update_name=@update_name,update_time=@update_time,orders_source=@orders_source,Import_status=@Import_status,before_orderId=@before_orderId where maintain_id=@maintain_id";
                }
                obj.Param = dicParam;
                listSql.Add(obj);
            }
        ...
        private void SaveBalanceInfo(List<SQLObj> listSql, string strOrderId)
        {
            try
            {
                ...
                dicParam.Add("payment_terms", new ParamObj("payment_terms", !string.IsNullOrEmpty(CommonCtrl.IsNullToString(cobBalanceType.SelectedValue)) ? cobBalanceType.SelectedValue.ToString() : null, SysDbType.VarChar, 40));//结算方式
                ...
                ,make_invoice_type,payment_terms,settlement_account,settle_company,maintain_id,settlement_id,responsible_opid,org_id,create_by,create_name,create_time)
                values (@man_hour_sum_money,@man_hour_tax_rate,@man_hour_tax_cost,@man_hour_sum,@fitting_sum_money,@fitting_tax_rate
                ,@fitting_tax_cost,@fitting_sum,@other_item_sum_money,@other_item_tax_rate,@other_item_tax_cost,@other_item_sum,@privilege_cost,@should_sum,@received_sum
                ,@make_invoice_type,@payment_terms,@settlement_account,@settle_company,@maintain_id,@settlement_id,@create_by,@create_name,@create_time);";
                }
                ...
                ,make_invoice_type=@make_invoice_type,payment_terms=@payment_terms,settlement_account=@settlement_account,settle_company=@settle_company,update_by=@update_by,update_name=@update_name,update_time=@update_time,responsible_opid=@responsible_opid,org_id=@org_id where maintain_id=@maintain_id";
                }
                obj.Param = dicParam;
                listSql.Add(obj);
            }
        ...
        private void GetMemberInfo(string strCid)
        {
            ...
            labMemberNoS.Text = mInfo.strMemberNo;
            labMemberGradeS.Text = mInfo.strMemberGrade;
            labMemberPZkS.Text =!string.IsNullOrEmpty(mInfo.strMemberPZk)?mInfo.strMemberPZk+"%":"";
            labMemberLZkS.Text =!string.IsNullOrEmpty(mInfo.strMemberLZk)?mInfo.strMemberLZk+"%":"";
        ...
        private void BindData()
        {
            try
            {
                ...
                #region 如果具有前置单据则设置部分数据不能修改
                if (wStatus != WindowStatus.Copy)
                {
                    string strBid = CommonCtrl.IsNullToString(dr["before_orderId"]);
                    if (!string.IsNullOrEmpty(strBid))
                    {
                        strBefore_orderId = strBid;
        ...
        public void BindSetmentData()
        {
            try
            {
                ...
                txtCustomNO.Tag = CommonCtrl.IsNullToString(dr["customer_id"]);//客户关联id
                GetMemberInfo(CommonCtrl.IsNullToString(dr["customer_id"]));
                txtBalanceCompany.Text = CommonCtrl.IsNullToString(dr["customer_name"]);//结算单位
                txtBalanceCompany.Tag = CommonCtrl.IsNullToString(dr["customer_id"]);//结算单位关联id               
                txtContact.Caption = CommonCtrl.IsNullToString(dr["linkman"]);//联系人
                txtContact.Enabled = false;
                txtContactPhone.Caption = CommonCtrl.IsNullToString(dr["link_man_mobile"]);//联系人手机  
                txtContactPhone.Enabled = false;
        ...
        {
            string strHRate = txtHTaxRate.Caption.Trim();
            if (strHRate.Length > 0)
            {
                if (strHRate.Substring(0, 1) == "0" && strHRate.Length > 1)
                {
                    txtHTaxRate.Caption = strHRate.Substring(1, strHRate.Length - 1);
                    txtHTaxRate.SelectStart = strHRate.Length;
                }
            }
        ...
        {
            string strFRate = txtFTaxRate.Caption.Trim();
            if (strFRate.Length > 0)
            {
                if (strFRate.Substring(0, 1) == "0" && strFRate.Length > 1)
                {
                    txtFTaxRate.Caption = strFRate.Substring(1, strFRate.Length - 1);
                    txtFTaxRate.SelectStart = strFRate.Length;
                }
            }
        ...
        {
            string strORate = txtOTaxRate.Caption.Trim();
            if (strORate.Length > 0)
            {
                if (strORate.Substring(0, 1) == "0" && strORate.Length > 1)
                {
                    txtOTaxRate.Caption = strORate.Substring(1, strORate.Length - 1);
                    txtOTaxRate.SelectStart = strORate.Length;
                }
            }
            ...
            try
            {
                ...
                if (dgvMaterials.Rows[e.RowIndex].Cells["parts_name"].Value != null)
                {
                    if (e.ColumnIndex == 7 || e.ColumnIndex == 8)
                    {
                        ...
                        dgvMaterials.Rows[e.RowIndex].Cells["quantity"].Value = !string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvMaterials.Rows[e.RowIndex].Cells["quantity"].Value)) ? Math.Round((Convert.ToDecimal(dgvMaterials.Rows[e.RowIndex].Cells["quantity"].Value)), 2).ToString("0.00") : "0.00";
        ...
        #region 选择结算方式,修改结算帐户为默认帐户
        private void cobBalanceType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cobBalanceType.SelectedItem == null)
            {
                return;
            }
            DataRowView drv = (DataRowView)cobBalanceType.SelectedItem;
            cobSetAccount.SelectedValue = drv["default_account"];
        }
        #endregion
                    ...
                    if (!string.IsNullOrEmpty(strPcode))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            if (dpt.Rows.Count > 0)
                            {
                                ...
                                string strNum = !string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvMaterials.Rows[intMrows].Cells["quantity"].Value)) ? Math.Round(Convert.ToDecimal(CommonCtrl.IsNullToString(dgvMaterials.Rows[intMrows].Cells["quantity"].Value)), 2).ToString("0.0") : "0.0";
                                string strUMoney = !string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvMaterials.Rows[intMrows].Cells["unit_price"].Value)) ? dgvMaterials.Rows[intMrows].Cells["unit_price"].Value.ToString() : "0.00";
                                //会员单价
