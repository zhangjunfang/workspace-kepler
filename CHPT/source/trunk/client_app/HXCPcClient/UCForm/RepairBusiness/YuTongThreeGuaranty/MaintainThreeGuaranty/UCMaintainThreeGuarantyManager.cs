...
namespace HXCPcClient.UCForm.RepairBusiness.YuTongThreeGuaranty.MaintainThreeGuaranty
{
    ...
    public partial class UCMaintainThreeGuarantyManager : UCBase
    {
        ...
        {
            btnEdit.Enabled= btnDelete.Enabled = btnStatus.Enabled = btnVerify.Enabled = btnConfirm.Enabled = btnActivation.Enabled = btnCommit.Enabled = btnRevoke.Enabled = btnSubmit.Enabled = false;
        ...
        private void ConvertDataGridColumnsData(DataGridViewCellFormattingEventArgs args) //转换数据列数据
        {
            ...
            UIAssistants.DgvCellDataConvert2DicData(dgv_table, args, "review_status_name_yt"); //宇通同步状态
        ...
        public void BindPageData(string where = "")
        {
            try
            {
                ...
                int recordCount;
                var dt = DBHelper.GetTableByPage("分页查询三包服务单信息", "v_maintain_three_guaranty_approve", "*",pnl_search.QueryString + where, "", "repairs_time desc", pageQ.PageIndex, pageQ.PageSize, out recordCount);
                dgv_table.DataSource = dt;
                pageQ.RecordCount = recordCount;
            }
            catch (Exception ex)
            {
                MessageBoxEx.Show(ex.Message, "查询三包服务单信息错误！");
            }
        }
