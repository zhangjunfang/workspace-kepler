    ...
    public partial class UCMaintainThreeGuarantyEdit : UCBase
    {
        ...
        private void InitEventHandle() //注册控件相关事件
        {
            ...
            btnImport.Click += delegate
            {
                ...
                {
                    BeforeOrderId = import.SelecttionMaintainId;
                    BindSetmentData();
                }
                return;
            };
        ...
        private void InitControlDataSource() //初始化数据字段控件数据
        {
            try
            {
                ...
                //}
                chk_whether_second_station_true.Checked = false;
                chk_whether_second_station_false.Checked = true;
                #endregion
                ...
                txt_appraiser_name.ChooserClick += delegate   //鉴定人选择器
                {
                    ...
                    chooser.AppendWhere = " and len(cont_crm_guid) > 0";
                ...
                txt_vehicle_use_corp.ChooserClick += delegate  //车辆使用单位选择器
                {
                    ...
                    custChooser.AppendWhere = " and len(sap_code) > 0";
                    ...
                    if (result == DialogResult.OK)
                    {
                        ...
                        txt_vehicle_use_corp.Tag = custChooser.CustomerInfo.SAPCode;
                            ...
                            //设置客户编码速查
                txt_customer_code.SetBindTable("tb_customer", "cust_code", "cust_code");
                txt_customer_code.GetDataSourced += new ServiceStationClient.ComponentUI.TextBox.TextChooser.GetDataSourceHandler(tc_GetDataSourced);
                txt_customer_code.DataBacked += new TextChooser.DataBackHandler(txtCustomNO_DataBacked);
                txt_customer_code.ChooserClick += delegate  //客户信息选择器
                {
                    var custChooser = new frmCustomerInfo();
                    custChooser.AppendWhere = " and len(cust_code) > 0";
                    var result = custChooser.ShowDialog();
                    if (result == DialogResult.OK)
                    {
                ...
                txt_vehicle_no.ChooserClick += delegate  //车辆选择器
                {
                    ...
                    chooser.AppendWhere = " and len(license_plate) > 0";
                    var result = chooser.ShowDialog();
                    if (result != DialogResult.OK) return;
                ...
                dtp_complete_work_time.ValueChanged += delegate
                {
                    if (dtp_complete_work_time.Value > dtp_repair_completion_time.Value)
                    {
                        MessageBoxEx.Show("维修结束时间不能大于整车完工时间", "操作提醒", MessageBoxButtons.OK);
                        dtp_complete_work_time.Value = dtp_repair_completion_time.Value;
                    }
                };
            }
            catch (Exception ex)
        ...
        {
            txt_customer_code.Text = CommonCtrl.IsNullToString(dr["cust_code"]);
            txt_customer_code.Tag = CommonCtrl.IsNullToString(dr["cust_id"]);
        }
        ...
        private void InitDataGridCellFormatting()   //初始化数据表格
        {
            try
            {
                ...
                dgv_tb_maintain_three_guaranty_item.CellClick += delegate(object sender, DataGridViewCellEventArgs args)
                {
                    ...
                    var frm = new UCMaintainWorkhourImport();
                    if (String.IsNullOrEmpty(BeforeOrderId))
                    {
                        MessageBoxEx.Show("无法获取维修单信息,请先导入维修单！");
                        return;
                    }
                    frm.MaintainId = BeforeOrderId;
                    var result1 = frm.ShowDialog();
                    if (result1 != DialogResult.OK) return;
                    for (var i = 0; i < dgv_tb_maintain_three_guaranty_item.Rows.Count; i++)
                    {
                        var value = dgv_tb_maintain_three_guaranty_item.Rows[i].Cells[drtxt1_pre_order_id.Name].Value;
                        if (value != null && value.ToString() == frm.ItemId)
                        {
                            MessageBoxEx.Show("已经导入的维修项目,不允许重复导入！");
                            return;
                        }
                    }
                    dgv_tb_maintain_three_guaranty_item.Rows[args.RowIndex].Cells[drtxt1_pre_order_id.Name].Value = frm.ItemId;
                ...
                dgv_tb_maintain_three_matrial_detail.CellClick += delegate(object sender, DataGridViewCellEventArgs args)
                {
                    ...
                    var frmPart = new UCMaintainMatrialDetailImport();
                    if (String.IsNullOrEmpty(BeforeOrderId))
                    {
                        MessageBoxEx.Show("无法获取维修单信息,请先导入维修单！");
                        return;
                    }
                    frmPart.MaintainId = BeforeOrderId;
                    var result1 = frmPart.ShowDialog();
                    if (result1 != DialogResult.OK) return;
                    for (var i = 0; i < dgv_tb_maintain_three_matrial_detail.Rows.Count; i++)
                    {
                        var value = dgv_tb_maintain_three_matrial_detail.Rows[i].Cells[drtxt2_pre_order_id.Name].Value;
                        if (value != null && value.ToString() == frmPart.ItemId)
                        {
                            MessageBoxEx.Show("已经导入的维修项目,不允许重复导入！");
                            return;
                        }
                    ...
                    var dpr = dpt.Rows[0];
                    dgv_tb_maintain_three_matrial_detail.Rows[args.RowIndex].Cells[drtxt2_pre_order_id.Name].Value = frmPart.ItemId;
                    ...
                    dgv_tb_maintain_three_matrial_detail.Rows[args.RowIndex].Cells["drtxt2_redeploy_no"].Value = CommonCtrl.IsNullToString(dpr["highest_out_price"]);   //三包调件号 unknow
        ...
        private Boolean CheckControlValue() //数据必填项非空判断
        {
            ...
            if (cbo_receipt_type.SelectedValue == null)
            {
                ...
                erp_error.Clear(); erp_error.SetError(cbo_receipt_type,"宇通单据类型不允许为空!!!");
                return false;
            }
            #endregion
            ...
            if (selectedValue == DbDic2Enum.BILL_TYPE_YT_100000005)
            {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(cbo_approver_name_yt,"宇通批准人不允许为空!");
                    return false;
                }
                #endregion
                ...
                {
                    erp_error.Clear(); erp_error.SetError(cbo_cost_type_service_zczg,"费用类型不允许为空!");
                    return false;
                }
                #endregion
                ...
                {
                    erp_error.Clear(); erp_error.SetError(txt_describes,"情况简介不允许为空!");
                    return false;
                }
                #endregion
            }
            ...
            if (selectedValue == DbDic2Enum.BILL_TYPE_YT_100000006)
            {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(cbo_product_notice_no,"产品改进通知不允许为空!");
                    return false;
                }
                #endregion
            }
            ...
            if (selectedValue == DbDic2Enum.BILL_TYPE_YT_100000004)    //服务活动
            {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(cbo_cost_type_service_fwhd, "费用类型不允许为空!");
                    return false;
                }
                #endregion
            }
            ...
            if (selectedValue != DbDic2Enum.BILL_TYPE_YT_100000007) //配件三包
            {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(txt_customer_code,"客户编码不允许为空!");
                    return false;
                }
                if (cbo_customer_property.SelectedIndex == -1)
                {
                    erp_error.Clear(); erp_error.SetError(cbo_customer_property,"客户性质不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_customer_address.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_customer_address,"客户地址不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_vehicle_use_corp.Tag)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_vehicle_use_corp,"车辆使用单位不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_vehicle_location.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_vehicle_location,"车辆所在地不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_repairer_name.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_repairer_name,"报修人不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_repairer_mobile.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_repairer_mobile,"报修人手机不允许为空!");
                    return false;
                }
                if (!txt_repairer_mobile.Verify())
                {
                    erp_error.Clear(); erp_error.SetError(txt_repairer_mobile, "报修人手机格式校验位通过");
                    return false;
                }
                if (!txt_customer_postcode.Verify())
                {
                    erp_error.Clear(); erp_error.SetError(txt_customer_postcode, "客户邮编格式校验位通过");
                    return false;
                }
                if (!txt_link_man_mobile.Verify())
                {
                    erp_error.Clear(); erp_error.SetError(txt_link_man_mobile, "联系人手机格式校验位通过");
                    return false;
                }
                #endregion
                ...
                {
                    erp_error.Clear(); erp_error.SetError(txt_vehicle_no,"车牌号不允许为空!");
                    return false;
                }

                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_vehicle_model.Tag)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_vehicle_model,"车型不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_travel_mileage.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_travel_mileage,"行驶里程不允许为空!");
                    return false;
                }
                #endregion
            }
            ...
            {
                erp_error.Clear(); erp_error.SetError(txt_depot_no,"车工号不允许为空!");
                return false;
            }
            #endregion
            ...
            {
                erp_error.Clear(); erp_error.SetError(txt_vehicle_vin,"VIN不允许为空!");
                return false;
            }
            #endregion
            ...
            {
                erp_error.Clear(); erp_error.SetError(txt_engine_num, "发动机号不允许为空!");
                return false;
            }
            #endregion
            ...
            {
                erp_error.Clear(); erp_error.SetError(dtp_start_work_time, "维修开始时间不允许为空!");
                return false;
            }
            #endregion
            ...
            {
                erp_error.Clear(); erp_error.SetError(dtp_complete_work_time, "维修结束时间不允许为空!");
                return false;
            }
            #endregion
            #region 维修结束时间
            if (String.IsNullOrEmpty(dtp_repair_completion_time.Value.ToString()))
            {
                erp_error.Clear(); erp_error.SetError(dtp_repair_completion_time, "整车完工时间不允许为空!");
                return false;
            }
            #endregion
            ...
            {
                erp_error.Clear(); erp_error.SetError(txt_appraiser_name, "鉴定人不允许为空!");
                return false;
            }
            #endregion
                ...
                selectedValue == DbDic2Enum.BILL_TYPE_YT_100000004)
            {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(cbo_fault_duty_corp,"故障责任单位不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_fault_system.Tag)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_fault_system,"故障系统不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_fault_assembly.Caption)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_fault_assembly,"故障总成不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_fault_part.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_fault_part,"故障部件不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_fault_schema.Tag)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_fault_schema,"故障模式不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_fault_describe.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_fault_describe,"故障描述不允许为空!");
                    return false;
                }
                if (cbo_fault_cause.SelectedIndex == -1)
                {
                    erp_error.Clear(); erp_error.SetError(cbo_fault_cause,"故障原因不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_dispose_result.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_dispose_result,"处理结果不允许为空!");
                    return false;
                }
            }
            #endregion
            ...
            if (Equals(cbo_whether_go_out.SelectedValue.ToString(), DbDic2Enum.TRUE))
            {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(txt_goout_cause,"外出事由不允许为空!");
                    return false;
                }
                if (cbo_goout_approver.SelectedIndex == -1)
                {
                    erp_error.Clear(); erp_error.SetError(cbo_goout_approver,"外出批准人不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_goout_place.Caption)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_goout_place,"外出地点不允许为空!");
                    return false;
                }
                if (cbo_means_traffic.SelectedIndex == -1)
                {
                    erp_error.Clear(); erp_error.SetError(cbo_means_traffic,"交通方式不允许为空!");
                    return false;
                }
                if (cbo_means_traffic.SelectedIndex == -1)
                {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(dtp_goout_time,"外出时间不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(dtp_goout_back_time.Value.ToString()))
                {
                    erp_error.Clear(); erp_error.SetError(dtp_goout_back_time,"外出回长时间不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(txt_goout_mileage.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_goout_mileage,"外出里程不允许为空!");
                    return false;
                }
                ...
                if (selectedValue4GooutMan == DbDic2Enum.TRUE)
                {
                    ...
                    {
                        erp_error.Clear(); erp_error.SetError(txt_goout_people_num,"外出人数必须为1到3之间!");
                        return false;
                    }
                }
                #endregion
                ...
                {
                    erp_error.Clear(); erp_error.SetError(txt_journey_subsidy,"路程补助不允许为空!");
                    return false;
                }

                if (String.IsNullOrEmpty(txt_ccf.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_ccf,"车船费不允许为空!");
                    return false;
                }

                if (String.IsNullOrEmpty(txt_man_hour_subsidy.Caption))
                {
                    erp_error.Clear(); erp_error.SetError(txt_man_hour_subsidy,"工时补助不允许为空!");
                    return false;
                }
            ...
            if (selectedValue == DbDic2Enum.BILL_TYPE_YT_100000007) //配件三包
            {
                ...
                {
                    erp_error.Clear(); erp_error.SetError(dtp_parts_buy_time ,"配件购买日期不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_parts_buy_corp.Caption)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_parts_buy_corp,"配件购买单位不允许为空!");
                    return false;
                }
                if (cbo_contain_man_hour_cost.SelectedIndex == -1)
                {
                    erp_error.Clear(); erp_error.SetError(cbo_contain_man_hour_cost,"是否包含工时费不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_parts_code.Text)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_parts_code,"配件编号不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_materiel_describe.Caption)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_materiel_describe,"物料描述不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_first_install_station.Caption)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_first_install_station,"首次安装服务站不允许为空!");
                    return false;
                }
                if (cbo_part_guarantee_period.SelectedIndex == -1)
                {
                    erp_error.Clear(); erp_error.SetError(cbo_part_guarantee_period,"协议保养日期不允许为空!");
                    return false;
                }
                if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(txt_feedback_num.Caption)))
                {
                    erp_error.Clear(); erp_error.SetError(txt_feedback_num,"反馈数量不允许为空!");
                    return false;
                }
            }
            #endregion
            ...
            if (!txt_man_hour_subsidy.Verify())
            {
                erp_error.Clear(); erp_error.SetError(txt_man_hour_subsidy, "数据格式校验位通过");
                return false;
            }

            if (!txt_ccf.Verify())
            {
                erp_error.Clear(); erp_error.SetError(txt_ccf, "数据格式校验位通过");
                return false;
            }

            if (!txt_journey_subsidy.Verify())
            {
                erp_error.Clear(); erp_error.SetError(txt_journey_subsidy, "数据格式校验位通过");
                return false;
            }

            if (!txt_maintain_mileage.Verify())
            {
                erp_error.Clear(); erp_error.SetError(txt_maintain_mileage, "协议保养里程格式校验位通过");
                return false;
            }

            if (!txt_travel_mileage.Verify())
            {
                erp_error.Clear(); erp_error.SetError(txt_travel_mileage, "行驶里程格式校验位通过");
                return false;
            }

            if (!txt_feedback_num.Verify())
            {
                erp_error.Clear(); erp_error.SetError(txt_feedback_num, "反馈数量格式校验位通过");
                return false;
            }
            #endregion
            return true;
        ...
        private Boolean SaveThreeGuarantyItem(String infoStatus)
        {
            ...
            var dicFields = new Dictionary<string, string>();
            dicFields.Add("pre_order_id", BeforeOrderId);               //前置单据ID -- varchar(40)
            ...
            dicFields.Add("start_work_time", Common.LocalDateTimeToUtcLong(dtp_start_work_time.Value).ToString());         //维修开始时间 -- bigint
            dicFields.Add("complete_work_time", Common.LocalDateTimeToUtcLong(dtp_complete_work_time.Value).ToString());   //维修结束时间 -- bigint
            dicFields.Add("repair_completion_time", Common.LocalDateTimeToUtcLong(dtp_repair_completion_time.Value).ToString());   //维修结束时间 -- bigint
            ...
            dicFields.Add("parts_buy_corp", CommonCtrl.IsNullToString(txt_parts_buy_corp.Caption));                                               //配件购买单位 -- varchar(40)
            ...
            dicFields.Add("first_install_station", txt_first_install_station.Caption);                                 //配件首次安装服务站 -- varchar(40)
        ...
        private void SaveMaterialDetail(string tgId)    //三包服务单维修用料
        {
            ...
            for (var i = 0; i < dgv_tb_maintain_three_matrial_detail.Rows.Count; i++)
            {
                ...
                diFileds.Add("pre_order_id", CommonCtrl.IsNullToString(dgv_tb_maintain_three_matrial_detail.Rows[i].Cells[drtxt2_pre_order_id.Name].Value));
        ...
        private void SaveProjectData(string tgId)   //三包服务单维修项目
        {
            ...
            for (var i = 0; i < dgv_tb_maintain_three_guaranty_item.Rows.Count; i++)
            {
                ...
                dicFileds.Add("pre_order_id", CommonCtrl.IsNullToString(dgv_tb_maintain_three_guaranty_item.Rows[i].Cells[drtxt1_pre_order_id.Name].Value));
        ...
        private Double GetManHourSumMoney() //工时费用
        {
            ...
            {
                var money = 0.0;
                var result = Double.TryParse(CommonCtrl.IsNullToString(dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_sum_money"].Value), out money);
                if(result) sumMoney += money;
            }
        ...
        {
            if (cbo_whether_go_out.SelectedValue == null) return 0;
            var selectedValue = cbo_whether_go_out.SelectedValue == null ? "" : cbo_whether_go_out.SelectedValue.ToString().ToUpper();
            if (selectedValue == DbDic2Enum.FALSE) return 0;
        ...
        public void BindSetmentData()
        {
            ...
            //维修项目数据                
            var dpt = DBHelper.GetTable("维修项目数据", "tb_maintain_item", "*",
                string.Format(" maintain_id='{0}' and three_warranty = '1'", BeforeOrderId), "", "");
            if (dpt.Rows.Count > 0)
            {
                ...
                for (var i = 0; i < dpt.Rows.Count; i++)
                {
                    ...
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_item_id"].Value = CommonCtrl.IsNullToString(dpr["item_id"]);
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_item_no"].Value = CommonCtrl.IsNullToString(dpr["item_no"]);
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_item_type"].Value = CommonCtrl.IsNullToString(dpr["item_type"]);
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_item_name"].Value = CommonCtrl.IsNullToString(dpr["item_name"]);
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_man_hour_type"].Value = CommonCtrl.IsNullToString(dpr["man_hour_type"]) == "1" ? "工时" : "定额";
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_man_hour_quantity"].Value =CommonCtrl.IsNullToString(dpr["man_hour_quantity"]);
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_man_hour_unitprice"].Value = CommonCtrl.IsNullToString(dpr["man_hour_norm_unitprice"]);
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_sum_money"].Value = CommonCtrl.IsNullToString(dpr["sum_money_goods"]);
                    //dgv_tb_maintain_three_guaranty_item.Rows[i].Cells["drtxt1_remarks"].Value = CommonCtrl.IsNullToString(dpr["remarks"]);
                    if (String.IsNullOrEmpty(CommonCtrl.IsNullToString(dpr["start_work_time"])) || String.IsNullOrEmpty(CommonCtrl.IsNullToString(dpr["complete_work_time"]))) continue;
                    var time = Convert.ToDateTime(Common.UtcLongToLocalDateTime(CommonCtrl.IsNullToString(dpr["start_work_time"])));
                    if (startTime > time) startTime = time;
                    var time1 = Convert.ToDateTime(Common.UtcLongToLocalDateTime(CommonCtrl.IsNullToString(dpr["complete_work_time"])));
                    if (_endTime < time) _endTime = time1;
                }
            }
            ...
            //维修用料数据
            //var dmt = DBHelper.GetTable("维修用料数据", "tb_maintain_material_detail", "*", string.Format(" maintain_id='{0}' and three_warranty = '1'", BeforeOrderId), "", "");
            //if (dmt.Rows.Count > 0)
            //{
            //    if (dmt.Rows.Count > dgv_tb_maintain_three_matrial_detail.Rows.Count)
            //    {
            //        dgv_tb_maintain_three_matrial_detail.Rows.Add(dmt.Rows.Count - dgv_tb_maintain_three_matrial_detail.Rows.Count + 1);
            //    }
            ...
            //        var dmt1 = DBHelper.GetTable("维修用料数据", "v_parts_chooser", "car_parts_code", string.Format(" ser_parts_code='{0}'", CommonCtrl.IsNullToString(dmr["parts_code"])), "", "");
            //        if (dmt1 != null && dmt1.Rows.Count > 0)
            //        {
            //            dgv_tb_maintain_three_matrial_detail.Rows[i].Cells["drtxt2_depot_code"].Value = CommonCtrl.IsNullToString(dmt1.Rows[0]["car_parts_code"]);
            //        }
            ...
            txt_repairer_name.Caption = CommonCtrl.IsNullToString(dr["driver_name"]);
            txt_repairer_mobile.Caption = CommonCtrl.IsNullToString(dr["driver_mobile"]);
            try
            {
                txt_depot_no.Text = CommonCtrl.IsNullToString(dr["turner"]);
            ...
            dtp_start_work_time.Value = startTime;
            if(startTime != DateTime.MaxValue) dtp_start_work_time.Enabled = false;
            try
            {
                var time = DateTime.Now;

                if (!String.IsNullOrEmpty(CommonCtrl.IsNullToString(dr["complete_work_time"])))
                {
                    time = Convert.ToDateTime(Common.UtcLongToLocalDateTime(CommonCtrl.IsNullToString(dr["complete_work_time"])));
                }
                dtp_complete_work_time.Value = dtp_repair_completion_time.Value = time;
            }
            catch (Exception ex)
            {
                
            }
            dtp_repair_completion_time.Enabled = false;
            ValidationDepto();
        ...
        public static void UpdateMaintainInfo(ICollection<SQLObj> listSql, string strReservId, string status)
        {
        ...
        private void BindData()
        {
            try
            {
                ...
                {
                    BeforeOrderId = CommonCtrl.IsNullToString(dr["pre_order_id"]);
                    ...
                    txt_approve_status_yt.Caption = UIAssistants.GetDicName(CommonCtrl.IsNullToString(dr["approve_status_yt"]));
                }
                ...
                dtp_parts_buy_time.Value = !String.IsNullOrEmpty(Common.UtcLongToLocalDateTime(dr["parts_buy_time"]))
                    ...
                    : DBHelper.GetCurrentTime();
                txt_parts_buy_corp.Caption = CommonCtrl.IsNullToString(dr["parts_buy_corp"]);
                ...
                txt_first_install_station.Caption = CommonCtrl.IsNullToString(dr["first_install_station"]);
                ...
                if (dpt.Rows.Count > 0)
                {
                    if (dpt.Rows.Count > dgv_tb_maintain_three_guaranty_item.Rows.Count)
                    {
                        dgv_tb_maintain_three_guaranty_item.Rows.Add(dpt.Rows.Count - dgv_tb_maintain_three_guaranty_item.Rows.Count + 1);
                    }
                    for (var i = 0; i < dpt.Rows.Count; i++)
                    {
                        var dpr = dpt.Rows[i];
                        dgv_tb_maintain_three_guaranty_item.Rows[i].Cells[drtxt1_pre_order_id.Name].Value =
                            CommonCtrl.IsNullToString(dpr["pre_order_id"]);
                ...
                if (dmt.Rows.Count > 0)
                {
                    ...
                    {
                        var dmr = dmt.Rows[i];
                        dgv_tb_maintain_three_matrial_detail.Rows[i].Cells[drtxt2_pre_order_id.Name].Value = CommonCtrl.IsNullToString(dmr["pre_order_id"]);
                        dgv_tb_maintain_three_matrial_detail.Rows[i].Cells["drtxt2_material_id"].Value = CommonCtrl.IsNullToString(dmr["material_id"]);
                        dgv_tb_maintain_three_matrial_detail.Rows[i].Cells[drtxt2_depot_code.Name].Value = CommonCtrl.IsNullToString(dmr["depot_code"]);
                        dgv_tb_maintain_three_matrial_detail.Rows[i].Cells[drtxt2_parts_code.Name].Value =  CommonCtrl.IsNullToString(dmr["parts_code"]);
                        dgv_tb_maintain_three_matrial_detail.Rows[i].Cells["drtxt2_parts_name"].Value = CommonCtrl.IsNullToString(dmr["parts_name"]);
