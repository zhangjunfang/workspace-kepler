...
namespace HXCPcClient.UCForm.RepairBusiness.Reserve
{
    ...
    public partial class ReserveOrderAddOrEdit : UCBase
    {
                ...
                if (dgvproject.Rows.Count > 0)
                {
                    for (int i = 0; i < dgvproject.Rows.Count; i++)
                    {
                        if (dgvproject.Rows[i].Cells["item_name"].Value != null)
                        {
                            ...
                            dicPFileds.Add("data_source", CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["data_source"].Value));                            
                            if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
                            {
                ...
                if (dgvMaterials.Rows.Count > 0)
                {
                    for (int i = 0; i < dgvMaterials.Rows.Count; i++)
                    {
                        if (dgvMaterials.Rows[i].Cells["parts_name"].Value != null)
                        {
                            ...
                            dicMFileds.Add("data_source", dgvMaterials.Rows[i].Cells["Mdata_source"].Value.ToString());      
                            if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
                            {
        ...
        private void ReserveOrderAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            ControlsConfig.TextForPhone(txtRepPersonPhone);
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    for (int i = 0; i <= intProws; i++)
                    {
                        if (string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["item_no"].Value)))
                        {
                            ...
                            dgvproject.Rows[i].Cells["three_warranty"].Value = "否";
                            dgvproject.Rows[i].Cells["data_source"].Value = frmHours.strData_source;
                            dgvproject.Rows[i].Cells["data_source"].ReadOnly = frmHours.strData_source == Convert.ToInt32(DataSources.EnumDataSources.SELFBUILD).ToString() ? true : false;
                            dgvproject.Rows.Add(1);
                            break;
                        }
                    }
                    ...
                    if (!string.IsNullOrEmpty(strPcode))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            if (dpt.Rows.Count > 0)
                            {
                                ...
                                string strNum = CommonCtrl.IsNullToString(dgvMaterials.Rows[intMrows].Cells["quantity"].Value);
                                ...
                                dgvMaterials.Rows[intMrows].Cells["Msum_money"].Value = Convert.ToString(Convert.ToDecimal(strNum) * Convert.ToDecimal(strUMoney = strUMoney == "" ? "0" : strUMoney));
                                dgvMaterials.Rows[intMrows].Cells["drawn_no"].Value = CommonCtrl.IsNullToString(dpr["drawing_num"]);
                                dgvMaterials.Rows[intMrows].Cells["vehicle_model"].Value = CommonFuncCall.GetCarTypeForMa((CommonCtrl.IsNullToString(dpr["ser_parts_code"])));
