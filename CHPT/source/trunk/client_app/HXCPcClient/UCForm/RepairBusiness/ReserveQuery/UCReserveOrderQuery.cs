...
namespace HXCPcClient.UCForm.RepairBusiness.ReserveQuery
{
    public partial class UCReserveOrderQuery : UCBase
    {
        ...
        #region 设置事件
        void UCReserveOrderQuery_SetEvent(object sender, EventArgs e)
        {
        ...
        #region 打印事件
        void UCReserveOrderQuery_PrintEvent(object sender, EventArgs e)
        {
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "维修预约单查询" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, dgvRData);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【维修预约单查询】" + ex.Message, "server");
