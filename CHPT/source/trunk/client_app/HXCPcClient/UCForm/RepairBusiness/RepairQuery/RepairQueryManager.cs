...
namespace HXCPcClient.UCForm.RepairBusiness.RepairQuery
{
    ...
    public partial class RepairQueryManager : UCBase
    {
        ...
        private void InitUC()
        {
            ...
            base.ExportEvent += new ClickHandler(UC_ExportEvent);
            base.PrintEvent -= new ClickHandler(RepairQueryManager_PrintEvent);
            base.PrintEvent += new ClickHandler(RepairQueryManager_PrintEvent);
            base.SetEvent += new ClickHandler(RepairQueryManager_SetEvent);
            #region 预览、打印设置
            paperSize.Width = 297;
        ...
        #region 设置事件
        void RepairQueryManager_SetEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
            ...
            else if (tcQuery.SelectedIndex == 1)
            {
                ...
                businessPrint.PrintSet(dgvProjectData);
            }
            else if (tcQuery.SelectedIndex == 2)
            {
                ...
                businessPrint.PrintSet(dgvYlData);
            }
        }
        #endregion

        #region 打印事件
        void RepairQueryManager_PrintEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
                printObject = "tb_maintain_info_Q";
            ...
            else if (tcQuery.SelectedIndex == 1)
            {
                ...
                printTitle = "按维修项目查询";
                businessPrint = new BusinessPrint(dgvProjectData, printObject, printTitle, paperSize, listNotPrint);
                businessPrint.Print(dgvProjectData.GetBoundData());
            }
            else if (tcQuery.SelectedIndex == 2)
            {
                printObject = "tb_maintain_info_M";
                printTitle = "按维修用料查询";
                businessPrint = new BusinessPrint(dgvYlData, printObject, printTitle, paperSize, listNotPrint);
                businessPrint.Print(dgvYlData.GetBoundData());
            }
        }
        #endregion
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
                ExportToxls(dgvQData,"按维修单查询");
            }
            else if (tcQuery.SelectedIndex == 1)
            {
                ExportToxls(dgvProjectData, "按维修项目查询");
            }
            else if (tcQuery.SelectedIndex == 2)
            {
                ExportToxls(dgvYlData, "按维修用料查询");
            }
        }
        ...
        {
            DataTable dt = DBHelper.GetTable("", "tb_balance_way", "balance_way_id,balance_way_name,default_account", "enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and status='" + Convert.ToInt32(DataSources.EnumStatus.Start).ToString() + "'", "", "");
        ...
        public void _BindPageData()
        {
            try
            {
                ...
                left join tb_vehicle_models v on v.vm_id=a.vehicle_model
                left join sys_user u on u.user_id=a.responsible_opid";
                string strFiles = @" a.maintain_no,a.reception_time,a.vehicle_no,v.vm_name as vehicle_model,a.customer_name,a.customer_code,a.dispatch_status
                ,a.driver_name,a.driver_mobile,a.maintain_man,u.user_name as responsible_opid,a.vehicle_vin,a.engine_no,b.man_hour_sum
                ,b.fitting_sum,b.other_item_sum,b.privilege_cost,b.should_sum,b.received_sum,b.debt_cost,a.travel_mileage,b.create_time
                ,a.oil_into_factory,a.fault_describe
                ,(select dic_name from sys_dictionaries c where c.dic_id=a.maintain_type) as maintain_type
                ,(select dic_name from sys_dictionaries c where c.dic_id=a.maintain_payment) as maintain_payment
                ,(select balance_way_name from tb_balance_way c where c.balance_way_id=b.payment_terms) as payment_terms
                ,a.remark,a.maintain_id,a.orders_source,a.info_status";
        ...
        public void BindProjectPageData()
        {
            try
            {
                ...
                left join sys_user u on u.user_id=a.responsible_opid
                left join tb_organization o on o.org_id=a.org_name ";
                ...
                ,b.sum_money_goods,case b.three_warranty when '1'  then '是'  when '0' then  '否' else '' end  as three_warranty,a.dispatch_status
                ,o.org_name as org_name,u.user_name as responsible_opid
                ,(select dic_name from sys_dictionaries c where c.dic_id=a.maintain_type) as maintain_type
                ,(select dic_name from sys_dictionaries c where c.dic_id=a.maintain_payment) as maintain_payment
                ,(select balance_way_name from tb_balance_way d where d.balance_way_id=c.payment_terms) as payment_terms
                ,a.remark,a.maintain_id,a.orders_source,a.info_status ";
                string strWhere = string.Format("a.enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "'");//enable_flag 1未删除
                ...
                {
                    strWhere += string.Format(" and  a.maintain_type = '{0}'", cobPReType.SelectedValue.ToString());
                }
                ...
                {
                    strWhere += string.Format(" and  a.responsible_opid = '{0}'", cobPHandle.SelectedValue.ToString());
                }
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(cobPBalanceType.SelectedValue)))//结算方式
                {
                    strWhere += string.Format(" and  c.payment_terms = '{0}'", cobPBalanceType.SelectedValue.ToString());
                }
        ...
        public void BindYLPageData()
        {
            try
            {
                ...
                ,b.norms,b.unit,b.quantity,b.sum_money,case b.three_warranty when '1'  then '是'  when '0' then  '否' else '' end  as three_warranty,a.dispatch_status,o.org_name as org_name,a.customer_code,a.vehicle_no
                ,u.user_name as responsible_opid
                ,(select dic_name from sys_dictionaries c where c.dic_id=a.maintain_type) as maintain_type
                ,(select dic_name from sys_dictionaries c where c.dic_id=a.maintain_payment) as maintain_payment
                ,(select balance_way_name from tb_balance_way d where d.balance_way_id=c.payment_terms) as payment_terms
                ,a.remark,a.maintain_id,a.orders_source,a.info_status";
                ...
                {
                    strWhere += string.Format(" and  a.maintain_type like '%{0}%'", cobPartRepairType.SelectedValue.ToString());
                }
