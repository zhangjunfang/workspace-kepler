...
namespace HXCPcClient.UCForm.RepairBusiness.FetchMaterialReturn
{
    ...
    public partial class UCFMaterialReturnAddOrEdit : UCBase
    {
        ...
        /// 领料单的Id
        /// </summary>
        public string strfetchId = string.Empty;
        /// <summary>
        /// 单据详情Id
        /// </summary>
        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            try
            {
                ...
                dicParam.Add("customer_name", new ParamObj("customer_name", txtCustomName.Caption.Trim(), SysDbType.NVarChar, 20));//客户名称
                ...
                dicParam.Add("fetch_id", new ParamObj("fetch_id", CommonCtrl.IsNullToString(strfetchId), SysDbType.VarChar, 40));//关联的领料单Id                
                if (status == DataSources.EnumAuditStatus.SUBMIT)//提交操作时生成单号
                {
                    dicParam.Add("refund_no", new ParamObj("refund_no",!string.IsNullOrEmpty(labReserveNoS.Text.Trim())?labReserveNoS.Text.Trim():CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.ReturnMaterialNo), SysDbType.VarChar, 40));//领料退货单号 
                    //单据状态
                    dicParam.Add("info_status", new ParamObj("info_status", Convert.ToInt32(status).ToString(), SysDbType.VarChar, 40));
                    //更新前置单据导入状态
                    if (!string.IsNullOrEmpty(strfetchId))
                    {
                        UpdateMaintainInfo(listSql, strfetchId, Convert.ToInt32(DataSources.EnumImportStaus.LOCK).ToString());
                    }
                }
                else
                {
                ...
                if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
                {
                 ...
                 ,refund_opid,material_no,fetch_opid,packing_time,remarks,responsible_opid,responsible_name,org_id,enable_flag,fetch_id,info_status,refund_no,refund_id
                ...
                else if (wStatus == WindowStatus.Edit)
                {
                 ...
                 ,refund_opid=@refund_opid,material_no=@material_no,fetch_opid=@fetch_opid,packing_time=@packing_time,remarks=@remarks,responsible_opid=@responsible_opid,responsible_name=@responsible_name,org_id=@org_id,enable_flag=@enable_flag,fetch_id=@fetch_id,info_status=@info_status,refund_no=@refund_no
        ...
        public void BindSetmentData()
        {
            ...
            {
                #region 基本信息
                DataTable dt = DBHelper.GetTable("查询领料单", "tb_maintain_fetch_material", "*", " fetch_id ='" + strfetchId + "'", "", "");
                ...
                #region 配件信息
                DataTable dmt = DBHelper.GetTable("领料明细数据", "tb_maintain_fetch_material_detai", "*", string.Format(" fetch_id='{0}'", strfetchId), "", "");
                if (dmt.Rows.Count > 0)
                {
                ...
                List<SQLObj> listSql = new List<SQLObj>();
                UpdateMaintainInfo(listSql, strfetchId, Convert.ToInt32(DataSources.EnumImportStaus.OCCUPY).ToString());
                DBHelper.BatchExeSQLMultiByTrans("更新前置单据状体", listSql);
            }
            catch (Exception ex)
            {
        ...
        private void BindData()
        {
            try
            {
                ...
                strfetchId = CommonCtrl.IsNullToString(dr["fetch_id"]);//关联的领料单Id
                #region 车辆信息
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.OK)
                {
                    strfetchId = MateriaImport.strMaterialId;
