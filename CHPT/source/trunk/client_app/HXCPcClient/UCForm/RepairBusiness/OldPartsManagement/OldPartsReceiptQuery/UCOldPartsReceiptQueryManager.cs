...
namespace HXCPcClient.UCForm.RepairBusiness.OldPartsManagement.OldPartsReceiptQuery
{
    ...
    public partial class UCOldPartsReceiptQueryManager : UCBase
    {
        ...
        public UCOldPartsReceiptQueryManager()
        {
            ...
            base.ExportEvent += new ClickHandler(UC_ExportEvent);
            base.PrintEvent += new ClickHandler(UCOldPartsReceiptQueryManager_PrintEvent);
            base.SetEvent += new ClickHandler(UCOldPartsReceiptQueryManager_SetEvent);
            SetQuick();
            #region 预览、打印设置
            paperSize.Width = 297;
        ...
        #region 打印事件
        void UCOldPartsReceiptQueryManager_PrintEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
                printObject = "tb_maintain_oldpart_receiv_send_r";
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
                ExportToxls(dgvRData, "旧件收货单查询");
            }
            else if (tcQuery.SelectedIndex == 1)
            {
                ExportToxls(dgvPartsData, "按配件查询旧件收货单");
            }
        }
        ...
        public void BindPageData()
        {
            try
            {
                ...
                int recordCount;
                DataTable dt = DBHelper.GetTableByPage("分页查询旧件收货单管理", strTables, strFiles, strWhere, "", " order by a.create_time desc", page.PageIndex, page.PageSize, out recordCount);
