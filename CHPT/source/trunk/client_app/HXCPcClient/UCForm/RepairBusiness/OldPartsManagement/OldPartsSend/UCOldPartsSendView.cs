...
namespace HXCPcClient.UCForm.RepairBusiness.OldPartsManagement.OldPartsSend
{
    ...
    /// AddTime：2014.11.05
    /// </summary>
    public partial class UCOldPartsSendView : UCBase
    {
        #region 属性设置
        /// <summary>
        /// 旧件发货单的Id值
        ...
        #region 初始化窗体
        public UCOldPartsSendView(string strPId)
        {
            InitializeComponent();
            InitializeComponent();
            strPartsId = strPId;
            ...
            base.InvalidOrActivationEvent += new ClickHandler(UCOldPartsSendView_InvalidOrActivationEvent);
        }
        #endregion


        #region 作废激活事件
        void UCOldPartsSendView_InvalidOrActivationEvent(object sender, EventArgs e)
        {
        ...
        void UCOldPartsReceiptView_SubmitEvent(object sender, EventArgs e)
        {
            ...
            {
                labMaterialNoS.Text = CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.PartsSend);
            }
        ...
        void UCOldPartsReceiptView_DeleteEvent(object sender, EventArgs e)
        {
            ...
            bool flag = DBHelper.BatchUpdateDataByIn("删除旧件发货单", "tb_maintain_oldpart_receiv_send", comField, "oldpart_id", listField.ToArray());
            if (flag)
            {
        ...
        void UCOldPartsReceiptView_AddEvent(object sender, EventArgs e)
        {
            ...
            PartsAdd.uc = uc;
            base.addUserControl(PartsAdd, "旧件发货单-新增", "PartsAdd" + PartsAdd.strId, this.Tag.ToString(), this.Name);
            deleteMenuByTag(this.Tag.ToString(), PartsAdd.Name);
        }
        #endregion

        #region 窗体Load事件
        private void UCOldPartsSendView_Load(object sender, EventArgs e)
        {
            GetRescueData(strPartsId);
        }
        #endregion

        #region 根据旧件发货单Id获取相应的详细信息
        /// <summary>
        /// 根据旧件发货单Id获取相应的详细信息
        /// </summary>
