    ...
    public partial class UCOldPartsReceiptAddOrEdit : UCBase
    {
        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            try
            {
                ...
                {
                    dicParam.Add("receipts_no", new ParamObj("receipts_no",!string.IsNullOrEmpty(labReceiptNoS.Text.Trim())?labReceiptNoS.Text.Trim():CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.PartsReceipt), SysDbType.VarChar, 40));//收货单号 
                    //单据状态
                ...
                else
                {
                    ...
                    else
                    {
                        //单据状态
                        dicParam.Add("info_status", new ParamObj("info_status", Convert.ToInt32(status).ToString(), SysDbType.VarChar, 40));
                        dicParam.Add("receipts_no", new ParamObj("receipts_no", labReceiptNoS.Text.Trim(), SysDbType.VarChar, 40));//旧件收货单号
                    }
                    //更新前置单据导入状态
                    if (!string.IsNullOrEmpty(strBefore_orderId))
                    {
