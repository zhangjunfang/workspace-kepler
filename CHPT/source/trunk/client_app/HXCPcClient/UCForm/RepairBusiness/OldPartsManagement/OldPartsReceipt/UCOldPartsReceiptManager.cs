...
namespace HXCPcClient.UCForm.RepairBusiness.OldPartsManagement.OldPartsReceipt
{
    ...
    public partial class UCOldPartsReceiptManager : UCBase
    {
        ...
        #region 设置事件
        void UCOldPartsReceiptManager_SetEvent(object sender, EventArgs e)
        {
        ...
        #region 打印事件
        void UCOldPartsReceiptManager_PrintEvent(object sender, EventArgs e)
        {
        ...
        void UCOldPartsReceiptManager_EditEvent(object sender, EventArgs e)
        {
            ...
            UCOldPartsReceiptAddOrEdit PartsReceiptEdit = new UCOldPartsReceiptAddOrEdit();
            PartsReceiptEdit.uc = this;
            PartsReceiptEdit.wStatus = WindowStatus.Edit;
            PartsReceiptEdit.strId = strReId;  //编辑单据的Id值
            base.addUserControl(PartsReceiptEdit, "旧件收货单-编辑", "PartsReceiptEdit" + PartsReceiptEdit.strId, this.Tag.ToString(), this.Name);
        }
        #endregion
        ...
        void UCOldPartsReceiptManager_CopyEvent(object sender, EventArgs e)
        {
            ...
            int intIndex = dgvRData.CurrentRow.Index;
            strReId = CommonCtrl.IsNullToString(dgvRData.Rows[intIndex].Cells["oldpart_id"].Value);
            UCOldPartsReceiptAddOrEdit PartsReceiptCopy = new UCOldPartsReceiptAddOrEdit();
            PartsReceiptCopy.uc = this;
            PartsReceiptCopy.wStatus = WindowStatus.Copy;
            PartsReceiptCopy.strId = strReId;  //复制单据的Id值
            base.addUserControl(PartsReceiptCopy, "旧件收货单-复制", "PartsReceiptCopy" + PartsReceiptCopy.strId, this.Tag.ToString(), this.Name);
        }
        #endregion
        ...
        {
            UCOldPartsReceiptAddOrEdit PartsReceiptAdd = new UCOldPartsReceiptAddOrEdit();
            PartsReceiptAdd.uc = this;
            PartsReceiptAdd.wStatus = WindowStatus.Add;
            base.addUserControl(PartsReceiptAdd, "旧件收货单-新增", "PartsReceiptAdd", this.Tag.ToString(), this.Name);
        }
        #endregion
...
//                    page.RecordCount = recordCount;
        ...
        private void _BindPageData(object obj)
        {
            string strFiles = @"*,(SELECT SUM(b.sum_money) FROM tb_maintain_oldpart_material_detail b  WHERE b.maintain_id=a.oldpart_id ) AS sum_money,
                                   (SELECT SUM(b.quantity) FROM tb_maintain_oldpart_material_detail b  WHERE b.maintain_id=a.oldpart_id ) AS quantity ";
            string strTables = "tb_maintain_oldpart_receiv_send a ";
            DataTable dt = DBHelper.GetTableByPage("分页查询旧件收货单管理", strTables, strFiles, obj.ToString(), "", " order by a.create_time desc", page.PageIndex, page.PageSize, out this.recordCount);
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "旧件收货单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, dgvRData);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【旧件收货单】" + ex.Message, "server");
