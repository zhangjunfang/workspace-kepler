    ...
    public partial class UCOldPartsSendAddOrEdit : UCBase
    {
        ...
        private Boolean CheckControlValue()
        {
            try
            {
                ...
                {
                    Validator.SetError(errorProvider1, dtpReceiptTime, "发货日期不能为空!");
                    return false;
                }
        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            try
            {
                ...
                dicParam.Add("remarks", new ParamObj("remarks", txtRemark.Caption.Trim(), SysDbType.NVarChar, 300));//备注
                #endregion
                ...
                {
                    dicParam.Add("receipts_no", new ParamObj("receipts_no",!string.IsNullOrEmpty(labReceiptNoS.Text.Trim())?labReceiptNoS.Text.Trim():CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.PartsSend), SysDbType.VarChar, 40));//发货单号 
                    //单据状态
                ...
                if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
                {
                 ...
                 ,responsible_name,org_id,enable_flag,info_status,receipt_type,receipts_no,oldpart_id, create_by,create_name,create_time)
                ...
                else if (wStatus == WindowStatus.Edit)
                {
                 ...
                 ,responsible_name=@responsible_name,org_id=@org_id,enable_flag=@enable_flag,info_status=@info_status,receipt_type=@receipt_type,receipts_no=@receipts_no,update_by=@update_by,update_name=@update_name,update_time=@update_time
        ...
        private void BindData()
        {
            try
            {
                ...
                #region 配件信息
                DataTable dmt = DBHelper.GetTable("旧件发货明细数据", "tb_maintain_oldpart_material_detail", "*", string.Format(" maintain_id='{0}' and enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "'", strId), "", "");
                if (dmt.Rows.Count > 0)
                {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvMaterials.Rows[e.RowIndex].Cells["parts_code"].Value)) && !string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvMaterials.Rows[e.RowIndex].Cells["parts_name"].Value)))
                {
                    if (e.ColumnIndex == 8 || e.ColumnIndex == 7)
                    {
                        ...
                        string strUnitPrice = !string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvMaterials.Rows[e.RowIndex].Cells["unit_price"].Value)) ? CommonCtrl.IsNullToString(dgvMaterials.Rows[e.RowIndex].Cells["unit_price"].Value) : "0";
                    ...
                    if (!string.IsNullOrEmpty(strPcode))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            if (dpt.Rows.Count > 0)
                            {
                                ...
                                string unit_price = CommonCtrl.IsNullToString(dpr["ref_out_price"]);
                                dgvMaterials.Rows[intMrows].Cells["unit_price"].Value = !string.IsNullOrEmpty(unit_price) ? unit_price : "0";
