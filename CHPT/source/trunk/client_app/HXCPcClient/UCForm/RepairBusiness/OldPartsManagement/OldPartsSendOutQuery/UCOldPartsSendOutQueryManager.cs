...
namespace HXCPcClient.UCForm.RepairBusiness.OldPartsManagement.OldPartsSendOutQuery
{
    ...
    /// AddTime：2014.11.05
    /// </summary>
    public partial class UCOldPartsSendOutQueryManager : UCBase
    {
        #region 属性设置
        /// <summary>
        /// 查询条件
        ...
        #region 初始化窗体
        public UCOldPartsSendOutQueryManager()
        {
            InitializeComponent();
            InitializeComponent();
            SetDgvAnchor();
            ...
            base.ExportEvent += new ClickHandler(UC_ExportEvent);
            base.PrintEvent += new ClickHandler(UCOldPartsSendOutQueryManager_PrintEvent);
            base.SetEvent += new ClickHandler(UCOldPartsSendOutQueryManager_SetEvent);
            SetQuick();
            #region 预览、打印设置
            paperSize.Width = 297;
        ...
        #region 设置事件
        void UCOldPartsSendOutQueryManager_SetEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
        ...
        #region 打印事件
        void UCOldPartsSendOutQueryManager_PrintEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
                printObject = "tb_maintain_oldpart_receiv_send_r";
            ...
            else if (tcQuery.SelectedIndex == 1)
            {
                ...
                printTitle = "按配件查询旧件发货单";
                businessPrint = new BusinessPrint(dgvPartsData, printObject, printTitle, paperSize, listNotPrint);
                businessPrint.Print(dgvPartsData.GetBoundData());
            }
        }
        #endregion
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            if (tcQuery.SelectedIndex == 0)
            {
                ExportToxls(dgvRData, "旧件发货单查询");
            }
            else if (tcQuery.SelectedIndex == 1)
            {
                ExportToxls(dgvPartsData, "按配件查询旧件发货单");
            }
        }
        ...
        private void SetQuick()
        {
            ...
            //按配件查询旧货发货单设置配件编码速查
            txtPartsCode.SetBindTable("v_parts_chooser", "ser_parts_code", "ser_parts_code");
            ...
            //按配件查询旧货发货单设置客户编码速查
            txtPCustomCode.SetBindTable("tb_customer", "cust_code", "cust_code");
        ...
        void txtPartsCode_DataBacked(DataRow dr)
        {
            ...
            txtPartsCode.Tag = CommonCtrl.IsNullToString(dr["parts_id"]);
        ...
        public void BindPageData()
        {
            try
            {
                ...
                {
                    MessageBoxEx.Show("发货日期,开始日期不能大于结束日期", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                #endregion
                strWhere = string.Format(" a.enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and a.receipt_type='2' and info_status in('1','2')");//enable_flag 1未删除receipt_type='2'为发货单
                ...
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(txtOrder.Caption.Trim())))//发货单号
                {
                ...
                int recordCount;
                DataTable dt = DBHelper.GetTableByPage("分页查询旧件发货单管理", strTables, strFiles, strWhere, "", " order by a.create_time desc", page.PageIndex, page.PageSize, out recordCount);
        ...
        #region 按旧件发货单查询-重写datagridview中的时间、状态等
        ...
        {
            string strTabTag = tcQuery.SelectedIndex.ToString();//0为按旧件发货单查询，1为按配件查询旧件发货单
            if (strTabTag == "0")
            {
            ...
            UCOldPartsSendView Qview = new UCOldPartsSendView(strReId);
            base.addUserControl(Qview, "旧件发货单-预览", "Qview" + strReId, this.Tag.ToString(), this.Name);
        }
        #endregion
        ...
        private void DoubleClickCell(DataGridView dgvView, string strIdName)
        {
            ...
            UCOldPartsSendView view = new UCOldPartsSendView(strReId);
            base.addUserControl(view, "旧件发货单-预览", "view" + strReId, this.Tag.ToString(), this.Name);
        }
        #endregion

        #region 窗体Load事件
        private void UCOldPartsSendOutQueryManager_Load(object sender, EventArgs e)
        {
        ...
        public void PBindPageData()
        {
            try
            {
                ...
                {
                    strWhere += string.Format(" and a.customer_code = '{0}'", txtPCustomCode.Text.Trim());
                }
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(txtPCustomName.Caption.Trim())))//客户名称
                {
                    strWhere += string.Format(" and a.customer_name = '{0}'", txtPCustomName.Caption.Trim());
                }
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(txtPContract.Caption.Trim())))//联系人
                {
                    strWhere += string.Format(" and a.linkman = '{0}'", txtPContract.Caption.Trim());
                }
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(cobProcessMode.SelectedValue)))//处理方式
                {
                    strWhere += string.Format(" and b.process_mode = '{0}'", cobProcessMode.SelectedValue.ToString());
                }
                ...
                a.create_name,a.org_name,a.responsible_name,b.remarks,a.oldpart_id,b.process_mode";
                ...
                int recordCount;
                DataTable dt = DBHelper.GetTableByPage("按配件查询旧件分页查询", strTables, strFiles, strWhere, "", " order by a.create_time desc", pageP.PageIndex, pageP.PageSize, out recordCount);
        ...
        #region 按配件查询旧件发货单查询-重写datagridview中的时间、状态等
        private void dgvPartsData_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (fieldNmae.Equals("process_mode"))
            {
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(e.Value)))
                {
                    e.Value = DataSources.GetDescription(typeof(DataSources.EnumOldSendProcessMode), int.Parse(e.Value.ToString()));
                }
            }
        ...
        #region 按配件查询旧件发货单查询-双击单元格进入预览窗体
