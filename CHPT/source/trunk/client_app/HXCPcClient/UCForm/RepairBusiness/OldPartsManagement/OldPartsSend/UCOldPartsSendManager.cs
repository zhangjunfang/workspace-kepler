...
namespace HXCPcClient.UCForm.RepairBusiness.OldPartsManagement.OldPartsSend
{
    ...
    /// AddTime：2014.11.05
    /// </summary>
    public partial class UCOldPartsSendManager : UCBase
    {
        #region 属性设置
        /// <summary>
        /// 查询条件
        ...
        #region 设置事件
        void UCOldPartsSendManager_SetEvent(object sender, EventArgs e)
        {
        ...
        #region 打印事件
        void UCOldPartsSendManager_PrintEvent(object sender, EventArgs e)
        {
        ...
        private void SubmitAndVerify(string strMessage, DataSources.EnumAuditStatus status)
        {
            ...
            foreach (DataGridViewRow dr in dgvRData.Rows)
            {
                ...
                if (isCheck != null && (bool)isCheck)
                {
                    ...
                    dicParam.Add("receipts_no", new ParamObj("receipts_no",!string.IsNullOrEmpty(strOrderNo)?strOrderNo: CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.PartsSend), SysDbType.VarChar, 40));//单据编号                   
        ...
        void UCOldPartsReceiptManager_DeleteEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                bool flag = DBHelper.BatchUpdateDataByIn("批量删除旧件发货单", "tb_maintain_oldpart_receiv_send", comField, "oldpart_id", listField.ToArray());
                if (flag)
                {
                    BindPageData();
        ...
        void UCOldPartsReceiptManager_EditEvent(object sender, EventArgs e)
        {
            ...
            UCOldPartsSendAddOrEdit PartsEdit = new UCOldPartsSendAddOrEdit();
            PartsEdit.uc = this;
            PartsEdit.wStatus = WindowStatus.Edit;
            PartsEdit.strId = strReId;  //编辑单据的Id值
            base.addUserControl(PartsEdit, "旧件发货单-编辑", "PartsEdit" + PartsEdit.strId, this.Tag.ToString(), this.Name);
        }
        #endregion
        ...
        void UCOldPartsReceiptManager_CopyEvent(object sender, EventArgs e)
        {
            ...
            int intIndex = dgvRData.CurrentRow.Index;
            strReId = CommonCtrl.IsNullToString(dgvRData.Rows[intIndex].Cells["oldpart_id"].Value);
            UCOldPartsSendAddOrEdit PartsCopy = new UCOldPartsSendAddOrEdit();
            PartsCopy.uc = this;
            PartsCopy.wStatus = WindowStatus.Copy;
            PartsCopy.strId = strReId;  //复制单据的Id值
            base.addUserControl(PartsCopy, "旧件发货单-复制", "PartsCopy" + PartsCopy.strId, this.Tag.ToString(), this.Name);
        }
        #endregion
        ...
        {
            UCOldPartsSendAddOrEdit PartsAdd = new UCOldPartsSendAddOrEdit();
            PartsAdd.uc = this;
            PartsAdd.wStatus = WindowStatus.Add;
            base.addUserControl(PartsAdd, "旧件发货单-新增", "PartsAdd", this.Tag.ToString(), this.Name);
        }
        #endregion
        ...
        #region 窗体Load事件
        private void UCOldPartsSendManager_Load(object sender, EventArgs e)
        {
            SetTopbuttonShow();     
            dtpReceiptSTime.Value = DateTime.Now.AddMonths(-1);
            dtpReceiptETime.Value = DateTime.Now;
        ...
        public void BindPageData()
        {
            try
            {
                if (this.myLock)
                {
                    ...
                    #endregion
                    strWhere = string.Format(" a.enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and a.receipt_type='2'");//enable_flag 1未删除receipt_type='1'为发货单
...
//                    int recordCount;
//                    DataTable dt = DBHelper.GetTableByPage("分页查询旧件发货单管理", strTables, strFiles, strWhere, "", " order by a.receipt_time desc", page.PageIndex, page.PageSize, out recordCount);
...
//                    page.RecordCount = recordCount;
        ...
        private void _BindPageData(object obj)
        {
            string strFiles = @"*,(SELECT SUM(b.sum_money) FROM tb_maintain_oldpart_material_detail b  WHERE b.maintain_id=a.oldpart_id ) AS sum_money,
                                   (SELECT SUM(b.quantity) FROM tb_maintain_oldpart_material_detail b  WHERE b.maintain_id=a.oldpart_id ) AS quantity ";
            string strTables = "tb_maintain_oldpart_receiv_send a ";
            DataTable dt = DBHelper.GetTableByPage("分页查询旧件发货单管理", strTables, strFiles, obj.ToString(), "", " order by a.create_time desc", page.PageIndex, page.PageSize, out this.recordCount);
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "旧件发货单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, dgvRData);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【旧件发货单】" + ex.Message, "server");
