...
namespace HXCPcClient.UCForm.RepairBusiness.RepairRescue
{
    ...
    public partial class UCRepairRescueAddOrEdit : UCBase
    {
        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            ...
            {
                dicParam.Add("rescue_no", new ParamObj("rescue_no",!string.IsNullOrEmpty(labReserveNoS.Text.Trim())?labReserveNoS.Text.Trim():CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.Rescue), SysDbType.VarChar, 40));//救援单号 
                //单据状态
            ...
            if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
            {
                ...
                obj.sqlString = @"insert into tb_maintain_rescue_info (remarks,vehicle_no,make_time,vehicle_vin,engine_type,vehicle_model,vehicle_brand,driver_name,driver_mobile,turner
                 ...
                 ,privilege_cost,should_sum,received_sum,rescue_id,create_by,create_name,create_time,responsible_opid,responsible_name,org_id,enable_flag,document_status,rescue_no,customer_code)
        ...
        private void BindData()
        {
            ...
            txtremarks.Caption = CommonCtrl.IsNullToString(dr["remarks"]);//备注
            #endregion
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    for (int i = 0; i <= intProws; i++)
                    {
                        if (string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["item_no"].Value)))
                        {
                            ...
                            #endregion
                            //工时单价
                            dgvproject.Rows[i].Cells["man_hour_norm_unitprice"].Value = Math.Round((Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strQuotaPrice)?frmHours.strQuotaPrice:"0.00")), 2).ToString("0.00");
                            //会员折扣
                            ...
                            dgvproject.Rows[i].Cells["man_hour_quantity"].Value = Math.Round((Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strWhoursNum)?frmHours.strWhoursNum:"0.0")), 1).ToString("0.0");
                ...
                if (intCurrentIndex >= 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(strItemNo))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            #endregion
                            //工时单价
                            dgvproject.Rows[intCurrentIndex].Cells["man_hour_norm_unitprice"].Value = Math.Round((Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strQuotaPrice) ? frmHours.strQuotaPrice : "0.00")), 2).ToString("0.00");
                            //会员折扣
                            ...
                            dgvproject.Rows[intCurrentIndex].Cells["man_hour_quantity"].Value = Math.Round((Convert.ToDecimal(!string.IsNullOrEmpty(frmHours.strWhoursNum) ? frmHours.strWhoursNum : "0.0")), 1).ToString("0.0");
