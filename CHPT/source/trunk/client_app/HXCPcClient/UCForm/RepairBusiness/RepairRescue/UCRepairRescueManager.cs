...
namespace HXCPcClient.UCForm.RepairBusiness.RepairRescue
{
    ...
    public partial class UCRepairRescueManager : UCBase
    {
        ...
        #region 设置事件
        void UCRepairRescueManager_SetEvent(object sender, EventArgs e)
        {
        ...
        #region 打印事件
        void UCRepairRescueManager_PrintEvent(object sender, EventArgs e)
        {
        ...
        private void SubmitAndVerify(string strMessage, DataSources.EnumAuditStatus status)
        {
            ...
            foreach (DataGridViewRow dr in dgvRData.Rows)
            {
                ...
                if (isCheck != null && (bool)isCheck)
                {
                    ...
                    strOrderNo = dr.Cells["rescue_no"].Value.ToString();
                    SQLObj obj = new SQLObj();
                    obj.cmdType = CommandType.Text;
                    Dictionary<string, ParamObj> dicParam = new Dictionary<string, ParamObj>();
                    dicParam.Add("rescue_no", new ParamObj("rescue_no", !string.IsNullOrEmpty(strOrderNo) ? strOrderNo : CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.Rescue), SysDbType.VarChar, 40));//单据编号                   
        ...
        {
            DataTable dt = DBHelper.GetTableByPage("分页查询救援单管理", "tb_maintain_rescue_info", "*", obj.ToString(), "", " order by depart_time desc", page.PageIndex, page.PageSize, out this.recordCount);
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "维修救援单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, dgvRData);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【维修救援单】" + ex.Message, "server");
        ...
        private void dgvRData_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (fieldNmae.Equals("vehicle_brand") || fieldNmae.Equals("rescue_type"))
            {
