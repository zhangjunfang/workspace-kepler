...
namespace HXCPcClient.UCForm.BusinessAnalysis.RepairReport
{
    public partial class UCRepairClientReconciliation : UCReport
    {
            ...
            : base("v_repair_client_reconciliation_report", "维修客户对账统计")
        {
            ...
            quick.BindCustomer(txtcCustomerCode);
            txtcCustomerCode.DataBacked += new ServiceStationClient.ComponentUI.TextBox.TextChooser.DataBackHandler(txtcCustCode_DataBacked);
        }

        void txtcCustCode_DataBacked(DataRow dr)
        {
            txtCustomerName.Caption = CommonCtrl.IsNullToString(dr["cust_name"]);
            txtcCustomerCode.Text = CommonCtrl.IsNullToString(dr["cust_code"]);
        }
        //车牌号选择
