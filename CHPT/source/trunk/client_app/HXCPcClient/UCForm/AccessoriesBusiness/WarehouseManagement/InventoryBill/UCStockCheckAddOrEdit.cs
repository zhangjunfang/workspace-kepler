...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.InventoryBill
{
    public partial class UCStockCheckAddOrEdit : UCBase
    {
       ...
       private  void UCStockCheckAddOrEdit_DeleteEvent(object sender, EventArgs e)
        {
        ...
        private void UCStockCheckAddOrEdit_SaveEvent(object send, EventArgs e)
        {
            ...
            {
                MessageBoxEx.Show("请您导入要生成盘点单的配件信息！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void gvPartsMsgList_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    if (gvPartsMsgList.Rows[i].Cells["FmOffCount"].Value == null || gvPartsMsgList.Rows[i].Cells["FmOffCount"].Value.ToString() == "")
                    {
                        return;
                    }
                    ...
                    {
                        decimal PaperCount = Convert.ToDecimal(gvPartsMsgList.Rows[i].Cells["Papcounts"].Value.ToString());//账面数量
                        decimal FirmCount = Convert.ToDecimal(gvPartsMsgList.Rows[i].Cells["FmOffCount"].Value.ToString());//实盘数量 
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (gvPartsMsgList.Rows.Count > 0)
                    {
                        foreach (DataGridViewRow dr in gvPartsMsgList.Rows)
                        {
                            ...
                            {
                                MessageBoxEx.Show("该配件信息已经存在与列表中!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }
                    }
        ...
        {
            //有问题，需要修改
            DataTable dt = DBHelper.GetTable("配件信息查询", "v_stock_part_import", "*", string.Format("ser_parts_code='{0}'", PartsCode), "", "");
            return dt;
        }
        ...
        private void GetGridViewRowByDr(DataGridViewRow dgvr, DataRow dr)
        {
            try
            {
                ...
                dgvr.Cells["CarFactoryCode"].Value = dr["car_parts_code"];
                dgvr.Cells["BarCode"].Value = dr["parts_barcode"];
                dgvr.Cells["Papcounts"].Value = dr["paper_count"];//账面数量
                dgvr.Cells["BusinesPrice"].Value = dr["ref_in_price"];//成本价
                dgvr.Cells["remarks"].Value = dr["remark"];
            }
            catch (Exception ex)
            {
        ...
        private bool CheckListInfo()
        {
            ...
            {
                if (dgvr.Cells["FmOffCount"].Value==null || dgvr.Cells["FmOffCount"].Value.ToString()=="")
                {
                    MessageBoxEx.Show("请输入实盘数量!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return false;
                }

                decimal FirmCount = Convert.ToDecimal(dgvr.Cells["FmOffCount"].Value.ToString());//实盘数量 
                ...
                else if (FirmCount <= decimal.Zero)
                {
            ...
            try
            {
                if (gvPartsMsgList.Rows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow DgVRow in gvPartsMsgList.Rows)
                    {
                        ...
                        DicPartParam.Add("profitloss_count", DgVRow.Cells["ProLossCount"].Value.ToString() == string.Empty ? "" : DgVRow.Cells["ProLossCount"].Value.ToString());
