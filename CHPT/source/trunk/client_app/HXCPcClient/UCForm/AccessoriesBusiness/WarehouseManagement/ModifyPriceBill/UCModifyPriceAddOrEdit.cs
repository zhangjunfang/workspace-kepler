    ...
    public partial class UCModifyPriceAddOrEdit : UCBase
    {
       ...
       private  void UCModifyPriceAddOrEdit_DeleteEvent(object sender, EventArgs e)
        ...
        private bool CheckListInfo()
        {
            ...
            {
                decimal MdyUntPrice = Convert.ToDecimal(dgvr.Cells["ModyUnitprice"].Value.ToString());
                if (MdyUntPrice <= decimal.Zero)
                {
                    MessageBoxEx.Show("调整单价不能为负数或零!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return false;
                }
                if (dgvr.Cells["ModyAfterPric"].Value == null || dgvr.Cells["ModyAfterPric"].Value.ToString()=="")
                {
                    MessageBoxEx.Show("请输入调价比率!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return false;
                }
                else if (dgvr.Cells["ModyPricRate"].Value == null || dgvr.Cells["ModyPricRate"].Value.ToString()=="")
                {
