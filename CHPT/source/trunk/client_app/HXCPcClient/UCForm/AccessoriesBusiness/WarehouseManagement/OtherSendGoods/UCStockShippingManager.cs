       ...
       private void UCStockShippingManager_ExportEvent(object sender, EventArgs e)
        ...
        private void UCStockShippingManager_VerifyEvent(object send, EventArgs e)
        {
            try
            {
                ...
                if (ShippingIDValuelist.Count == 0 && gvShippingBillList.SelectedRows.Count == 0)
                {
                ...
                bool flag = DBHelper.BatchUpdateDataByIn(ShippingVerifyLogMsg, ShippingTable, ShippingBillField, ShippingID, ShippingIDValuelist.ToArray());//批量审核获取的其它发货单记录
                if (flag)
                {
                    if (UcVerifyStatus == DataSources.EnumAuditStatus.AUDIT)
                    {
                        CommonFuncCall.StatisticStock(GetShipPart(OrderIDDateDic), "其它发货单配件表");//同步更新其它发货单账面库存
                    }
        ...
        /// <summary>
        /// 获取其它发货单配件表已审核的单据配件信息
        /// </summary>
        /// <param name="OrderIDDateDic">单据ID与日期对</param>
        /// <returns>返回待统计配件信息</returns>
        private DataTable GetShipPart(Dictionary<string, long> OrderIDDateDic)
        {
            try
            {
                ...
                foreach (KeyValuePair<string, long> KVPair in OrderIDDateDic)
                {
                    ...
                    sbField.AppendFormat("{0},{1},PtTb.{2},ShipTb.{3},ShipTb.{4},ShipTb.{5},ShipTb.{6},ShipTb.{7},ShipTb.{8},{9},{10}",
                     WarehID, WarehName, PartID, PartsCode, PartName, PartSpec, PartBarCode, CarPartsCode, DrawNum, UnitName, PartCount);
                    string RelationTable = ShippingPartTable + " as ShipTb left join " + PartsTable +
                    " as PtTb on ShipTb.parts_code=PtTb.ser_parts_code ";//要查询的关联表名
                    TemplateTable = CommonFuncCall.CreatePartStatisticTable();//获取要填充的公用表
                    //获取对应单据的配件信息
                    DataTable IOPartTable = DBHelper.GetTable("查询其它发货单配件信息", RelationTable, sbField.ToString(), "stock_shipping_id='" + KVPair.Key.ToString() + "'", "", "");
        ...
        public void GetShippingBillList(string WhereStr)
        {
            try
            {
                ...
                for (int i = 0; i < ShipBillTable.Rows.Count; i++)
                {
                    ...
                    if (ShipBillTable.Rows[i][OrderStatus].ToString() == DataSources.GetDescription(DataSources.EnumAuditStatus.DRAFT, true))
                    {
            ...
            foreach (DataGridViewRow dr in gvShippingBillList.Rows)
            {
                ...
                if (isCheck != null && (bool)isCheck)
                {
                    ...
                    if (BillStatusSUBMIT == ColOrderStatus || BillStatusNOTAUDIT == ColOrderStatus)
                    {
                        ...
                        DicField.Add(dr.Cells["ShipId"].Value.ToString(),OrdeDate);//添加已审核单据主键ID和单据日期键值对
                        ShippingIDValuelist.Add(dr.Cells["ShipId"].Value.ToString());//添加其它发货单id
                    }
                }
            }
            return DicField;
