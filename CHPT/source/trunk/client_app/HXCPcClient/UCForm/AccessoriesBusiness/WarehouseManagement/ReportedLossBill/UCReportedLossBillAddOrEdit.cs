    ...
    public partial class UCReportedLossBillAddOrEdit : UCBase
    {
        ...
        private void UCStockReceiptAddOrEdit_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                CommonFuncCall.BindWarehouse(WhName);//给配件列表绑定仓库名称
                //公司ID
        ...
        private void BillImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UCImportSaleReturnBilling UCPB = new UCImportSaleReturnBilling();
        ...
        private void GetGridViewRowByDrImport(DataTable partsTable, string BillID)
        {
            try
            {
                ...
                for (int i = 0; i < partsTable.Rows.Count; i++)
                {
                   ...
                   ((DataGridViewComboBoxCell) dgRow.Cells["WhName"]).DisplayMember = partsTable.Rows[i]["wh_name"].ToString();
        ...
        private void GetGridViewRowByDr(DataGridViewRow dgvr, DataRow dr)
        {
            try
            {
                dgvr.Cells["BillingID"].Value = "";
                dgvr.Cells["billingno"].Value = "";
                dgvr.Cells["WHName"].Value = "";
                ...
                dgvr.Cells["UntName"].Value = dr["unit"];
                ...
                dgvr.Cells["Unitprice"].Value = dr["ref_in_price"];//成本价
        ...
        private void AddOrEditPartsListMsg(List<SysSQLString> listSQL, string LossBillId, WindowStatus WinState)
        {
            try
            {
                ...
                if (gvPartsMsgList.Rows.Count > 0)
                {
                    ...
                    sb.Append(sb_ColName.ToString() + ",create_time,update_time) values(@stock_loss_parts_id,@stock_loss_id,@num" + sb_ColValue.ToString() + ",@create_time,@update_time);");
                    int SerialNum = 1;
                    foreach (DataGridViewRow DgVRow in gvPartsMsgList.Rows)
                    {
                        ...
                        DicPartParam.Add("wh_id", DgVRow.Cells["WhName"].Value.ToString() == string.Empty ? "" : DgVRow.Cells["WhName"].Value.ToString());
                        DicPartParam.Add("wh_name", DgVRow.Cells["WhName"].EditedFormattedValue.ToString() == string.Empty ? "" : DgVRow.Cells["WhName"].EditedFormattedValue.ToString());
        ...
        private bool CheckListInfo()
        {
            ...
            foreach (DataGridViewRow dgvr in gvPartsMsgList.Rows)
            {
                ...
                {
                    decimal LossCount = Convert.ToDecimal(dgvr.Cells["counts"].Value.ToString());//报损数量
                    if (LossCount <= decimal.Zero)
                    {
                        MessageBoxEx.Show("您输入的数量不能小于等于零！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        gvPartsMsgList.CurrentCell.Style.BackColor = Color.Red;
                        return false;
                    }
                }
        ...
        private void GetBillPartsMsg(string LossBillIdValue)
        {
            try
            {
                ...
                if (PartsMsgList.Rows.Count > 0)
                {
                    ...
                    {
                        DataGridViewRow dgvr = gvPartsMsgList.Rows[gvPartsMsgList.Rows.Add()];
                        ((DataGridViewComboBoxCell)dgvr.Cells["WhName"]).DisplayMember = dr["wh_name"].ToString();//获取仓库名称
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void gvPartsMsgList_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0 || e.ColumnIndex < 0)
            {
                return;
            }
            switch (gvPartsMsgList.Columns[e.ColumnIndex].Name)
            {
                case "counts":
                case "Unitprice":
                    decimal LossCount = Common.ConvertToDecimal(gvPartsMsgList.Rows[e.RowIndex].Cells["counts"].Value);//报损数量
                    decimal LossUntPrice = Common.ConvertToDecimal(gvPartsMsgList.Rows[e.RowIndex].Cells["Unitprice"].Value);//报损单价
                    gvPartsMsgList.Rows[e.RowIndex].Cells["Calcmoney"].Value = LossUntPrice * LossCount;//计算报损总金额
                    break;
            }
        }
