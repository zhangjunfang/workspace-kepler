...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.AllocationBill
{
    public partial class UCAllocationBillDetails : UCBase
    {
        ...
        #endregion
        public UCAllocationBillDetails(string BillID)
        {
            InitializeComponent();
        ...
        private void UCAllocationBillDetails_Load(object sender, EventArgs e)
        {
            ...
            //CommonCtrl.DgCmbBindDict(partsbrandname, "sys_parts_brand");
        }
        ...
        private void GetBillPartsList()
        {
            try
            {
                ...
                int RowCount = 0;
                DataTable PartsTable = DBHelper.GetTableByPage(InOutQueryPartLogMsg, InOutPartTable, "*", StockInoutId + "='" + InoutBillID + "'",
                ...
                for (int i = 0; i < PartsTable.Rows.Count; i++)
                {
                    ...
                    if (CommonCtrl.IsNullToString(PartsTable.Rows[i]["make_date"]) != string.Empty && CommonCtrl.IsNullToString(PartsTable.Rows[i]["make_date"]) != "0")
                        DgRow.Cells["ProductionDate"].Value = Common.UtcLongToLocalDateTime(PartsTable.Rows[i]["make_date"].ToString());
                    if (CommonCtrl.IsNullToString(PartsTable.Rows[i]["validity_date"]) != string.Empty && CommonCtrl.IsNullToString(PartsTable.Rows[i]["validity_date"]) != "0")
                        DgRow.Cells["ValidityDate"].Value = Common.UtcLongToLocalDateTime(PartsTable.Rows[i]["validity_date"].ToString());
                    if (PartsTable.Rows[i]["is_gift"].ToString() == "1")
                    {
                        ((DataGridViewCheckBoxCell)DgRow.Cells["isgift"]).Value = Gift;
                    }
                    else if (PartsTable.Rows[i]["is_gift"].ToString() == "0")
                    {
