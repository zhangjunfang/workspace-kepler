    ...
    public partial class UCRequisitionAddOrEdit : UCBase
    {
        ...
        private void GetGridViewRowByDr(DataGridViewRow dgvr, DataRow dr)
        {
            try
            {
                ...
                dgvr.Cells["partcounts"].Value = dr["paper_count"];//该配件的账面库存数量
                dgvr.Cells["callinprice"].Value = dr["ref_in_price"];//成本价
        ...
        private bool CheckListInfo()
        {
            ...
            {
                if (dgvr.Cells["partcounts"].Value == null || dgvr.Cells["partcounts"].Value.ToString()=="")
                {
                ...
                else if (dgvr.Cells["callinprice"].Value == null || dgvr.Cells["callinprice"].Value.ToString() == "")
                {
                    MessageBoxEx.Show("请输入调入单价!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    gvPartsMsgList.CurrentCell = dgvr.Cells["callinprice"];
                    return false;
                }

                else
                {
                    decimal AllotCount = Convert.ToDecimal(dgvr.Cells["partcounts"].Value.ToString());//调拨数量
                    decimal AllotUntPrice = Convert.ToDecimal(dgvr.Cells["callinprice"].Value.ToString());//调拨单价
                    if (AllotCount <= decimal.Zero)
                    {
                        MessageBoxEx.Show("您输入配件数量不能小于等于零!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        gvPartsMsgList.CurrentCell = dgvr.Cells["partcounts"];
                        return false;
                    }
                    else if (AllotUntPrice <= decimal.Zero)
                    {
                        MessageBoxEx.Show("您输入调入单价不能小于等于零!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        gvPartsMsgList.CurrentCell = dgvr.Cells["callinprice"];
                        return false;
                    }
                }
        ...
        private void gvPartsMsgList_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (gvPartsMsgList.Columns[e.ColumnIndex].Name)
            {
                case "callinprice":
                case "partcounts":
                    decimal AllotCount = Common.ConvertToDecimal(gvPartsMsgList.Rows[e.RowIndex].Cells["partcounts"].Value);//调拨数量
                    decimal AllotUntPrice = Common.ConvertToDecimal(gvPartsMsgList.Rows[e.RowIndex].Cells["callinprice"].Value);//调拨单价
                    gvPartsMsgList.Rows[e.RowIndex].Cells["totalmoney"].Value = AllotUntPrice * AllotCount;//计算调拨总金额
                    break;
            }
        }
    }
