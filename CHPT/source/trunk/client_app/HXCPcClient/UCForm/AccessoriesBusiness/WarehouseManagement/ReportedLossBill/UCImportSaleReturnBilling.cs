...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.ReportedLossBill
{
    public partial class UCImportSaleReturnBilling : FormChooser
    {
        ...
        /// 获取销售退货单查询结果
        /// </summary>
        private void GetSaleBillResult()
        { 
           try
            {
        ...
        private void dgBillList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex > -1 && e.ColumnIndex > -1)  
            {
                ...
                BinddgPartsList(BillingNumID);//查询对应开单中的配件信息表               
        ...
        private void BinddgBilling(string BillTable, string BillPrimID, string BillLogMsg, string WhereStr)
        {
            try
            {
                ...
                ordertypename, ordernum, orderdate, allmoney, orgname, handlename, operatorname, rmk);
        ...
        { 
           try
           {
                ...
                for(int i=0;i<BillDataTable.Rows.Count;i++)
                {
                        ...
                        dgRow.Cells["balance_money"].Value = BillDataTable.Rows[i]["allmoney"].ToString();
     ...
     /// <param name="SubOrderType">开单中的子单据类型名称</param>
        private void BinddgPartsList(string BillingId)
        {
            try
            {
        ...
        private string BuildQueryWhere()
        {
            try
            {
                ...
                //获取出入库单与开单类型信息
                string StrWhere = " enable_flag='" + NODelState + "'" + " and is_occupy_stock='"
                + OpenState + "'"+" and order_status='" + Verified + "'";//查询条件字符串
                //销售退货开单
                ...
                if (StartTime.ToShortDateString() == EndTime.ToShortDateString())
                {
                    ...
                    DateWhere = " order_date>=" + Common.LocalDateTimeToUtcLong(Convert.ToDateTime(StartTime.Date.ToString())) + " and ";
                ...
                else
                {
                    ...
                    {
                        DateWhere += " order_date>=" + Common.LocalDateTimeToUtcLong(StartTime);
                    }
                    if (!string.IsNullOrEmpty(dateTimeEnd.Value.Trim().ToString()))
                    {
                        DateWhere += " and order_date<=" + Common.LocalDateTimeToUtcLong(EndTime) + " and ";
                    }
                }
