...
namespace HXCPcClient.UCForm.AccessoriesBusiness.SaleManagement.SaleBilling
{
    public partial class UCChooseSaleBill : FormChooser
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (List_PlanID.Count > 0)
                {
                    ...
                    {
                        DataTable dt_parts_sale = DBHelper.GetTable("查询销售开单配件表信息", "tb_parts_sale_billing_p", "*", " sale_billing_id='" + billid + "' and isnull(finish_count,0)<isnull(business_count,0) ", "", "");
        ...
        {
            //销售开单可以导入的前提是：单据状态是已审核、导入状态是正常的 三种情况
        ...
        private void BinddgAccessoriesDetail(string sale_billing_id)
        {
            if (!string.IsNullOrEmpty(sale_billing_id))
            {
                ...
                (
                select 
                ISNULL(business_price,0)*((ISNULL(tax_rate,0)/100)+1) tax_prices,
                case when (ISNULL(business_count,0)-ISNULL(finish_count,0))>0 
                        then (ISNULL(business_count,0)-ISNULL(finish_count,0))
                        else 0 end surplus_count,* 
                from tb_parts_sale_billing_p
                ) a");
                DataTable dt_parts_sale = DBHelper.GetTable("查询销售开单配件表信息", TableName, "*", " sale_billing_id='" + sale_billing_id + "' and isnull(finish_count,0)<isnull(business_count,0) ", "", "");
