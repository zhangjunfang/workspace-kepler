...
namespace HXCPcClient.UCForm.AccessoriesBusiness.SaleManagement.SaleBilling
{
    public partial class UCSaleBillAddOrEdit : UCBase
    {
        ...
        private void UCSaleBillAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            string[] NotReadOnlyColumnsName = new string[] { "colCheck", "wh_id", "unit_id", "original_price", "business_price", "business_count", "discount", "tax_rate", "is_gift", "make_date", "arrival_date", "remark","is_loss" };
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void UCSaleBillAddOrEdit_CancelEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (list_dr.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow dr in list_dr)
                    {
                        if (dr.Cells["parts_code"].Value != null)
                        {
                            #region 当删除成功时，将销售订单或是开单状态释放为正常
                            string relation_order = dr.Cells["relation_order"].Value == null ? "" : dr.Cells["relation_order"].Value.ToString();
        ...
        private void GetAccessories(string sale_billing_id)
        {
            ...
            if (dt_parts_sale != null && dt_parts_sale.Rows.Count > 0)
            {
                foreach (DataRow dr in dt_parts_sale.Rows)
                {
                    ...
                    dgvr.Cells["is_loss"].Value = dr["is_loss"];
                    dgvr.Cells["loss_count"].Value = dr["loss_count"];
                }
            }
        }
        /// <summary>
        ...
        private void DealAccessories(List<SysSQLString> listSql, string sale_billing_id)
        {
            ...
            if (gvPurchaseList.Rows.Count > 0)
            {
                foreach (DataGridViewRow dr in gvPurchaseList.Rows)
                {
                    ...
                    dic.Add("is_loss", dr.Cells["is_loss"].Value == null ? "0" : dr.Cells["is_loss"].Value.ToString());
                    dic.Add("loss_count", dr.Cells["loss_count"].Value == null ? "0" : dr.Cells["loss_count"].Value.ToString());
                    ...
                    update_time,ImportOrderType,finish_count,finish_count_stock,is_loss,loss_count,vm_name,parts_type_id,parts_type_name) values(@sale_billing_parts_id,@sale_billing_id,@wh_id,@wh_name,
                    @parts_id,@parts_code,@parts_name,@model,@drawing_num,@unit_id,@unit_name,@parts_brand,@parts_brand_name,@business_count,@original_price,@discount,@business_price,
                    @tax_rate,@tax,@payment,@valorem_together,@auxiliary_count,@relation_order,@is_gift,@make_date,@arrival_date,@remark,
                    @return_count,@library_count,@car_factory_code,@create_by,@create_name,@create_time,@update_by,@update_name,@update_time,
                    @ImportOrderType,@finish_count,@finish_count_stock,@is_loss,@loss_count,@vm_name,@parts_type_id,@parts_type_name);");
                        ...
                        if (dt_surplus != null && dt_surplus.Rows.Count > 0)
                        {
                            if (gvPurchaseList.Rows.Count > 0)
                            {
                                foreach (DataGridViewRow gvdr in gvPurchaseList.Rows)
                                {
                                    ...
                                    {
                                        business_count = Convert.ToDecimal(gvdr.Cells["business_count"].Value == null ? "0" : gvdr.Cells["business_count"].Value.ToString());
                                        business_count = business_count < 0 ? -business_count : business_count;
                                    }
                                    else
                                    { business_count = Convert.ToDecimal(gvdr.Cells["business_count"].Value == null ? "0" : gvdr.Cells["business_count"].Value.ToString()); }

                                    //decimal business_count = decimal.Parse(gvdr.Cells["business_count"].Value == null ? "0" : gvdr.Cells["business_count"].Value.ToString());
                                    ...
                                    else if (status == WindowStatus.Edit)
                                    {
                                        ...
                                        if (dt_old != null && dt_old.Rows.Count > 0)
                                        {
                                            ...
                                            {
                                                decimal oldcount = decimal.Parse(dr_old[0]["business_count"].ToString()) < 0 ? 0 : decimal.Parse(dr_old[0]["business_count"].ToString());
                                                if (oldcount > SurplusCount)
                                                {
                                                    MessageBoxEx.Show("引用单号为:" + order_num + ",配件编码为:" + parts_code + "的业务数量超出剩余数量，请修改!");
                                                    return;
                                                }
                                                        ...
                                                        select 
                                                         tb_sal_bill_revice.order_num,tb_sal_bill_revice.parts_code,tb_sal_bill_revice.business_count,
                                                          isnull(sum(case when (order_status='2' or order_status='3') and  tb_sal_bill.business_count<0 
                                                                          then ABS(tb_sal_bill.business_count) 
                                                                          else tb_sal_bill.business_count end),0) relation_count,
                                                          tb_sal_bill.ImportOrderType
                                                         from
                                                         (
                                                          --销售开单信息
        ...
        void SetOrderStatus(string orderid)
        {
            ...
            if (dt_Business.Rows.Count > 0)
            {
                ...
                {
                    bool isfinish = true;
                    decimal BusinessCount = decimal.Parse(dt_Business.Rows[i]["business_count"].ToString());
