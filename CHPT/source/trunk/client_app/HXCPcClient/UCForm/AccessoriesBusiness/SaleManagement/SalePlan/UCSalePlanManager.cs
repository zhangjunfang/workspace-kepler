...
namespace HXCPcClient.UCForm.AccessoriesBusiness.SaleManagement.SalePlan
{
    public partial class UCSalePlanManager : UCBase
    {
        ...
        #region 打印事件
        void UCSalePlanManager_PrintEvent(object sender, EventArgs e)
        {
        ...
        #region 预览事件
        void UCSalePlanManager_ViewEvent(object sender, EventArgs e)
        {
        ...
        #region 导出事件
        void UCSalePlanManager_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "销售计划单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, gvSalePlanList);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【销售计划单】" + ex.Message, "server");
        ...
        #region 预览、打印设置
        void UCSalePlanManager_SetEvent(object sender, EventArgs e)
        {
