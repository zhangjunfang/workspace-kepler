...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchasePlan
{
    public partial class UCPurchasePlanOrderManager : UCBase
    {
        ...
        public UCPurchasePlanOrderManager()
        {
            ...
            base.ViewEvent += new ClickHandler(UCPurchasePlanOrderManager_ViewEvent);
            base.PrintEvent += new ClickHandler(UCPurchasePlanOrderManager_PrintEvent);
            base.ExportEvent += new ClickHandler(UCPurchasePlanOrderManager_ExportEvent);
            base.SetEvent += new ClickHandler(UCPurchasePlanOrderManager_SetEvent);
            #region 预览、打印设置
            string printObject = "tb_parts_purchase_plan_o";
            string printTitle = "采购计划单";
        ...
        #region 导出事件
        void UCPurchasePlanOrderManager_ExportEvent(object sender, EventArgs e)
        {
            if (this.gvPurchasePlanOrderList.Rows.Count == 0)
            {
                return;
            }
            try
            {
                string fileName = "采购计划单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, gvPurchasePlanOrderList);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【采购计划单】" + ex.Message, "server");
        ...
        #region 打印事件
        void UCPurchasePlanOrderManager_PrintEvent(object sender, EventArgs e)
        {
            businessPrint.Print(gvPurchasePlanOrderList.GetBoundData());
        }
        #endregion

        #region 预览事件
        void UCPurchasePlanOrderManager_ViewEvent(object sender, EventArgs e)
        {
            businessPrint.Preview(gvPurchasePlanOrderList.GetBoundData());
        }
        #endregion

        #region 预览、打印设置
        void UCPurchasePlanOrderManager_SetEvent(object sender, EventArgs e)
        {
            businessPrint.PrintSet(gvPurchasePlanOrderList);
        } 
        #endregion
        ...
        void UCPurchasePlanOrderManager_VerifyEvent(object sender, EventArgs e)
        {
            ...
            if (UcVerify.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int i = 0; i < listField.Count; i++)
                {
                    ...
                    dic.Add("plan_id", listField[i]);
                    sysStringSql.sqlString = @"update tb_parts_purchase_plan set 
                ...
                if (DBHelper.BatchExeSQLStringMultiByTrans("采购计划单审核操作", list_sql))
                {
                    BindgvPurchasePlanOrderList();
                    MessageBoxEx.Show("操作成功！");
                }
