...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchaseOrder
{
    public partial class UCPurchaseOrderManager : UCBase
    {
        ...
        public UCPurchaseOrderManager()
        {
            ...
            base.ExportEvent += new ClickHandler(UCPurchaseOrderManager_ExportEvent);
            base.ViewEvent += new ClickHandler(UCPurchaseOrderManager_ViewEvent);
            base.PrintEvent += new ClickHandler(UCPurchaseOrderManager_PrintEvent);
            base.SetEvent += new ClickHandler(UCPurchaseOrderManager_SetEvent);
            #region 预览、打印设置
            string printObject = "tb_parts_purchase_order";
            string printTitle = "采购订单";
            List<string> listNotPrint = new List<string>();
            listNotPrint.Add(order_id.Name);
        ...
        #region 预览、打印设置
        void UCPurchaseOrderManager_SetEvent(object sender, EventArgs e)
        {
        ...
        #region 打印事件
        void UCPurchaseOrderManager_PrintEvent(object sender, EventArgs e)
        {
        ...
        #region 预览事件
        void UCPurchaseOrderManager_ViewEvent(object sender, EventArgs e)
        {
        ...
        #region 导出事件
        void UCPurchaseOrderManager_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "采购订单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, gvPurchaseOrderList);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【采购订单】" + ex.Message, "server");
        ...
        void UCPurchaseOrderManager_VerifyEvent(object sender, EventArgs e)
        {
            ...
            if (UcVerify.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int i = 0; i < listField.Count; i++)
                {
                    ...
                    dic.Add("order_id", listField[i]);
                    sysStringSql.sqlString = @"update tb_parts_purchase_order set 
                ...
                if (DBHelper.BatchExeSQLStringMultiByTrans("采购订单审核操作", list_sql))
                {
                    if (UcVerifyStatus == DataSources.EnumAuditStatus.AUDIT)
                    {
                ...
                foreach (DataGridViewRow dr in gvPurchaseOrderList.Rows)
                {
                    ...
                    if (isCheck != null && (bool)isCheck)
                    {
                        ...
                        if (dr.Cells["order_status"].Value.ToString() == Convert.ToInt32(DataSources.EnumAuditStatus.DRAFT).ToString())//草稿状态
                        {
                            ...
                            {
                                order_num = CommonUtility.GetNewNo(DataSources.EnumProjectType.PurchaseOrder);
                            }
                        }
        ...
        private void gvPurchaseOrderList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用   
            {
                ...
                string order_Id = this.gvPurchaseOrderList.CurrentRow.Cells["order_id"].Value.ToString();
