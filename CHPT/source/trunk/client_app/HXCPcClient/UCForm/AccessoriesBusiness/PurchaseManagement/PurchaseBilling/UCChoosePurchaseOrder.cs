...
{
    public partial class UCChoosePurchaseOrder : FormChooser
    {
        #region 变量、类
        public int dgPlanRowIndex = -1;
        private string sup_id = string.Empty;
        public List<PartsInfoClassByPurchaseOrder> List_PartInfo = new List<PartsInfoClassByPurchaseOrder>();
        public DataTable dt_PurchaseOrder = null;
        #endregion
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (List_PlanID.Count > 0)
                {
                    ...
                    {
                        DataTable dt_parts_purchase = DBHelper.GetTable("查询采购订单配件表信息", "tb_parts_purchase_order_p", "*", " order_id='" + order_id + "' and is_suspend=1 and isnull(finish_counts,0)<isnull(business_counts,0) ", "", "");
        ...
        private void dgPurchaseOrder_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用  
            {
                ...
                string order_id = Convert.ToString(this.dgPurchaseOrder.CurrentRow.Cells[0].Value.ToString());
                BinddgAccessoriesDetail(order_id);
            }
        }
        /// <summary> 采购订单信息列表 单元格内容点击事件
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        void dgAccessoriesDetail_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            try
            {
                if (e.Value == null || e.Value.ToString().Length == 0)
                ...
                string fieldNmae = dgAccessoriesDetail.Columns[e.ColumnIndex].DataPropertyName;
                if (fieldNmae.Equals("arrival_date"))
                {
                    long ticks = (long)e.Value;
                    e.Value = Common.UtcLongToLocalDateTime(ticks).ToShortDateString();
                }
        ...
        {
            ////采购订单可以导入的前提是：已审核、在有效期、未中止、开单数量小于订单数量、导入状态是正常的 五种情况
            ...
            //}
            if (dateTimeArrival_date.Value != null)
            {
                DateTime dtime = Convert.ToDateTime(dateTimeArrival_date.Value.ToShortDateString() + " 23:59:59");
                Str_Where += " and arrival_date<=" + Common.LocalDateTimeToUtcLong(dtime);
            }
        ...
        private void BinddgPurchaseOrder()
        {
            try
            {
                DataTable gvPurchaseOrder_dt = DBHelper.GetTable("查询采购订单列表信息", "tb_parts_purchase_order", "*", BuildString(), "", "");
                dgPurchaseOrder.DataSource = gvPurchaseOrder_dt;
                dt_PurchaseOrder = gvPurchaseOrder_dt;
            }
            catch (Exception ex)
            {
                //异常日志
        ...
        /// <summary>绑定采购订单配件列表
        /// </summary>
        private void BinddgAccessoriesDetail(string order_id)
        {
            if (!string.IsNullOrEmpty(order_id))
            {
                string TableName = string.Format(@" (
                                                     ...
                                                     from tb_parts_purchase_order_p 
                                                    ) a");
                DataTable dt_parts_purchase = DBHelper.GetTable("查询采购订单配件表信息", TableName, "*", " order_id='" + order_id + "' and is_suspend=1 and isnull(finish_counts,0)<isnull(business_counts,0) ", "", "");
