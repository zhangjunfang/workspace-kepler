...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchaseOrder
{
    public partial class UCPurchaseOrderManagerView : UCBase
    {
        ...
        public UCPurchaseOrderManagerView()
        {
            ...
            base.ExportEvent += new ClickHandler(UCPurchaseOrderManagerView_ExportEvent);
            base.ViewEvent += new ClickHandler(UCPurchaseOrderManagerView_ViewEvent);
            base.PrintEvent += new ClickHandler(UCPurchaseOrderManagerView_PrintEvent);
            base.SetEvent += new ClickHandler(UCPurchaseOrderManagerView_SetEvent);
            #region 预览、打印设置            
            paperSize.Width = 297;
        ...
        #region 打印事件
        void UCPurchaseOrderManagerView_PrintEvent(object sender, EventArgs e)
        {
        ...
        #region 导出功能
        void UCPurchaseOrderManagerView_ExportEvent(object sender, EventArgs e)
        {
            if (tabControlEx1.SelectedIndex == 0)
            {
                ExportToxls(gvPurchaseOrderList, "按采购订单查询");
            }
            else if (tabControlEx1.SelectedIndex == 1)
            {
                ExportToxls(gvPurchaseList2, "按配件货供应商查询");
            }
        }
        ...
        #region 预览、打印设置
        void UCPurchaseOrderManagerView_SetEvent(object sender, EventArgs e)
        {
            if (tabControlEx1.SelectedIndex == 0)
            {
                printObject = "tb_parts_purchase_order_s";
        ...
        private void SupName2_DataBack(DataRow dr)
        {
            ...
            {
                this.txtsup_name2.Caption = dr["sup_full_name"].ToString();
            }
        }
