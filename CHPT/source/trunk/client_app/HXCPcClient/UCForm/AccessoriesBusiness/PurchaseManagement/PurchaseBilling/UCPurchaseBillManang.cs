...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchaseBilling
{
    public partial class UCPurchaseBillManang : UCBase
    {
        ...
        #region 打印事件
        void UCPurchaseBillManang_PrintEvent(object sender, EventArgs e)
        {
        ...
        #region 预览事件
        void UCPurchaseBillManang_ViewEvent(object sender, EventArgs e)
        {
        ...
        #region 导出事件
        void UCPurchaseBillManang_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "采购开单" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, gvPurchaseOrderList);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("【采购开单】" + ex.Message, "server");
        ...
        #region 预览、打印设置
        void UCPurchaseBillManang_SetEvent(object sender, EventArgs e)
        {
...
//                            }
        ...
        /// <returns></returns>
        bool GetIsSubmit(string purchase_billing_id, string order_status)
        {
            ...
            if (dt_surplus != null && dt_surplus.Rows.Count > 0)
            {
                ...
                if (dtPurchaseList.Rows.Count > 0)
                {
                    foreach (DataRow drr in dtPurchaseList.Rows)
                    {
                        ...
                        //decimal business_counts = decimal.Parse(drr["business_counts"] == null ? "0" : drr["business_counts"].ToString());
                        if (order_num != "" && (order_status == "2" || order_status == "3"))
                        {
                            business_counts = Convert.ToDecimal(drr["business_counts"] == null ? "0" : drr["business_counts"].ToString());
