...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchaseBilling
{
    public partial class UCPurchaseBillManangView : UCBase
    {
        ...
        public UCPurchaseBillManangView()
        {
            ...
            base.ExportEvent += new ClickHandler(UCPurchaseBillManangView_ExportEvent);
            base.ViewEvent += new ClickHandler(UCPurchaseBillManangView_ViewEvent);
            base.PrintEvent += new ClickHandler(UCPurchaseBillManangView_PrintEvent);
            base.SetEvent += new ClickHandler(UCPurchaseBillManangView_SetEvent);
            #region 预览、打印设置
            paperSize.Width = 297;
        ...
        #region 打印事件
        void UCPurchaseBillManangView_PrintEvent(object sender, EventArgs e)
        {
        ...
        #region 导出事件
        void UCPurchaseBillManangView_ExportEvent(object sender, EventArgs e)
        {
            if (tabControlEx1.SelectedIndex == 0)
            {
                ExportToxls(gvPurchaseOrderList, "按采购开单查询");
            }
            else if (tabControlEx1.SelectedIndex == 1)
            {
                ExportToxls(gvPurchaseList2, "采购开单按配件货供应商查询");
            }
        }
