...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.YuTongPurchaseOrder
{
    public partial class UserControlXuQiu : UserControl
    {
        ...
        {
            InitializeComponent();
            ddlallot_type.SelectedValueChanged += new EventHandler(ddlallot_type_SelectedValueChanged);
        ...
        #region 当选择正常备货时，紧急程度修改为A5
        void ddlallot_type_SelectedValueChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(ddlallot_type.SelectedValue.ToString()))
            {
                if (ddlallot_type.SelectedValue.ToString() == "1")
                {
                    if (ddlemergency_level.Items.Count > 0)
                    {
                        foreach (var item in ddlemergency_level.Items)
                        {
                            if (((ListItem)item).Text.Contains("A5"))
                            {
                                ddlemergency_level.SelectedValue = ((ListItem)item).Value;
                                return;
                            }
                        }
                        ddlemergency_level.SelectedValue = "";
                    }
                }
            }
        } 
