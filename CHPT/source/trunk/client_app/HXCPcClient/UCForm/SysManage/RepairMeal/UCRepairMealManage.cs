...
namespace HXCPcClient.UCForm.SysManage.RepairMeal
{
    ...
    public partial class UCRepairMealManage : UCBase
    {
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            dtiVaild.StartDate = string.Empty;
            dtiVaild.EndDate = string.Empty;
        ...
        public void BindPageData()
        {
            ...
            {
                #region 事件选择判断
                if (dtiVaild.StartDate != string.Empty && dtiVaild.StartDate != string.Empty && DateTime.Parse(dtiVaild.StartDate) > DateTime.Parse(dtiVaild.EndDate))
                {
                ...
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(dtiVaild.StartDate)))//有效期起
                {
                    strWhere += string.Format(" and period_validity >= '{0}'", Common.LocalDateTimeToUtcLong(DateTime.Parse(dtiVaild.StartDate)));
                }
                if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(dtiVaild.EndDate)))//有效期至
                {
                    strWhere += string.Format(" and valid_until <= '{0}'", Common.LocalDateTimeToUtcLong(DateTime.Parse(dtiVaild.EndDate).Date.AddDays(1).AddMilliseconds(-1)));
                }
                int recordCount;
