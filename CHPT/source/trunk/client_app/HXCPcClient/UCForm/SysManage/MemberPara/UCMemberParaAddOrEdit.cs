    ...
    public partial class UCMemberParaAddOrEdit : UCBase
    {
        ...
        private void BindDgvParts(DataTable dt)
        {
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    decimal iValue = Convert.ToDecimal(dr["parts_discount"].ToString());
                    bool bln = false;
        ...
        private void BindDgvProjrct(DataTable dt)
        {
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    decimal iValue = Convert.ToDecimal(dr["service_project_discount"].ToString());
                    bool bln = false;
                    decimal quota_price = 0;
        ...
        void UCMemberParaAddOrEdit_SaveEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DBHelper.IsExist("查询会员等级", "tb_CustomerSer_member_setInfo", " member_grade_id='" + cbomember_grade_id.SelectedValue.ToString() + "' and enable_flag='1' ") && windowStatus == WindowStatus.Add)
                {
                    Utility.Common.Validator.SetError(errorProvider1, cbomember_grade_id, "会员等级已存在，请先到字典码表新增!");
                    return;
                }

                string Service_project_discount = Utility.Common.Validator.IsDecimal(txtservice_project_discount.Caption.Trim(), 2, 2, ref bln);
                bln = decimal.Parse(txtservice_project_discount.Caption.Trim()) > 100 ? false : true;
                if (!bln)
                {
                    Utility.Common.Validator.SetError(errorProvider1, txtservice_project_discount, "服务项目折扣格式不正确!限定0-100");
                    return;
                }
                string Parts_discount = Utility.Common.Validator.IsDecimal(txtparts_discount.Caption.Trim(), 2, 2, ref bln);
                bln = decimal.Parse(txtparts_discount.Caption.Trim()) > 100 ? false : true;
                if (!bln)
                {
                ...
                string Subscription_Ratio = Utility.Common.Validator.IsDecimal(txtSubscription_Ratio.Caption.Trim(), 2, 2, ref bln);
                bln = decimal.Parse(txtSubscription_Ratio.Caption.Trim()) > 100 ? false : true;
                if (!bln)
                {
        ...
        private string GetUpdteInsertPartsSql(string setInfo_id)
        {
            ...
            foreach (DataGridViewRow dr in dgvparts.Rows)
            {
                if (dr.Cells[this.setInfo_parts_id.Name].Value.ToString().Length == 0)
                {
                    ...
                    if (dr.Index == dgvprojrct.Rows.Count - 1)
