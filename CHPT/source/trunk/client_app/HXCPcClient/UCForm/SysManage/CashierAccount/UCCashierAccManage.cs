...
namespace HXCPcClient.UCForm.SysManage.CashierAccount
{
    ...
    public partial class UCCashierAccManage : UCBase
    {
        ...
        void DeleteData()
        {
            ...
            foreach (DataGridViewRow dgvr in dgvRecord.Rows)
            {
                ...
                if (isCheck != null && (bool)isCheck)
                {
                    ...
                    listName.Add(dgvr.Cells[this.colAccountName.Name].Value.ToString());
                }
            }
            ...
            {
                if (!CanDelete(listField[i]))
                {
                    MessageBoxEx.Show(listName[i] + "已被占用,不可删除!");
                    return;
                }
            }
        ...
        #region 账户占用判断
        bool CanDelete(string accountid)
        {
            SQLObj sqlinfo = new SQLObj();
            sqlinfo.Param = new Dictionary<string, ParamObj>();
            sqlinfo.sqlString = string.Format("select * from tb_payment_detail where payment_account='{0}'", accountid);
            sqlinfo.cmdType = CommandType.Text;
            DataTable dt = DBHelper.GetDataSet("查询出纳账户是否占用", sqlinfo).Tables[0];
