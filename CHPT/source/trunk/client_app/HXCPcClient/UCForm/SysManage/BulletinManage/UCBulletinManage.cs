...
namespace HXCPcClient.UCForm.SysManage.BulletinManage
{
    ...
    public partial class UCBulletinManage : UCBase
    {
        ...
        private void InitEvent()
        {
            ...
            base.PrintEvent -= new ClickHandler(UC_PrintEvent);
            base.PrintEvent += new ClickHandler(UC_PrintEvent);
        }
        ...
        void UC_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "公告管理" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, dgvRecord);
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("公告管理" + ex.Message, "server");
                MessageBoxEx.ShowWarning("导出失败！");
            }
        }
        //报表设计器
        void UC_SetEvent(object sender, EventArgs e)
        {
        ...
        #region --绘制标题
        private void DrawHead(Graphics g, int width, int x, int y)
        {
            string title = "公告信息表";
            Font headFont = new Font("宋体", 14F);
            int tWidth = (int)g.MeasureString(title, headFont).Width;
            g.DrawString(title, headFont, new SolidBrush(Color.Black),
            x + (width - tWidth) / 2, y);
        }
        #endregion
        #endregion
        ...
        public void BindPageData()
        {
            if (this.myLock)
            {
                ...
                if (!string.IsNullOrEmpty(txtKeyNmae.Caption.Trim()))
                {
                    ...
                    where += string.Format(" or  content like '%{0}%' ) ", txtKeyNmae.Caption.Trim());
                }
                ...
                {
                    where += string.Format(" and date_up >= {0} and  date_up<= {1}",
        ...
        {
            if (this.verify_list.Count + this.revoke_list.Count > 0)
            {
        ...
        private void dgvRecord_HeadCheckChanged()
        {
            ...
            if (this.btnRevoke.Enabled)
            {
                this.btnRevoke.Enabled = false;
            }
            if (this.btnVerify.Enabled)
            {
                this.btnVerify.Enabled = false;
            }
            if (this.btnEdit.Enabled)
