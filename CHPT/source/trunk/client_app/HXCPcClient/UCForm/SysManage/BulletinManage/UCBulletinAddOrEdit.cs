...
namespace HXCPcClient.UCForm.SysManage.BulletinManage
{
    ...
    public partial class UCBulletinAddOrEdit : UCBase
    {
        ...
        private void SetBtnStatus()
        {
            ...
            {
                SetSysManageEditBtn();
                btnVerify.Visible = true;//审核按钮
                btnStatus.Visible = false;//启用
            }
        ...
        void UCBulletinAddOrEdit_SaveEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                //如果有接收组织 而没有接收个人 那就只给组织发送
                if (!string.IsNullOrEmpty(rtbOrganization.Text.Trim())
                    && string.IsNullOrEmpty(rtbPerson.Text.Trim()))
                {
                    this.SaveOrg(listSql, id);
                }
                //如果有接收个人，而没有接收组织，就只能个人发
                else if (!string.IsNullOrEmpty(rtbPerson.Text.Trim())
                    && string.IsNullOrEmpty(rtbOrganization.Text.Trim()))
                {
        ...
        private void SaveOrg(List<SQLObj> listSql, string strId)
        {
            ...
            string strOrgId = "";
            if (rtbOrganization.Tag != null)
            {
                strOrgId = rtbOrganization.Tag.ToString();
            }



            if (!string.IsNullOrEmpty(strOrgId) && !string.IsNullOrEmpty(strOrgName))
            {
            ...
            else
            {
                ...
                for (int i = 0; i < ds.Rows.Count; i++)
                {
                    ...
                    Dictionary<string, ParamObj> dicParam = new Dictionary<string, ParamObj>();
                    dicParam.Add("announcement_id", new ParamObj("announcement_id", strId, SysDbType.VarChar, 40));//公告Id 
                    dicParam.Add("org_id", new ParamObj("org_id", ds.Rows[i]["org_id"], SysDbType.VarChar, 40));//组织结构ID
                    dicParam.Add("org_name", new ParamObj("org_name", ds.Rows[i]["org_name"], SysDbType.VarChar, 40));//组织结构名
                    dicParam.Add("announcement_org_id", new ParamObj("announcement_org_id", Guid.NewGuid().ToString("N"), SysDbType.VarChar, 40));//Id
                    obj.sqlString = @"insert into sys_announcement_org (announcement_id,org_id,org_name,announcement_org_id)
                    values (@announcement_id,@org_id,@org_name,@announcement_org_id);";
                    obj.Param = dicParam;
                    listSql.Add(obj);
        ...
        private void SavePerson(List<SQLObj> listSql, string strId)
        {
            ...
            if (string.IsNullOrEmpty(strPerName))
            {
                strPerName = "[全部]";
            }
            if (strPerName == "[全部]")
            {
                DataTable ds = Organization();
                var a = ds.AsEnumerable().Select(A => A.Field<string>("org_id"));
                string strOrgId = "";
                foreach (var item in a)
                {
                    strOrgId += item.ToString() + ",";
                }

                strOrgId = strOrgId.Remove(strOrgId.Length - 1, 1);
