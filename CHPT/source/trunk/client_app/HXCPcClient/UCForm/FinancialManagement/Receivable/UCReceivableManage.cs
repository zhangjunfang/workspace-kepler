...
namespace HXCPcClient.UCForm.FinancialManagement.Receivable
{
    public partial class UCReceivableManage : UCBase
    {
        ...
        private void SetQuick()
        {
            ...
            {
                //设置供应商编码速查
                txtCustCode.SetBindTable("tb_supplier", "sup_code", "sup_full_name");
            }
        ...
        void txtcCustName_DataBacked(DataRow dr)
        {
            if (orderType == DataSources.EnumOrderType.RECEIVABLE)
            {
                ...
                txtCustCode.Text = CommonCtrl.IsNullToString(dr["cust_code"]);
            }
            else if (orderType == DataSources.EnumOrderType.PAYMENT)
            {
                txtCustName.Caption = CommonCtrl.IsNullToString(dr["sup_full_name"]);
                txtCustCode.Text = CommonCtrl.IsNullToString(dr["sup_code"]);
            }
        }
        ...
        #region 工具栏事件
        //设置
        void UCReceivableManage_SetEvent(object sender, EventArgs e)
        {
            businessPrint.PrintSet(dgvBillReceivable);
        }
