    ...
    public partial class UCReceivableAdd : UCBase
    {
        ...
        private void UCReceivableAdd_Load(object sender, EventArgs e)
        {
            ...
            if (windowStatus != WindowStatus.View)
            {
                ...
                tsmiImport.Visible = true;
                tsmiImportRepair.Visible = true;
                tsmiImportThree_.Visible = true;
            }
            else
            {
                ...
                tsmiImportRepair.Visible = false;
                tsmiImportThree_.Visible = false;
            }
        ...
        private void SetQuick()
        {
            ...
            {
                //设置客户编码速查
                txtcCustName.SetBindTable("tb_customer", "cust_name");
            }
            else if (orderType == DataSources.EnumOrderType.PAYMENT)
            {
                //设置供应商编码速查
                txtcCustName.SetBindTable("tb_supplier", "sup_full_name");
            }
            txtcCustName.GetDataSourced += new TextChooser.GetDataSourceHandler(txtcCustName_GetDataSourced);
            txtcCustName.DataBacked += new TextChooser.DataBackHandler(txtcCustName_DataBacked);
        ...
        void txtcCustName_DataBacked(DataRow dr)
        {
            ...
            if (orderType == DataSources.EnumOrderType.RECEIVABLE)
            {
                ...
                txtcCustName.Tag = custID;
                cust_code = CommonCtrl.IsNullToString(dr["cust_code"]);
                txtBankAccount.Caption = CommonCtrl.IsNullToString(dr["bank_account"]);
                txtBankOfDeposit.Caption = CommonCtrl.IsNullToString(dr["dic_name"]);
            }
            else if (orderType == DataSources.EnumOrderType.PAYMENT)
            {
                custID = CommonCtrl.IsNullToString(dr["sup_id"]);
                txtcCustName.Text = CommonCtrl.IsNullToString(dr["sup_full_name"]);
                txtcCustName.Tag = custID;
                cust_code = CommonCtrl.IsNullToString(dr["sup_code"]);
            }
        ...
        //设置
        void UCReceivableAdd_SetEvent(object sender, EventArgs e)
        {
            GetPrintData();
        ...
        {
            GetPrintData();
            detailPrint.DetailPreview(dicPrint);
        }
        //删除
        ...
        void GetPrintData()
        {
            ...
            dtFinancial.Columns.Add("order_num");
            dtFinancial.Columns.Add("order_date");
            dtFinancial.Columns.Add("order_status");
            dtFinancial.Columns.Add("cust_name");
            dtFinancial.Columns.Add("payment_type");
            dtFinancial.Columns.Add("payment_money");
            dtFinancial.Columns.Add("dealings_balance");
            dtFinancial.Columns.Add("bank_of_deposit");
            dtFinancial.Columns.Add("bank_account");
            dtFinancial.Columns.Add("remark");
            dtFinancial.Columns.Add("org_name");
            dtFinancial.Columns.Add("handle_name");
            dtFinancial.Columns.Add("operator_name");
            dtFinancial.Columns.Add("create_name");
            dtFinancial.Columns.Add("create_time");
            dtFinancial.Columns.Add("update_name");
            dtFinancial.Columns.Add("update_time");
            dtFinancial.Columns.Add("print_name");
            DataRow drFinancial = dtFinancial.NewRow();
            drFinancial["order_num"] = txtOrderNum.Caption;
            drFinancial["order_date"] = dtpOrderDate.Value.ToString("yyyy-MM-dd");
            drFinancial["order_status"] = txtOrderStatus.Caption;
            drFinancial["cust_name"] = txtcCustName.Text;
            drFinancial["payment_type"] = cboOrderType.Text;
            drFinancial["payment_money"] = txtAdvance.Caption;
            drFinancial["dealings_balance"] = txtBalance.Caption;
            drFinancial["bank_of_deposit"] = txtBankOfDeposit.Caption;
            drFinancial["bank_account"] = txtBankAccount.Caption;
            drFinancial["remark"] = txtRemark.Caption;
            drFinancial["org_name"] = cboOrgId.Text;
            drFinancial["handle_name"] = cboHandle.Text;
            drFinancial["operator_name"] = lblOperator.Text;
            drFinancial["create_name"] = lblCreateBy.Text;
            drFinancial["create_time"] = lblCreateTime.Text;
            drFinancial["update_name"] = lblUpdateBy.Text;
            drFinancial["update_time"] = lblUpdateTime.Text;
            drFinancial["print_name"] = GlobalStaticObj.UserName;
            dtFinancial.Rows.Add(drFinancial);
            dicPrint.Add("tb_bill_receivable", dtFinancial);
            #endregion
            dicPrint.Add("tb_payment_detail", dgvPaymentDetail.GetBoundData());
            dicPrint.Add("tb_balance_documents", dgvBalanceDocuments.GetBoundData());
        }
        //删除
        void DeleteData()
        {
...
where a.order_status in ('1','2') and b.order_id=order_id
        ...
        bool CheckReceivable()
        {
            ...
            else if (txtBankAccount.Caption.Trim().Length > 0 && !ValidateUtil.IsBankCard(txtBankAccount.Caption.Trim()))
            {
                epError.SetError(txtBankAccount, "银行账号格式有误！");
                txtBankAccount.Focus();
                check = false;
                return false;
            }
        ...
        {
            if ((dgvPaymentDetail.AllowUserToAddRows && dgvPaymentDetail.RowCount <= 1) || (!dgvPaymentDetail.AllowUserToAddRows && dgvPaymentDetail.RowCount <= 0))
            {
            ...
            foreach (DataGridViewRow dgvr in dgvPaymentDetail.Rows)
            {
                ...
                else if (Common.CheckDecimalLength(dgvr.Cells[colMoney.Name].Value, 13))
                {
                    MessageBoxEx.ShowError("金额超出系统限制，请检查输入！");
                    check = false;
                    break;
                }
                ...
                string checkNumber = CommonCtrl.IsNullToString(dgvr.Cells[colCheckNumber.Name].Value);
                ...
                else if (checkNumber.Length > 0 && !ValidateUtil.IsCharNumber(checkNumber))
                {
                    MessageBoxEx.Show("票号只能输入英文字符或数字！");
                    dgvPaymentDetail.CurrentCell = dgvr.Cells[colCheckNumber.Name];
                    check = false;
                    break;
                }
        ...
        bool CheckBalanceDocuments(ref decimal settlement_money)
        {
            ...
            foreach (DataGridViewRow dgvr in dgvBalanceDocuments.Rows)
            {
                ...
                else if (Common.CheckDecimalLength(dgvr.Cells[colSettlementMoney.Name].Value, 13))
                {
                    MessageBoxEx.ShowError("本次结算超出系统限制，请检查输入！");
                    check = false;
                    break;
                }
                ...
                else if (Common.CheckDecimalLength(dgvr.Cells[colPaidMoney.Name].Value, 13))
                {
                    MessageBoxEx.ShowError("实收金额超出系统限制，请检查输入！");
                    check = false;
                    break;
                }
        ...
        bool CheckIsDecimalOrInt(DataGridView dgv, DataGridViewRow dgvr, DataGridViewColumn dgvc)
        {
            ...
            {
                decimal de = 0;
                if (!decimal.TryParse(num, out de))
                {
        ...
        {
            //预付、预收且没有单据时，不验证总合计
            if (CommonCtrl.IsNullToString(cboOrderType.SelectedValue) == "0" && dgvBalanceDocuments.RowCount == 0)
            {
                return true;
            }
        ...
        bool CheckDocuments()
        {
            string strOrderType = CommonCtrl.IsNullToString(cboOrderType.SelectedValue);
            //预收,预付不用验证
            if (strOrderType.Length == 0 || strOrderType == "0")
            {
                return true;
            }
...
,@create_by,@create_time)";
     ...
     VALUES
        ...
        void BindData()
        {
            ...
            if (orderType == DataSources.EnumOrderType.PAYMENT)
            {
                DataSources.EnumPaymentType paymentType = (DataSources.EnumPaymentType)Convert.ToInt32(dr["payment_type"]);
                if (paymentType == DataSources.EnumPaymentType.PAYMENT)
                {
                    detailPrint.PrintTitle = "财务应付款单";
                }
                else if (paymentType == DataSources.EnumPaymentType.ADVANCES)
                {
                    detailPrint.PrintTitle = "财务预付款单";
                }
                dt.DateTableToEnum("payment_type", typeof(DataSources.EnumPaymentType));
            }
            else if (orderType == DataSources.EnumOrderType.RECEIVABLE)
            {
                DataSources.EnumReceivableType receivableType = (DataSources.EnumReceivableType)Convert.ToInt32(dr["payment_type"]);
                if (receivableType == DataSources.EnumReceivableType.RECEIVABLE)
                {
                    detailPrint.PrintTitle = "财务应收款单";
                }
                else if (receivableType == DataSources.EnumReceivableType.ADVANCES)
                {
                    detailPrint.PrintTitle = "财务预收款单";
                }
                dt.DateTableToEnum("payment_type", typeof(DataSources.EnumReceivableType));
            }
            dt.Columns.Add("print_name");//打印人
        ...
        void BindDetail()
        {
            string strTable = @"tb_payment_detail a 
  left join tb_balance_way b on a.balance_way=b.balance_way_id
  left join tb_cashier_account c on a.bank_account=c.cashier_account";
            string files = "a.*,b.balance_way_name,c.account_name";
            DataTable dt = DBHelper.GetTable("", strTable, files, string.Format("order_id='{0}'", orderID), "", "order by create_time");
            dgvPaymentDetail.Rows.Clear();
            if (dt == null || dt.Rows.Count == 0)
            {
                if (dt == null)
                ...
                if (dt.Columns.Count == 0)
                {
                    ...
                    dt.Columns.Add("balance_way");
                    dt.Columns.Add("payment_account");
                    dt.Columns.Add("bank_of_deposit");
        ...
        void BindBalanceDocuments(DataSources.EnumAuditStatus enumAuditStatus)
        {
            ...
            if (dt == null || dt.Rows.Count == 0)
            {
                ...
                if (dt.Columns.Count == 0)
                {
                    dt.Columns.Add("documents_name");
                    dt.Columns.Add("documents_num");
                    dt.Columns.Add("documents_date");
                    dt.Columns.Add("billing_money");
                    dt.Columns.Add("settled_money");
                    dt.Columns.Add("wait_settled_money");
                    dt.Columns.Add("settlement_money");
                    dt.Columns.Add("paid_money");
                }
            ...
            dt.DataTableToDate("documents_date");
            dicPrint.Add("tb_balance_documents", dt);
        }
        #endregion
        ...
        private void txtcCustName_ChooserClick(object sender, EventArgs e)
        {
            ...
            if (orderType == DataSources.EnumOrderType.RECEIVABLE)
            {
                ...
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    string tables = "tb_customer a left join sys_dictionaries b on a.open_bank=b.dic_id ";
                    //字动带出账户信息
                    DataTable dt = DBHelper.GetTable("", tables, "b.dic_name,a.bank_account", string.Format("cust_id='{0}'", frm.strCustomerId), "", "");
                    if (dt == null || dt.Rows.Count == 0)
                    {
                        return;
                    }
                    txtBankAccount.Caption = CommonCtrl.IsNullToString(dt.Rows[0]["bank_account"]);
                    txtBankOfDeposit.Caption = CommonCtrl.IsNullToString(dt.Rows[0]["dic_name"]);
                    epError.Clear();
            ...
            else if (orderType == DataSources.EnumOrderType.PAYMENT)
            {
                ...
                switch (enumType)
                {
                    case DataSources.EnumPaymentType.ADVANCES:
                        tsmiImport.Text = "导入采购订单";
                        isVisible = false;
                        detailPrint.PrintTitle = "财务预付款单";
                        break;
                    case DataSources.EnumPaymentType.PAYMENT:
                        isVisible = true;
                        tsmiImport.Text = "导入采购开单";
                        detailPrint.PrintTitle = "财务应付款单";
                        break;
                }
            }
        ...
        private void txtBankAccount_ValueChanged(object sender, EventArgs e)
        {
            epError.Clear();
        }
        ...
        private void tsmiImport_Click(object sender, EventArgs e)
        {
            ...
            switch (tsmiImport.Text)
            {
                case "导入销售开单":
                    tsmiSale_Click(sender, e);
                    break;
                case "导入销售订单":
                    ImportReceivableByAdvances();
                    break;
