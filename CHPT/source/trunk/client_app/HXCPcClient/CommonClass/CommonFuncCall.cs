...
namespace HXCPcClient
{
    ...
    public class CommonFuncCall
    {
                    ...
                    Label_01D9:
                        ...
                        if (info.GetValue(p_ModelObject, null).ToString().Trim() != "0")
                        {
                            ...
                            if (list.Items.Count > 0)
                            {
                                ...
                                {
                                    if (((ListItem)item).Value.ToString() == Convert.ToString(obj2))
                                    {
                                        list.SelectedValue = Convert.ToString(obj2);
                                        break;
                                    }
        ...
        {
            DataTable dt = DBHelper.GetTable("", "tb_company", "com_id,com_name", " status=1 and enable_flag=1 and (data_source is null or  data_source !='2') ", "", "");
            List<ListItem> list = new List<ListItem>();
            list.Add(new ListItem("", typename));
            foreach (DataRow dr in dt.Rows)
            {
        ...
        public static void BindDepartment(ComboBoxEx ControlName, string com_id, string typename)
        {
            ...
            {
                DataTable dt = DBHelper.GetTable("", "tb_organization", "org_id,org_name", string.Format("com_id='{0}' and status=1 and enable_flag='1'", com_id), "", "");
                foreach (DataRow dr in dt.Rows)
                {
        ...
        {
            string sql_where = " enable_flag=1  and status=1 ";
            List<ListItem> list = new List<ListItem>();
            list.Add(new ListItem("", typename));
        ...
        public static void BindUser(ComboBoxEx ControlName, bool isShowFrist, string typename)
        {
            ...
            //公司ID
            string com_id = GlobalStaticObj.CurrUserCom_Id;
            DataTable dt = DBHelper.GetTable("查询收货人信息", "sys_user", "cont_crm_guid,user_name", " LEN(cont_crm_guid)>0 and enable_flag='1' ", "", "");
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
        ...
        public static void GetStationInfo(ref string com_tel, ref string com_phone, ref string street,ref string com_person)
        {
            try
            {
                ...
                string tableanme = string.Format(@" tb_company ");
                DataTable dt = DBHelper.GetTable("从帐套通用表中获取当前服务站信息", GlobalStaticObj.CommAccCode, tableanme, files, " data_source='2' ", "", "");
                if (dt != null && dt.Rows.Count > 0)
                {
        ...
        /// <summary> 根据服务站联系人crmid，获取联系人的手机和固话
        /// </summary>
        /// <param name="cont_crm_guid"></param>
        /// <param name="user_phone"></param>
        /// <param name="user_telephone"></param>
        public static void GetUserPhone(string cont_crm_guid,ref string user_phone,ref string user_telephone)
        {
            if (!string.IsNullOrEmpty(cont_crm_guid))
            {
                DataTable dt = DBHelper.GetTable("查询收货人信息", "sys_user", "user_phone,user_telephone", " cont_crm_guid='" + cont_crm_guid + "' and enable_flag='1' ", "", "");
                if (dt != null && dt.Rows.Count > 0)
                {
                    user_phone = dt.Rows[0]["user_phone"].ToString();
                    user_telephone = dt.Rows[0]["user_telephone"].ToString();
                }
            }
        }
            ...
            try
            {
                ...
                for (int i = 0; i < FieldsArray.Length; i++)
                {
                    ...
                    if (TxtSubCtl != null)
                    {
                        ...
                        {
                            if (decimal.TryParse(txtMsg.Caption.ToString(), out ModifyRatio))
                            {
                                FieldsArray[i].SetValue(ObjEntity, Convert.ToDecimal(txtMsg.Caption.ToString()), null);//设置属性值
                            }
                            else
                            {
        ...
        public static void StatisticStock(DataTable BusinessPartsTable, string OperateLogMsg)
        {
            try
            {
                ...
                for (int i = 0; i < StatisUniqueTable.Rows.Count; i++)
                {
                    ...
                    StockStatisTable = DBHelper.GetTable("查询库存统计表", "tb_parts_stock_p", "statistic_date,stock_part_id,statistic_count,statistic_Type",
                    "statistic_date=" +Convert.ToInt64(StatisUniqueTable.Rows[i]["OrderDate"].ToString()) + " and stock_part_id='" +
                    StatisUniqueTable.Rows[i]["PartID"].ToString() + "'" + " and statistic_Type=" + StatisUniqueTable.Rows[i]["StatisticType"].ToString(), "", "");
                    ...
                    if (StockStatisTable.Rows.Count == 0)//统计库存
                    {
                        ...
                        DicPartParam.Add(StockUpdateDate, "0");//更新日期
                    ...
                    {
                        decimal OldStatisticCount = Convert.ToDecimal(StockStatisTable.Rows[0][PartStatisticCount].ToString());//原来统计的配件数量
                        decimal NewStatisticCount = 0;//新添加的配件数量
                        if (RepeatPartCount == 0)
                        {
                            NewStatisticCount = Convert.ToDecimal(StatisUniqueTable.Rows[i]["PartCount"].ToString());
                        }
                        else
                        {
                            NewStatisticCount = RepeatPartCount;
                        }
                        decimal StatisticCount = OldStatisticCount + NewStatisticCount;//获取当前最新配件统计数量
                ...
                bool ret=DBHelper.BatchExeSQLStringMultiByTrans(OperateLogMsg, SqlStr);//执行sql语句操作
            }
            catch (Exception ex)
            {
       ...
       /// <param name="typename"></param>
        public static void BindCompanyWarehouse(ComboBox ControlName, string com_id, string typename)
        {
            List<ListItem> list = new List<ListItem>();
            ...
            if (string.IsNullOrEmpty(com_id))
            {
                where_str = " enable_flag='1' ";
            }
            else
            {
                where_str = " enable_flag='1' and com_id='" + com_id + "'";
            }
       ...
       /// <param name="FTimePoint">时间点1</param>
       /// <param name="STimePoint">时间点2</param>
       /// <param name="TTimePoint">时间点3</param>
       /// <returns></returns>
        public static DataTable GetStockAge(string StrWhere,int FTimePoint,int STimePoint,int TTimePoint)
        { 
          try
          {
              DataTable StockTable = DBHelper.GetTable("库龄分析查询", "v_stockage", "*", StrWhere, "", "");
              DataTable StockAgeAnalysisTable = CreateStockAgeTable();//获取库龄分析表结构
              StockTable.Columns.Add("InStock_date", typeof(string));
              for (int i = 0; i < StockTable.Rows.Count;i++ )
              {
                  string CreateDate = Common.UtcLongToLocalDateTime(StockTable.Rows[i]["create_time"].ToString());
                  int SpaceIndex=CreateDate.IndexOf(" ",0);
                  StockTable.Rows[i]["InStock_date"] = CreateDate.Remove(SpaceIndex);
              }
              DataView StockUniqueView = StockTable.DefaultView;
              DataTable StockUniqueTable = StockUniqueView.ToTable("UniqueRows", true, "parts_code");//去除重复记录数据
              //过滤分组统计库龄
              for (int j = 0; j < StockUniqueTable.Rows.Count; j++)
              {
                  string FilterWhere = "parts_code='" + StockUniqueTable.Rows[j]["parts_code"].ToString() + "'";//过滤条件
                    DataView StockView = StockTable.DefaultView;
                    StockView.RowFilter = FilterWhere;
                    DataTable FilterTable= StockView.ToTable();
                    DataRow dr = StockAgeAnalysisTable.NewRow();
                    dr["parts_code"] = FilterTable.Rows[0]["parts_code"].ToString();
                    dr["parts_name"] = FilterTable.Rows[0]["parts_name"].ToString();
                    dr["wh_id"] = FilterTable.Rows[0]["wh_id"].ToString();
                    dr["model"] = FilterTable.Rows[0]["model"].ToString();
                    dr["drawing_num"] = FilterTable.Rows[0]["drawing_num"].ToString();
                    dr["parts_brand"] = FilterTable.Rows[0]["parts_brand"].ToString();
                    dr["car_parts_code"] = FilterTable.Rows[0]["car_parts_code"].ToString();
                    dr["unit_name"] = FilterTable.Rows[0]["unit_name"].ToString();
                    if (string.IsNullOrEmpty(FilterTable.Rows[0]["ref_in_price"].ToString()))
                    {
                        dr["ref_in_price"] =decimal.Zero;
                    }
                    else
                    {
                        dr["ref_in_price"] = FilterTable.Rows[0]["ref_in_price"].ToString();
                    }
                    //总库存量和金额
                    decimal RefInPrice = Convert.ToDecimal(dr["ref_in_price"].ToString());//当前配件单价
                    decimal  TotalCount=Convert.ToDecimal( FilterTable.Compute("sum(statistic_count)", ""));//当前配件总库存量
                    dr["TotalPhyCount"] = TotalCount;
                    dr["TotalAmount"] = RefInPrice * TotalCount;
                    decimal ThirtyCount = decimal.Zero;
                    decimal SixtyCount = decimal.Zero;
                    decimal MoreSixtyCount = decimal.Zero;
                    //不同库龄的库存量和金额
                    for (int k = 0; k < FilterTable.Rows.Count; k++)
                    {
                        DateTime CreateDate = Convert.ToDateTime(FilterTable.Rows[k]["InStock_date"].ToString());
                        DateTime CurrentDate = Convert.ToDateTime(DateTime.Now.ToShortDateString());
                        TimeSpan ts = CurrentDate.Subtract(CreateDate);
                        int TimeInterval=ts.Days;//获取时间间隔
                        if (TimeInterval >= 0 && TimeInterval<FTimePoint)
                        {
                            ThirtyCount += Convert.ToDecimal(FilterTable.Rows[k]["statistic_count"].ToString());//当前配件0--30天数量
                        }
                        else if (TimeInterval >= FTimePoint && TimeInterval < STimePoint)
                        {
                            SixtyCount += Convert.ToDecimal(FilterTable.Rows[k]["statistic_count"].ToString());//当前配件30--60天数量
                        }
                        else if (TimeInterval >= TTimePoint)
                        {
                            MoreSixtyCount += Convert.ToDecimal(FilterTable.Rows[k]["statistic_count"].ToString());//当前配件60天以上的数量
                        }

                    }
                    dr["ThirtyPhyCount"] = ThirtyCount;
                    dr["ThirtyAmount"] = RefInPrice * ThirtyCount;
                    dr["SixtyPhyCount"] = SixtyCount;
                    dr["SixtyAmount"] = RefInPrice * SixtyCount;
                    dr["MoreSixtyPhyCount"] = MoreSixtyCount;
                    dr["MoreSixtyAmount"] = RefInPrice * MoreSixtyCount;
                    StockAgeAnalysisTable.Rows.Add(dr);  
              }

              return StockAgeAnalysisTable;
        ...
        public static DataTable CreateStockAgeTable()
        {
            DataTable StockAgeTable = new DataTable();
            StockAgeTable.Columns.Add("parts_code",typeof(string));
            StockAgeTable.Columns.Add("parts_name", typeof(string));
            StockAgeTable.Columns.Add("wh_id", typeof(string));
            StockAgeTable.Columns.Add("model", typeof(string));
            StockAgeTable.Columns.Add("drawing_num", typeof(string));
            StockAgeTable.Columns.Add("parts_brand", typeof(string));
            StockAgeTable.Columns.Add("car_parts_code", typeof(string));
            StockAgeTable.Columns.Add("unit_name", typeof(string));
            StockAgeTable.Columns.Add("ref_in_price", typeof(string));
            StockAgeTable.Columns.Add("TotalPhyCount", typeof(string));//配件总库存量
            StockAgeTable.Columns.Add("TotalAmount", typeof(string));//配件总库存金额
            StockAgeTable.Columns.Add("ThirtyPhyCount", typeof(string));//配件0--30天库存量
            StockAgeTable.Columns.Add("ThirtyAmount", typeof(string));//配件0--30天库存金额
            StockAgeTable.Columns.Add("SixtyPhyCount", typeof(string));//配件30--60天库存量
            StockAgeTable.Columns.Add("SixtyAmount", typeof(string));//配件30--60天库存金额
            StockAgeTable.Columns.Add("MoreSixtyPhyCount", typeof(string));//配件60天以上的库存量
            StockAgeTable.Columns.Add("MoreSixtyAmount", typeof(string));//配件60天以上的库存金额
            return StockAgeTable;
        }
