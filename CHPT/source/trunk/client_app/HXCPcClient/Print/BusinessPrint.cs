...
namespace HXCPcClient
{
    ...
    public class BusinessPrint
    {
        ...
        /// 打印设置
        /// </summary>
        public void PrintSet(DataGridView dgv)
        {
            //报表设置
            frmReportSet frmSet = new frmReportSet();
            frmSet.setObject = printObject;
        ...
        void SetReportSet(DataGridView dgv)
        {
            //查询用户对报表的设置
            DataTable dt = DBHelper.GetTable("", "tb_report_set", "*", string.Format("set_object='{0}' and set_user='{1}'", printObject, GlobalStaticObj.UserID), "", "");
            if (dt == null || dt.Rows.Count == 0)
            {
                return;
            }
            ...
            {
                string dataName = CommonCtrl.IsNullToString(dr["set_data_name"]);
                foreach (DataGridViewColumn dgvc in dgv.Columns)
                {
                    if (dgvc.DataPropertyName == dataName)
                    {
                        dgvc.Visible = CommonCtrl.IsNullToString(dr["is_show"]) == "1";//控制是否显示
                        dgvc.Width = Convert.ToInt32(dr["set_width"]);//列宽
                        break;
                    }
                }
            }
