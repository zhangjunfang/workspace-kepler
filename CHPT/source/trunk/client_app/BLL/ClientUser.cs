...
namespace BLL
{
    public class ClientUser
    {
        public static string UserLogin(LoginInput loginInput, string IPStr, string currAccDbName, out DataSet ds)
        {
            ...
            {
                return "对不起，您的登陆账号不存在或被删除，不能登入系统，请检查您的登陆账号。";
            }
            ...
            {
                return "对不起，您的登录密码不正确，不能登入系统，请联系管理员。";
            }
            if (status != DataSources.EnumStatus.Start.ToString("d"))
            {
                return "对不起，您的账号被停用，不能登入系统，请联系管理员。";
            }
            #endregion
        ...
        public static void UserLoginOut(string UserID, string currAccCode)
        {
            ...
            sysSqlString = new SysSQLString();
            sysSqlString.cmdType = CommandType.Text;
            sysSqlString.sqlString = "UPDATE [sys_log_log] set exit_time=" + exitTime.ToString() + " where  login_time=(select max(login_time) from sys_log_log where user_id='" + UserID + "' and exit_time is null)";
            list.Add(sysSqlString);
