...
namespace HXCServerWinForm
{
    public partial class HXCServerMainForm : FormExhyh
    {
        ...
        public HXCServerMainForm()
        {
            ...
            InitContentMenu();  //add by kord
        }

        #region add by kord -- 更改系统菜单按钮样式
        ...
        private void deleteMenu(string strTag, string PUCName)
        {
            try
            {
                ...
                if (this.menus.ContainsKey(strArr[0]))
                {
                    //删除菜单
                    this.menus.Remove(strArr[0]);
                }
        ...
        private void tSMenuItem_PWDReSet_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //}
                HXCServerWinForm.UCForm.frmPersionSet frm = new HXCServerWinForm.UCForm.frmPersionSet();
                frm.Text = "修改密码";
                frm.ShowDialog();
            }
            catch (Exception ex)
        ...
        private void tSMenuItem_Softupdate_Click(object sender, EventArgs e)
        {
            string currVersion = "V" + Assembly.GetExecutingAssembly().GetName().Version.ToString();
            currVersion = currVersion.Substring(0, currVersion.Length - 2);
            //检测升级
            string strUpData = AutoUpdate.CheckUpdate.ClientUpDataExe("HXCServerWinForm.exe", currVersion, "1");
            if (strUpData != "")
            {
                string[] arr = strUpData.Split('|');
                if (arr.Length == 2 && arr[0] == "UP")
                {
                    string version = arr[1];
                    AutoUpdate.frmDownload frm = new AutoUpdate.frmDownload();
                    frm.ShowDialog();
                    return;
                }
                else
                {
                    MessageBox.Show("检测升级版本失败，" + strUpData, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            }
            else
            {
                MessageBoxEx.ShowInformation("已经是最新版本");
            }
        }

        private void tSMenuItem_Help_Click(object sender, EventArgs e)
        {
        ...
        //数据初始化
        private void tSMenuItem_Initialize_Click(object sender, EventArgs e)
        {
            yuTongWebService.WebServ_YT_BasicData.InitData_Customer();
            yuTongWebService.WebServ_YT_BasicData.InitData_Contact();
        ...
        private void loadHome()
        {
            ...
            if (panel_LeftMenu.Controls.Find("S_SysMaintenance", true).Length == 0)
            {
                ...
                this.ucHome = new UCForm.UUCHomePage(); //首页
        ...
        /// <summary> 同步crm_id
        /// </summary>
        public string SyncCrmID()
        {
            //DataTable dt = DBHelper.GetTable("获取缺少crmid的用户", GlobalStaticObj_Server.DbPrefix + GlobalStaticObj_Server.Instance.MainAccCode, "sys_user", "*", "cont_crm_guid is null or cont_crm_guid=''", "", "");
            //List<Model.tb_contacts_ex> list = new List<Model.tb_contacts_ex>();
