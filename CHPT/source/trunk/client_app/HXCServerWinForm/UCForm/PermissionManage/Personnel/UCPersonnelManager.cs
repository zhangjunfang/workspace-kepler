...
namespace HXCServerWinForm.UCForm.Personnel
{
    ...
    public partial class UCPersonnelManager : UCBase
    {
        ...
        private void UCPersonnelManager_Load(object sender, EventArgs e)
        {
            ...
            DataSources.BindComBoxDataEnum(cbbstatus, typeof(DataSources.EnumStatus), false, false);//绑定状态 启用 停用
            DataSources.BindComBoxDataEnum(cbbis_operator, typeof(DataSources.EnumYesNo), false, false);//是否操作员
            BindTree();
        ...
        void UCPersonnelManager_EditEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                UCPersonnelAddOrEdit.userid = dgvUser.CurrentRow.Cells["user_id"].Value.ToString();  //参数 人员管理ID
                base.addUserControl(UCPersonnelAddOrEdit, "人员管理-编辑", "UCPersonnelEdit" + UCPersonnelAddOrEdit.userid, this.Tag.ToString(), this.Name);
            }
            catch (Exception ex)
            {
                GlobalStaticObj_Server.GlobalLogService.WriteLog("人员管理", ex);
        ...
        void UCPersonnelManager_DeleteEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                if (flag)
                {
                    ...
                    whereStr += ")";
                    DBHelper.BatchDeleteDataByWhere("批量删除角色人员关系", GlobalStaticObj_Server.DbPrefix + GlobalStaticObj_Server.CommAccCode, "tr_user_role", whereStr);
                    #endregion
        ...
        private void BindTree()
        {
            ...
            {
                InitTree(tvCompany.Nodes[0].Nodes, rootid, ds.Tables[0].DefaultView);
            }
