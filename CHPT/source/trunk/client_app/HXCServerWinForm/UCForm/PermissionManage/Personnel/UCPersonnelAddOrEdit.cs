    ...
    public partial class UCPersonnelAddOrEdit : UCBase
    {
        ...
        void UCPersonnelAddOrEdit_SaveEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                dicFileds.Add("org_id", cmbOrganize.SelectedValue.ToString());//组织id 
                //dicFileds.Add("com_id", cmbCompany.SelectedValue.ToString());//公司id 
                ...
                {
                    dicFileds.Add("user_code", txtuser_code.Caption.Trim());//人员编码
                ...
                SQLObj sqlObj1 = new SQLObj();
                sqlObj1.cmdType = CommandType.Text;
                sqlObj1.sqlString = string.Format("delete from tr_user_role where user_id='{0}'", userid);
                listSql.Add(sqlObj1);
                sqlObj1 = new SQLObj();
                sqlObj1.cmdType = CommandType.Text;
                sqlObj1.sqlString = string.Format("insert into tr_user_role(user_role_id,user_id,role_id) values('{0}','{1}','{2}')", Guid.NewGuid(), userid, cmbRole.SelectedValue.ToString());
                listSql.Add(sqlObj1);
        ...
        private bool Validator(ref string msg)
        {
            ...
            if (cmbOrganize.SelectedIndex <= 0)// string.IsNullOrEmpty(txcorg_name.Tag.ToString().Trim()))
            {
                Utility.Common.Validator.SetError(errorProvider1, cmbOrganize, "请选择组织!");
                return false;
            }
            ...
            if (cbois_operator.SelectedValue.ToString() == "1" && string.IsNullOrEmpty(cmbRole.SelectedValue.ToString().Trim()))
            {
                Utility.Common.Validator.SetError(errorProvider1, cmbRole, "请选择角色!");
                return false;
            }
            if (cbois_operator.SelectedValue.ToString() == "1" && string.IsNullOrEmpty(txtland_name.Caption.Trim()))
            {
        ...
        private void BindControls(DataTable dt)
        {
            ...
            cmbCompany.SelectedValue = dt.Rows[0]["com_id"].ToString();
            cmbOrganize.SelectedValue = dt.Rows[0]["org_id"].ToString();
            ...
            if (cbois_operator.SelectedValue.ToString() == "1")
            {
                if (!string.IsNullOrEmpty(txtland_name.Caption))
                {
                    txtland_name.ReadOnly = true;
                }
                else
            ...
            DataTable dt_pic = DBHelper.GetTable("查询用户图片", GlobalStaticObj_Server.DbPrefix + GlobalStaticObj_Server.CommAccCode, "attachment_info", "*", "relation_object_id='" + userid + "' and att_name='用户图片' and att_type='图片'", "", "");
            ...
            BindAData("sys_user", userid);
            string roleid = DBHelper.GetSingleValue("获取关联的角色id", GlobalStaticObj_Server.DbPrefix + GlobalStaticObj_Server.CommAccCode, "tr_user_role", "role_id", "user_id='" + userid + "'", "");
