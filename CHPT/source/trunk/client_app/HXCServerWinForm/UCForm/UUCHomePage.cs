...
left join sys_dictionaries h on b.maintain_type=h.dic_id";
        ...
        private void btnStock_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int PartsCount = StockTable.Rows[i]["statis_count"].ToString() == string.Empty ? 0 : Convert.ToInt32(StockTable.Rows[i]["statis_count"]);
        ...
        private void btnTest_Click(object sender, EventArgs e)
        {
            ...
            yuTongWebService.WebServ_YT_BasicData.LoadHitchMode();
        }
        ...
        public void BindDatatoFrm()
        {
            ...
            if (m.Success)
            {
                GlobalStaticObj_Server.Instance.DbIP = m.Value;
        ...
        public void BindSignInfo()
        {
            ...
            GlobalStaticObj_Server.Instance.StationID = dr["sign_id"].ToString();
            ...
            if (string.IsNullOrEmpty(dtStr) || Convert.ToInt64(dtStr) < Common.LocalDateTimeToUtcLong(GlobalStaticObj_Server.Instance.CurrentDateTime))
            {
                long dateInt = 0;
                dateInt = CommonUtility.GetSoftValidate();
                if (dateInt > 0)
                {
                    DateTime date = Common.UtcLongToLocalDateTime(dateInt);
                    lblprotocol_expires_time.Text = date.ToString("yyyy-MM-dd");
                    if (dateInt < Common.LocalDateTimeToUtcLong(GlobalStaticObj_Server.Instance.CurrentDateTime))
                    {
                        lblprotocol_expires_time.Text += "(已过期)";
                        lblprotocol_expires_time.ForeColor = System.Drawing.Color.Red;
                        MessageBox.Show("您的软件已过期，请联系软件提供商！");
                        Application.Exit();
                        return;
                    }
                }
                ...
                {
                    MessageBox.Show("获取软件有效期出现异常，请重新启动！");
                    Application.Exit();
                    return;
                }
            }
