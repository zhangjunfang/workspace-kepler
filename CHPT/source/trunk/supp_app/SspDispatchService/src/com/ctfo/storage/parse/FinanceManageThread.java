 ...
 * @since JDK1.6
 */
public class FinanceManageThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(FinanceManageThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 财务管理数据Map */
	private static Map<String, List<Object>> financeMap = new ConcurrentHashMap<String, List<Object>>();
	...
	private void process(Object o) {
		List<Object> list = financeMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = financeMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = financeMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		FinanceManageThread.lastTime = lastTime;
	}
