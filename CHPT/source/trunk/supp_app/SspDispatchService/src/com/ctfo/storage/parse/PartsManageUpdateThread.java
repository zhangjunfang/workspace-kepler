 ...
 * @since JDK1.6
 */
public class PartsManageUpdateThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(PartsManageUpdateThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 库存模块更新数据Map */
	private static Map<String, List<Object>> partsUpdateMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public PartsManageUpdateThread() {
	...
	private void process(Object o) {
		List<Object> list = partsUpdateMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = partsUpdateMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = partsUpdateMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		PartsManageUpdateThread.lastTime = lastTime;
	}
