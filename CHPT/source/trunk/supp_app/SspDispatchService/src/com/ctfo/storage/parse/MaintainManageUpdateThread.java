 ...
 * @since JDK1.6
 */
public class MaintainManageUpdateThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(MaintainManageUpdateThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 维修数据Map */
	private static Map<String, List<Object>> maintainUpdateMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public MaintainManageUpdateThread() {
	...
	public void run() {
		while (true) {
			try {
				...
				logger.error("维修管理更新数据处理队列异常:" + e.getMessage());
			}
		}
	}

	private void process(Object o) {
		List<Object> list = maintainUpdateMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = maintainUpdateMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = maintainUpdateMap.get(key);
	...
	public static Map<String, List<Object>> getMaintainUpdateMap() {
		return maintainUpdateMap;
	}

	public static void setMaintainUpdateMap(Map<String, List<Object>> maintainUpdateMap) {
		MaintainManageUpdateThread.maintainUpdateMap = maintainUpdateMap;
	}
	...
	public static void setLastTime(long lastTime) {
		MaintainManageUpdateThread.lastTime = lastTime;
	}
