 ...
 * @since JDK1.6
 */
public class BaseDataManageThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(BaseDataManageThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 基础数据Map */
	private static Map<String, List<Object>> baseDataMap = new ConcurrentHashMap<String, List<Object>>();
	...
	private void process(Object o) {
		List<Object> list = baseDataMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = baseDataMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = baseDataMap.get(key);
	...
	public static Map<String, List<Object>> getBaseDataMap() {
		return baseDataMap;
	}

	public static void setBaseDataMap(Map<String, List<Object>> baseDataMap) {
		BaseDataManageThread.baseDataMap = baseDataMap;
	}
	...
	public static void setLastTime(long lastTime) {
		BaseDataManageThread.lastTime = lastTime;
	}
