 ...
 * @since JDK1.6
 */
public class PartsManageThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(PartsManageThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 库存模块数据Map */
	private static Map<String, List<Object>> partsMap = new ConcurrentHashMap<String, List<Object>>();
	...
	private void process(Object o) {
		List<Object> list = partsMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = partsMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = partsMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		PartsManageThread.lastTime = lastTime;
	}
