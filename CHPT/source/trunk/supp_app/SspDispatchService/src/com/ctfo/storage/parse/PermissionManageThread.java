 ...
 * @since JDK1.6
 */
public class PermissionManageThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(PermissionManageThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 权限数据Map */
	private static Map<String, List<Object>> permissionMap = new ConcurrentHashMap<String, List<Object>>();
	...
	private void process(Object o) {
		List<Object> list = permissionMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		...
		while (iter.hasNext()) {
			...
			List<Object> value = permissionMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		PermissionManageThread.lastTime = lastTime;
	}
