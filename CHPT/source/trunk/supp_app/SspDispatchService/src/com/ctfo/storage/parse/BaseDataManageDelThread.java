 ...
 * @since JDK1.6
 */
public class BaseDataManageDelThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(BaseDataManageDelThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 基础数据Map */
	private static Map<String, List<Object>> baseDataDelMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public BaseDataManageDelThread() {
	...
	private void process(Object o) {
		List<Object> list = baseDataDelMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = baseDataDelMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = baseDataDelMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		BaseDataManageDelThread.lastTime = lastTime;
	}
