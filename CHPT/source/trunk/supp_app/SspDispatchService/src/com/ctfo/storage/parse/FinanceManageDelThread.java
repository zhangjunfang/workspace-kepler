 ...
 * @since JDK1.6
 */
public class FinanceManageDelThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(FinanceManageDelThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 财务管理更新数据Map */
	private static Map<String, List<Object>> financeDelMap = new ConcurrentHashMap<String, List<Object>>();
	...
	private void process(Object o) {
		List<Object> list = financeDelMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = financeDelMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = financeDelMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		FinanceManageDelThread.lastTime = lastTime;
	}
