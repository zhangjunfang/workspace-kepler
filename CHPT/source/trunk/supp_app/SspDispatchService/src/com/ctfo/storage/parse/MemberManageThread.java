 ...
 * @since JDK1.6
 */
public class MemberManageThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(MemberManageThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 会员管理数据Map */
	private static Map<String, List<Object>> memberMap = new ConcurrentHashMap<String, List<Object>>();
	...
	private void process(Object o) {
		List<Object> list = memberMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = memberMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = memberMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		MemberManageThread.lastTime = lastTime;
	}
