 ...
 * @since JDK1.6
 */
public class SystemSettingsManageUpdateThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(SystemSettingsManageUpdateThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 权限数据更新Map */
	private static Map<String, List<Object>> systemSettingsUpdateMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public SystemSettingsManageUpdateThread() {
	...
	private void process(Object o) {
		List<Object> list = systemSettingsUpdateMap.get(o.getClass().getSimpleName());
		if (null == list) {
		...
		if (list.size() == batchSize) {
			...
			systemSettingsUpdateMap.remove(o.getClass().getSimpleName());
			logger.info("【{}】 批量提交:{}, 耗时:{}ms, 共提交总数:{}", o.getClass().getSimpleName(), list.size(), (System.currentTimeMillis() - start), processSize);
		} else {
			systemSettingsUpdateMap.put(o.getClass().getSimpleName(), list);
		}
	}
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = systemSettingsUpdateMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = systemSettingsUpdateMap.get(key);
			...
			systemSettingsUpdateMap.remove(key);
	...
	public static void setLastTime(long lastTime) {
		SystemSettingsManageUpdateThread.lastTime = lastTime;
	}
