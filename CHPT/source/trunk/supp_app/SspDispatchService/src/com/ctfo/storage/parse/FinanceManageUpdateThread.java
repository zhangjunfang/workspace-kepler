 ...
 * @since JDK1.6
 */
public class FinanceManageUpdateThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(FinanceManageUpdateThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 财务管理更新数据Map */
	private static Map<String, List<Object>> financeUpdateMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public FinanceManageUpdateThread() {
	...
	public void run() {
		while (true) {
			try {
				...
				logger.error("财务管理数据处理队列数据异常:" + e.getMessage());
			}
		}
	}

	private void process(Object o) {
		List<Object> list = financeUpdateMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = financeUpdateMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = financeUpdateMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		FinanceManageUpdateThread.lastTime = lastTime;
	}
