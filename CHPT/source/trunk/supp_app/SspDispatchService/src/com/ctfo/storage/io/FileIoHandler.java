...
import com.ctfo.storage.parse.FileParse;
...
public class FileIoHandler extends IoHandlerAdapter {
	...
	private final static Logger logger = LoggerFactory.getLogger(FileIoHandler.class);
	...
	public void messageReceived(IoSession iosession, Object message) throws Exception {
		try {
			if (message instanceof String) {
				...
				if (FileResponseListen.getFileMsgMap().containsKey(arr[0])) {
					parse.put(message.toString());
				}
			}
		} catch (Exception e) {
			logger.info("---附件通讯接收处理器异常:" + e.getMessage(), e);
		}
	}
	...
	public void sessionClosed(IoSession session) throws Exception {
		...
		if (FileResponseListen.getFileMsgMap().size() > 0) {
			for (Map.Entry<String, IoSession> entry : FileResponseListen.getFileMsgMap().entrySet()) {
				if (entry.getValue().equals(session)) {
					FileResponseListen.getFileMsgMap().remove(entry.getKey());
				}
			}
		}
	}
