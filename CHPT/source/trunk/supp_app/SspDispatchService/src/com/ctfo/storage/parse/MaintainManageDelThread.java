 ...
 * @since JDK1.6
 */
public class MaintainManageDelThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(MaintainManageDelThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 维修数据Map */
	private static Map<String, List<Object>> maintainDelMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public MaintainManageDelThread() {
	...
	private void process(Object o) {
		List<Object> list = maintainDelMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = maintainDelMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = maintainDelMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		MaintainManageDelThread.lastTime = lastTime;
	}
