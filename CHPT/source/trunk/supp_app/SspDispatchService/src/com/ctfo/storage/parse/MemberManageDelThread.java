 ...
 * @since JDK1.6
 */
public class MemberManageDelThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(MemberManageDelThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 会员管理删除数据Map */
	private static Map<String, List<Object>> memberDelMap = new ConcurrentHashMap<String, List<Object>>();
	...
	private void process(Object o) {
		List<Object> list = memberDelMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = memberDelMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = memberDelMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		MemberManageDelThread.lastTime = lastTime;
	}
