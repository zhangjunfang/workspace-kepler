...
public class IoServer {
	...
	public void start() throws Exception {
		...
		acceptor.getFilterChain().addLast("codec", new ProtocolCodecFilter(new ByteCodecFactory()));
		...
		// acceptor.getSessionConfig().setIdleTime(IdleStatus.BOTH_IDLE, 120); // 读写通道在60秒内无任何操作进入空闲状态
		...
		try {
			acceptor.bind();
			logger.info("服务端启动成功，端口：" + port + ",时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
		} catch (IOException e) {
			logger.error("服务端" + port + "异常 " + ",时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + ", 连接MSG异常,请检查MSG端口、IP是否正确,MSG服务是否启动,异常内容:" + e.getMessage(), e);
		}
		logger.info("TCP服务启动，端口：" + port);
	}
