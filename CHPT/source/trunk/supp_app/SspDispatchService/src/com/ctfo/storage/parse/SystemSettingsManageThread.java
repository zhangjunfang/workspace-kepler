 ...
 * @since JDK1.6
 */
public class SystemSettingsManageThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(SystemSettingsManageThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 权限数据Map */
	private static Map<String, List<Object>> systemSettingsMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public SystemSettingsManageThread() {
	...
	private void process(Object o) {
		List<Object> list = systemSettingsMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = systemSettingsMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = systemSettingsMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		SystemSettingsManageThread.lastTime = lastTime;
	}
