 ...
 * @since JDK1.6
 */
public class PermissionManageUpdateThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(PermissionManageUpdateThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 权限数据更新Map */
	private static Map<String, List<Object>> permissionUpdateMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public PermissionManageUpdateThread() {
	...
	private void process(Object o) {
		List<Object> list = permissionUpdateMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = permissionUpdateMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = permissionUpdateMap.get(key);
			...
			permissionUpdateMap.remove(key);
	...
	public static void setLastTime(long lastTime) {
		PermissionManageUpdateThread.lastTime = lastTime;
	}
