 ...
 * @since JDK1.6
 */
public class MaintainManageThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(MaintainManageThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 维修数据Map */
	private static Map<String, List<Object>> maintainMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public void run() {
		while (true) {
			try {
				...
				logger.error("维修管理处理队列数据异常:" + e.getMessage());
			}
		}
	}

	private void process(Object o) {
		List<Object> list = maintainMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = maintainMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = maintainMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		MaintainManageThread.lastTime = lastTime;
	}

	public static Map<String, List<Object>> getMaintainMap() {
		return maintainMap;
	}

	public static void setMaintainMap(Map<String, List<Object>> maintainMap) {
		MaintainManageThread.maintainMap = maintainMap;
	}
