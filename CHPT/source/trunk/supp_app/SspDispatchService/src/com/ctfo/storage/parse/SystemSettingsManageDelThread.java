 ...
 * @since JDK1.6
 */
public class SystemSettingsManageDelThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(SystemSettingsManageDelThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 权限数据删除Map */
	private static Map<String, List<Object>> systemSettingsDelMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public SystemSettingsManageDelThread() {
	...
	private void process(Object o) {
		List<Object> list = systemSettingsDelMap.get(o.getClass().getSimpleName());
		if (null == list) {
		...
		if (list.size() == batchSize) {
			...
			systemSettingsDelMap.remove(o.getClass().getSimpleName());
			logger.info("【{}】 批量提交:{}, 耗时:{}ms, 共提交总数:{}", o.getClass().getSimpleName(), list.size(), (System.currentTimeMillis() - start), processSize);
		} else {
			systemSettingsDelMap.put(o.getClass().getSimpleName(), list);
		}
	}
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = systemSettingsDelMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = systemSettingsDelMap.get(key);
			...
			systemSettingsDelMap.remove(key);
	...
	public static void setLastTime(long lastTime) {
		SystemSettingsManageDelThread.lastTime = lastTime;
	}
