 ...
 * @since JDK1.6
 */
public class BaseDataManageUpdateThread extends Thread {

	private static Logger logger = LoggerFactory.getLogger(BaseDataManageUpdateThread.class);

	/** 数据队列 */
	private static ArrayBlockingQueue<Object> queue = new ArrayBlockingQueue<Object>(500000);

	/** 基础数据Map */
	private static Map<String, List<Object>> baseDataUpdateMap = new ConcurrentHashMap<String, List<Object>>();
	...
	public BaseDataManageUpdateThread() {
	...
	private void process(Object o) {
		List<Object> list = baseDataUpdateMap.get(o.getClass().getSimpleName());
		if (null == list) {
	...
	public static void updateList(MySqlService mySqlService) {
		Iterator<String> iter = baseDataUpdateMap.keySet().iterator();
		while (iter.hasNext()) {
			long start = System.currentTimeMillis();
			String key = (String) iter.next();
			List<Object> value = baseDataUpdateMap.get(key);
	...
	public static void setLastTime(long lastTime) {
		BaseDataManageUpdateThread.lastTime = lastTime;
	}
