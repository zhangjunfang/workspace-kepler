/**
 * [ 用户档案列表]                                                                                        resize(ch [description]
 * @return {[type]}         [description]
 */
CTFO.Model.UserFileListManage = (function() {
  var uniqueInstance;
  ...
  function constructor() {
    ...
    var cHeight = 0,
      ...
      userFileFormContent = null,
      userFileGridContent = null,
      userFileDetailForm = null,
      updateRowAuth = 'FG_MEMU_DATAMANAGER_USERFILELISTMANAGE_U', // 修改记录权限
      detailRowAuth = 'FG_MEMU_DATAMANAGER_USERFILELISTMANAGE_I', // 查看记录详情权限
      deleteRowAuth = 'FG_MEMU_DATAMANAGER_USERFILELISTMANAGE_D', // 删除记录权限
      addRowAuth = 'FG_MEMU_DATAMANAGER_USERFILELISTMANAGE_C', // 新增记录权限
      exportRowAuth = 'FG_MEMU_DATAMANAGER_USERFILELISTMANAGE_E', // 导出记录权限
      opLogRowAuth = 'FG_MEMU_DATAMANAGER_USERFILELISTMANAGE_O', // 操作记录
    ...
    var columns = [{
      ...
      align: 'center',
      render: function(row) {
        return '<span class="mr10 hand cBlue" actionType="showDetailStatement">' + row.userId + '</span>';
      }
    }, {
      display: '人员姓名',
      name: 'userName',
      ...
      align: 'center',
      render: function(row) {
        return CTFO.utilFuns.dateFuns.utc2date(row.loginTime);
      }
    }];
    // grid初始化参数
    var gridOptions = {
      ...
      url: CTFO.config.sources.userFileGrid, // 数据请求地址
      pageSize: pageSize,
    ...
    var bindGridRowEvent = function(rowData, rowIndex, rowDom, eDom) {
      ...
      switch (actionType) {
        case 'showDetailStatement':
          showUserFileListDetail(rowData.userId);
          break;
      }
      return !actionType;
    };

    var showUserFileListDetail = function(tbUserOnlineId) {
        $.ajax({
          url: CTFO.config.sources.userFileSingleDetail,
          type: 'POST',
          dataType: 'json',
          data: {
            'tbUserOnlineId': tbUserOnlineId
          },
          complete: function(xhr, textStatus) {
            //called when complete
          },
          success: function(data, textStatus, xhr) {
            userFileFormContent.removeClass('none');
            userFileGridContent.addClass('none');
            resetThis();
    ...
    var compileFormData = function(r) {
      ...
      for (var n in r) {
        var key = n;
        if (key == 'createTime' || key == 'updateTime' || key == 'entryDate' || key == 'graduateDate' || key == 'birthday') {
          d[key] = (key > 0) ? CTFO.utilFuns.dateFuns.dateFormat(new Date(r[n]), 'yyyy-MM-dd') : '';
        } else {
          d[key] = r[n];
        }
      }
      $(userFileDetailForm).find('input[type=text]').each(function() {
    ...
    var resetThis = function() {
      $(userFileFormContent).find('input[type="text"]').each(function() {
        $(this).val("");
      ...
      //错误标签
      $(userFileFormContent).find('label[class="error"]').each(function() {
        $(this).remove();
      });
      $(userFileFormContent).find('.error').removeClass('error');
    };
    /**
    ...
    var inisertVehicleTeam = function() {
      var defaultData = {
          parentName: userFileForm.find('.parentCorpDesc').text(),
          corpCode: CTFO.cache.user.corpCode,
          parentId: userFileForm.find('input[name=parentId]').val()
        },
      ...
      var param = {
        ...
        onLoad: function(w, d, g) {
          ...
          var grandpaId = userFileForm.find('input[name=grandpaId]').val();
          if (grandpaId !== "-1") {
    ...
    var initVehicleDetailUpdate = function(t, w, d, g) {
      ...
      $(w).find('.saveButton').click(function(event) {
          ...
          saveVehicleTeamDetailFrom(actionUrl, param, function() {
            $(obj).attr("disabled", false);
            g.close();
            userFileForm.find('.queryButton').trigger('click');
          });
        }).end()
    ...
    var bindEvent = function() {
      userFileTerm.eq(1).find("span[name='cancelSave']").click(function() {
        userFileFormContent.addClass('none');
        userFileGridContent.removeClass('none');
      });
      userFileForm.find('.addVehicleTeam').click(function(event) {
          inisertVehicleTeam();
          //日志统计
        ...
        .find('.exportGrid').click(function(event) {
          CTFO.utilFuns.commonFuns.exportGrid({
            grid: grid,
            url: CTFO.config.sources.exportUserFileExcelData
          });
        ...
        .find('.cancelSave').click(function(event) {
            CTFO.cache.frame.changeModel('archivesManage', '', null, 0);
        });
    ...
    var initForm = function() {
      $(userFileForm).find('input[name=registerTimeStart]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(userFileForm).find('input[name=registertTimeEnd]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(userFileForm).find('input[name=loadTimeStart]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(userFileForm).find('input[name=loadTimeEnd]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      userFileForm.find('.queryButton').click(function(event) {
        searchGrid();
      }).end().find('.resetButton').click(function(event) {
        resetThis();
      });
    };
    /**
     ...
     */
    var resetThis = function() {
      $(userFileForm).find('input[type="text"]').each(function() {
        $(this).val("");
      }).end().find('select').each(function() {
        $(this).val("");
      }).end().find('textarea').each(function() {
...
/*      var provinceOption = $(authManageForm).find('select[name=corpProvince]'),
    ...
    var validateParams = function() {
      var param = [],
        data = userFileForm.serializeArray();
      $(data).each(function(event) {
        var name = '';
        if (this.name === 'clientAccount' || this.name === 'comName' || this.name === 'setbookName')
    ...
    var resize = function(ch) {
      ...
      gridContainerWrap.height(p.mainContainer.height() - pageLocation.outerHeight() - userFileTerm.eq(0).outerHeight() - parseInt(gridContainerWrap.css('margin-top')) * 3 - parseInt(gridContainerWrap.css('border-top-width')) * 2);
    ...
    return {
      init: function(options) {
        ...
        userFileTerm = p.mainContainer.find('.userFileTerm');
        userFileForm = p.mainContainer.find('form[name=userFileForm]');
        userFileDetailForm = p.mainContainer.find('form[name=userFileDetailForm]');
        ...
        userFileGridContent = p.mainContainer.find('.userManageContent:eq(0)'); //查询条件以及表格容器
        userFileFormContent = p.mainContainer.find('.userManageContent:eq(1)');
        ...
        initForm();
        initGrid();
        initAuth(userFileForm);


        userFileForm.find('input[name="comId"]').val(p.ep.comId);
        userFileForm.find('.queryButton').trigger('click');
