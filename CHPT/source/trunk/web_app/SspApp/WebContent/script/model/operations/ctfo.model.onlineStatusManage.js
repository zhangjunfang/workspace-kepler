/**
 * [ 系统监控 - 在线状态]                                                                                        resize(ch [description]
 * @return {[type]}         [description]
 */
CTFO.Model.OnlineStatusManage = (function() {
  var uniqueInstance;
  ...
  function constructor() {
    ...
    var cHeight = 0,
      ...
      updateRowAuth = 'FG_MEMU_SYSMONITOR_ONLINESTATUSMANAGE_U', // 修改记录权限
      detailRowAuth = 'FG_MEMU_SYSMONITOR_ONLINESTATUSMANAGE_I', // 查看记录详情权限
      deleteRowAuth = 'FG_MEMU_SYSMONITOR_ONLINESTATUSMANAGE_D', // 删除记录权限
      addRowAuth = 'FG_MEMU_SYSMONITOR_ONLINESTATUSMANAGE_C', // 新增记录权限
      exportRowAuth = 'FG_MEMU_SYSMONITOR_ONLINESTATUSMANAGE_E', // 导出记录权限
      opLogRowAuth = 'FG_MEMU_SYSMONITOR_ONLINESTATUSMANAGE_O', // 操作记录
    ...
    var columns = [{
      ...
      render: function(row) {
        if (row.total == '-') {
          ...
          if (row.serviceStatus == '0') {
            return '<span class="mr10 hand cBlue" actionType="showDetailStatement">' + row.total + '</span>';
          } else {
            return '-';
          }
        }
      ...
      render: function(row) {
        if (row.serviceStatus == '1') {
          return '<font color="#FF0000">在线</font>';
        } else if (row.serviceStatus == '0') {
          return '<font color="#06ff00">离线</font>';
        }
      }
    }];
    // grid初始化参数
    var gridOptions = {
      ...
      url: CTFO.config.sources.monitorServer, // 数据请求地址
      pageSize: pageSize,
    ...
    var inisertVehicleTeam = function() {
      var defaultData = {
          parentName: onlineForm.find('.parentCorpDesc').text(),
          corpCode: CTFO.cache.user.corpCode,
          parentId: onlineForm.find('input[name=parentId]').val()
        },
      ...
      var param = {
        ...
        onLoad: function(w, d, g) {
          ...
          var grandpaId = onlineForm.find('input[name=grandpaId]').val();
          if (grandpaId !== "-1") {
    ...
    var initVehicleDetailUpdate = function(t, w, d, g) {
      ...
      $(w).find('.saveButton').click(function(event) {
          ...
          saveVehicleTeamDetailFrom(actionUrl, param, function() {
            $(obj).attr("disabled", false);
            g.close();
            onlineForm.find('.queryButton').trigger('click');
          });
        }).end()
    ...
    var bindEvent = function() {
      onlineForm.find('.addVehicleTeam').click(function(event) {
          inisertVehicleTeam();
          //日志统计
          cLog.addOperatorLog($.extend({}, CTFO.cache.cLogInfo, {
            opType: '新增车队信息', // (必填)
        ...
        .find('.exportGrid').click(function(event) {
          CTFO.utilFuns.commonFuns.exportGrid({
            grid: grid,
            url: CTFO.config.sources.exportMonitorServerExcelData
          });
    ...
    var initForm = function() {
      $(onlineForm).find('input[name=registTimeStart]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(onlineForm).find('input[name=registEndTimeEnd]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      var provinceOption = onlineForm.find('select[name=corpProvince]'),
        cityOption = onlineForm.find('select[name=corpCity]'),
        countyOption = onlineForm.find('select[name=corpCounty]');
      //生成省
      CTFO.utilFuns.codeManager.getProvinceList(provinceOption);
      //联动市
      ...
      onlineForm.find('.queryButton').click(function(event) {
        searchGrid();
      }).end()
    ...
    var resetThis = function() {
      $(onlineForm).find('input[type="text"]').each(function() {
        $(this).val("");
    ...
    var validateParams = function() {
      var param = [],
        data = onlineForm.serializeArray();
    ...
    return {
      init: function(options) {
        ...
        vehicleTeamTerm = p.mainContainer.find('.onlineStatusTerm');
        onlineForm = p.mainContainer.find('form[name=onlineForm]');
