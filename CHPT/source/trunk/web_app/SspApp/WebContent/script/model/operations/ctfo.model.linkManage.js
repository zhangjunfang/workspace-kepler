/**
 * [ 系统监控 - 链路管理]                                                                                        resize(ch [description]
 * @return {[type]}         [description]
 */
CTFO.Model.LinkManage = (function() {
  var uniqueInstance;
  ...
  function constructor() {
    ...
    var cHeight = 0,
      ...
      updateRowAuth = 'FG_MEMU_SYSMONITOR_LINKMANAGE_U', // 修改记录权限
      detailRowAuth = 'FG_MEMU_SYSMONITOR_LINKMANAGE_I', // 查看记录详情权限
      deleteRowAuth = 'FG_MEMU_SYSMONITOR_LINKMANAGE_D', // 删除记录权限
      addRowAuth = 'FG_MEMU_SYSMONITOR_LINKMANAGE_C', // 新增记录权限
      exportRowAuth = 'FG_MEMU_SYSMONITOR_LINKMANAGE_E', // 导出记录权限
      opLogRowAuth = 'FG_MEMU_SYSMONITOR_LINKMANAGE_O', // 操作记录
    ...
    var columns = [{
      ...
      display: '宇通CRM系统链路',
      name: 'ytCrmLinkedStatus',
      width: 120,
      sortable: true,
      align: 'center',
      render: function(row) {
        if (row.ytCrmLinkedStatus == '0') {
          return '<font color="#FF0000">在线</font>';
        } else if (row.ytCrmLinkedStatus == '1') {
    ...
    var gridOptions = {
      ...
      url: CTFO.config.sources.monitorLink, // 数据请求地址
      pageSize: pageSize,
    ...
    var inisertVehicleTeam = function() {
      var defaultData = {
          parentName: linkForm.find('.parentCorpDesc').text(),
          corpCode: CTFO.cache.user.corpCode,
          parentId: linkForm.find('input[name=parentId]').val()
        },
      ...
      var param = {
        ...
        onLoad: function(w, d, g) {
          ...
          var grandpaId = linkForm.find('input[name=grandpaId]').val();
          if (grandpaId !== "-1") {
    ...
    var initVehicleDetailUpdate = function(t, w, d, g) {
      ...
      $(w).find('.saveButton').click(function(event) {
          ...
          saveVehicleTeamDetailFrom(actionUrl, param, function() {
            $(obj).attr("disabled", false);
            g.close();
            linkForm.find('.queryButton').trigger('click');
          });
        }).end()
    ...
    var bindEvent = function() {
      linkForm.find('.addVehicleTeam').click(function(event) {
          inisertVehicleTeam();
          //日志统计
          cLog.addOperatorLog($.extend({}, CTFO.cache.cLogInfo, {
            opType: '新增车队信息', // (必填)
        ...
        .find('.exportGrid').click(function(event) {
          CTFO.utilFuns.commonFuns.exportGrid({
            grid: grid,
            url: CTFO.config.sources.exportMonitorLinkExcelData
          });
    ...
    var selectOrgTreeNode = function(d, columns) {
      linkForm.find('.parentCorpDesc').text(d[columns.text]).end()
    ...
    var initForm = function() {
      $(linkForm).find('input[name=registTimeStart]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(linkForm).find('input[name=registEndTimeEnd]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      var provinceOption = linkForm.find('select[name=corpProvince]'),
        cityOption = linkForm.find('select[name=corpCity]'),
        countyOption = linkForm.find('select[name=corpCounty]');
      CTFO.utilFuns.codeManager.initProvAndCityAndCounty(provinceOption, cityOption, countyOption);
      linkForm.find('.queryButton').click(function(event) {
          searchGrid();
        }).end()
    ...
    var resetThis = function() {
      $(linkForm).find('input[type="text"]').each(function() {
        $(this).val("");
    ...
    return {
      init: function(options) {
        ...
        vehicleTeamTerm = p.mainContainer.find('.linkManageForm');
        linkForm = p.mainContainer.find('form[name=linkForm]');
