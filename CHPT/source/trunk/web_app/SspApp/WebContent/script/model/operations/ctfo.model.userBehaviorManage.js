/**
 * [ 系统监控 - 用户行为监控]                                                                                        resize(ch [description]
 * @return {[type]}         [description]
 */
CTFO.Model.UserBehaviorManage = (function() {
  var uniqueInstance;
  ...
  function constructor() {
    ...
    var cHeight = 0,
      ...
      updateRowAuth = 'FG_MEMU_SYSMONITOR_USERBEHAVIORMANAGE_U', // 修改记录权限
      detailRowAuth = 'FG_MEMU_SYSMONITOR_USERBEHAVIORMANAGE_I', // 查看记录详情权限
      deleteRowAuth = 'FG_MEMU_SYSMONITOR_USERBEHAVIORMANAGE_D', // 删除记录权限
      addRowAuth = 'FG_MEMU_SYSMONITOR_USERBEHAVIORMANAGE_C', // 新增记录权限
      exportRowAuth = 'FG_MEMU_SYSMONITOR_USERBEHAVIORMANAGE_E', // 导出记录权限
      opLogRowAuth = 'FG_MEMU_SYSMONITOR_USERBEHAVIORMANAGE_O', // 操作记录
    ...
    var columns = [{
      ...
      align: 'center',
      render: function(row) {
        return CTFO.utilFuns.dateFuns.utc2date(row.watchTime);
      }
    }, {
      display: '在线行为类型',
      name: 'onlineType',
    ...
    var gridOptions = {
      ...
      url: CTFO.config.sources.monitorUserBehavior, // 数据请求地址
      pageSize: pageSize,
    ...
    var inisertVehicleTeam = function() {
      var defaultData = {
          parentName: userBehavior.find('.parentCorpDesc').text(),
          corpCode: CTFO.cache.user.corpCode,
          parentId: userBehavior.find('input[name=parentId]').val()
        },
      ...
      var param = {
        ...
        onLoad: function(w, d, g) {
          ...
          var grandpaId = userBehavior.find('input[name=grandpaId]').val();
          if (grandpaId !== "-1") {
    ...
    var initVehicleDetailUpdate = function(t, w, d, g) {
      ...
      $(w).find('.saveButton').click(function(event) {
          ...
          saveVehicleTeamDetailFrom(actionUrl, param, function() {
            $(obj).attr("disabled", false);
            g.close();
            userBehavior.find('.queryButton').trigger('click');
          });
        }).end()
    ...
    var bindEvent = function() {
      userBehavior.find('.addVehicleTeam').click(function(event) {
          inisertVehicleTeam();
          //日志统计
          cLog.addOperatorLog($.extend({}, CTFO.cache.cLogInfo, {
            opType: '新增车队信息', // (必填)
        ...
        .find('.exportGrid').click(function(event) {
          CTFO.utilFuns.commonFuns.exportGrid({
            grid: grid,
            url: CTFO.config.sources.exportMonitorUserBehaviorExcelData
          });
    ...
    var selectOrgTreeNode = function(d, columns) {
      userBehavior.find('.parentCorpDesc').text(d[columns.text]).end()
    ...
    var initForm = function() {
      $(userBehavior).find('input[name=timeStart]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(userBehavior).find('input[name=timeEnd]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      userBehavior.find('.queryButton').click(function(event) {
          searchGrid();
        }).end()
    ...
    var resetThis = function() {
      $(userBehavior).find('input[type="text"]').each(function() {
        $(this).val("");
    ...
    var validateParams = function() {
      var param = [],
        data = userBehavior.serializeArray();
      $(data).each(function(event) {
        var name = '';
        if (this.name === 'comName' || this.name === 'setbookName' || this.name === 'clientAccount')
    ...
    return {
      init: function(options) {
        ...
        vehicleTeamTerm = p.mainContainer.find('.userBehaviorTerm');
        userBehavior = p.mainContainer.find('form[name=userBehavior]');
