...
//			msg.append(Base64_URl.base64Encode(json)); //操作对象 对象序列化json串，进行base64编码
	...
	public Map<String, String> getValidate(String remoteComId) {
		...
		map.put("comId", remoteComId);
		int isUpdate = this.countForRemoteByComId(map);
		
		if(isUpdate!=0){
			CompanyInfo companyInfo = this.selectPKById(map);
			returnJson.put("isSuccess", "1");
			returnJson.put("errMsg", "");
			returnJson.put("validate", Long.toString(companyInfo.getValidDate()));
			return returnJson;
		}else{
			returnJson.put("isSuccess", "0");
			returnJson.put("errMsg", "不存在此企业信息");
			returnJson.put("validate", "");
			return returnJson;
		}
	}
	...
	/**
	 * 根据ComId查询对应在数据库中的数据数
	 * @param map
	 * @return
	 */
	@SuppressWarnings("rawtypes")
	public int countForRemoteByComId(Map map) {
		return authManageDAO.countForRemoteByComId(map);
	}
	/**
	 * 根据机器码查询公司信息
	 * @param map
