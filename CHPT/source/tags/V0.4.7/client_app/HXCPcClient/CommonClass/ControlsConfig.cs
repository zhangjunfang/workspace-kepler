...
namespace HXCPcClient.CommonClass
{
   ...
   public class ControlsConfig
   {
                       ...
                       cellEdit.KeyPress += delegate(object o, KeyPressEventArgs e1)
                       {
                           ...
                           if (colnameList.Contains(dgv.Columns[dgv.CurrentCell.ColumnIndex].Name))
                           {
                               if ((Convert.ToInt32(e1.KeyChar) < 48 || Convert.ToInt32(e1.KeyChar) > 57) && Convert.ToInt32(e1.KeyChar) != 46 && Convert.ToInt32(e1.KeyChar) != 8 && Convert.ToInt32(e1.KeyChar) != 13)
                               {
                                   ...
                                   if (b_minus && Convert.ToInt32(e1.KeyChar) == 45)
                                       return;
       ...
       #region 手机号输入限制-只能输入数字并且可以实现组合件功能如：Ctrl+A，Ctrl+C，Ctrl+V等
      ...
      /// <param name="isint">是否只能输入整数，isint=true可以输入小数否则仅能输入整数</param>
       ...
       #region 负数格式化红色,并不显示负数
       /// <summary>
       /// 负数格式化红色,并不显示负数
       /// </summary>
       /// <param name="dgv"></param>
       public static void NegativeFormatting(DataGridView dgv)
       {
       ...
       static void dgv_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
       {
           ...
           if (dgv.Columns[e.ColumnIndex].ValueType==typeof(decimal))
           {
               ...
               if (num < 0)
               {
                   e.CellStyle.ForeColor = System.Drawing.Color.Red;//字体为红色
                   e.Value = num * -1;//不显示负号
               }
           }
       }
