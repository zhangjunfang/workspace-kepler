    ...
    public class LocalCache
    {
        ...
        private static void LoadAnnounce()
        {
            try
            {
                ...
                GlobalStaticObj.CurrentDateTime = DBHelper.GetCurrentTime(); //从数据库同步当前时间
            }
            catch
            {
                DtAnnounce = null;
        ...
        public static DataTable GetParentFunction(string parent_id)
        {
            if (GlobalStaticObj.gLoginDataSet != null
                && GlobalStaticObj.gLoginDataSet.Tables.Count > 1)
            {
                GlobalStaticObj.gLoginDataSet.Tables[2].DefaultView.Sort = "fun_idx asc";
                DataTable dt = GlobalStaticObj.gLoginDataSet.Tables[2].Clone();
                DataTable fundt = GlobalStaticObj.gLoginDataSet.Tables[2].DefaultView.ToTable();
                //建议菜单放在缓存里面
                DataRow[] drs = fundt.Select("parent_id='" + parent_id + "'");
