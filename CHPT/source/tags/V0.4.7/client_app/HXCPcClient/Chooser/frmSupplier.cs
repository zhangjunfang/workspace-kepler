...
namespace HXCPcClient.Chooser
{
    public partial class frmSupplier : FormChooser
    {
        ...
        void BindData()
        {
            ...
            sbWhere.AppendFormat(" enable_flag = 1  and status= 1 ");
        ...
        private void tvNature_AfterSelect(object sender, TreeViewEventArgs e)
        {
            BindData();
        }
        //回车事件
        private void dgvSupper_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                //回车时,DataGridView会自动向下一行,这里向上取一行,就可以取当前行了
                if (dgvSupper.CurrentRow.Index != dgvSupper.Rows.Count - 1)
                {
                    dgvSupper.CurrentCell = dgvSupper.Rows[dgvSupper.CurrentRow.Index - 1].Cells[colSupperCode.Name];
                }
                SelectedSupper();
            }
        }
