...
namespace HXCPcClient.Chooser
{
    ...
    public partial class frmCustomerInfo : FormChooser
    {
        ...
        public String AppendWhere = String.Empty;   //add by krod
        /// <summary>
        /// 客户信息
        /// </summary>
        /// <remarks>
        ...
        private void BindTvCustom()
        {
            ...
            root.Text = "客户类别";
            root.Name = "";
            tvCustom.Nodes.Add(root);
            CreateTreeViewChildNode(root, "sys_customer_category");
        ...
        private void BindPageData()
        {
            ...
            left join tr_base_contacts c on c.relation_object_id=a.cust_id and c.relation_object='tb_customer'
            ...
            if (!string.IsNullOrEmpty(CommonCtrl.IsNullToString(tvCustom.SelectedNode.Name)))
            {
                strWhere += string.Format(" and a.cust_type like '%{0}%'", tvCustom.SelectedNode.Name.ToString());
            }
            int recordCount;
            DataTable dt = DBHelper.GetTableByPage("分页查询客户", strTable, strfileds, strWhere + AppendWhere, "", " order by a.create_time desc", page.PageIndex, page.PageSize, out recordCount);
        ...
        private void dgvCustom_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                //回车时,DataGridView会自动向下一行,这里向上取一行,就可以取当前行了
                if (dgvCustom.CurrentRow.Index != dgvCustom.Rows.Count - 1)
                {
                    dgvCustom.CurrentCell = dgvCustom.Rows[dgvCustom.CurrentRow.Index - 1].Cells[cust_code.Name];
                }
                strCustomerNo = dgvCustom.CurrentRow.Cells["cust_code"].Value.ToString();
                strCustomerName = dgvCustom.CurrentRow.Cells["cust_name"].Value.ToString();
                strCustomerId = dgvCustom.CurrentRow.Cells["cust_id"].Value.ToString();
        ...
        #region 左侧树选中后的事件
        private void tvCustom_AfterSelect(object sender, TreeViewEventArgs e)
        {
            BindPageData();
        }
        #endregion
