...
namespace HXCPcClient.Chooser
{
    ...
    public partial class frmWorkHours : FormChooser
    {
        ...
        string BuildString()
        {
            ...
            if (!string.IsNullOrEmpty(cboDataSources.SelectedValue.ToString()))
            {
                Str_Where += string.Format(" and  data_source = '{0}'", cboDataSources.SelectedValue.ToString());
            }
            return Str_Where;
        }
        #endregion
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            strWhours_id = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["whours_id"].Value);
            strRepairType = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["repair_type"].Value);
            strProjectNum = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["project_num"].Value);
            strProjectName = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["project_name"].Value);
            strRemark = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["remark"].Value);
            strQuotaPrice = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["quota_price"].Value);
            strWhoursNum = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["whours_num_a"].Value);
            strWhoursType = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["whours_type"].Value);
            if (strWhoursType == "1")
            {
            ...
            strWhoursChange = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells[drtxt_whours_change.Name].Value);
            strData_source = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["data_source"].Value);
        ...
        private void gvWorkList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                strWhours_id = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["whours_id"].Value);
                strRepairType = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["repair_type"].Value);
                strProjectNum = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["project_num"].Value);
                strProjectName = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["project_name"].Value);
                strRemark = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["remark"].Value);
                strQuotaPrice = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["quota_price"].Value);
                strWhoursNum = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["whours_num_a"].Value);
                strWhoursType = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells["whours_type"].Value);
                if (strWhoursType == "1")
                {
                    DataTable dt = DBHelper.GetTable("查询维修业务参数-工时标准", GlobalStaticObj.CommAccCode, "sys_repair_param", "time_standards", "book_id='" + GlobalStaticObj.CurrAccID + "'", "", "");
                    strQuotaPrice = CommonCtrl.IsNullToString(dt.Rows[0]["time_standards"]);
                }
                strWhoursChange = CommonCtrl.IsNullToString(dgvWorkList.Rows[e.RowIndex].Cells[drtxt_whours_change.Name].Value);
                strData_source = CommonCtrl.IsNullToString(dgvWorkList.CurrentRow.Cells["data_source"].Value);
                this.DialogResult = DialogResult.OK;
            }
        }
        ...
        private void gvWorkList_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            //选择当前行
            dgvWorkList.Rows[e.RowIndex].Cells[colCheck.Name].Value = true;
        }
        #endregion
