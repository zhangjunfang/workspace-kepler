...
namespace HXCPcClient.UCForm.DataManage.Contacts
{
    public partial class UCContactsManage : UCBase
    {
        ...
        private void BindEvent()
        {
            ...
            base.ExportEvent += new ClickHandler(UCContactsManage_ExportEvent);
        ...
        //导出
        void UCContactsManage_ExportEvent(object sender, EventArgs e)
        {
            ...
            try
            {
                string fileName = "联系人档案" + DateTime.Now.ToString("yyyy-MM-dd") + ".xls";
                ExcelHandler.ExportExcel(fileName, ExcelHandler.HandleDataTableForExcel(dgvContacts.GetBoundData(), dgvContacts));
            }
            catch (Exception ex)
            {
                Utility.Log.Log.writeLineToLog("联系人档案" + ex.Message, "client");
        ...
        private void ViewData()
        {
            ...
            string cont_id = dgvContacts.CurrentRow.Cells[colContID.Name].Value.ToString();
            if (string.IsNullOrEmpty(cont_id))
            {
                return;
            }
            UCContactsView view = new UCContactsView(cont_id, this);
        ...
        private void RecordData(List<string> listFiles)
        {
            ...
            foreach (DataGridViewRow dgvr in dgvContacts.Rows)
            {
                if (Convert.ToBoolean(dgvr.Cells[colChk.Name].Value))
                {
                    listFiles.Add(dgvr.Cells[colDataSources.Name].Tag.ToString());
                    string cont_id = dgvr.Cells[colContID.Name].Value.ToString();
                    listIDs.Add(cont_id);
        ...
        private void dgvContacts_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            ////单击选择框
            //if (dgvContacts.CurrentCell != null && dgvContacts.CurrentCell.OwningColumn.Name == colChk.Name)
            //{
            //    SetSelectedStatus();
            //}
        }
