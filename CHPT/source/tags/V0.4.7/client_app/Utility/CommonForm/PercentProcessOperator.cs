 ...
 * 2，赋值BackgroundWork（类型为一个参数，无返回值的委托，其中参数是一个具有一个int类型参数无返回值的委托，用来预报进度）属性为要在后台执行的方法
 * 3，调用Start方法开始执行
 ...
 */
    public class PercentProcessOperator
    {
        ...
        {
            _processForm.MessageInfo = _inforMessage + ",已完成：" + (e.ProgressPercentage * 100 / _processForm.Maximum).ToString() + "%";
        ...
        public int Maximum
        {
            set { _processForm.Maximum = value; }
        }
