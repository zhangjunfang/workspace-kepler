    ...
    public partial class UCDispatchDetails : UCDispatchBase
    {
        ...
        private void UpdateRepairOrderInfo(List<SQLObj> listSql, DataSources.EnumDispatchStatus DStatus)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(strInspection))
                {
                    ...
                    obj.sqlString = @"update tb_maintain_info set dispatch_status=@dispatch_status,set_meal=@set_meal,update_by=@update_by,update_name=@update_name,update_time=@update_time,Verify_advice=@Verify_advice,complete_work_time=@complete_work_time
            ...
            try
            {
                foreach (DataGridViewRow dgvr in dgvproject.Rows)
                {
                    ...
                    if (strPname.Length > 0)
                    {
                        ...
                        ,repair_progress,three_warranty,start_work_time,complete_work_time,shut_down_time,shut_down_reason,shut_down_duration,continue_time,remarks,enable_flag,whours_id)
                        ...
                        ,shut_down_reason=@shut_down_reason,shut_down_duration=@shut_down_duration,continue_time=@continue_time,remarks=@remarks,enable_flag=@enable_flag,whours_id=@whours_id where item_id=@item_id";
                        }
                        obj.Param = dicParam;
                        listSql.Add(obj);
                    }
                ...
                foreach (DataGridViewRow dgvr in dgvMaterials.Rows)
                {
                    ...
                    if (strPname.Length > 0 && strPCmoney.Length > 0)
                    {
                        ...
                        if (strPID.Length == 0)
                        {
                            ...
                            obj.sqlString = "insert into [tb_maintain_material_detail] (material_id,parts_code,parts_name,norms,unit,whether_imported,quantity,unit_price,member_discount,member_price,member_sum_money,sum_money,drawn_no,vehicle_brand,three_warranty,remarks,enable_flag,maintain_id,parts_id) ";
                            obj.sqlString += " values (@material_id,@parts_code,@parts_name,@norms,@unit,@whether_imported,@quantity,@unit_price,@member_discount,@member_price,@member_sum_money,@sum_money,@drawn_no,@vehicle_brand,@three_warranty,@remarks,@enable_flag,@maintain_id,@parts_id);";
                        }
                        else
                        {
                            ...
                            obj.sqlString += "drawn_no=@drawn_no,vehicle_brand=@vehicle_brand,member_sum_money=@member_sum_money, three_warranty=@three_warranty,remarks=@remarks,enable_flag=@enable_flag,parts_id=@parts_id where material_id=@material_id";
                        }
                        obj.Param = dicParam;
                        listSql.Add(obj);
                    }
        ...
        private void GetReservData(string strRId)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    #region 分配工时人员信息
                    //其他项目收费数据             
                    DataTable dut = DBHelper.GetTable("分配工时人员信息", "tb_maintain_dispatch_worker a left join tb_maintain_item b on a.item_id=b.item_id", "*", string.Format(" a.maintain_id='{0}'", strRId), "", "");
                    if (dut.Rows.Count > 0)
                    {
                        if (dut.Rows.Count > dgvUserData.Rows.Count)
                        {
        ...
        private void SetTopbuttonShow()
        {
            ...
            switch (strProjectSattus)
            {
                ...
                case "2"://已完工
                    ...
                    dgvAttachment.Enabled = false;
                    dgvFData.Enabled = false;
                    dgvMaterials.Enabled = false;
                    dgvOther.Enabled = false;
                    dgvproject.Enabled = false;
                    dgvUserData.Enabled = false;
        ...
        private void dgvproject_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                //其他项目收费数据             
                DataTable dut = DBHelper.GetTable("分配工时人员信息", "tb_maintain_dispatch_worker a left join tb_maintain_item b on a.item_id=b.item_id ", "*", string.Format("a.maintain_id='{0}' and a.item_id='{1}'", strReceiveId, strProjectID), "", "");
                if (dut.Rows.Count > 0)
                {
                    //清空已选择框
                    foreach (DataGridViewRow dgvr in dgvproject.Rows)
