...
namespace HXC_FuncUtility
{
    ...
    public class LocalVariable
    {
        ...
        {
            //string entConnSTr = Secret.Encrypt3DES(desConnStr, Key);
        ...
        {
            if (htParas.ContainsKey(ConfigConst.CloundData))
            {
                string[] dataConfig = htParas[ConfigConst.CloundData].ToString().Split(':');
                if (dataConfig.Length == 2)
                {
                    GlobalStaticObj_Server.Instance.CloundServerIp = dataConfig[0];
                    int port = 0;
                    if (int.TryParse(dataConfig[1], out port))
                    {
                        GlobalStaticObj_Server.Instance.CloundServerPort = port;
                    }
                }
            }

            if (htParas.ContainsKey(ConfigConst.CloundFile))
            {
                string[] fileConfig = htParas[ConfigConst.CloundFile].ToString().Split(':');
                if (fileConfig.Length == 2)
                {
                    GlobalStaticObj_Server.Instance.CloundFileIp = fileConfig[0];
                    int port = 0;
                    if (int.TryParse(fileConfig[1], out port))
                    {
                        GlobalStaticObj_Server.Instance.CloundFilePort = port;
                    }
                }
            }
            ...
            if (htParas.ContainsKey(ConfigConst.UserUploadTime))
            {
                GlobalStaticObj_Server.Instance.UserUploadTime = htParas[ConfigConst.UserUploadTime].ToString();
            }
        }
        ...
        public static string GetConnString(string dbName, string dbConfig)
        {
            ...
            //dbName = "hxc"; //测试连接hxc
            //conn = "Data Source=192.168.35.104;Initial Catalog=@@@;User ID=sa;Password=123456;";
