...
namespace HXCServerWinForm.UCForm.AcountSet
{
    public partial class frmAccountEdit : FormEx
    {
        ...
        private void frmAccountEdit_Load(object sender, EventArgs e)
        {
            ...
            if (OpType == 2)
            {
                Common.SetControlForLable(pnlContainer, true);
        ...
        public void CreateAccount()
        {
            ...
            DBHelper.BatchExeSQLStringMultiByTrans("创建帐套", GlobalStaticObj_Server.DbPrefix + GlobalStaticObj_Server.CommAccCode, list);
        }
        ...
        public void ToStep(bool toDirection)
        {
            ...
            string title = string.Format(frmTitle + "帐套引导（{0}/6）", currStep);
            if (OpType == 2)
            {
                title = string.Format("{0}-{1}详细信息（{2}/6）", txtsetbook_name.Caption, txtsetbook_code.Caption, currStep);
            }
            this.Text = title;
        ...
        public bool HandleStep()
        {
            if (currStep == 1)
            {
                ...
                if (!string.IsNullOrEmpty(txtbank_account.Caption))
                {
                    if (Common.ValidateStr(txtbank_account.Caption.Trim(), @"[\d]+") != 1)
                    {
                        Validator.SetError(errProvider, txtbank_account, "请录入正确的银行账号");
                        return false;
                    }
                }
