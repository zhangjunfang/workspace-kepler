    ...
    public partial class UCImportBilling : FormChooser
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    BinddgBilling(PurBillTable, PurPartTable, purchasebillid, PurBillingLogMsg, WhereQuery);//根据条件查询采购开单信息列表
                }
                else if (BillingType == SaleBilling)
                {
                    BinddgBilling(SaleBillTable, SalePartTable, salebillid, SaleBillingLogMsg, WhereQuery);//根据条件查询销售开单信息列表
                }
     ...
     /// <param name="SubOrderType">开单中的子单据类型名称</param>
        private void BinddgPartsList(string BillingId,string BType,string SubOrderType)
        {
            try
            {
                ...
                sb_Part.AppendFormat("{0},{1},{2},{3},{4},{5},{6},{7},{8}",
                billID, partscode, partsname, drawingnum, unitname,originalprice,
                businessprice,valoremtogether,carpartscode);
                ...
                {                
                    //获取采购换货入库配件信息列表
                    dt_parts_Billing = DBHelper.GetTable(PurpartsLogMsg, PurPartTable, sb_Part.ToString() + ",business_counts," + arrivaldate, purchasebillid + "='" + BillingId + "' and business_counts>0", "", "");
                ...
                {
                    //获取采购收货入库配件信息列表
                    dt_parts_Billing = DBHelper.GetTable(PurpartsLogMsg, PurPartTable, sb_Part.ToString() + ",business_counts," + arrivaldate, purchasebillid + "='" + BillingId + "'", "", "");
                }
                ...
                {
                    //获取入库配件信息列表
                    dt_parts_Billing = DBHelper.GetTable(SalepartsLogMsg, SalePartTable, sb_Part.ToString() + ",business_count", salebillid + "='" + BillingId + "' and business_count>0", "", "");
                ...
                {
                    //获取销售退货入库配件信息列表
                    dt_parts_Billing = DBHelper.GetTable(SalepartsLogMsg, SalePartTable, sb_Part.ToString() + ",business_count", salebillid + "='" + BillingId + "'", "", "");
                }
                //出库单销售发货
            ...
            try
            {
                for (int i = 0; i < partsTable.Rows.Count; i++)
                {
                        ...
                        if (BillTypeName == purchaseBilling)
                        {
                            ...
                            dgRow.Cells["BusCounts"].Value = partsTable.Rows[i]["business_counts"].ToString();//采购开单业务数量
                        ...
                        else if (BillTypeName == SaleBilling)
                        {
                            ...
                            dgRow.Cells["BusCounts"].Value = partsTable.Rows[i]["business_count"].ToString();//销售开单业务数量
        ...
        private string BuildQueryWhere()
        {
            try
            {
                ...
                //获取出入库单与开单类型信息
                string StrWhere = " enable_flag='" + NODelState + "'" + " and is_occupy='" +
                OpenState + "'" + " and order_status='" + Verified + "'" + " and is_lock='" + OpenState + "'";//查询条件字符串
                int StIdx = dateTimeStart.Value.ToString().IndexOf(" ", 0);
                int EdIdx = dateTimeEnd.Value.ToString().IndexOf(" ", 0);
                string StDate = dateTimeStart.Value.ToString().Remove(StIdx);
                string EdDate = dateTimeEnd.Value.ToString().Remove(EdIdx);
                DateTime StartTime = Convert.ToDateTime(StDate + " " + DateTime.Now.ToLongTimeString());//开始时间
                DateTime EndTime = Convert.ToDateTime(EdDate + " " + DateTime.Now.ToLongTimeString());//结束时间
                //获取导入单据的查询条件信息
