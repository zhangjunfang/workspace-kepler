...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchaseOrder
{
    public partial class UCChoosePurchasePlanOrder : FormChooser
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (List_PlanID.Count > 0)
                {
                    ...
                    {
                        DataTable dt_parts_purchase = DBHelper.GetTable("查询采购计划单配件表信息", "tb_parts_purchase_plan_p", "*", " plan_id='" + planid + "' and is_suspend=1 and isnull(finish_counts,0)<isnull(business_counts,0) ", "", "");
        ...
        private void BinddgAccessoriesDetail(string plan_id)
        {
            ...
            {
                string wherestr = " plan_id='" + plan_id + "' and is_suspend=1 and isnull(finish_counts,0)<isnull(business_counts,0) ";
                if (this.RelationType == "宇通采购订单")
                {
                    wherestr = wherestr + " and len(car_factory_code)>0";
                }
                DataTable dt_parts_purchase = DBHelper.GetTable("查询采购计划单配件表信息", "tb_parts_purchase_plan_p", "*", wherestr, "", "");
