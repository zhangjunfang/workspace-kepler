        ...
        private void GetInOutBillList(string RecWhereStr,string ShpWhereStr)
        {
            try
            {
                ...
                if (RecWhereStr != null)
                {
                   ...
                   "", "RptBillTb.order_date desc", winFormInoutPage.PageIndex, winFormInoutPage.PageSize, out InCount);//获取其它收货单表查询记录
                }
                ...
                if (ShpWhereStr != null)
                {
                   ...
                   "", "  ShpBillTb.order_date desc", winFormInoutPage.PageIndex, winFormInoutPage.PageSize, out OutCount);//获取其它发货单表查询记录
                }
                //获取总记录行
        ...
        private void GetPartList(string RecWhereStr,string ShpWhereStr)
        {
            try
            {
                ...
                if (RecWhereStr != null)
                {
                    ...
                    sbPartField.AppendFormat("RptBillTb.{0},RptPartTb.{1},{2},{3},{4},{5},RptPartTb.{6},RptPartTb.{7},RptPartTb.{8},RptPartTb.{9},{10},PartAmount,{11},RptPartTb.{12},RptPartTb.{13}",
                    ReceiptID, WareHouseName, InTypeName, OrderNo, OrdDate, PartsNum, CarFactoryCode, PartsName, DrawNum, PartSpec, UnitName,//10
                    VmName, PartsBrand, PartRemk);
                    //其它收货单数据表联合查询集
                    ...
                    "", "  RptBillTb.order_date desc", winFormInoutPartPager.PageIndex, winFormInoutPartPager.PageSize, out InCount);//获取其它收货单表查询记录
                ...
                if (ShpWhereStr != null)
                {
                    ...
                    sbPartField.AppendFormat("ShpBillTb.{0},ShpBillTb.{1},{2},{3},{4},{5},ShpPartTb.{6},ShpPartTb.{7},ShpPartTb.{8},ShpPartTb.{9},PartAmount,{10},{11},ShpPartTb.{12},ShpPartTb.{13}",
                    ...
                    "", " ShpBillTb.order_date desc", winFormInoutPartPager.PageIndex, winFormInoutPartPager.PageSize, out OutCount);//获取其它发货单表查询记录
                }
                //获取总记录行
                TotalCount = InCount + OutCount;
