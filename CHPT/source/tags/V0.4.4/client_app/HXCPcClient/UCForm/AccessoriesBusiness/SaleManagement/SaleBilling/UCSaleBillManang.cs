    ...
    public partial class UCSaleBillManang : UCBase
    {
            ...
            try
            {
                ...
                foreach (DataGridViewRow dr in gvPurchaseOrderList.Rows)
                {
                    ...
                    if (isCheck != null && (bool)isCheck)
                    {
                        ...
                        obj.Param = dicParam;
                        listSql.Add(obj);
                        if (GetIsSubmit(dr.Cells["sale_billing_id"].Value.ToString()))
                        {
                ...
                for (int i = 0; i < listField.Count; i++)
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        if (IOPartTable != null && IOPartTable.Rows.Count > 0)
                        {
                            ...
                            //dr["PartCount"] = IOPartTable.Rows[i]["business_count"].ToString();//配件业务数量
                            dr["PartCount"] = Convert.ToDecimal(IOPartTable.Rows[i]["business_count"].ToString());//配件业务数量
                            ...
                            if (CreateIntoStock("出库单", model, ref stock_inout_id))
                            {
                                ...
                                if (dt_parts != null && dt_parts.Rows.Count > 0)
                                {
                                    for (int a = 0; a < dt_parts.Rows.Count; a++)
                                    {
                                        ...
                                        bool ret = CreateIntoPartsStock(stock_inout_id, bill_p_model);
                                    }
                                }
                            }
                        } 
                            ...
                            if (CreateIntoStock("入库单", model, ref stock_in_id))
                            {
                                ...
                                if (dt_parts != null && dt_parts.Rows.Count > 0)
                                {
                                    for (int a = 0; a < dt_parts.Rows.Count; a++)
                                    {
                                        tb_parts_sale_billing_p bill_p_model = new tb_parts_sale_billing_p();
                                        CommonFuncCall.SetModlByDataTable(bill_p_model, dt_parts, a);
                                        bool ret = CreateIntoPartsStock(stock_in_id, bill_p_model);
                                    }
                                }
                            }
                            ...
                            if (CreateIntoStock("出库单", model, ref stock_out_id))
                            {
                                ...
                                if (dt_parts != null && dt_parts.Rows.Count > 0)
                                {
                                    for (int a = 0; a < dt_parts.Rows.Count; a++)
                                    {
                                        tb_parts_sale_billing_p bill_p_model = new tb_parts_sale_billing_p();
                                        CommonFuncCall.SetModlByDataTable(bill_p_model, dt_parts, a);
                                        bool ret = CreateIntoPartsStock(stock_out_id, bill_p_model);
                                    }
                                }
                            }
                        } 
        ...
        bool CreateIntoStock(string ordertype, tb_parts_sale_billing bill_model, ref string stock_inout_id)
        {
            ...
            if (ordertype == "入库单")
            {
                ...
                stoct_inout_model.order_type = Convert.ToInt32(DataSources.EnumAllocationBillType.Storage).ToString();//入库单编号
                stoct_inout_model.order_type_name = DataSources.GetDescription(DataSources.EnumAllocationBillType.Storage, true);//入库单名称
            }
            else if (ordertype == "出库单")
            {
                stoct_inout_model.order_num = CommonUtility.GetNewNo(DataSources.EnumProjectType.OutBill);//获取出库单编号
                stoct_inout_model.order_type = Convert.ToInt32(DataSources.EnumAllocationBillType.OutboundOrder).ToString();//出库单编号
                stoct_inout_model.order_type_name = DataSources.GetDescription(DataSources.EnumAllocationBillType.OutboundOrder, true);//出库单名称
            }
        ...
        /// <returns></returns>
        bool GetIsSubmit(string sale_billing_id)
        {
            ...
            if (dt_surplus != null && dt_surplus.Rows.Count > 0)
            {
                if (dtPurchaseList.Rows.Count > 0)
                {
                    foreach (DataRow drr in dtPurchaseList.Rows)
                    {
                        ...
                        int business_count = int.Parse(drr["business_count"] == null ? "" : drr["business_count"].ToString());
