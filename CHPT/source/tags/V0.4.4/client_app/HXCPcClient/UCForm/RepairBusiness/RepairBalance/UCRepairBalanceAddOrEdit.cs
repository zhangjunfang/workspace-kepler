    ...
    public partial class UCRepairBalanceAddOrEdit : UCBase
    {
        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            try
            {
                ...
                if ((wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy) && string.IsNullOrEmpty(strBefore_orderId))
                {
                 ...
                 ,responsible_name,org_name,enable_flag,info_status,maintain_no,maintain_id,create_by,create_name,create_time,orders_source,Import_status,before_orderId)
        ...
        private void SaveBalanceInfo(List<SQLObj> listSql, string strOrderId)
        {
            try
            {
                ...
                ,make_invoice_type=@make_invoice_type,payment_terms=@payment_terms,settlement_account=@settlement_account,settle_company=@settle_company,update_by=@update_by,update_name=@update_name,update_time=@update_time where maintain_id=@maintain_id";
                }
                obj.Param = dicParam;
                listSql.Add(obj);
            }
