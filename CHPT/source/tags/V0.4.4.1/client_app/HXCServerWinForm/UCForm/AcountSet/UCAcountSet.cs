...
namespace HXCServerWinForm.UCForm.AcountSet
{
    public partial class UCAcountSet : UCBase
    {
        ...
        public void DelAcc()
        {
            ...
            DBHelper.Submit_AddOrEdit("删除选中帐套", GlobalStaticObj_Server.DbPrefix + GlobalStaticObj_Server.CommAccCode, "sys_setbook", "setbook_code", code, dicField);

            #region 分离被删除的数据库
            string sqlStr = string.Format("if db_id('{0}') is not null begin exec sp_killspid '{0}' exec sp_detach_db {0} end", GlobalStaticObj_Server.DbPrefix + code);
