...
namespace HXCPcClient.UCForm.SysManage.Stock
{
    ...
    public partial class UCStockManager : UCBase
    {
        ...
        private bool ValidatorDgv(ref string msg)
        {
            ...
            foreach (int index in this.rowList)
            {
                if (!int.TryParse(this.dgvRecord[this.columnMax.Name, index].EditedFormattedValue.ToString(), out max))
                {
                    msg = "第" + index + "行库存上限应输入数字！";
                    return false;
                }
                if (!int.TryParse(this.dgvRecord[this.columnMin.Name, index].EditedFormattedValue.ToString(), out min))
                {
                    msg = "第" + index + "行库存下限应输入数字！";
                    return false;
                }
                if (max < min)
                {
                    msg = "第" + index + "行库存下限应大于上限值！";
                    return false;
                }
            }
            return true;
