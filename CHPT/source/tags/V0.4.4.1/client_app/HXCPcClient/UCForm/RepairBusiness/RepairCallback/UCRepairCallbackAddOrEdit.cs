        ...
        private void SaveOrderInfo(List<SQLObj> listSql, DataSources.EnumAuditStatus status)
        {
            try
            {
                ...
                if (wStatus == WindowStatus.Add || wStatus == WindowStatus.Copy)
                {
                    ...
                    ,turner,customer_code,customer_name,customer_id,linkman,linkman_mobile,responsible_opid,responsible_name,org_name,enable_flag,document_status,repairer_name
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    for (int i = 0; i <= intProws; i++)
                    {
                        if (string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvproject.Rows[i].Cells["item_no"].Value)))
                        {
                            ...
                            //会员工时费
                            dgvproject.Rows[i].Cells["member_price"].Value = Convert.ToString(Convert.ToDecimal(frmHours.strQuotaPrice) * (Convert.ToDecimal(strPzk) / 100));
                ...
                if (intCurrentIndex >= 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(strItemNo))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            //会员工时费
                            dgvproject.Rows[intCurrentIndex].Cells["member_price"].Value = Convert.ToString(Convert.ToDecimal(frmHours.strQuotaPrice) * (Convert.ToDecimal(strPzk) / 100));
                            if (dgvproject.Rows[intCurrentIndex].Cells["member_price"].Value.ToString() != "0")
                            {
                                //折扣额
                                dgvproject.Rows[intCurrentIndex].Cells["member_sum_money"].Value = Convert.ToString(Convert.ToDecimal(frmHours.strQuotaPrice) - Convert.ToDecimal(dgvproject.Rows[intCurrentIndex].Cells["member_price"].Value));
                            }
                            else
                            {
