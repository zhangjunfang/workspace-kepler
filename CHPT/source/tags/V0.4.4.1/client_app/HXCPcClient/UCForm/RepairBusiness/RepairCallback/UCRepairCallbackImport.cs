    ...
    public partial class UCRepairCallbackImport : FormChooser
    {
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow dr in dgvRData.Rows)
            {
                ...
                {
                    strRId =CommonCtrl.IsNullToString(dr.Cells["maintain_id"].Value);
                    listField.Add(CommonCtrl.IsNullToString(dr.Cells["maintain_id"].Value));
                }
            }
            #endregion
        ...
        private void BindData()
        {
            try
            {
                ...
                strWhere = string.Format(" b.enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and (b.info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.AUDIT).ToString() + "' or b.info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.Balance).ToString() + "') and b.Import_status='" + Convert.ToInt32(DataSources.EnumImportStaus.OPEN).ToString() + "'");//enable_flag 1未删除
        ...
        private void BindDataP()
        {
            try
            {
                ...
                #endregion
                string strFiles = " *";
                strWhere = string.Format(" enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and (info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.SUBMIT).ToString() + "' or info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.AUDIT).ToString() + "')");//enable_flag 1未删除
                if (!string.IsNullOrEmpty(txtCarNO.Text.Trim()))//车牌号
                {
        ...
        private void BindDataP1()
        {
            try
            {
                ...
                #endregion
                string strFiles = " *";
                strWhere = string.Format(" enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and (info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.Balance) + "' or info_status='" +  + Convert.ToInt32(DataSources.EnumAuditStatus.AUDIT) + "')");//enable_flag 1未删除
                if (!string.IsNullOrEmpty(txtCarNO.Text.Trim()))//车牌号
                {
                    strWhere += string.Format(" and  vehicle_no like '%{0}%'", txtCarNO.Text.Trim());
                }
                ...
                string strTables = "tb_maintain_info";
                DataTable dt = DBHelper.GetTable("维修结算单信息列表", strTables, strFiles, strWhere, "", "order by maintain_no desc");
                dgvRData.DataSource = dt;
            }
            catch (Exception ex)
            {
        ...
        private void dgvRData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            strRId = CommonCtrl.IsNullToString(dgvRData.Rows[e.RowIndex].Cells["maintain_id"].Value);
            if (strTag == "0")//返修单
            {
                uc.strBefore_orderId = strRId;  //导入单据的Id值          
                uc.BindSetmentData();
        ...
        private void dgvRData_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                strRId = CommonCtrl.IsNullToString(dgvRData.CurrentRow.Cells["maintain_id"].Value);
                if (strTag == "0")//返修单
                {
                    uc.strBefore_orderId = strRId;  //导入单据的Id值          
                    uc.BindSetmentData();
