...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.StockQuery
{
    public partial class UCStockQuery : UCBase
    {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CombWarehouse_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(CombCompany.SelectedValue.ToString()))
                {
                    CommonFuncCall.BindCompanyWarehouse(CombWarehouse, null, "请选择"); //获取仓库名称
                    MessageBoxEx.Show("请您选择仓库所属的机构！","提示",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                    return;
                }
            }
            catch (Exception ex)
       ...
       private  void UCStockQuery_ExportEvent(object sender, EventArgs e)
        ...
        private void GetPartList(String WhereStr)
        {
            try
            {
                ...
                {
                    StockTable = DBHelper.GetTableByPage(StockQueryLogMsg, StockWHouseTable, "*", 
                     WhereStr +" and wh_name='"+ CombWarehouse.Text.ToString()+"'",
        ...
        private void GetYuTongPartList(String WhereStr)
        {
            try
            {
                ...
                {
                    decimal PhyCount = Convert.ToDecimal(YuTongInoutTable.Rows[i]["PhyCount"].ToString());//实际库存
                    decimal OcyCount = Convert.ToDecimal(YuTongInoutTable.Rows[i]["OcpyCount"].ToString());//占用库存
                    ...
                    dgRow.Cells["AvailableCount"].Value = AvblCount;//可用库存
                    dgRow.Cells["UntName"].Value = YuTongInoutTable.Rows[i]["unit"].ToString();
                    ...
                    dgRow.Cells["BarCode"].Value = YuTongInoutTable.Rows[i][PtBarCode].ToString();
