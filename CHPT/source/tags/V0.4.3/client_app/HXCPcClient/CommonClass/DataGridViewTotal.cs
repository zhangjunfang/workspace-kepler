...
namespace HXCPcClient.CommonClass
{
    ...
    public class DataGridViewTotal
    {
        ...
        /// 设置DataGridView位置
        /// </summary>
        void SetDataGridViewLocation()
        {
            if (IsShowScroll(dgv))
            {
                this.dgv.Height = this.dgv.Parent.Height - ROW_HEIGHT - dgv.Location.Y - 20;
                dgvSum.Location = new Point(dgv.Location.X, dgv.Location.Y + dgv.Height);
                dgvSum.Height = ROW_HEIGHT + 18;
            }
            else
            {
                this.dgv.Height = this.dgv.Parent.Height - ROW_HEIGHT - dgv.Location.Y - 2;
                dgvSum.Location = new Point(dgv.Location.X, dgv.Location.Y + dgv.Height);
                dgvSum.Height = ROW_HEIGHT + 2;

            }
            this.dgv.ResumeLayout(true);
            this.dgv.PerformLayout();
            //this.dgvSum.ResumeLayout(true);
            //this.dgvSum.PerformLayout();
            //this.dgv.Parent.PerformLayout();
            this.dgv.Refresh();
            //this.dgv.Invalidate();
        }
        /// <summary>
        ...
        private void InitDataGridViewSum()
        {
            ...
            for (int i = 0; i < this.dgv.Columns.Count; i++)
            {
                ...
                //this.dgvSum.DefaultCellStyle = this.dgv.Columns[i].DefaultCellStyle;
                this.dgvSum.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                this.dgvSum.DefaultCellStyle.Format = "N2";
        ...
        public void SetSum()
        {
            for (int i = 0; i < this.dgv.ColumnCount; i++)
            {
                if (this.sumColumns.Contains(this.dgv.Columns[i].Name) && this.dgv.Columns[i].Visible)
                {
                    ...
                    this.dgvSum[i, 0].Value = sum.ToString("N2");
                }
            }
        }
        /// <summary>
