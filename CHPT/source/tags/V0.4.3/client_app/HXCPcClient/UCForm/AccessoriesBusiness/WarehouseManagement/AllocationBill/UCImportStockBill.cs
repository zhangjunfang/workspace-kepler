    ...
    public partial class UCImportStockBill : FormChooser
    {
        ...
        private void UCImportStockBill_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //领料单/退料单字段列表
                FetchRefundFields = BillID + "," + partscode + "," + partsname + ",drawn_no,norms,unit,unit_price,quantity,sum_money";
                //调拨单字段列表
            ...
            try
            {
                ...
                {//调拨单入库
                    ...
                    { //获取根据配件条件查询到的调拨单主键ID
                        ...
                        for (int i = 0; i < gvBillID_dt.Rows.Count; i++)
                        {
                            ...
                            BuildQueryWhere() + " and call_in_wh_name='" + WareHouseName
                ...
                {//调拨单出库
                    ...
                    {   //获取根据配件条件查询到的调拨单主键ID
                        ...
                        {
                            gvBill_dt = DBHelper.GetTable(QueryLogMsg, BillTable, sb_Bill.ToString(),
                            BuildQueryWhere() + " and call_out_wh_name='" + WareHouseName
                            + "'" + " and stock_allot_id='" + gvBillID_dt.Rows[i]["stock_allot_id"] + "'", "", "");
                            if (gvBill_dt.Rows.Count == 0) return;//如果未查询到数据返回
                            GetBillLists(gvBill_dt);//显示查询结果
                        }
                ...
                else if (BillType == ReceiveBill && BillingType == RefundMaterial)
                {//退料单入库
                    //获取开单信息列表
                ...
                else if (BillType == ReceiveBill && BillingType == InventoryBill)
                {//盘点单盘亏入库
                    //获取开单信息列表
                    sb_Bill.AppendFormat("{0},{1},{2},{3},{4},{5},{6}", BillPrimaryID,
                    ordernum, orderdate, orgname, handlename, operatorname, rmk);//查询字段
                ...
                else if (BillType == ReceiveBill && BillingType == OtherReceiptBill)
                {   //其它收货单入库
                    //获取开单信息列表
                    sb_Bill.AppendFormat("{0},{1},{2},{3},{4},{5},{6}", BillPrimaryID,
                    ordernum, orderdate, orgname, handlename, operatorname, rmk);//查询字段
