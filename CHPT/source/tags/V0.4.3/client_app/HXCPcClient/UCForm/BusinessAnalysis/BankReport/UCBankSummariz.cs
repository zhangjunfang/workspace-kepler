...
ISNULL(b.期初余额,0)+SUM(ISNULL(a.本期收入,0))-SUM(ISNULL(a.本期支出,0)) 期末余额,a.bank_account";
            string table = string.Format(@"v_bank_summariz a left join (
select bank_account,SUM(ISNULL(本期收入,0)-ISNULL(本期支出,0)) 期初余额 from v_bank_summariz where 单据日期<0
group by bank_account
)b on a.bank_account=b.bank_account", Common.LocalDateTimeToUtcLong(Convert.ToDateTime(diDate.StartDate).Date));
            string groupBy = "group by a.账户名称,a.银行名称,b.期初余额,a.bank_account order by 账户名称";
