                ...
                foreach (DataGridViewRow dgvr in dgvMaterials.Rows)
                {
                    ...
                    if (strPname.Length > 0 && strPCmoney.Length > 0)
                    {
                        ...
                        if (strPID.Length == 0)
                        {
                            ...
                            obj.sqlString = "insert into [tb_maintain_material_detail] (material_id,parts_code,parts_name,norms,unit,whether_imported,quantity,unit_price,member_discount,member_price,sum_money,drawn_no,vehicle_brand,three_warranty,remarks,enable_flag,maintain_id,parts_id) ";
                            obj.sqlString += " values (@material_id,@parts_code,@parts_name,@norms,@unit,@whether_imported,@quantity,@unit_price,@member_discount,@member_price,@sum_money,@drawn_no,@vehicle_brand,@three_warranty,@remarks,@enable_flag,@maintain_id,@parts_id);";
                        }
                        else
                        {
            ...
            try
            {
                ...
                if (dpt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dpt.Rows.Count; i++)
                    {
                        ...
                        {
                             strPzk = !string.IsNullOrEmpty(txtMemberPZk.Caption) ? txtMemberPZk.Caption : "10";
                            //会员折扣
                        ...
                        {
                            //会员工时费
                            dgvproject.Rows[i].Cells["member_price"].Value = Convert.ToString(Convert.ToDecimal(!string.IsNullOrEmpty(CommonCtrl.IsNullToString(dpr["man_hour_norm_unitprice"])) ? CommonCtrl.IsNullToString(dpr["man_hour_norm_unitprice"]) : "0") * (Convert.ToDecimal(strPzk) / 10));
                        }
            ...
            try
            {
                ...
                if (dmt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dmt.Rows.Count; i++)
                    {
                        ...
                        {
                            string strPzk = !string.IsNullOrEmpty(txtMemberLZk.Text) ? txtMemberLZk.Text : "10";
                            //会员折扣
