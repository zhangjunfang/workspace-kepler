...
public class ServiceOnlineController extends BaseController{
	...
	public Map<String, Object> queryList(HttpServletRequest request) throws Exception{
	    ...
	    int total = onlineService.count(param);
	    PaginationResult<CompanyInfo> list = onlineService.selectPageForOnline(param);
//	    List<CompanyInfo> list = onlineService.selectPageForOnline(param);
	 ...
	 * @modifyInformation：
	 */
	@RequestMapping(value="/exportMonitorServerExcelData.do")
	@ResponseBody
	public void exportOnlineUsersManageExcelData(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		String exportDataHeader = "comName=公司名称&comContact=联系人&registTime=注册时间&province=所在地&machineSerial=机器码&authorizationCode=慧修车授权码&macAddress=计算机MAC地址&serviceVersion=服务端安装版本&registIp=IP&total=客户端在线数&serviceStatus=在线状态";
		String excel_result = onlineService.exportData(exportDataHeader, json, this.getUrl(request),"CompanyInfo","服务端状态");
