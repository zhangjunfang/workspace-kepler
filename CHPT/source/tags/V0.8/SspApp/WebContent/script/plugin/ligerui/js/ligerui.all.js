    ...
    $.ligerui.win = {
        ...
        setFront: function (win)
        {
            ...
            for (var i in wins)
            {
                ...
                if (w == win)
                {
                    $(w.element).css("z-index", "66100");
                    this.activeTask(w);
                }
                else
                {
                    $(w.element).css("z-index", "66000");
                }
            }
        }
    };
...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        //改变排序
        changeSort: function (columnName, sortOrder, data)
        {
            var g = this, p = this.options;
            ...
            if (p.dataAction == "local")
            {
                ...
                if (!g.sortedData || (g.sortedData && g.sortedData.Rows.length < 1))
                    g.sortedData = g.filteredData;
                if ($.type(data) === 'array' && data.Rows) g.sortedData = data;
        ...
        _addTreeDataIndexToData: function (data)
        {
            ...
            $(data).each(function ()
            {
                if (this.treedataindex != undefined) {
                    // edit by fanxuean in 2013.7.5 此处，如果第一次加载数据时有treedataindex，
                    // 而子节点是异步获取再加载的, treedataindex会出现重复，导致触发select节点的时候,
                    // 返回的节点数据不是你所选的节点的数据
                    g.treedataindex = this.treedataindex;
                    // return;
                } else
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                else
                {
                    ...
                    if (p.checkbox)
                    {
                        if (o.ischecked) // fanshine 于2013.6.27 添加了id
                            treehtmlarr.push('<div id="' + ((p.idFieldName && o[p.idFieldName]) ? o[p.idFieldName] : Math.ceil(Math.random()*100000)) + '_checkbox_tree_leaf" nodeid="' + ((p.idFieldName && o[p.idFieldName]) ? o[p.idFieldName] : '') + '" class="l-box l-checkbox l-checkbox-checked"></div>');
                        else
                            treehtmlarr.push('<div id="' + ((p.idFieldName && o[p.idFieldName]) ? o[p.idFieldName] : Math.ceil(Math.random()*100000)) + '_checkbox_tree_leaf" nodeid="' + ((p.idFieldName && o[p.idFieldName]) ? o[p.idFieldName] : '') + '" class="l-box l-checkbox l-checkbox-unchecked"></div>');
                    }
                    if (p.childIcon)
                    {
                        //node icon
        ...
        _setTreeEven: function ()
        {
            ...
            g.tree.click(function (e)
            {
                ...
                if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("l-box"))
                    ...
                    treeitem = $(obj).parent();
                else if ($(obj).parents("li:eq(0)").attr("treedataindex"))
                    treeitem = $(obj).parents("li:eq(0)");
                else
                    treeitem = $(obj);
                if ($(obj).hasClass("extendButton"))
                    treeitem = $(obj).parents(".l-body").parent();
                if (!treeitem) return;
                var treedataindex = parseInt(treeitem.attr("treedataindex"));
                var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
                ...
                g.trigger('click', [{ data: treenodedata, target: treeitem[0], srcElement: obj}]);
            });
