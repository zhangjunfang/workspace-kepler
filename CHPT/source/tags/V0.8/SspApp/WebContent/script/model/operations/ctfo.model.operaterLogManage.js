/**
 * [ 操作记录 ]                                                                                        resize(ch [description]
 * @return {[type]}         [description]
 */
CTFO.Model.OperaterLogManage = (function() {
  var uniqueInstance;
  ...
  function constructor() {
    ...
    var columns = [{
      ...
      align: 'center',
      render: function(row) {
        return '<div class="" title="' + row.valBefore + '">' + row.valBefore + '</div>';
      }
    }, {
      display: '变更后值',
      name: 'valAfter',
      ...
      align: 'center',
      render: function(row) {
        return '<div class="" title="' + row.valAfter + '">' + row.valAfter + '</div>';
      }
    }, {
      display: '操作时间',
      name: 'opTime',
    ...
    var gridOptions = {
      ...
      url: CTFO.config.sources.queryOpLogList, // 数据请求地址
      pageSize: pageSize,
    ...
    var initVehicleDetailUpdate = function(t, w, d, g) {
      ...
      $(w).find('.saveButton').click(function(event) {
          ...
          saveVehicleTeamDetailFrom(actionUrl, param, function() {
            $(obj).attr("disabled", false);
            g.close();
            vehicleTeamForm.find('.queryButton').trigger('click');
          });
        }).end()
    ...
    var bindEvent = function() {
      vehicleTeamForm.find('.addVehicleTeam').click(function(event) {
          inisertVehicleTeam();
          //日志统计
          cLog.addOperatorLog($.extend({}, CTFO.cache.cLogInfo, {
            opType: '新增车队信息', // (必填)
    ...
    var initForm = function() {
      ...
      initFunctionSelectList(vehicleTeamForm.find("select[name='funId']"));

      //初始化时，需要将当前用户的企业信息添加到所属企业和parentId字段，用于查询
      vehicleTeamForm.find('.parentCorpDesc').text(CTFO.cache.user.entName).end()
      ...
      var provinceOption = vehicleTeamForm.find('select[name=corpProvince]'),
        ...
        countyOption = vehicleTeamForm.find('select[name=corpCounty]');
      //生成省
      ...
      $(vehicleTeamForm).find('input[name=startTime]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(vehicleTeamForm).find('input[name=endTime]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
    ...
    var searchGrid = function() {
      ...
      op.push({
        name: "requestParam.equal.opId",
        value: CTFO.cache.user.opId
      });
      op.push({
      ...
      op.push({
        name: "requestParam.equal.comId",
        value: CTFO.cache.user.comId
      });
      grid.setOptions({
        parms: op
      });
    ...
    var validateParams = function() {
      ...
      $(data).each(function(event) {
        var name = '';
        if (this.name === 'content' || this.name === 'opName') name = 'requestParam.like.' + this.name;
    ...
    var initFunctionSelectList = function(container, defaultVal) {
      $.ajax({
        ...
        success: function(data, textStatus, xhr) {
          CTFO.utilFuns.commonFuns.getSelectList(_.filter(_.map(data, function(item) {
            if ($.inArray(item.funId, CTFO.cache.auth) > 0) {
              return {
                "code": item.funId,
                "name": item.funName
              };
