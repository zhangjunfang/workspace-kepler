/**
 * [ 系统监控 - 访问统计]                                                                                        resize(ch [description]
 * @return {[type]}         [description]
 */
CTFO.Model.VisitStat = (function() {
  var uniqueInstance;
  ...
  function constructor() {
    ...
    var cHeight = 0,
      ...
      updateRowAuth = 'FG_MEMU_SYSMONITOR_VISITSTAT_U', // 修改记录权限
      detailRowAuth = 'FG_MEMU_SYSMONITOR_VISITSTAT_I', // 查看记录详情权限
      deleteRowAuth = 'FG_MEMU_SYSMONITOR_VISITSTAT_D', // 删除记录权限
      addRowAuth = 'FG_MEMU_SYSMONITOR_VISITSTAT_C', // 新增记录权限
      exportRowAuth = 'FG_MEMU_SYSMONITOR_VISITSTAT_E', // 导出记录权限
      opLogRowAuth = 'FG_MEMU_SYSMONITOR_VISITSTAT_O', // 操作记录
    ...
    var gridOptions = {
      ...
      url: CTFO.config.sources.monitorVisit, // 数据请求地址
      pageSize: pageSize,
    ...
    var inisertVehicleTeam = function() {
      var defaultData = {
          parentName: visitForm.find('.parentCorpDesc').text(),
          corpCode: CTFO.cache.user.corpCode,
          parentId: visitForm.find('input[name=parentId]').val()
        },
      ...
      var param = {
        ...
        onLoad: function(w, d, g) {
          ...
          var grandpaId = visitForm.find('input[name=grandpaId]').val();
          if (grandpaId !== "-1") {
    ...
    var initVehicleDetailUpdate = function(t, w, d, g) {
      ...
      $(w).find('.saveButton').click(function(event) {
          ...
          saveVehicleTeamDetailFrom(actionUrl, param, function() {
            $(obj).attr("disabled", false);
            g.close();
            visitForm.find('.queryButton').trigger('click');
          });
        }).end()
    ...
    var bindEvent = function() {
      visitForm.find('.addVehicleTeam').click(function(event) {
          inisertVehicleTeam();
          //日志统计
          cLog.addOperatorLog($.extend({}, CTFO.cache.cLogInfo, {
            opType: '新增车队信息', // (必填)
        ...
        .find('.exportGrid').click(function(event) {
          CTFO.utilFuns.commonFuns.exportGrid({
            grid: grid,
            url: CTFO.config.sources.exportMonitorVisitExcelData
          });
    ...
    var selectOrgTreeNode = function(d, columns) {
      visitForm.find('.parentCorpDesc').text(d[columns.text]).end()
    ...
    var initForm = function() {
      initFunctionSelectList(visitForm.find("select[name='funId']"));
      //初始化时，需要将当前用户的企业信息添加到所属企业和parentId字段，用于查询
      $(visitForm).find('input[name=accessTimeStart]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      $(visitForm).find('input[name=accessTimeEnd]').click(function(event) {
        WdatePicker({
          dateFmt: 'yyyy-MM-dd',
          isShowClear: false
        });
      });
      visitForm.find('.queryButton').click(function(event) {
          searchGrid();
        }).end()
    ...
    /**
     * [initFunctionSelectList 初始化功能名称下拉列表 select]
     *
     *
     * @param container
     *            审批人容器
     ...
     * @return {[type]} [description]
     */
    var initFunctionSelectList = function(container, defaultVal) {
      $.ajax({
        url: CTFO.config.sources.selectOpLogQuery,
        dataType: 'json',
        complete: function(xhr, textStatus) {
          //called when complete
        },
    ...
    return {
      init: function(options) {
        ...
        vehicleTeamTerm = p.mainContainer.find('.visitStatTerm');
        visitForm = p.mainContainer.find('form[name=visitForm]');
