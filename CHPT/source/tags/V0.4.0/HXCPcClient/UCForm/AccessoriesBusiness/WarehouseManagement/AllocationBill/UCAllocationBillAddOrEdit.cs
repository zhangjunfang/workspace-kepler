        ...
        private void ImportBillingMethod(string BillTable, string PartTable, string BillingPrimID, string QueryLogMsg, string ModifyLogMsg,string BillType)
        {
            try
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    BillingPrimID, PartCode, PartsName,PartModel,DrawNum, UnitName, PartsBrand, Remark);//要查询的字段名称格式项
                ...
                for (int i = 0; i < partsTable.Rows.Count; i++)
                {
                   ...
                   if (BillingType == PurchaseBilling)
                    ...
                    else if (BillingType == MaterialRequisition )
                    {
                        ...
                        dgRow.Cells["partbrand"].Value = string.Empty;//品牌
            ...
            try
            {
                ...
                if (gvPartsMsgList.Rows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow DgVRow in gvPartsMsgList.Rows)
                    {
                        ...
                        DicPartParam.Add("billing_id", DgVRow.Cells["billing_ID"].Value.ToString() == string.Empty ? "" : DgVRow.Cells["billing_ID"].Value.ToString());
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (DgResult == DialogResult.Yes)
                    {
                        ...
                        if (MaxCount == 0)
                        {
                            for (int j = 0; j < gvPartsMsgList.Rows.Count; j++)
                            {
                                if (gvPartsMsgList.Rows[j].Selected) gvPartsMsgList.Rows.RemoveAt(j);//删除选中行
                            }
                        }
