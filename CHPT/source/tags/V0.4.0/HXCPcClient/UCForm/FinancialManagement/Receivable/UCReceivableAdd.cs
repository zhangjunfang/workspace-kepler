            ...
            if (frmVerify.ShowDialog() == DialogResult.OK)
            {
                ...
                if (frmVerify.auditStatus == DataSources.EnumAuditStatus.NOTAUDIT)
                {
                    ...
                    if (orderType == DataSources.EnumOrderType.RECEIVABLE)
                    {
                        ...
                        receivableSql.sqlString = string.Format(@"update tb_parts_sale_billing set is_lock=@is_lock where where sale_billing_id in (select documents_id from tb_balance_documents where order_id='{0}') and  exists (
...
update tb_parts_sale_order set is_lock=@is_lock where where sale_order_id in (select documents_id from tb_balance_documents where order_id='{0}') and  exists (
select a.documents_id from tb_balance_documents a inner join tb_bill_receivable b on a.order_id=b.payable_single_id
...
update tb_maintain_settlement_info set is_lock=@is_lock where where settlement_id in (select documents_id from tb_balance_documents where order_id='{0}') and  exists (
select a.documents_id from tb_balance_documents a inner join tb_bill_receivable b on a.order_id=b.payable_single_id
...
update tb_maintain_three_guaranty_settlement set is_lock=@is_lock where where st_id in (select documents_id from tb_balance_documents where order_id='{0}') and  exists (
select a.documents_id from tb_balance_documents a inner join tb_bill_receivable b on a.order_id=b.payable_single_id
...
,@create_by,@create_time)";
     ...
     VALUES
        ...
        {
            //dgvBalanceDocuments.Width = dgvPaymentDetail.Width;
        }
