    ...
    public partial class UCCallBackImport : FormChooser
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                strWhere = string.Format("enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and (document_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.AUDIT).ToString() + "' or document_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.SUBMIT).ToString() + "') and import_status='" + Convert.ToInt32(DataSources.EnumImportStaus.OPEN).ToString() + "'");//enable_flag 1未删除
        ...
        private void dgvRData_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                uc.strBefore_orderId = dgvRData.CurrentRow.Cells["repair_id"].Value.ToString();  //导入单据的Id值
                uc.strOrdersSouse = "2";
                uc.BindCallBackData();
                this.Close();
            }
