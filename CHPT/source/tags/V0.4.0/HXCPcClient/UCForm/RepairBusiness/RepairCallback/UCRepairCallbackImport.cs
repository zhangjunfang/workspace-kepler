    ...
    public partial class UCRepairCallbackImport : FormChooser
    {
        ...
        private void BindData()
        {
            try
            {
                ...
                strWhere = string.Format(" b.enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and (b.info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.AUDIT).ToString() + "' or b.info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.SUBMIT).ToString() + "') and b.Import_status='" + Convert.ToInt32(DataSources.EnumImportStaus.OPEN).ToString() + "'");//enable_flag 1未删除
        ...
        private void BindDataP()
        {
            try
            {
                ...
                #endregion
                string strFiles = " *";
                strWhere = string.Format(" enable_flag='" + Convert.ToInt32(DataSources.EnumEnableFlag.USING).ToString() + "' and (info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.SUBMIT).ToString() + "' or info_status='" + Convert.ToInt32(DataSources.EnumAuditStatus.AUDIT).ToString() + "') and Import_status='" + Convert.ToInt32(DataSources.EnumImportStaus.OPEN).ToString() + "'");//enable_flag 1未删除
                if (!string.IsNullOrEmpty(txtCarNO.Text.Trim()))//车牌号
                {
        ...
        private void BindDataP1()
        {
            try
            {
                ...
                string strTables = "tb_maintain_info";
                DataTable dt = DBHelper.GetTable("维修结算单信息列表", strTables, strFiles, strWhere, "", "order by update_time desc");
                dgvRData.DataSource = dt;
            }
            catch (Exception ex)
            {
