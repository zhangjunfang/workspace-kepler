...
namespace HXCPcClient.UCForm.BusinessAnalysis.ARAPReport
{
    public partial class UCReceivableSummariz : UCReport
    {
        ...
        void BindData()
        {
            string filed = @"cust_code,cust_name,期初应收款,sum(本期发生) 本期增加应收款,SUM(其中商业折扣) 其中商业折扣,SUM(money) 本期承收应收款,
SUM(deduction) 其中现金折扣,期初应收款+sum(本期发生)-SUM(money) 期末结存应收额";
            string table = string.Format(@"(
select c.cust_code,c.cust_name,d.期初应收款,a.本期发生,a.其中商业折扣,e.money,e.deduction,c.cust_type,c.is_member
from v_receivable_document a 
left join v_YingShou b on a.ID=b.documents_id
left join tb_customer c on a.cust_id=c.cust_id
left join (
select cust_id,SUM(本期发生) 期初应收款 from v_receivable_document where 单据日期<{0}
group by cust_id
) d on a.cust_id=d.cust_id
left join v_YingShou_detail e on a.cust_id=e.cust_id
