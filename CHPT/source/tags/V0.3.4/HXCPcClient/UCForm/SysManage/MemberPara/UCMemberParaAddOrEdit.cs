    ...
    public partial class UCMemberParaAddOrEdit : UCBase
    {
        ...
        private void _LoadData(object obj)
        {
           ...
           sqlobj.sqlString = "SELECT p.*,w.project_name,w.project_num,w.quota_price,'' as discount_price  FROM tb_CustomerSer_member_setInfo_projrct p left join tb_workhours w on w.whours_id=p.service_project_id  "
            ...
            sqlobj.sqlString += "; SELECT p2.*,w2.ser_parts_code,w2.parts_name,w2.ref_out_price,'' as discount_price  FROM tb_CustomerSer_member_setInfo_parts p2 left join tb_parts w2 on w2.parts_id=p2.parts_id  "
        ...
        private string GetUpdteInsertPartsSql(string setInfo_id)
        {
            ...
            foreach (DataGridViewRow dr in dgvparts.Rows)
            {
                if (dr.Cells[this.setInfo_parts_id.Name].Value.ToString().Length == 0)
                {
                    ...
                    strSql += "('" + guid + "',";
                    strSql += "'" + setInfo_id + "',";
                    strSql += "'" + dr.Cells[this.setInfo_parts_id.Name].Value.ToString() + "',";//配件id
                    strSql += dr.Cells["parts_discount"].Value.ToString() + ",";//折扣
                ...
                else
                {
                    ...
                    updateSql += "parts_discount='" + dr.Cells["parts_discount"].Value.ToString() + "',";
        ...
        private string GetUpdteInsertProjrctSql(string setInfo_id)
        {
            ...
            foreach (DataGridViewRow dr in dgvprojrct.Rows)
            {
                if (dr.Cells[this.setInfo_projrct_id.Name].Value.ToString().Length == 0)
                {
                    ...
                    strSql += "('" + guid + "',";
                    strSql += "'" + setInfo_id + "',";
                    strSql += "'" + dr.Cells[this.setInfo_projrct_id.Name].Value.ToString() + "',";//维修项目id
                    strSql += dr.Cells["project_service_project_discount"].Value.ToString() + ",";//折扣
                ...
                else
                {
                    ...
                    updateSql += "service_project_discount='" + dr.Cells["project_service_project_discount"].Value.ToString() + "',";
