    ...
    public partial class UCRepairMealView : UCBase
    {
        ...
        private void GetReservData(string strRId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                #region 维修项目数据
                DataTable dpt = DBHelper.GetTable("维修项目数据", "sys_b_set_repair_package_set_project", "*", string.Format(" repair_package_set_id='{0}'", strRId), "", ""); ;
                if (dpt.Rows.Count > 0)
                {
                ...
                #region 维修用料数据
                DataTable dmt = DBHelper.GetTable("维修用料数据", "sys_b_set_repair_package_set_materials", "*", string.Format(" repair_package_set_id='{0}'", strRId), "", "");
                if (dmt.Rows.Count > 0)
                {
