...
namespace HXCPcClient.UCForm.SysManage.BulletinManage
{
    ...
    public partial class UCBulletinAddOrEdit : UCBase
    {
        ...
        private void BindData()
        {
            ...
            #region 接收组织信息
            DataTable dot = DBHelper.GetTable("接收组织信息", "sys_announcement_org", "*", "announcement_id='" + id + "'", "", "");
            if (dot.Rows.Count > 0)
            {
            ...
            #region 接收人员信息
            DataTable dut = DBHelper.GetTable("接收人员信息", "sys_announcement_user", "*", "announcement_id='" + id + "'", "", "");
            if (dut.Rows.Count > 0)
            {
