    ...
    public partial class UCAllocationQuery : UCBase
    {
        ...
        private void gvAllocPartList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用   
                {
                    string InOutIdValue = gvAllocPartList.CurrentRow.Cells["Inout_Id"].Value.ToString();//获取出入库单ID
                    string WHName = gvAllocPartList.CurrentRow.Cells["PartWHName"].Value.ToString();//获取当前出入库单仓库名称
                    UCAllocationBillDetails UCAllocPartDetails = new UCAllocationBillDetails(InOutIdValue, WHName);
        ...
        private void GetAllocBillList(string WhereStr)
        {
            try
            {
                ...
                sbBillField.AppendFormat("IOBillTb.{0},{1},{2},{3},{4},IOBillTb.{5},{6},{7},PartAmount,{8},{9},{10},IOBillTb.{11}", 
                InOutID, OrderTypeName,BillingTypeName, OrderNo, OrdDate, WareHouseName, BussinUnit, ArrivalPlace,OrgName, 
                ...
                "", " order by IOBillTb.order_date desc", winFormBillPager.PageIndex, winFormBillPager.PageSize, out RecCount);//获取出入库单表查询记录
                ...
                for (int i = 0; i < InoutBillTable.Rows.Count; i++)
                {
                        ...
                        dgRow.Cells["WHName"].Value = InoutBillTable.Rows[i][WareHouseName].ToString();
        ...
        private void GetPartOrBusUntList(String WhereStr)
        {
            try
            {
                ...
                //sbPartField.AppendFormat("IOBillTb.{0},IOBillTb.{1},{2},{3},{4},{5},IOPartTb.{6},IOPartTb.{7},IOPartTb.{8},IOPartTb.{9},IOPartTb.{10},{11}," +
                //"PartAmount,tb_vehicle_models.{12},IOPartTb.{13},{14},{15},{16},{17},IOPartTb.{18}",
                //InOutID,WareHouseName,OrderTypeName,BillingTypeName, OrderNo, OrdDate, PartsNum, CarFactoryCode, PartsName, DrawNum, PartSpec, UnitName,//11
                //VmName, PartsBrand, SupplierCode, Custcode, BussinUnit, PartGift, PartRemk);
                ////数据表联合查询集
                //StringBuilder sbRelationTable = new StringBuilder();
                //sbRelationTable.AppendFormat("{0} as IOBillTb inner join {1} as IOPartTb on IOBillTb.{9}=IOPartTb.{9} "+
                //" inner join (select {0}.{9},sum({21}) as PartAmount from {0} inner join {1} on {0}.{9}={1}.{9} group by {0}.{9}) as IOBillPartTb "+
                //" on IOBillPartTb.{9}= IOBillTb.{9} " +
                //" left join {2} on IOBillTb.{10}={2}.{11} left join {3} on IOBillTb.{10}={3}.{12} " +
                //" left join {4} on {2}.{13}={4}.{16} or  {3}.{14}={4}.{16} left join {5} on {5}.{15}={4}.{15} left join {6} on {6}.{19}=IOPartTb.{18}" +
                //" left join {7} on {6}.{17} = {7}.{17} left join {8} on {8}.{20}={7}.{20}",
                //InOutBillTable, InOutPartTable, SupplierTable, CustomerTable, RelationContactTable, ContactTable, PartsTable, PartForVhcTable, VehModelTable,//8
                //InOutID, BussinUnit, SupFullName, CustName, SupplierID, CustomerID, ContactId, RelObjId, PartID, PartsNum, PartsSerCode, VehicleTypeId, PartCount);//21

                DataTable InoutTable = DBHelper.GetTableByPage(InOutQueryLogMsg, VPartOrBusUntView, "*", WhereStr,
                "", "order_date desc", winFormBillPager.PageIndex, winFormBillPager.PageSize, out RecCount);//获取出入库单表查询记录
                winFormBillPager.RecordCount = RecCount;//获取总记录行
                if (RecCount == 0)
                {
                    MessageBoxEx.Show("报歉没有找到您要的出入库单据！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                for (int i = 0; i < InoutTable.Rows.Count; i++)
                {
                    ...
                    DataGridViewRow dgRow = gvAllocPartList.Rows[gvAllocPartList.Rows.Add()];//创建新行项
                        ...
                        dgRow.Cells["ID"].Value = SerialNum;
        ...
        private string QueryBillWhereCondition()
        {
            try
            {
                ...
                {
                    Str_Where += " and order_num = '" + txtBillNum.Caption.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(CombCompany.SelectedValue.ToString()))
                {
        ...
        private string QueryPartOrBusUntWhereCondition()
        {
            try
            {
                ...
                {
                    Str_Where += " and is_gift='" + Combis_gift.SelectedValue.ToString() + "'";
                }
