    ...
    public partial class UCAllocationBillManager : UCBase
    {
        ...
        public UCAllocationBillManager()
        {
           ...
           // DataGridViewEx.SetDataGridViewStyle(gvAllocBillList, OrderState);//美化表格控件
            ...
            gvAllocBillList.HeadCheckChanged += new DataGridViewEx.DelegateOnClick(gvAllocBillList_HeadCheckChanged); //复选框标题显示为复选框状态  
        }
        //复选框选择状态
        private void gvAllocBillList_HeadCheckChanged()
        {
        ...
        private void IsDataGridViewCheckBox()
        {
            try
            {
                gvAllocBillList.EndEdit();//单据列表
                if (gvAllocBillList.Rows.Count == 0)
                {
                ...
                {
                    bool ischeck = (bool)((DataGridViewCheckBoxCell)gvAllocBillList.Rows[0].Cells["colCheck"]).EditedFormattedValue;
                    bool SetCheck = ischeck == true ? true : false;
                    for (int i = 1; i < gvAllocBillList.Rows.Count; i++)
                    {
                        ((DataGridViewCheckBoxCell)gvAllocBillList.Rows[i].Cells["colCheck"]).Value = SetCheck;//开单列表选择状态
                    }
                }
