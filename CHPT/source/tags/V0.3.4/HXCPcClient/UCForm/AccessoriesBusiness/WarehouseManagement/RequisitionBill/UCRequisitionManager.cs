...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.RequisitionBill
{
    public partial class UCRequisitionManager : UCBase
    {
        ...
        //复选框选择状态
        private void gvAllotBillList_HeadCheckChanged()
        {
       ...
       private  void UCRequisitionManager_ExportEvent(object sender, EventArgs e)
       {
        ...
        private void IsDataGridViewCheckBox()
        {
            try
            {
                ...
                if (gvAllotBillList.Rows.Count != 0)
                {
                    bool ischeck = (bool)((DataGridViewCheckBoxCell)gvAllotBillList.Rows[0].Cells["colCheck"]).EditedFormattedValue;
                    bool SetCheck = ischeck == true ? true : false;
                    for (int i = 1; i < gvAllotBillList.Rows.Count; i++)
                    {
                        ((DataGridViewCheckBoxCell)gvAllotBillList.Rows[i].Cells["colCheck"]).Value = SetCheck;//开单列表选择状态
                    }
                }
