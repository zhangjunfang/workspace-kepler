...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.ModifyPriceBill
{
    public partial class UCModifyPriceManager : UCBase
    {
        ...
        public UCModifyPriceManager()
        {
            ...
            gvModifyPriceBillList.HeadCheckChanged += new DataGridViewEx.DelegateOnClick(gvModifyPriceBillList_HeadCheckChanged); //复选框标题显示为复选框状态  
        ...
        //复选框选择状态
       private  void gvModifyPriceBillList_HeadCheckChanged()
        {
       ...
       private  void UCModifyPriceManager_ExportEvent(object sender, EventArgs e)
                ...
                else 
                {
                    ...
                    if (DgResult == DialogResult.OK)
                    {
                        ...
                        if (flag)
                        {
                            ...
                            GetModifyPriceBillList(QueryWhere);//刷新调价单列表
        ...
        private void IsDataGridViewCheckBox()
        {
            try
            {
                ...
                if (gvModifyPriceBillList.Rows.Count != 0)
                {
                    bool ischeck = (bool)((DataGridViewCheckBoxCell)gvModifyPriceBillList.Rows[0].Cells["colCheck"]).EditedFormattedValue;
                    bool SetCheck = ischeck == true ? true : false;
                    for (int i = 1; i < gvModifyPriceBillList.Rows.Count; i++)
                    {
                        ((DataGridViewCheckBoxCell)gvModifyPriceBillList.Rows[i].Cells["colCheck"]).Value = SetCheck;//开单列表选择状态
                    }
                }
        ...
        public void GetModifyPriceBillList(string WhereStr)
        {
            try
            {
                ...
                "", " MdyBillTb.order_date desc", winFormModifyPage.PageIndex, winFormModifyPage.PageSize, out RecCount);//获取出入库单表查询记录
