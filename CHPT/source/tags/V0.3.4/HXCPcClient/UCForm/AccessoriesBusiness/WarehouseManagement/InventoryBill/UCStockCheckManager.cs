    ...
    public partial class UCStockCheckManager : UCBase
    {
        ...
        public UCStockCheckManager()
        {
           ...
           // DataGridViewEx.SetDataGridViewStyle(gvCheckBillList, OrderState);//美化表格控件
            ...
            gvCheckBillList.HeadCheckChanged += new DataGridViewEx.DelegateOnClick(gvCheckBillList_HeadCheckChanged); //复选框标题显示为复选框状态 
        }
        //复选框选择状态
        private void gvCheckBillList_HeadCheckChanged()
        {
       ...
       private void UCStockCheckManager_ExportEvent(object sender, EventArgs e)
       {
        ...
        private void IsDataGridViewCheckBox()
        {
            try
            {
                gvCheckBillList.EndEdit();//单据列表
                if (gvCheckBillList.Rows.Count == 0)
                {
                ...
                {
                    bool ischeck = (bool)((DataGridViewCheckBoxCell)gvCheckBillList.Rows[0].Cells["colCheck"]).EditedFormattedValue;
                    bool SetCheck = ischeck == true ? true : false;
                    for (int i = 1; i < gvCheckBillList.Rows.Count; i++)
                    {
                        ((DataGridViewCheckBoxCell)gvCheckBillList.Rows[i].Cells["colCheck"]).Value = SetCheck;//开单列表选择状态
                    }
                }
        ...
        public void GetCheckBillList(string WhereStr)
        {
            try
            {
                ...
                "", " CkBillTb.order_date desc", winFormCheckPage.PageIndex, winFormCheckPage.PageSize, out RecCount);//获取盘点单表查询记录
