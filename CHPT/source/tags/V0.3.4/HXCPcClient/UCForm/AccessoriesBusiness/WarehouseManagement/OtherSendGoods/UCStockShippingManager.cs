...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.OtherSendGoods
{
    public partial class UCStockShippingManager : UCBase
    {
        ...
        public UCStockShippingManager()
        {
            ...
            gvShippingBillList.HeadCheckChanged += new DataGridViewEx.DelegateOnClick(gvShippingBillList_HeadCheckChanged); //复选框标题显示为复选框状态
        ...
        //复选框选择状态
        private void gvShippingBillList_HeadCheckChanged()
        {
       ...
       private void UCStockShippingManager_ExportEvent(object sender, EventArgs e)
                ...
                else
                {
                    ...
                    if (DgResult == DialogResult.OK)
                    {
                        ...
                        if (flag)
                        {
                            ...
                            GetShippingBillList(QueryWhere);//刷新其它发货单列表
        ...
        private void winFormReceiptPage_PageIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (SearchFlag == (int)DataSources.SearchState.InitSearch)
                {
                    ...
                    string DefaultWhere = " enable_flag=1 and ShpBillTb.create_time between  " + StartDate + " and " + EndDate;//默认查询条件
        ...
        private void IsDataGridViewCheckBox()
        {
            try
            {
                gvShippingBillList.EndEdit();//单据列表
                if (gvShippingBillList.Rows.Count == 0)
                {
                ...
                {
                    bool ischeck = (bool)((DataGridViewCheckBoxCell)gvShippingBillList.Rows[0].Cells["colCheck"]).EditedFormattedValue;
                    bool SetCheck = ischeck == true ? true : false;
                    for (int i = 1; i < gvShippingBillList.Rows.Count; i++)
                    {
                        ((DataGridViewCheckBoxCell)gvShippingBillList.Rows[i].Cells["colCheck"]).Value = SetCheck;//开单列表选择状态
                    }
                }
        ...
        public void GetShippingBillList(string WhereStr)
        {
            try
            {
                ...
                if (RecCount == 0)
                {
                    MessageBoxEx.Show("报歉没有找到您要的其它发货单据！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
