...
namespace HXCPcClient.UCForm.AccessoriesBusiness.SaleManagement.SaleBilling
{
    public partial class UCChooseSaleOrder : FormChooser
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (List_PlanID.Count > 0)
                {
                    ...
                    {
                        DataTable dt_parts_purchase = DBHelper.GetTable("查询销售订单配件表信息", "tb_parts_sale_order_p", "*", " sale_order_id='" + sale_order_id + "' and is_suspend=1 and finish_count<business_count ", "", "");
                        if (dt_parts_purchase.Rows.Count > 0)
                        {
                            for (int i = 0; i < dt_parts_purchase.Rows.Count; i++)
                            {
                                partsInfo = new PartsInfoClassBySaleOrder();
                                partsInfo.sale_order_id = dt_parts_purchase.Rows[i]["sale_order_id"].ToString();
        ...
        private void BinddgAccessoriesDetail(string sale_order_id)
        {
            ...
            {
                string wherestr = " sale_order_id='" + sale_order_id + "' and is_suspend=1 and finish_count<business_count ";
                if (this.RelationType == "宇通采购订单")
                {
                    wherestr = wherestr + " and len(car_factory_code)>0";
                }
                DataTable dt_parts_purchase = DBHelper.GetTable("查询销售订单配件表信息", "tb_parts_sale_order_p", "*", wherestr, "", "");
