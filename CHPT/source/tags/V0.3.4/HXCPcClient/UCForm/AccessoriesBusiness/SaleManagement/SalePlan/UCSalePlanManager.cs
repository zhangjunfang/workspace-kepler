    ...
    public partial class UCSalePlanManager : UCBase
    {
        ...
        void UCSalePlanManager_VerifyEvent(object sender, EventArgs e)
        {
            ...
            if (UcVerify.ShowDialog() == DialogResult.OK)
            {
                ...
                bool flag = DBHelper.BatchUpdateDataByIn("批量审核销售计划单表", "tb_parts_sale_plan", purchasePlanField, "sale_plan_id", listField.ToArray());
                if (flag)
                {
                    BindgvSalePlanList();
                    MessageBoxEx.Show("操作成功！");
            ...
            try
            {
                ...
                foreach (DataGridViewRow dr in gvSalePlanList.Rows)
                {
                    ...
                    if (isCheck != null && (bool)isCheck)
                    {
                        ...
                        dicParam.Add("order_num", CommonUtility.GetNewNo(DataSources.EnumProjectType.SalePlan));//单据编号
