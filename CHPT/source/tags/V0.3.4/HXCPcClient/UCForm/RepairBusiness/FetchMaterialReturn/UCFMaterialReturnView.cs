    ...
    public partial class UCFMaterialReturnView : UCBase
    {
        ...
        void UCFMaterialReturnView_InvalidOrActivationEvent(object sender, EventArgs e)
        {
            ...
            if (DBHelper.BatchExeSQLMultiByTrans("更新单据状态为" + strmsg + "", listSql))
            {
                ...
                uc.BindPageData();
                deleteMenuByTag(this.Tag.ToString(), "UCFetchMaterialView");
            }
            else
            {
        ...
        void UCFMaterialReturnView_CopyEvent(object sender, EventArgs e)
        {
            ...
            deleteMenuByTag(this.Tag.ToString(), "UCFMaterialReturnView");
        }
        #endregion
        ...
        private void GetRescueData(string strRId,string strDRId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string strFetchTime = CommonCtrl.IsNullToString(dr["refund_time"]); //退料时间
            ...
            #region 领料信息
                DataTable dlt = DBHelper.GetTable("领料数据", "tb_maintain_refund_material_detai", "*", string.Format(" refund_id='{0}'", strDRId), "", "");
                if (dlt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dlt.Rows.Count; i++)
                    {
                        ...
                        dgvMaterials.Rows[i].Cells["warehouse"].Value = GetDicName(CommonCtrl.IsNullToString(dor["warehouse"]));
                        dgvMaterials.Rows[i].Cells["goods_allocation"].Value = GetDicName(CommonCtrl.IsNullToString(dor["goods_allocation"]));
        ...
        private void dgvMaterials_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (fieldNmae.Equals("warehouse") || fieldNmae.Equals("goods_allocation"))
            {
