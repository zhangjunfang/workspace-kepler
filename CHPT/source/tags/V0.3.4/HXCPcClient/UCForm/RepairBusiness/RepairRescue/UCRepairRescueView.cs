    ...
    public partial class UCRepairRescueView : UCBase
    {
        ...
        private void GetRescueData(string strRId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                labGsseS.Text = CommonCtrl.IsNullToString(dr["man_hour_tax_cost"]);//工时税额
                labGssjhjS.Text = CommonCtrl.IsNullToString(dr["man_hour_sum"]);//工时价税合计
                ...
                labPjsjhjS.Text = CommonCtrl.IsNullToString(dr["fitting_sum"]);//配件价税合计
