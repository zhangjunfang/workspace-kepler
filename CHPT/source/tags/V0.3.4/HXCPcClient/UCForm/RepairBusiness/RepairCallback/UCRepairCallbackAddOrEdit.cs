       ...
       List<string> listProject = new List<string>();
        ...
        private void addProject_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                //会员工时费
                dgvproject.CurrentRow.Cells["member_price"].Value = Convert.ToString(Convert.ToDecimal(frmHours.strQuotaPrice) * (Convert.ToDecimal(strPzk) / 10));
                if (dgvproject.CurrentRow.Cells["member_price"].Value.ToString() != "0")
                {
                    //折扣额
                    dgvproject.CurrentRow.Cells["member_sum_money"].Value = Convert.ToString(Convert.ToDecimal(frmHours.strQuotaPrice) - Convert.ToDecimal(dgvproject.CurrentRow.Cells["member_price"].Value));
                }
                else
                {
                ...
                dgvproject.CurrentRow.Cells["sum_money_goods"].Value = Convert.ToString(Convert.ToDecimal(frmHours.strWhoursNum) * Convert.ToDecimal(!string.IsNullOrEmpty(CommonCtrl.IsNullToString(dgvproject.CurrentRow.Cells["member_price"].Value)) ? CommonCtrl.IsNullToString(dgvproject.CurrentRow.Cells["member_price"].Value) : "0"));
                dgvproject.Rows.Add(1);
            }
        }
        #endregion
        ...
        private void addParts_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                if (dpt.Rows.Count > 0)
                {
                    ...
                    string strUMoney = dgvMaterials.CurrentRow.Cells["unit_price"].Value != null ? dgvMaterials.CurrentRow.Cells["unit_price"].Value.ToString() : "0";
                    //会员单价
