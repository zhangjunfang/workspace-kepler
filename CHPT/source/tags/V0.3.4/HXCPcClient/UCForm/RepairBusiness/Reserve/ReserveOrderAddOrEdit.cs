        ...
        private void addParts_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                if (dpt.Rows.Count > 0)
                {
                    ...
                    string strNum = CommonCtrl.IsNullToString(dgvMaterials.CurrentRow.Cells["quantity"].Value);
                    ...
                    dgvMaterials.CurrentRow.Cells["Msum_money"].Value = Convert.ToString(Convert.ToDecimal(strNum) * Convert.ToDecimal(strUMoney = strUMoney == "" ? "0" : strUMoney));
                    dgvMaterials.CurrentRow.Cells["drawn_no"].Value = CommonCtrl.IsNullToString(dpr["drawing_num"]);
                    dgvMaterials.CurrentRow.Cells["vehicle_model"].Value = CommonCtrl.IsNullToString(dpr["v_brand_name"]);
