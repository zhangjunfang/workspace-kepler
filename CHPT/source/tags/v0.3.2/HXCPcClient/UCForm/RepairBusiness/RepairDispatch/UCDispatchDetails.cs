...
namespace HXCPcClient.UCForm.RepairBusiness.RepairDispatch
{
    ...
    public partial class UCDispatchDetails : UCDispatchBase
    {
        ...
        #region 人员派工信息保存
        private void DispatchUserInfo(List<SQLObj> listSql, string partID)
        {
            SQLObj obj = new SQLObj();
            obj.cmdType = CommandType.Text;
            Dictionary<string, ParamObj> dicParam = new Dictionary<string, ParamObj>();
            foreach (DataGridViewRow dgvr in dgvUserData.Rows)
            {
                ...
                if (strUNo.Length > 0)
                {
                    dicParam.Add("maintain_id", new ParamObj("maintain_id", partID, SysDbType.VarChar, 40));//Id
                    ...
                    string strDId = CommonCtrl.IsNullToString(dgvr.Cells["dispatch_id"].Value);
                    if (strDId.Length== 0)
                    {
                        opName = "新增派工人员信息";
                        strDId = Guid.NewGuid().ToString();
                        dicParam.Add("dispatch_id", new ParamObj("dispatch_id", strDId, SysDbType.VarChar, 40));
                        obj.sqlString = @"insert into tb_maintain_dispatch_worker(maintain_id,dispatch_no,dispatch_name,org_name,team_name,man_hour,sum_money,create_time,dispatch_id)
                        values (@maintain_id,@dispatch_no,@dispatch_name,@org_name,@team_name,@man_hour,@sum_money,@create_time,@create_time);";
                    }
                    else
                    {
                        dicParam.Add("dispatch_id", new ParamObj("dispatch_id", dgvr.Cells["dispatch_id"].Value, SysDbType.VarChar, 40));
                        opName = "更新派工人员信息";
                        obj.sqlString = @"update tb_maintain_dispatch_worker set maintain_id=@maintain_id,dispatch_no=@dispatch_no,dispatch_name=@dispatch_name,org_name=@org_name
                        ,team_name=@team_name,man_hour=@man_hour,sum_money=@sum_money,create_time=@create_time 
        ...
        {           
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string strWhere = " a.maintain_id='" + strRId + "'";
        ...
        private void SetTopbuttonShow()
        {
            ...
            switch (strProjectSattus)
            {
                case "0"://未派工
                    ...
                    base.btnAffirm.Enabled = false;//确认派工
        ...
        private bool IsChooserProject()
        {
            ...
            else if (listField.Count > 1)
            {
                MessageBoxEx.Show(" 一次仅能选择1条派工项目！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void dgvUserData_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(strUName))
            {
                if (e.ColumnIndex == 5)
                {
                    ...
                    //分配金额
                    string strUSumMoney = (Convert.ToDecimal(strMHourMoney) * Convert.ToDecimal(strHourNum)).ToString();
