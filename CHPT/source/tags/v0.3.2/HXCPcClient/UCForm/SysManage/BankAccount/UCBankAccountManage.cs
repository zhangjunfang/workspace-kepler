...
namespace HXCPcClient.UCForm.SysManage.BankAccount
{
    ...
    public partial class UCBankAccountManage : UCBase
    {
        ...
        void BindData()
        {
            ...
            {
                sbWhere.AppendFormat(" and bank_account like '%{0}%'", bankAccount);
            }
            string bankName = txtBankName.Caption.Trim();//银行名称
            if (bankName.Length > 0)
            {
                sbWhere.AppendFormat(" and bank_name like '%{0}%'", bankName);
            }
            ...
            DataTable dt = DBHelper.GetTable("", "tb_bank_account", "*", sbWhere.ToString(), "", "order by create_time");
