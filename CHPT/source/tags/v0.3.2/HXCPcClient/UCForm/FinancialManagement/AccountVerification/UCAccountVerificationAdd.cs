...
namespace HXCPcClient.UCForm.FinancialManagement.AccountVerification
{
    public partial class UCAccountVerificationAdd : UCBase
    {
        ...
        private void UCAccountVerificationAdd_Load(object sender, EventArgs e)
        {
            ...
            {
                Utility.Common.Common.SetControlEnable(this, false);
                //设置页面按钮可见性
                var btnCols = new ObservableCollection<ButtonEx_sms>
                {
...
where a.order_status in ('1','2') and b.order_id=purchase_billing_id
        ...
        void UCAccountVerificationAdd_CancelEvent(object sender, EventArgs e)
        {
            ...
            List<SysSQLString> listSql = new List<SysSQLString>();
            SetDocumentImportStatus("is_lock", DataSources.EnumImportStaus.OPEN, listSql);
            if (listSql.Count > 0)
            {
                if (!DBHelper.BatchExeSQLStringMultiByTrans("解除往来核销前置单据锁定", listSql))
                {
                    return;
                }
            }
        ...
        void BindDocuments(List<DataGridViewRow> listRows)
        {
            ...
            foreach (DataGridViewRow dgvrd in listRows)
            {
                ...
                dgvr.Cells[colOrderDate.Name].Value = dgvrd.Cells["colReceivablesDate"].Value;//单据日期
        ...
        void BindDocuments2(List<DataGridViewRow> listRows)
        {
            ...
            foreach (DataGridViewRow dgvrd in listRows)
            {
                ...
                dgvr.Cells[colOrderDate2.Name].Value = dgvrd.Cells["colReceivablesDate"].Value;//单据日期
        ...
        /// <returns></returns>
        bool CheckDetail(DataGridViewEx dgv, string colName, string colWaitMoneyName)
        {
            bool check = true;
        ...
        bool CheckYuE(DataSources.EnumAuditStatus auditStatus)
        {
                ...
                || enumAccount == DataSources.EnumAccountVerification.YuShouToYuShou || enumAccount == DataSources.EnumAccountVerification.YuFuToYuFu)
            {
                ...
                {
                    yuE = DBOperation.WaiLaiYuE(txtcCustName1.Tag.ToString(), DataSources.EnumOrderType.RECEIVABLE);
                }
                else
                {
                    yuE = DBOperation.WaiLaiYuE(txtcCustName1.Tag.ToString(), DataSources.EnumOrderType.PAYMENT);
                }
                ...
                {
                    MessageBoxEx.Show(string.Format("本次核销合计超过了【{0}】的预收余额！", lblCustName1.Text.TrimEnd('：')));
                    return false;
                }
            }
            return true;
        ...
        #region 生成sql
        //往来核销sql
        void AddAccountVerificationSqlString(List<SysSQLString> list, DataSources.EnumAuditStatus auditStatus)
        {
            SysSQLString sql = new SysSQLString();
            sql.cmdType = CommandType.Text;
            sql.Param = new Dictionary<string, string>();
...
,org_id,handle,operator,create_by,create_time,status,enable_flag,remark)
     VALUES
           (@account_verification_id,@order_num,@order_date,@order_status,@order_type,@advance_balance,@cust_id1,@cust_code1,@cust_name1,@cust_id2,@cust_code2,@cust_name2
,@org_id,@handle,@operator,@create_by,@create_time,@status,@enable_flag,@remark);";
            }
...
,org_id = @org_id,handle = @handle,operator = @operator,update_by = @update_by,update_time = @update_time,remark=@remark
...
,order_num,order_date,settled_money,wait_settled_money,verification_money,is_verification,remark,create_by,create_time)
     VALUES
(@verificationn_documents_id,@account_verification_id,@order_name,@money,@order_id,@order_num,@order_date,@settled_money,@wait_settled_money,@verification_money,@is_verification,@remark
...
,@create_by,@create_time)";
        ...
        /// 设置单据导入状态
        /// </summary>
        void SetDocumentImportStatus(string statusName, DataSources.EnumImportStaus importStaus, List<SysSQLString> listSql)
        {
            ...
            foreach (DataGridViewRow dgvr in dgvDocuments.Rows)
            {
                ids += string.Format("'{0}',", dgvr.Cells[colOrderID.Name].Value);
            }
            if (dgvDocuments2.Visible)
            {
                foreach (DataGridViewRow dgvr in dgvDocuments2.Rows)
                {
                    ids += string.Format("'{0}',", dgvr.Cells[colOrderID2.Name].Value);
                }
        ...
        void Save(DataSources.EnumAuditStatus auditStatus)
        {
            ...
            SetDocumentImportStatus("is_lock", DataSources.EnumImportStaus.OPEN, list);
            if (auditStatus == DataSources.EnumAuditStatus.SUBMIT)
            {
                SetDocumentImportStatus("is_occupy", DataSources.EnumImportStaus.OCCUPY, list);
            }
        ...
        void BindDetail()
        {
            ...
            {
                List<DataRow> listYS = dt.Select("order_name='销售开单'").ToList();
                DataRow[] listYF = dt.Select("order_name='采购开单'");
                BindDocuments(listYS);
                BindDocuments2(listYF);
            }
            else if (enumAccount == DataSources.EnumAccountVerification.YingFuToYingShou)
            {
                DataRow[] listYS = dt.Select("order_name='销售开单'");
                List<DataRow> listYF = dt.Select("order_name='采购开单'").ToList();
        ...
        void BindDocuments(List<DataRow> drs)
        {
            ...
            foreach (DataRow dr in drs)
            {
                ...
                dgvr.Cells[colOrderDate.Name].Value = Common.UtcLongToLocalDateTime(dr["order_date"]);
        ...
        void BindDocuments2(DataRow[] drs)
        {
            ...
            foreach (DataRow dr in drs)
            {
                DataGridViewRow dgvr = dgvDocuments.Rows[dgvDocuments2.Rows.Add()];
        ...
        void SetLable()
        {
            ...
            else
            {
                ...
                pnlDocument1.Height = pnlMan.Height;
            }
        ...
        private void dgvDocuments2_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == colIsVerification2.Index)//点击核销
            {
                ...
                if (gathering)
                {
                    object waitMoney = dgvDocuments.Rows[e.RowIndex].Cells[colWaitMoney2.Name].Value;
                    dgvDocuments.Rows[e.RowIndex].Cells[colVerificationMoney2.Name].Value = waitMoney;//本次核销等于为结算金额
                }
                else
                {
                    dgvDocuments.Rows[e.RowIndex].Cells[colVerificationMoney2.Name].Value = null;
                }
            }
        }
        //点击核销
