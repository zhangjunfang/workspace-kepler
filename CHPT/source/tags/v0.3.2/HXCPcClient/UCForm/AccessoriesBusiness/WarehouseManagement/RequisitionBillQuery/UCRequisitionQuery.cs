       ...
       private  void UCRequisitionQuery_ExportEvent(object sender, EventArgs e)
        ...
        private void UCRequisitionQuery_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //调拨出入库状态
                CommonFuncCall.BindBillInOutStatus(Combinout_status, true, "请选择");
                //单据状态
        ...
        private void GetAllotBillList(string WhereStr)
        {
            try
            {
                ...
                "", " order by create_time desc", winFormAllotQueryPage.PageIndex, winFormAllotQueryPage.PageSize, out RecCount);//获取调拨单表查询记录
        ...
        private string BuildWhereCondation()
        {
            try
            {
                ...
                {
                    Str_Where += " and " + OrderTypeName + " = '" + ComBorder_type_name.SelectedValue.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(ComBcall_out_wh_name.SelectedValue.ToString()))
                {
                    Str_Where += " and " + CallOutWhouse + "= '" + ComBcall_out_wh_name.SelectedValue.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(ComBcall_in_org_name.SelectedValue.ToString()))
                {
                    Str_Where += " and " + CallInOrg + "= '" + ComBcall_in_org_name.SelectedValue.ToString() + "'";
                }

                 if (!string.IsNullOrEmpty(ComBcall_in_wh_name.SelectedValue.ToString()))
                {
                    Str_Where += " and " + CallInWhouse + "='" + ComBcall_in_wh_name.SelectedValue.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(ComBcom_name.SelectedValue.ToString()))
                {
                    Str_Where += " and " + ComName + "='" + ComBcom_name.SelectedValue.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(ComBorg_name.SelectedValue.ToString()))
                {
                    Str_Where += " and " + OrgName + "='" + ComBorg_name.SelectedValue.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(ComBhandle_name.SelectedValue.ToString()))
                {
                    Str_Where += " and " + HandleName + "='" + ComBhandle_name.SelectedValue.ToString() + "'";
                }
                else if (!string.IsNullOrEmpty(Combtrans_way_name.SelectedValue.ToString()))
                {
                    Str_Where += " and " + TranWay + "='" + Combtrans_way_name.SelectedValue.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(Combinout_status.SelectedValue.ToString()))
                {
                    Str_Where += " and " + InoutStatus + "='" + Combinout_status.SelectedValue.ToString() + "'";
                }
