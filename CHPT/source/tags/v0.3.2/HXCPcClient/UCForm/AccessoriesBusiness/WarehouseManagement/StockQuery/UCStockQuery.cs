...
namespace HXCPcClient.UCForm.AccessoriesBusiness.WarehouseManagement.StockQuery
{
    public partial class UCStockQuery : UCBase
    {
        ...
        private string LOutPrice = "lowest_out_price";//最低售价
       ...
       private  void UCStockQuery_ExportEvent(object sender, EventArgs e)
        ...
        private void GetPartList(String WhereStr)
        {
            try
            {
                ...
                DataTable InoutTable = DBHelper.GetTableByPage(StockQueryLogMsg, sbRelationTable.ToString(), sbPartField.ToString(), WhereStr,
        ...
        private void GetYuTongPartList(String WhereStr)
        {
            try
            {
                ...
                DataTable InoutTable = DBHelper.GetTableByPage(YuTongStockQueryLogMsg, sbRelationTable.ToString(), sbPartField.ToString(), WhereStr,
                "", "", winFormStockPager.PageIndex, winFormStockPager.PageSize, out RecCount);//获取库存单配件表查询记录
                winFormStockPager.RecordCount = RecCount;//获取总记录行
        ...
        private string QueryPartWhereCondition()
        {
            try
            {
                string Str_Where = " enable_flag=1 and " + YuTonngApproveStatus + "=" + Submited;
                if (!string.IsNullOrEmpty(CombCompany.Text.ToString()))
                {
                    Str_Where += " and com_name = '" + CombCompany.Text.ToString() + "'";
                }
                 if (!string.IsNullOrEmpty(CombWarehouse.Text.ToString()))
                {
                ...
                {
                    Str_Where += " and IOT.parts_num='" + txtparts_code.Text.ToString() + "'";
                }
