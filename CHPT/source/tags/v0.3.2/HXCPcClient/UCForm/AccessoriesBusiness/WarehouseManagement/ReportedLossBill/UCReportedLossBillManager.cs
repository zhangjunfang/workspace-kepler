    ...
    public partial class UCReportedLossBillManager : UCBase
    {
        ...
        public void GetRepLossBillList(string WhereStr)
        {
            try
            {
                ...
                "", " order by LsBillTb.create_time desc", winFormLossPage.PageIndex, winFormLossPage.PageSize, out RecCount);//获取报损单表查询记录
        ...
        private string BuildWhereCondation()
        {
            try
            {
                ...
                {
                    Str_Where += " and LsBillTb.create_time>=" + Common.LocalDateTimeToUtcLong(dStarttime);
                }
                 if (dateTimeEnd.Value != null)
                {

                    Str_Where += " and LsBillTb.create_time<=" + Common.LocalDateTimeToUtcLong(dEndtime);
                }
