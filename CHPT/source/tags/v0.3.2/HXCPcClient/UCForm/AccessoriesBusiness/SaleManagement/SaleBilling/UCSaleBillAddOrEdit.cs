...
namespace HXCPcClient.UCForm.AccessoriesBusiness.SaleManagement.SaleBilling
{
    public partial class UCSaleBillAddOrEdit : UCBase
    {
        ...
        private void UCSaleBillAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            string[] NotReadOnlyColumnsName = new string[] { "colCheck", "wh_id", "unit_id", "business_count", "discount", "tax_rate", "is_gift", "make_date", "arrival_date", "return_count", "library_count", "remark" };
        ...
        {
            CommonFuncCall.BindAccount(ddlbalance_account, ddlbalance_way.SelectedValue.ToString(), "请选择");
        }
            ...
            if (result == DialogResult.OK)
            {
                ...
                if (List_PartInfo.Count > 0)
                {
                    ...
                    for (int i = 0; i < List_PartInfo.Count; i++)
                    {
                        ...
                        if (!IsExist)
                        {
                            DataTable dt = DBHelper.GetTable("查询销售开单配件表信息", "tb_parts_sale_billing_p", "*", " sale_billing_id='" + billID + "' and parts_code='" + partscode + "' and finish_count<business_count ", "", "");
                            if (dt == null || dt.Rows.Count == 0)
                            {
                                break;
                            }
        ...
        void GetGridViewRowByDrImport(DataGridViewRow dgvr, DataRow dr, string relation_order, string OrderTypeName, int rowIndex, string ImportOrderType)
        {
            try
            {
                ...
                dgvr.Cells["business_count"].Value = int.Parse(dr["business_count"].ToString()) - int.Parse(dr["finish_count"].ToString());//业务数量
            ...
            try
            {
                ...
                if (CheckDataInfo())
                {
                    ...
                    if (HandleTypeName == "提交")
                    {
                        ...
                        {
                            lblorder_num.Text = CommonUtility.GetNewNo(DataSources.EnumProjectType.SaleOpenOrder);//获取销售计划单编号
                        }
        ...
        #region 限制文本框输入内容的事件
        private void txtreceivables_limit_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }

        private void txtcurrent_collect_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }

        private void txtcust_arrears_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }
        ...
        private void txtlast_cumulative_Integral_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }

        private void txtthis_cumulative_Integral_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }
        #endregion
        ...
        DataTable GetSurplusCount()
        {
            ...
            try
            {
                ...
                string TableName = string.Format(@" (
                                                        --销售订单
                                                        select 
                                                        tb_order.order_num,tb_order.parts_code,tb_order.business_count, sum(tb_bill.business_count) relation_count,tb_bill.ImportOrderType
                                                         from
                                                         (
                                                           --销售开单信息
                                                           select b.relation_order,b.parts_code,b.business_count,b.ImportOrderType from tb_parts_sale_billing a 
                                                        ...
                                                        --销售开单（已收货）
                                                        select 
                                                         tb_sal_bill_revice.order_num,tb_sal_bill_revice.parts_code,tb_sal_bill_revice.business_count,sum(tb_sal_bill.business_count) relation_count,tb_sal_bill.ImportOrderType
                                                         from
                                                         (
                                                          --销售开单信息
                                                          select b.relation_order,b.parts_code,b.business_count,b.ImportOrderType from tb_parts_sale_billing a 
