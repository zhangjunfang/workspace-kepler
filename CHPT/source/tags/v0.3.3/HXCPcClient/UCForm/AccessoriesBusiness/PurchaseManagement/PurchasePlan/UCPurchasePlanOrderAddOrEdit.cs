    ...
    public partial class UCPurchasePlanOrderAddOrEdit : UCBase
    {
        ...
        private void UCPurchasePlanOrderAddOrEdit_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string[] NotReadOnlyColumnsName = new string[] { "colCheck", "unit_id", "business_counts", "discount", "remark" };
                ...
                ddtplan_end_time.Value = DateTime.Now;
        ...
        /// <summary>
        /// 在编辑和添加时对主数据和采购计划单配件表中的配件信息进行操作
        /// </summary>
            ...
            try
            {
                ...
                if (gvPurchasePlanList.Rows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow dr in gvPurchasePlanList.Rows)
                    {
                        ...
                        string sql2 = "Insert Into tb_parts_purchase_plan_p(ID,plan_id,parts_code,parts_name,parts_brand,parts_brand_name,drawing_num,unit_id,"
                         ...
                         + "@ID,@plan_id,@parts_code,@parts_name,@parts_brand,@parts_brand_name,@drawing_num,@unit_id,"
        ...
        void GetBuessinessCount(ref int business_counts, ref int finish_counts)
        {
            foreach (DataGridViewRow dr in gvPurchasePlanList.Rows)
            {
                business_counts = business_counts + Convert.ToInt32(dr.Cells["business_counts"].Value == null ? "0" : dr.Cells["business_counts"].Value.ToString() == "" ? "0" : dr.Cells["business_counts"].Value.ToString());
                finish_counts = finish_counts + Convert.ToInt32(dr.Cells["finish_counts"].Value == null ? "0" : dr.Cells["finish_counts"].Value.ToString() == "" ? "0" : dr.Cells["finish_counts"].Value.ToString());
            }
        }
        ...
        void GetBuessinessMoney()
        {
            ...
            txtplan_finish_money.Caption = finish_money.ToString();
        }
