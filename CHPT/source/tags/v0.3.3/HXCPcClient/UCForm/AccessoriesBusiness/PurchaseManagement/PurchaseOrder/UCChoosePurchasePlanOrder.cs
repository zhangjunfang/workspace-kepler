...
{
    public partial class UCChoosePurchasePlanOrder : FormChooser
    {
        #region 变量、类
        public int dgPlanRowIndex = -1;
        public List<PartsInfoClassByPurchasePlan> List_PartInfo = new List<PartsInfoClassByPurchasePlan>();
        public DataTable dt_PurchasePlanOrder = null;
        #endregion

        #region 窗体初始化
        /// <summary> 窗体初始化
        /// </summary>
        public UCChoosePurchasePlanOrder()
        {
            InitializeComponent();
            dgPurchasePlan.ReadOnly = false;
        ...
        /// <summary> 单击采购计划单列表事件,加载配件明细信息
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dgPurchasePlan_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用  
            {
                dgPlanRowIndex = e.RowIndex;
                string plan_id = Convert.ToString(this.dgPurchasePlan.CurrentRow.Cells[0].Value.ToString());
                BinddgAccessoriesDetail(plan_id);
            }
        }
        /// <summary> 选择采购计划单信息，操作下级的配件明细信息的checkbox
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dgPurchasePlan_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用  
            {
                bool ischeck = (bool)((DataGridViewCheckBoxCell)dgPurchasePlan.Rows[dgPlanRowIndex].Cells["colCheck"]).EditedFormattedValue;
        ...
        private void dgAccessoriesDetail_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用   
                {
                    ...
                    ((DataGridViewCheckBoxCell)dgPurchasePlan.Rows[dgPlanRowIndex].Cells["colCheck"]).Value = isChkis_Suspend;
                }
            }
            catch (Exception ex)
            { }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dgPurchasePlan_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.Value == null || e.Value == string.Empty)
            {
                string ColumnName = dgPurchasePlan.Columns[e.ColumnIndex].Name;
            ...
            string fieldNmae = dgPurchasePlan.Columns[e.ColumnIndex].DataPropertyName;
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List_PartInfo = new List<PartsInfoClassByPurchasePlan>();
                PartsInfoClassByPurchasePlan partsInfo = new PartsInfoClassByPurchasePlan();
                string CheckPlanID = string.Empty;
                //1.判断获取采购计划单列表中选中的项
                if (dgPurchasePlan.Rows.Count > 0)
                {
                    foreach (DataGridViewRow dr in dgPurchasePlan.Rows)
                    {
                ...
                if (dgAccessoriesDetail.Rows.Count > 0)
                {
                    foreach (DataGridViewRow dr in dgAccessoriesDetail.Rows)
                    {
                        ...
                        if (isCheck != null && (bool)isCheck)
                        {
                            if (!List_PlanID.Contains(dr.Cells["plan_id"].Value.ToString()))
                            {
                                partsInfo = new PartsInfoClassByPurchasePlan();
                                partsInfo.planID = dr.Cells["plan_id"].Value.ToString();
                ...
                {
                    foreach (string planid in List_PlanID)
                    {
                        DataTable dt_parts_purchase = DBHelper.GetTable("查询采购计划单配件表信息", "tb_parts_purchase_plan_p", "*", " plan_id='" + planid + "' and is_suspend=1 and finish_counts<business_counts ", "", "");
                        if (dt_parts_purchase.Rows.Count > 0)
                        {
                            for (int i = 0; i < dt_parts_purchase.Rows.Count; i++)
                            {
                                partsInfo = new PartsInfoClassByPurchasePlan();
                                partsInfo.planID = dt_parts_purchase.Rows[i]["plan_id"].ToString();
        ...
        string BuildString()
        {
            ////采购计划单可以导入的前提是：已审核、在有效期、未中止、完成数量小于计划数量、导入状态是正常的 五种情况
            ...
            //采购计划单可以导入的前提是：已审核、在有效期、未中止、完成数量小于计划数量、导入状态是正常和部分导入 五种情况
        ...
        /// 绑定采购计划单列表
        /// </summary>
        private void BinddgPurchasePlan()
        {
            try
            {
                DataTable gvPurchasePlanOrder_dt = DBHelper.GetTable("查询采购计划单列表信息", "tb_parts_purchase_plan", "*", BuildString(), "", "");
                dgPurchasePlan.DataSource = gvPurchasePlanOrder_dt;
                dt_PurchasePlanOrder = gvPurchasePlanOrder_dt;
            }
            catch (Exception ex)
            {
                //异常日志
        ...
        {
            dgPurchasePlan.EndEdit();
            dgAccessoriesDetail.EndEdit();
            if (dgPurchasePlan.Rows.Count > 0)
            {
                for (int i = 0; i < dgPurchasePlan.Rows.Count; i++)
                {
                    ((DataGridViewCheckBoxCell)dgPurchasePlan.Rows[i].Cells["colCheck"]).Value = ischeck;
                }
            }
            if (dgAccessoriesDetail.Rows.Count > 0)
            {
        ...
        /// 绑定采购计划单配件列表
        /// </summary>
        private void BinddgAccessoriesDetail(string plan_id)
        {
            if (!string.IsNullOrEmpty(plan_id))
            {
                DataTable dt_parts_purchase = DBHelper.GetTable("查询采购计划单配件表信息", "tb_parts_purchase_plan_p", "*", " plan_id='" + plan_id + "' and is_suspend=1 and finish_counts<business_counts ", "", "");
