...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchaseBilling
{
    public partial class UCPurchaseBillAddOrEdit : UCBase
    {
        ...
        private void UCPurchaseBillAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            string[] NotReadOnlyColumnsName = new string[] { "colCheck", "wh_id", "unit_id", "business_counts", "discount", "tax_rate", "is_gift", "make_date", "arrival_date", "return_bus_count", "storage_count", "remark" };
        ...
        private bool CheckDataInfo()
        {
            ...
            {
                //string wh_id = CommonCtrl.IsNullToString(dgvr.Cells["wh_id"].Value);//配件仓库
                //if (wh_id.Length == 0)
                //{
                //    MessageBoxEx.Show("请选择配件仓库!");
                //    gvPurchaseList.CurrentCell = dgvr.Cells["wh_id"];
                //    return false;
                //}
        ...
        private void GetAccessories(string purchase_billing_id)
        {
            ...
            if (dt_parts_purchase != null && dt_parts_purchase.Rows.Count > 0)
            {
                foreach (DataRow dr in dt_parts_purchase.Rows)
                {
                    ...
                    dgvr.Cells["wh_id"].Value = dr["wh_id"];
                    ...
                    dgvr.Cells["is_gift"].Value = dr["is_gift"];//非赠品1，赠品0
        ...
        private void DealAccessories(List<SysSQLString> listSql, string purchase_billing_id)
        {
            ...
            if (gvPurchaseList.Rows.Count > 0)
            {
                foreach (DataGridViewRow dr in gvPurchaseList.Rows)
                {
                    ...
                    dic.Add("is_gift", dr.Cells["is_gift"].Value == null ? "1" : dr.Cells["is_gift"].Value.ToString());//非赠品1，赠品0
                    ...
                    parts_code,parts_name,model,drawing_num,unit_id,unit_name,parts_brand,parts_brand_name,business_counts,original_price,discount,business_price,
                    ...
                    update_time,ImportOrderType,finish_counts,vm_name,parts_type_id,parts_type_name) values(@id,@purchase_billing_id,@wh_id,@wh_name,
                    @parts_code,@parts_name,@model,@drawing_num,@unit_id,@unit_name,@parts_brand,@parts_brand_name,@business_counts,@original_price,@discount,@business_price,
                    @tax_rate,@tax,@payment,@valorem_together,@assisted_count,@relation_order,@is_gift,@make_date,@arrival_date,@remark,
                    @return_bus_count,@storage_count,@car_factory_code,@create_by,@create_name,@create_time,@update_by,@update_name,@update_time,
                    @ImportOrderType,@finish_counts,@vm_name,@parts_type_id,@parts_type_name);");
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (List_PartInfo.Count > 0)
                    {
                        ...
                        for (int i = 0; i < List_PartInfo.Count; i++)
                        {
                            ...
                            if (!IsExist)
                            {
                                DataTable dt = DBHelper.GetTable("查询采购订单配件表信息", "tb_parts_purchase_order_p", "*", " order_id='" + order_id + "' and parts_code='" + partscode + "' and is_suspend=1 and finish_counts<business_counts ", "", "");
                                if (dt == null || dt.Rows.Count == 0)
                                {
                                    break;
                                }
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (List_PartInfo.Count > 0)
                    {
                        ...
                        for (int i = 0; i < List_PartInfo.Count; i++)
                        {
                            ...
                            if (!IsExist)
                            {
                                DataTable dt = DBHelper.GetTable("查询采购开单配件表信息", "tb_parts_purchase_billing_p", "*", " purchase_billing_id='" + billID + "' and parts_code='" + partscode + "' and finish_counts<business_counts ", "", "");
                                if (dt == null || dt.Rows.Count == 0)
                                {
                                    break;
                                }
        ...
        void GetGridViewRowByDrImport(DataGridViewRow dgvr, DataRow dr, string relation_order, string OrderTypeName, int rowIndex, string ImportOrderType)
        {
            try
            {
                ...
                {
                    dgvr.Cells["wh_id"].Value = dr["wh_id"];//仓库
                }
                ...
                dgvr.Cells["unit_id"].Value = dr["unit_id"];//单位
                dgvr.Cells["parts_brand"].Value = dr["parts_brand"];//品牌
                //dgvr.Cells["business_counts"].Value = dr["business_counts"];//业务数量
                dgvr.Cells["business_counts"].Value = int.Parse(dr["business_counts"].ToString()) - int.Parse(dr["finish_counts"].ToString());//剩余的 业务数量
            ...
            try
            {
                ...
                if (CheckDataInfo())
                {
                    ...
                    if (HandleTypeName == "提交")
                    {
                        ...
                        if (dt_surplus != null && dt_surplus.Rows.Count > 0)
                        {
                            if (gvPurchaseList.Rows.Count > 0)
                            {
                                foreach (DataGridViewRow gvdr in gvPurchaseList.Rows)
                                {
                                    ...
                                    int business_counts = int.Parse(gvdr.Cells["business_counts"].Value == null ? "" : gvdr.Cells["business_counts"].Value.ToString());
                                    ...
                                    else if (status == WindowStatus.Edit)
                                    {
                                        ...
                                        if (dt_old != null && dt_old.Rows.Count > 0)
                                        {
                                            ...
                                            {
                                                int oldcount = int.Parse(dr_old[0]["business_counts"].ToString()) < 0 ? 0 : int.Parse(dr_old[0]["business_counts"].ToString());
                        ...
                        {
                            lblorder_num.Text = CommonUtility.GetNewNo(DataSources.EnumProjectType.PurchaseOpenOrder);//获取采购计划单编号
                        }
        ...
        #region 限制文本框输入内容的事件
        private void txtpayment_limit_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }

        private void txtthis_payment_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }

        private void txtsup_arrears_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }

        private void txtwhythe_discount_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        }

        private void txtbalance_money_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidationRegex.KeyPressFolat(sender, e);
        } 
        #endregion
        ...
        void dc_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //业务单价
                    GetBusiness_priceByOriginal_price(c.Text);
                    //货款
                    GetPayment();
                    //税款
                    GetTax();
                ...
                {
                    //业务单价
                    GetBusiness_priceByDiscount(c.Text);
                    //货款
                    GetPayment();
                    //税款
                    GetTax();
                ...
                {
                    //当修改业务单价时，自动更改折扣
                    string original_price = gvPurchaseList.Rows[rowIndex].Cells["original_price"].Value == null ? "1" : gvPurchaseList.Rows[rowIndex].Cells["original_price"].Value.ToString();
                    ...
                    gvPurchaseList.Rows[rowIndex].Cells["discount"].Value = Convert.ToDecimal(c.Text) / Convert.ToDecimal(original_price) * 100;

                    //货款
                    GetPaymentByBusiness_price(c.Text);
                    //税款
                    GetTax();
                    //税价合计
                    GetValorem_Together();
                ...
                else if (ColumnName == "business_counts")
                {
                    ...
                    //货款
                    GetPaymentByBusiness_counts(c.Text);
                    //税款
                    GetTax();
                    //税价合计
                    GetValorem_Together();
        ...
        /// </summary>
        void GetBusiness_priceByOriginal_price(string original_price)
        {
            //折扣
            string discount = gvPurchaseList.Rows[rowIndex].Cells["discount"].Value == null ? "100" : gvPurchaseList.Rows[rowIndex].Cells["discount"].Value.ToString();
            discount = string.IsNullOrEmpty(discount) ? "100" : discount;
            //业务单价
            gvPurchaseList.Rows[rowIndex].Cells["business_price"].Value = Convert.ToDecimal(original_price) * (Convert.ToDecimal(discount) / 100);
        }
        /// <summary>修改折扣后获取业务单价
        /// </summary>
        /// 
        void GetBusiness_priceByDiscount(string discount)
        {
            //原始单价
            string original_price = gvPurchaseList.Rows[rowIndex].Cells["original_price"].Value == null ? "0" : gvPurchaseList.Rows[rowIndex].Cells["original_price"].Value.ToString();
            original_price = string.IsNullOrEmpty(original_price) ? "0" : original_price;
            //业务单价
            gvPurchaseList.Rows[rowIndex].Cells["business_price"].Value = (Convert.ToDecimal(discount) / 100) * Convert.ToDecimal(original_price);
        }

        /// <summary>修改业务单价后, 获取货款
        /// </summary>
        void GetPaymentByBusiness_price(string business_price)
        {
            //业务数量
            string business_counts = gvPurchaseList.Rows[rowIndex].Cells["business_counts"].Value == null ? "0" : gvPurchaseList.Rows[rowIndex].Cells["business_counts"].Value.ToString();
            business_counts = string.IsNullOrEmpty(business_counts) ? "0" : business_counts;
            //货款
            gvPurchaseList.Rows[rowIndex].Cells["payment"].Value = Convert.ToDecimal(business_price) * Convert.ToDecimal(business_counts);
        }
        /// <summary>修改业务数量后, 获取货款
        /// </summary>
        void GetPaymentByBusiness_counts(string business_counts)
        {
            //业务单价
            string business_price = gvPurchaseList.Rows[rowIndex].Cells["business_price"].Value == null ? "" : gvPurchaseList.Rows[rowIndex].Cells["business_price"].Value.ToString();
            business_price = string.IsNullOrEmpty(business_price) ? "0" : business_price;
            //货款
            gvPurchaseList.Rows[rowIndex].Cells["payment"].Value = Convert.ToDecimal(business_counts) * Convert.ToDecimal(business_price);
        }
        /// <summary> 获取货款
        /// </summary>
        void GetPayment()
        ...
        /// <summary> 修改税率后，获取税款 
        /// </summary>
        void GetTaxByTax_rate(string tax_rate)
        {
            //货款
            string payment = gvPurchaseList.Rows[rowIndex].Cells["payment"].Value == null ? "" : gvPurchaseList.Rows[rowIndex].Cells["payment"].Value.ToString();
            payment = string.IsNullOrEmpty(payment) ? "0" : payment;
            //税款
            gvPurchaseList.Rows[rowIndex].Cells["tax"].Value = Convert.ToDecimal(tax_rate) / 100 * Convert.ToDecimal(payment);
        }
        /// <summary> 获取税款 
        /// </summary>
        void GetTax()
        {
            ...
            payment = string.IsNullOrEmpty(payment) ? "0" : payment;
            //税率
            string tax_rate = gvPurchaseList.Rows[rowIndex].Cells["tax_rate"].Value == null ? "0" : gvPurchaseList.Rows[rowIndex].Cells["tax_rate"].Value.ToString();
            tax_rate = string.IsNullOrEmpty(tax_rate) ? "0" : tax_rate;
            gvPurchaseList.Rows[rowIndex].Cells["tax"].Value = Convert.ToDecimal(payment) * Convert.ToDecimal(tax_rate) / 100;
        }
        /// <summary> 获取税价合计 
        /// </summary>
        void GetValorem_Together()
        {
            //税价合计
            gvPurchaseList.Rows[rowIndex].Cells["valorem_together"].Value = Convert.ToDecimal(gvPurchaseList.Rows[rowIndex].Cells["payment"].Value) + Convert.ToDecimal(gvPurchaseList.Rows[rowIndex].Cells["tax"].Value);
        }
                ...
                if (e.RowIndex > -1 && e.ColumnIndex > -1)//双击表头或列头时不起作用   
                {
                    ...
                    for (int i = 0; i < gvPurchaseList.Rows.Count; i++)
                    {
                        ...
                        if (ischeck != null && (bool)ischeck)
                        {
                            ...
                            gvPurchaseList.Rows[rowIndex].Cells["original_price"].Value = original_price;
                            //业务单价
                            GetBusiness_priceByOriginal_price(original_price);
                            //货款
                            GetPayment();
                            //税款
                            GetTax();
        ...
        DataTable GetSurplusCount()
        {
            ...
            try
            {
                ...
                string TableName = string.Format(@" (
                                                        --采购订单
                                                        select 
                                                        tb_order.order_num,tb_order.parts_code,tb_order.business_counts, sum(tb_bill.business_counts) relation_count,tb_bill.ImportOrderType
                                                         from
                                                         (
                                                           --采购开单信息
                                                           select b.relation_order,b.parts_code,b.business_counts,b.ImportOrderType from tb_parts_purchase_billing a 
                                                        ...
                                                        --采购开单（已收货）
                                                        select 
                                                         tb_pur_bill_revice.order_num,tb_pur_bill_revice.parts_code,tb_pur_bill_revice.business_counts,sum(tb_pur_bill.business_counts) relation_count,tb_pur_bill.ImportOrderType
                                                         from
                                                         (
                                                          --采购开单信息
                                                          select b.relation_order,b.parts_code,b.business_counts,b.ImportOrderType from tb_parts_purchase_billing a 
