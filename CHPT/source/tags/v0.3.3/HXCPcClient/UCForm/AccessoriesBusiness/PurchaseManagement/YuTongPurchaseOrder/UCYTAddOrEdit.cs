...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.YuTongPurchaseOrder
{
    public partial class UCYTAddOrEdit : UCBase
    {
        ...
        private void UCYTAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            string[] NotReadOnlyColumnsName = new string[] { "colCheck", "application_count", "conf_count", "parts_explain", "replaces", "center_library_explain", "cancel_reasons" };
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (List_PartInfo.Count > 0)
                    {
                        for (int i = 0; i < List_PartInfo.Count; i++)
                        {
                            ...
                            if (!IsExist)
                            {
                                DataTable dt = DBHelper.GetTable("查询销售订单配件表信息", "tb_parts_sale_order_p", "*", " sale_order_id='" + sale_order_id + "' and parts_code='" + partscode + "' and is_suspend=1 ", "", "");
                                if (dt == null || dt.Rows.Count == 0)
                                {
                                    break;
                                }
        ...
        private void DealAccessories(List<SysSQLString> listSql, string purchase_order_yt_id)
        {
            ...
            if (gvPurchaseList.Rows.Count > 0)
            {
                foreach (DataGridViewRow dr in gvPurchaseList.Rows)
                {
                    ...
                    string sql2 = string.Format(@"Insert Into tb_parts_purchase_order_p_2(id,purchase_order_yt_id,parts_code,parts_name,car_factory_code,
                    ...
                    update_by,update_name,update_time,ImportType) values(@id,@purchase_order_yt_id,@parts_code,@parts_name,@car_factory_code,
                    @drawing_num,@model,@application_count,@price,@money,@conf_count,@parts_explain,@replaces,
                    @unit_id,@unit_name,@center_library_explain,@cancel_reasons,@relation_order,@create_by,@create_name,@create_time,
        ...
        private void AddPurchaseOrderSqlString(List<SysSQLString> listSql, string purchase_order_yt_id, string HandleType)
        {
            ...
            int conf_count = 0;
            GetBuesinessCount(ref application_count,ref conf_count);
        ...
        void GetGridViewRowByDrImport(DataGridViewRow dgvr, DataRow dr, string relation_order, int rowIndex, string ImportType)
        {
            try
            {
                ...
                dgvr.Cells["unit_id"].Value = dr["unit_id"];//单位
                dgvr.Cells["application_count"].Value = int.Parse(dr["business_counts"].ToString()) - int.Parse(dr["finish_counts"].ToString());//剩余的 业务数量
        ...
        void GetBuesinessCount(ref int application_count, ref int conf_count)
        {
            int Newapplication_count = 0;
            int Newconf_count = 0;
            foreach (DataGridViewRow dr in gvPurchaseList.Rows)
            {
                Newapplication_count = Newapplication_count + Convert.ToInt32(dr.Cells["application_count"].Value == null ? "0" : dr.Cells["application_count"].Value.ToString() == "" ? "0" : dr.Cells["application_count"].Value.ToString());
                Newconf_count = Newconf_count + Convert.ToInt32(dr.Cells["conf_count"].Value == null ? "0" : dr.Cells["conf_count"].Value.ToString() == "" ? "0" : dr.Cells["conf_count"].Value.ToString());
            }
...
//                string FileName = string.Format(@" tb_plan.order_num,tb_plan.parts_code,sum(tb_order.application_count) relation_count,tb_order.ImportType ");
...
//                                                    group by tb_plan.order_num,tb_plan.parts_code,tb_order.ImportType ", files);
