...
namespace HXCPcClient.UCForm.AccessoriesBusiness.PurchaseManagement.PurchaseOrder
{
    public partial class UCPurchaseOrderAddOrEdit : UCBase
    {
        ...
        private void UCPurchaseOrderAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            string[] NotReadOnlyColumnsName = new string[] { "colCheck", "unit_id", "business_counts", "discount", "tax_rate", "is_gift", "arrival_date", "remark" };
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (List_PartInfo.Count > 0)
                    {
                        for (int i = 0; i < List_PartInfo.Count; i++)
                        {
                            ...
                            if (!IsExist)
                            {
                                DataTable dt = DBHelper.GetTable("查询采购计划单配件表信息", "tb_parts_purchase_plan_p", "*", " plan_id='" + planid + "' and parts_code='" + partscode + "' and is_suspend=1 and finish_counts<business_counts ", "", "");
        ...
        private void DealAccessories(List<SysSQLString> listSql, string order_id)
        {
            ...
            if (gvPurchaseList.Rows.Count > 0)
            {
                foreach (DataGridViewRow dr in gvPurchaseList.Rows)
                {
                    ...
                    dic.Add("arrival_date", Common.LocalDateTimeToUtcLong(Convert.ToDateTime(dr.Cells["arrival_date"].Value == null ? DateTime.Now.ToString() : dr.Cells["arrival_date"].Value.ToString())).ToString());
                    ...
                    string sql2 = string.Format(@"Insert Into tb_parts_purchase_order_p(ID,order_id,parts_code,parts_name,drawing_num,unit_id,unit_name,
                    ...
                    update_by,update_name,update_time,finish_counts,vm_name,parts_type_id,parts_type_name) values(@ID,@order_id,@parts_code,@parts_name,@drawing_num,@unit_id,@unit_name,
                    @parts_brand,@parts_brand_name,@business_counts,@original_price,@discount,@business_price,@tax_rate,@tax,@payment,@valorem_together,
                    @auxiliary_count,@relation_order,@is_gift,@is_suspend,@arrival_date,@remark,@ImportOrderType,@parts_barcode,@car_factory_code,@create_by,@create_name,@create_time,
        ...
        void GetGridViewRowByDrImport(DataGridViewRow dgvr, DataRow dr, string relation_order,int rowIndex,  string ImportOrderType)
        {
            try
            {
                ...
                if (ImportOrderType == "采购计划单")
                    ...
                    dgvr.Cells["business_counts"].Value = int.Parse(dr["business_count"].ToString()) - int.Parse(dr["finish_count"].ToString());//剩余的 业务数量
        ...
        void GetBuesinessMoney(ref int business_counts,ref decimal payment,ref decimal tax,ref decimal valorem_together)
        {
            int Newbusiness_counts = 0;
            decimal Newpayment = 0;
            decimal Newtax = 0;
            decimal Newvalorem_together = 0;
            foreach (DataGridViewRow dr in gvPurchaseList.Rows)
            {
                Newbusiness_counts = Newbusiness_counts + Convert.ToInt32(dr.Cells["business_counts"].Value == null ? "0" : dr.Cells["business_counts"].Value.ToString() == "" ? "0" : dr.Cells["business_counts"].Value.ToString());
...
//                string FileName = string.Format(@" tb_plan.order_num,tb_plan.parts_code,sum(tb_order.business_counts) relation_count,tb_order.ImportOrderType ");
//                string TableName = string.Format(@" (
...
//                                                    group by tb_plan.order_num,tb_plan.parts_code,tb_order.ImportOrderType ", files);
//                return dt = DBHelper.GetTable("查询采购订单导入采购计划单时,获取计划单中配件已完成的数量", TableName, FileName, "", "", "");
        ...
        DataTable GetBusinessCount(string order_id)
        {
            ...
            try
            {
//                string FileName = string.Format(@" a.plan_id,b.order_num,a.parts_code,a.business_counts ");
//                string TableName = string.Format(@" (
...
//		                                                  )
//	                                                   )
//                                                     ) a left join tb_parts_purchase_plan b on a.plan_id=b.plan_id ", order_id);
//                return dt = DBHelper.GetTable("查询采购订单导入采购计划单时,获取计划单中配件已完成的数量", TableName, FileName, "", "", "");
                string FileName = string.Format(@" * ");
                string TableName = string.Format(@" (
