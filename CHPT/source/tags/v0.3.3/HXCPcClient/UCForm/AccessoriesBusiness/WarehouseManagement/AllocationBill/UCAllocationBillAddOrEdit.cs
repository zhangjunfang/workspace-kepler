    ...
    public partial class UCAllocationBillAddOrEdit : UCBase
    {
        ...
        private void gvPartsMsgList_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    if (!IsNumber(gvPartsMsgList.Rows[i].Cells["counts"].Value.ToString()))
                    {
                    ...
                    else
                    {
                        ...
                        else if (gvPartsMsgList.Rows[i].Cells["counts"].Value==null)
                        {
        ...
        private static bool IsNumber(string strValue)
        {
            ...
            return RegNum.IsMatch(strValue.Trim());
        }
        ...
        private void ImportBillingMethod(string BillTable, string PartTable, string BillingPrimID, string QueryLogMsg, string ModifyLogMsg,string BillType)
        {
            try
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    BillingIDLst = UCPB.BillPrimIDLst;//获取导入开单主键ID
                    foreach (string item in UCPB.BillPrimIDLst)
                    {
                    ...
                    if (UCPB.PartHTable.Count != 0)
                    {
                        for (int k = 0; k < gvPartsMsgList.Rows.Count; k++)
                        {
                            foreach (DictionaryEntry DicEty in UCPB.PartHTable)
                            {
                                ...
                                {
                                    dt_Parts = DBHelper.GetTable(QueryLogMsg, PartTable, sb_Fields.ToString(), " parts_code='" + PartNumber + "'", "", "");
        ...
        private void ImportStockBillMethod(string BillTable, string PartTable, string BillingPrimID, string QueryLogMsg, string ModifyLogMsg, string BillType)
        {
            try
            {
                ...
                if (Combbilling_type_name.SelectedValue.ToString() == AllotBill)
                {//调拨单导入
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    string BillPrimIDValue = string.Empty;//要查询的开单主键ID条件
                    string PartNumber = string.Empty;//查询的配件编号条件
                    string BillTypeName = Combbilling_type_name.SelectedValue.ToString();//开单类型名
        ...
        private void GetGridViewRowByDrImport(DataTable partsTable, string BillID,string BillingType)
        {
            try
            {
                 int SerialId = 0;//序号初始化
                if(gvPartsMsgList.Rows.Count==0)SerialId = 1;
                else
                {
                    SerialId = gvPartsMsgList.Rows.Count + 1;//记录下次添加记录的初始序号
                }
                ...
                for (int i = 0; i < partsTable.Rows.Count; i++)
                {
                    ...
                    if (BillingType == PurchaseBilling || BillingType == SaleBilling)
                    {
                        dgRow.Cells["counts"].Value = partsTable.Rows[i]["business_counts"].ToString();//销售或采购
                    }
                    else if (BillingType == MaterialRequisition )
                    {
                        dgRow.Cells["counts"].Value = partsTable.Rows[i]["received_num"].ToString();//领料
                    }
                    else if ( BillingType == RefundBill)
                    {
                        dgRow.Cells["counts"].Value = partsTable.Rows[i]["retreat_num"].ToString();//退料
                    }
                    else if (BillingType == AllotBill || BillingType == OtherReceiveBill  ||  BillingType ==OtherSendBill)
                    {
                        dgRow.Cells["counts"].Value = partsTable.Rows[i]["count"].ToString();//调拨
                    }
            ...
            try
            {
                ...
                if (gvPartsMsgList.Rows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow DgVRow in gvPartsMsgList.Rows)
                    {
                        ...
                        DicPartParam.Add("num", DgVRow.Cells["ID"].Value.ToString() == string.Empty ? "" : DgVRow.Cells["ID"].Value.ToString());
        ...
        private void gvPartsMsgList_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == gvPartsMsgList.CurrentRow.Cells["counts"].ColumnIndex)
                {
                    gvPartsMsgList.CurrentRow.Cells["counts"].ReadOnly = false;//单击数量列变为可编辑状态
                }
                 if (e.ColumnIndex == gvPartsMsgList.CurrentRow.Cells["remarks"].ColumnIndex)
                {
                    gvPartsMsgList.CurrentRow.Cells["remarks"].ReadOnly = false;//单击备注列变为可编辑状态 
                }
        ...
        private void PartDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBoxEx.Show("您没有选中要删除的行项！", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    return;
                }
                else
                {
