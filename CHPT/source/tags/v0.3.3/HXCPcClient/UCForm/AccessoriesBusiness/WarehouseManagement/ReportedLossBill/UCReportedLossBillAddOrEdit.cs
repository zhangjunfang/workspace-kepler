    ...
    public partial class UCReportedLossBillAddOrEdit : UCBase
    {
        ...
        private void gvPartsMsgList_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                    ...
                    for (int i = 0; i < gvPartsMsgList.Rows.Count; i++)
                    {
                        ...
                        {
                            int LossCount = Convert.ToInt32(gvPartsMsgList.Rows[i].Cells["counts"].Value.ToString());//报损数量
        ...
        private void BillImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    sb_Fields.AppendFormat("{0},{1},{2},{3},{4},{5},{6},{7}",
                    SaleBillingId, PartCode, PartsName, PartsSpec, DrawNum, UnitName, PartsBrand, PartsFacCode,
                    ...
                    if (UCPB.PartHTable.Count != 0)
                    {
                        for (int k = 0; k < gvPartsMsgList.Rows.Count; k++)
                        {
                            foreach (DictionaryEntry DicEty in UCPB.PartHTable)
                            {
                                ...
                                {
                                    dt_Parts = DBHelper.GetTable(SalepartsLogMsg, SalePartTable, sb_Fields.ToString(), " parts_code='" + PartNumber + "'", "", "");
                                    GetGridViewRowByDrImport(dt_Parts, SaleBillingId);//导入配件信息列表
                                }
                            }
                        }
                        SetBillingStatus(SaleBillTable, SaleBillingId, BillPrimIDValue, SaleBillModifyLog);//修改开单状态为已占用
            ...
            try
            {
                ...
                if (DgResult == DialogResult.Yes)
                {
                    for (int i = 0; i < gvPartsMsgList.SelectedRows.Count; i++)
                    {
                        ...
                        gvPartsMsgList.Rows.RemoveAt(i);//删除选中行项
                ...
                if (CheckListInfo())
                {
                    ...
                    else if (status == WindowStatus.Edit)
                    {
                        ...
                        if (BillIDlist != null)
                        {
                            ...
                            {
                                SetBillingStatus(SaleBillTable, SaleBillingId, BillIDlist[j].ToString(), SaleBillModifyLog);//修改开单状态为已占用
                            }
                        }
                    ...
                    if (DBHelper.BatchExeSQLStringMultiByTrans(opName, listSql))
                    {
                        ...
                        UCLossBM.GetRepLossBillList(QueryWhere);//更新单据列表
        ...
        private void gvPartsMsgList_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                for (int i = 0; i < gvPartsMsgList.Rows.Count; i++)
                {
                    if (e.ColumnIndex == gvPartsMsgList.Rows[i].Cells["counts"].ColumnIndex)
                    {
                        gvPartsMsgList.Rows[i].Cells["counts"].ReadOnly = false;//单击数量列变为可编辑状态
                    }
