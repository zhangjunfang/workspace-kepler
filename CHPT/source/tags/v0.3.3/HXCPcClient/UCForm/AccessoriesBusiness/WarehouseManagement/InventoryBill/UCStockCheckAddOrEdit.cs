    ...
    public partial class UCStockCheckAddOrEdit : UCBase
    {
        ...
        private void gvPartsMsgList_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    if (!IsNumber(gvPartsMsgList.Rows[i].Cells["FmOffCount"].Value.ToString()))
                    {
                    ...
                    else
                    {
                        ...
                        if (FirmCount <= 0)
                        {
                            MessageBoxEx.Show("您输入的实盘数量不能小于等于零！");
                            return;
                        }
                        else if (gvPartsMsgList.Rows[i].Cells["FmOffCount"].Value==null)
                        {
                            MessageBoxEx.Show("您输入的实盘数量不能为空！");
                            return;
                        }
                        else
                        {
        ...
        private void gvPartsMsgList_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                for (int i = 0; i < gvPartsMsgList.Rows.Count;i++ )
                {
                    if(e.ColumnIndex==gvPartsMsgList.Rows[i].Cells["FmOffCount"].ColumnIndex)
                    {
                        gvPartsMsgList.Rows[i].Cells["FmOffCount"].ReadOnly = false;//单击数量列变为可编辑状态
                    }
