    ...
    public partial class UCRequisitionAddOrEdit : UCBase
    {
        ...
        private void UCRequisitionAddOrEdit_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                CommonFuncCall.BindBillInOutStatus(Combinout_status, true, "请选择");//调拨单的出入库状态
        ...
        private void gvPartsMsgList_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                for (int i = 0; i < gvPartsMsgList.Rows.Count; i++)
                {
                    ...
                    {
                        MessageBoxEx.Show("您输入的调入数量或单价不能为空！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
            ...
            try
            {
                if (gvPartsMsgList.Rows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow DgVRow in gvPartsMsgList.Rows)
                    {
                        ...
                        DicPartParam.Add("make_date", DgVRow.Cells["makedate"].Value.ToString() == string.Empty ? "" : DgVRow.Cells["makedate"].Value.ToString());
                        DicPartParam.Add("validity_date", DgVRow.Cells["validitydate"].Value.ToString() == string.Empty ? "" : DgVRow.Cells["validitydate"].Value.ToString());
        ...
        private void gvPartsMsgList_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                for (int i = 0; i < gvPartsMsgList.Rows.Count; i++)
                {
                    if (e.ColumnIndex == gvPartsMsgList.Rows[i].Cells["partcounts"].ColumnIndex)
                    {
                        gvPartsMsgList.Rows[i].Cells["partcounts"].ReadOnly = false;//单击数量列变为可编辑状态
                    }
                    else if (e.ColumnIndex == gvPartsMsgList.Rows[i].Cells["callinprice"].ColumnIndex)
                    {
                        gvPartsMsgList.Rows[i].Cells["callinprice"].ReadOnly = false;//单击调入单价列变为可编辑状态 
                    }
