       ...
       private  void UCModifyPriceManager_ExportEvent(object sender, EventArgs e)
        ...
        public void GetModifyPriceBillList(string WhereStr)
        {
            try
            {
                ...
                sbField.AppendFormat("MdyBillTb.{0},{1},{2},MdyBillTb.{3},{4},{5},PartMoney,{6},{7},{8},MdyBillTb.{8},{9}", ModifyPriceID,
                ...
                "", " order by MdyBillTb.create_time desc", winFormModifyPage.PageIndex, winFormModifyPage.PageSize, out RecCount);//获取出入库单表查询记录
                ...
                for (int i = 0; i < ModifyPriceTable.Rows.Count; i++)
                {
                    ...
                    if (Convert.ToDecimal(ModifyPriceTable.Rows[i][AmountMoney].ToString()) < decimal.Zero)
                    {
