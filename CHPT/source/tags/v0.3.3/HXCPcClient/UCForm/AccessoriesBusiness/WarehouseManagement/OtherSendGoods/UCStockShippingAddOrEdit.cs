            ...
            try
            {
                ...
                if (CheckListInfo())
                {
                    ...
                    if (DBHelper.BatchExeSQLStringMultiByTrans(opName, listSql))
                    {
                        ...
                        UCShippingBM.GetShippingBillList(QueryWhere);//更新单据列表
        ...
        private void gvPartsMsgList_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                for (int i = 0; i < gvPartsMsgList.Rows.Count; i++)
                {
                    ...
                    else if (e.ColumnIndex == gvPartsMsgList.Rows[i].Cells["Unitprice"].ColumnIndex)
                    {
                        gvPartsMsgList.Rows[i].Cells["Unitprice"].ReadOnly = false;//单击单价列变为可编辑状态 
                    }
