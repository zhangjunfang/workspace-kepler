...
namespace HXCPcClient.UCForm.AccessoriesBusiness.SaleManagement.SaleOrder
{
    public partial class UCChooseSalePlanOrder : FormChooser
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (List_PlanID.Count > 0)
                {
                    ...
                    {
                        DataTable dt_parts_sale = DBHelper.GetTable("查询销售计划单配件表信息", "tb_parts_sale_plan_p", "*", " sale_plan_id='" + sale_plan_id + "' and is_suspend=1 and finish_count<business_count ", "", "");
        ...
        private void BinddgAccessoriesDetail(string sale_plan_id)
        {
            ...
            {
                DataTable dt_parts_purchase = DBHelper.GetTable("查询销售计划单配件表信息", "tb_parts_sale_plan_p", "*", " sale_plan_id='" + sale_plan_id + "' and is_suspend=1 and finish_count<business_count ", "", "");
