...
namespace HXCPcClient.UCForm.AccessoriesBusiness.SaleManagement.SaleOrder
{
    public partial class UCSaleOrderAddOrEdit : UCBase
    {
        ...
        private void UCSaleOrderAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            string[] NotReadOnlyColumnsName = new string[] { "colCheck", "unit_id", "business_count", "discount", "tax_rate", "is_gift", "delivery_time", "remark" };
            ...
            if (result == DialogResult.OK)
            {
                ...
                if (List_PartInfo.Count > 0)
                {
                    for (int i = 0; i < List_PartInfo.Count; i++)
                    {
                        ...
                        if (!IsExist)
                        {
                            DataTable dt = DBHelper.GetTable("查询销售计划单配件表信息", "tb_parts_sale_plan_p", "*", " sale_plan_id='" + sale_plan_id + "' and parts_code='" + partscode + "' and is_suspend=1 and finish_count<business_count ", "", "");
        ...
        private void txtcust_code1_ChooserClick(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(cust_id))
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    txtfax.Caption = dt.Rows[0]["cust_tel"].ToString();
                    txtcontacts_tel.Caption = dt.Rows[0]["cust_fax"].ToString();
                }
        ...
        private void DealAccessories(List<SysSQLString> listSql, string sale_order_id)
        {
            ...
            if (gvPurchaseList.Rows.Count > 0)
            {
                foreach (DataGridViewRow dr in gvPurchaseList.Rows)
                {
                    ...
                    dic.Add("delivery_time", Common.LocalDateTimeToUtcLong(Convert.ToDateTime(dr.Cells["delivery_time"].Value == null ? DateTime.Now.ToString() : dr.Cells["delivery_time"].Value.ToString())).ToString());
                    dic.Add("remark", dr.Cells["remark"].Value == null ? "" : dr.Cells["remark"].Value.ToString());
                    //dic.Add("barcode", dr.Cells["barcode"].Value == null ? "" : dr.Cells["barcode"].Value.ToString());
                    //dic.Add("parts_factory_code", dr.Cells["parts_factory_code"].Value == null ? "" : dr.Cells["parts_factory_code"].Value.ToString());
                    ...
                    string sql2 = string.Format(@"Insert Into tb_parts_sale_order_p(sale_order_parts_id,sale_order_id,parts_code,parts_name,drawing_num,unit_id,unit_name,
                    parts_brand,parts_brand_name,business_count,original_price,discount,business_price,tax_rate,tax,payment,valorem_together,
                    auxiliary_count,relation_order,is_gift,is_suspend,delivery_time,remark,create_by,create_name,create_time,
                    update_by,update_name,update_time,finish_count,vm_name,parts_type_id,parts_type_name) values(@sale_order_parts_id,@sale_order_id,@parts_code,
        ...
        void GetGridViewRowByDrImport(DataGridViewRow dgvr, DataRow dr, string relation_order, int rowIndex)
        {
            try
            {
                ...
                dgvr.Cells["business_count"].Value = int.Parse(dr["business_count"].ToString()) - int.Parse(dr["finish_count"].ToString());//剩余的 业务数量
        ...
        {
            //业务数量
            string business_count = gvPurchaseList.Rows[rowIndex].Cells["business_count"].Value == null ? "0" : gvPurchaseList.Rows[rowIndex].Cells["business_count"].Value.ToString();
            business_count = string.IsNullOrEmpty(business_count) ? "0" : business_count;
            //货款
            gvPurchaseList.Rows[rowIndex].Cells["payment"].Value = Convert.ToDecimal(business_price) * Convert.ToDecimal(business_count);
        }
        /// <summary>修改业务数量后, 获取货款
        /// </summary>
        void GetPaymentByBusiness_counts(string business_count)
        {
            //业务单价
            ...
            //货款
            gvPurchaseList.Rows[rowIndex].Cells["payment"].Value = Convert.ToDecimal(business_count) * Convert.ToDecimal(business_price);
        }
        /// <summary> 获取货款
        /// </summary>
        void GetPayment()
        ...
        void GetBuesinessMoney(ref int business_counts, ref decimal payment, ref decimal tax, ref decimal valorem_together)
        {
            ...
            foreach (DataGridViewRow dr in gvPurchaseList.Rows)
            {
                Newbusiness_counts = Newbusiness_counts + Convert.ToInt32(dr.Cells["business_count"].Value == null ? "0" : dr.Cells["business_count"].Value.ToString() == "" ? "0" : dr.Cells["business_count"].Value.ToString());
