...
namespace HXCPcClient.UCForm.SysManage.RepairMeal
{
    ...
    public partial class UCRepairMealAddOrEdit : UCBase
    {
            ...
            foreach (DataGridViewRow dgvr in dgvproject.Rows)
            {
                ...
                if (strPname.Length > 0)
                {
                    ...
                    if (strPID.Length == 0)
                    {
                        ...
                        ,repair_type,whours_type,whours_counts,whours_price,tax_amount,remark,repair_package_set_project_id,create_by,create_time)
                    ...
                    else
                    {
                        ...
                        ,update_by=@update_by,update_time=@update_time where repair_package_set_project_id=@repair_package_set_project_id";
                    }
                    obj.Param = dicParam;
                    listSql.Add(obj);
                }
            ...
            foreach (DataGridViewRow dgvr in dgvMaterials.Rows)
            {
                ...
                if (strPname.Length > 0)
                {
                    ...
                    if (strPID.Length == 0)
                    {
                        ...
                        ,number,original_price,preferential_price,remarks,repair_package_set_materials_id,create_by,create_time)
                        values (@repair_package_set_id,@parts_code,@parts_name,@unit,@number,@original_price,@preferential_price
                        ,@remarks,@repair_package_set_materials_id,@create_by,@create_time);";
                    }
                    else
                    {
                        ...
                        ,number=@number,original_price=@original_price,preferential_price=@preferential_price,remarks=@remarks,update_by=@update_by,update_time=@update_time 
        ...
        private void BindPData(string strOrderId)
        {
            ...
            //维修项目数据                
            DataTable dpt = DBHelper.GetTable("维修项目数据", "sys_b_set_repair_package_set_project", "*", string.Format(" repair_package_set_id='{0}'", strOrderId), "", "");
            if (dpt.Rows.Count > 0)
            {
        ...
        private void BindMData(string strOrderId)
        {
            ...
            //维修用料数据
            DataTable dmt = DBHelper.GetTable("维修用料数据", "sys_b_set_repair_package_set_materials", "*", string.Format(" repair_package_set_id='{0}'", strOrderId), "", "");
            if (dmt.Rows.Count > 0)
            {
