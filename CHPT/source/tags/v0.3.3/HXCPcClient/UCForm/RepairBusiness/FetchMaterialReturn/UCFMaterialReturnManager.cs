...
namespace HXCPcClient.UCForm.RepairBusiness.FetchMaterialReturn
{
    ...
    public partial class UCFMaterialReturnManager : UCBase
    {
        ...
        private void SubmitAndVerify(string strMessage, DataSources.EnumAuditStatus status)
        {
            ...
            foreach (DataGridViewRow dr in dgvRData.Rows)
            {
                ...
                if (isCheck != null && (bool)isCheck)
                {
                    ...
                    dicParam.Add("refund_no", new ParamObj("refund_no", CommonUtility.GetNewNo(SYSModel.DataSources.EnumProjectType.ReturnMaterialNo), SysDbType.VarChar, 40));//单据编号                   
        ...
        private void ViewData(string strType = "0")
        {
            ...
            UCFMaterialReturnView view = new UCFMaterialReturnView(strReId, strDReId);
            view.uc = this;
        ...
        public void BindPageData()
        {
            try
            {
                ...
                {
                    strWhere += string.Format(" and a.refund_opid = '{0}'", cobWarehouse.SelectedValue.ToString());
                }
        ...
        private void txtFetchOpid_ChooserClick(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                txtFetchOpid.Text = frmCInfo.strCustomerName;
                txtFetchOpid.Tag = frmCInfo.strCustomerId;
            }
        }
        #endregion
