...
namespace HXCPcClient.CommonClass
{
    public class DBHelper
    {
        ...
        #region JsonOperate 契约  add by kord -- 2014.12.17
        /// <summary>
        /// 
        /// </summary>
        /// <returns></returns>
        public static RespFunStruct JsonOperate(String str)
        {
            try
            {
                GlobalStaticObj.GlobalLogService.WriteLog(1, HXCCommon.DotNetEncrypt.DESEncrypt.Decrypt(str));
                if (!WCFClientProxy.TestDataProxy())
                {
                ...
                GlobalStaticObj.GlobalLogService.WriteLog(resultStr.ToString());
                return resultStr;
            }
            catch (Exception ex)
            {
            ...
            string orderBy)
        {
            try
            {
                ...
                if (respO.IsSuccess == "0")
                {
                    GlobalStaticObj.GlobalLogService.WriteLog(respO.Msg);
                    return null;
                }
        ...
        public static DataTable GetTableByPage(DataTable _dt, int pageIndex, int pageSize)
        {
            ...
            int startIndex = 0;
            startIndex = _dt.Rows.Count - pageIndex * pageSize;
            if (startIndex < 0)
            {
                startIndex = _dt.Rows.Count - (pageIndex - 1) * pageSize;
            }
            startIndex--;
        ...
        public static RespFunStruct WebServHandlerByFun(string opName, SYSModel.EnumWebServFunName enumFuncName, object webServFuncObj)
        {
            ...
            {
                Utility.Log.Log.writeLineToLog(te, "接口调用");
                resp.IsSuccess = "0";
                resp.Msg = "操作超时";
            }
            catch (Exception e)
            {
                Utility.Log.Log.writeLineToLog(e, "接口调用");
                resp.IsSuccess = "0";
