                ...
                if (project == DataSources.EnumReminderType.YYDZ.ToString())
                {
                    if (second > 0)
                    {
                        sb.Append(string.Format("select * from tb_maintain_reservation where document_status='{0}' and enable_flag='{1}' and maintain_time>'{2}' and maintain_time<'{3}' order by update_time desc"
                         , DataSources.EnumAuditStatus.AUDIT.ToString("d"), DataSources.EnumEnableFlag.USING.ToString("d"), Common.LocalDateTimeToUtcLong(DateTime.Now), Common.LocalDateTimeToUtcLong(DateTime.Now.AddSeconds(second))));
                    }
                    else
                    {
                        sb.Append(string.Format("select * from tb_maintain_reservation where document_status='{0}' and enable_flag='{1}' and maintain_time>'{2}' order by update_time desc"
                         , DataSources.EnumAuditStatus.AUDIT.ToString("d"), DataSources.EnumEnableFlag.USING.ToString("d"), Common.LocalDateTimeToUtcLong(DateTime.Now)));
                    }                    
                    sb.Append(";");
                }
                else if (project == DataSources.EnumReminderType.DPG.ToString())
                {
                    sb.Append(string.Format("select * from tb_maintain_info where info_status='{0}' and enable_flag='{1}' order by update_time desc"
