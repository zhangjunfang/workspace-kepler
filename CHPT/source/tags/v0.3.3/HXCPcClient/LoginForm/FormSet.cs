...
namespace HXCPcClient.LoginForms
{
    public partial class FormSet : Form
    {
        ...
        private void panelYes_Click(object sender, EventArgs e)
        {
            ...
            {
                Utility.Common.Validator.SetError(this.errorProvider, this.tbServerIp, "服务器Ip不能为空!");
                return;
            }
            if (string.IsNullOrEmpty(this.tbServerPort.Caption.Trim()))
            {
                Utility.Common.Validator.SetError(this.errorProvider, this.tbServerPort, "服务器端口不能为空!");
                return;
            }
            
            if (string.IsNullOrEmpty(this.tbFileServerIp.Caption.Trim()))
            {
                Utility.Common.Validator.SetError(this.errorProvider, this.tbFileServerIp, "服务器Ip不能为空!");
                return;
            }
            if (string.IsNullOrEmpty(this.tbFilePort.Caption.Trim()))
            {
                Utility.Common.Validator.SetError(this.errorProvider, this.tbFilePort, "服务器端口不能为空!");
                return;
            }
            ...
            else
            {
                if (dataFlag)
                {
                    FormMessgeBox.ShowMsg(this, "数据通讯服务连接测试成功！", this.panelTop.BackColor);
                    if (this.RefreshDataStart != null)
                    {
                        this.RefreshDataStart();
                    }
                ...
                {
                    FormMessgeBox.ShowMsg(this, "文件通讯服务连接测试成功！", this.panelTop.BackColor); 
                }
                else
                {
