...
operator_name,create_by,create_name,create_time,enable_flag,is_lock,is_occupy,ration_send_code)
select @purchase_billing_id,order_num,order_date,@order_status,order_status_name,'1','采购收货',
null sup_id,null sup_code,null sup_name,null receipt_type,null receipt_type_name,null receipt_no,
consignee contacts,consignee_tel contacts_tel,null fax,req_delivery trans_way,req_delivery_name trans_way_name,
null payment_date,null payment_limit,null this_payment,null sup_arrears,null balance_unit,null balance_way,
...
operator_name,create_by,create_name,{0},enable_flag,0 is_lock,0 is_occupy,@ration_send_code
 ...
 where crm_bill_id=@crm_bill_id", nowDate);
            ...
            foreach (SUPartPutStore.Detail detail in details)
            {
                ...
                detailSql.sqlString = string.Format(@"insert into tb_parts_purchase_billing_p(id,purchase_billing_id,num,wh_code,wh_name,parts_code,
parts_name,model,parts_brand,drawing_num,unit_id,business_counts,original_price,discount,business_price,
tax_rate,tax,payment,valorem_together,is_gift,relation_order,return_bus_count,storage_count,remark,
assisted_count,create_by,create_time,unit_name,create_name,arrival_date,make_date)
select @id,@purchase_billing_id,num,'' wh_code,'' wh_name,parts_code,parts_name,model,'' parts_brand,
drawing_num,@unit,@num business_counts,price original_price,100 discount,price business_price,
null tax_rate,null tax,@num*price payment,a.conf_count*price+0 valorem_together,'0' is_gift,
b.order_num relation_order,null retum_bus_count,null storage_count,null remark,0 assisted_count,
a.create_by,{0},@unit unit_name,a.create_name,0 arrival_date,0 make_date
...
where b.crm_bill_id=@crm_bill_id and a.car_factory_code=@parts_code", nowDate);
        ...
        public static bool LoadPartPurchaseStauts(string crm_bill_id)
        {
            ...
            //如果没有接入码或者sap代码,则不调用接口
            if (string.IsNullOrEmpty(GlobalStaticObj_Server.Instance.ClientID) ||
                string.IsNullOrEmpty(GlobalStaticObj_YT.SAPCode))
            {
                return true;
            }
            ...
            try
            {
                ...
                sqlYT.Param.Add("order_status_yt", order_status);
                sqlYT.Param.Add("order_status_yt_name", WebServUtil.GetYTDicName(order_status));
                sqlYT.Param.Add("approval_record", result.approval_record);
        ...
        public static bool UpLoadPartPurchase(string jsonStr)
        {
            ...
            if (partsPurchase.create_time != null)
            {
                DateTime dtApply = Common.UtcLongToLocalDateTime(Convert.ToInt64(partsPurchase.create_time));
                purchase.apply_date_time = dtApply.ToString("yyyy-MM-dd HH:mm:ss");//申请时间
            }
            if (partsPurchase.req_delivery_time != null)
            {
        ...
        public static bool QuerySettleAccounts(string settlementNo)
        {
            ...
            else
            {
                ...
                strSql.Append("st_id,settlement_no,station_settlement_no,service_station_code,service_station_name,bill_sum_cost,sum_cost,oldpart_sum_cost,other_sum_cost,settlement_date,settlement_cycle_start,settlement_cycle_end,info_status,create_time,update_time,enable_flag,create_by,update_by,create_name,update_name");
                strSql.Append(") values (");
                strSql.Append("@st_id,@settlement_no,@station_settlement_no,@service_station_code,@service_station_name,@bill_sum_cost,@sum_cost,@oldpart_sum_cost,@other_sum_cost,@settlement_date,@settlement_cycle_start,@settlement_cycle_end,@info_status,@create_time,@update_time,@enable_flag,@create_by,@update_by,@create_name,@update_name) ");
                #endregion
