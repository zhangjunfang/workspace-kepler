...
public class ThvehicleAlarmDaoImpl implements ThVehicleAlarmDao{

	
	private static final Logger logger = Logger.getLogger(ThvehicleAlarmDaoImpl.class);
	/* (non-Javadoc)
	 * @see com.ctfo.dataanalysisservice.save.dao.ThVehicleAlarmDao#save(com.ctfo.dataanalysisservice.beans.ThVehicleAlarm)
	 */
	@Override
	public boolean save(List<ThVehicleAlarm> alarm) {
		...
		String sql="insert into TH_VEHICLE_ALARM(" +
				"ALARM_ID," + // 1 报警ID
				"VID," +      // 2 车辆ID
				"UTC," +      // 3 GPS报警时间UTC
				...
				"VEHICLE_NO," + // 17 车牌号
				"ALARM_TODO," + // 18  督办状态：0：未督办 
				"ALARM_SRC," + //  19  报警信息来源2：企业监控平
				"ALARM_ADD_INFO_START,MAPLAT,MAPLON)" + //  20  报警附加信息
				" values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
		try {
		   ...
		   /*conn.setAutoCommit(false);
			...
			for(ThVehicleAlarm bo : alarm)
			{
				...
				ps.setObject(10, System.currentTimeMillis());
				...
				ps.setObject(17, "");
				ps.setObject(18, 0);//督办状态：0：未督办   默认
				ps.setObject(19, 1);
				ps.setObject(20, bo.getAlarmAddInfoStart());
				ps.setObject(21, bo.getMaplat());
				ps.setObject(22, bo.getMaplon());
				logger.info(bo.getAlarmId()+"alarmsave addBatch");
				ps.addBatch();
			}
	...
	public boolean update(List<ThVehicleAlarm> alarm) {
		...
		String sql=" UPDATE TH_VEHICLE_ALARM " +
				"set " +
		...
		try 
		{
		   /*
			...
			for(ThVehicleAlarm bo : alarm)
			{
			 ...
			 ps.addBatch();
			 logger.info(bo.getAlarmId()+"alarmupdate addBatch");
			}
