...
//    	System.out.println(Base64.encode(s1));
    	...
    	String str = StringUtils.repeat("0", 32);
    	str = replace(str,0,'1');
    	str = replace(str,1,'1');
    ...
    /**
     * 反向替换字符串中的指定字符
     * @param soureceStr  原始字符串
     * @param index   需要替换的字符的位置
     * @param value   替换的字符
     * @return
     */
    public static String replace(String soureceStr,int index,char value){
    	char chr[] = soureceStr.toCharArray();
    	if (index>chr.length-1) return soureceStr;
    	chr[chr.length-1-index]=value;
