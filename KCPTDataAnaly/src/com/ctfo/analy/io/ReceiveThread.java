...
import com.ctfo.analy.util.SocketUtil;
...
public class ReceiveThread extends Thread {
	...
	public ReceiveThread(UserInfoBean userInfo) throws Exception {
	...
	private void connect() {
		socket = SocketUtil.createSocket(luserInfo.getMsgServiceAddr(),luserInfo.getMsgServicePort(), luserInfo.getReConnectTime());
...
//			logger.info(msginfo + " 30s接收：" + receivecount);
	...
	private void sendLoginData() {
		...
		String logindata = "LOGI " + luserInfo.getLogintype() + " "+ luserInfo.getUserid() + " " + luserInfo.getPassword()+ " \r\n";
		wirteData(logindata);
	}
			...
			try {
				if (in.ready()) {
					...
					if (!s.equals("NOOP_ACK") || !s.equals("LACK")) {
						...
						AnalyseServiceInit.getAnalyseServiceThread()[0].addPacket(message);
					}
					lastReceiveNoopTime = System.currentTimeMillis();
				...
				if (lastNoopTime - lastReceiveNoopTime >= luserInfo.getConnectStateTime()) {
					logger.info(msginfo + "长时间没收到数据重新连接");
					lastReceiveNoopTime = lastNoopTime = System.currentTimeMillis();
