...
public class JDCConnectionInit {
	...
	public  void init() throws ClassNotFoundException,
		...
		String oracledbPassword = config.getStringValue("database|JDBCPassword");// mysql数据库密码
		/*String mysqldbDriver = config.getStringValue("database|MysqlJDBCDriver");
		String mysqldbConString = config.getStringValue("database|MysqlJDBCUrl");
		String mysqldbUserName = config.getStringValue("database|MysqlJDBCUser");
		String mysqldbPassword = config.getStringValue("database|MysqlJDBCPassword");
		*/int timeout = config.getIntValue("database|DBReconnectWait");// 设置连接超时时间
		...
		// 初始MYSQL连接池
		//new JDCConnectionMysqlDriver(mysqldbDriver, mysqldbConString,mysqldbUserName, mysqldbPassword, connectionPoolSize, timeout,maxLimit);
