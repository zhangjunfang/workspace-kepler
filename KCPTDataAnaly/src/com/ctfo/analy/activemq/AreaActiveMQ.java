 ...
 * <li>文件名称：AreaActiveMQ.java </li><br>
 * <li>时        间：2013-5-9  下午12:54:04	</li><br>
 * <li>描        述：区域频道			</li><br>
 *****************************************/
public class AreaActiveMQ extends CreateActiveMQ {
	private static final Logger logger = Logger.getLogger(AreaActiveMQ.class);
	...
	public AreaActiveMQ(String mqUrl, String queueName, XmlConf config) {
		super.mqUrl = mqUrl;
		...
		try {
			...
			while (true) {
				...
				try {
					...
					logger.error("接收、处理基础数据更新通知消息异常:" + ex);
					destination = null;
					consumer.close();
					if (session!=null){
						session = null;
	...
	private void processMsg(String msg) {
		...
		if(msgStr.length == 3){
			...
			String name = "a.area_id";
			String value = (msgStr[2].split(":"))[1];
			//通知与区域相关的缓存更新
			if(type.equals("ADD")){
				//区域报警设置
				cacheUpdateDBAdapter.areaAlarmUpdate(name, value);
			//批量导入  全量导入通知与区域相关的缓存更新 
