...
DST_CONF="${LBS_HOME}/dataanaly/DataAnaly.xml"
CONSOLE_CONF=" -Djava.rmi.server.hostname=10.8.3.163 -Dcom.sun.management.jmxremote.port=9990 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false "
...
unset CLASSPATH
for i in ${LBS_HOME}/dataanaly/*.jar ; do
  if [ "$CLASSPATH" != "" ]; then
...
CLASSPATH=${CLASSPATH}:${LBS_HOME}/dataanaly/java/classes
...
case "$1" in
	...
	debug)
		...
		;;
	*)
		echo "Usage: $0 {start|stop|restart|status|verify|convert|frcnv|manual}"
esac
