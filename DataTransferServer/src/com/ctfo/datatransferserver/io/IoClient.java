 ...
 * </ul>
 *****************************************/
package com.ctfo.datatransferserver.io;
...
public class IoClient {
	 ...
	 * @throws Exception  
	 */
	public IoClient(XmlConf conf) throws Exception { 
		handler = new IoHandler();
		host = conf.getStringValue("msgServiceManage|msgServiceAddr");
		port = Integer.parseInt(conf.getStringValue("msgServiceManage|msgServicePort"));
		String userName = conf.getStringValue("msgServiceManage|userid");
		String password = conf.getStringValue("msgServiceManage|password");
		if(host == null || port == 0 || userName == null || password == null){
			throw new Exception("通讯登录信息错误");
		}
...
//		断线重连回调过滤器
		...
		connector.getSessionConfig().setIdleTime(IdleStatus.WRITER_IDLE, 30);
		// 设置默认访问地址
		...
		for (;;) {
			try {
				...
				// 获取会话
				session = future.getSession();
				System.out.println("连接服务端" + host + ":" + port + "[成功]" + ",,时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
				...
				Thread.sleep(5000);// 连接失败后,重连,间隔5s
			}
		}
	}
