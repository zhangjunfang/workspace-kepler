...
public class OracleConnectionPool {
	...
	private final static String CACHE_NAME = "KCPsT"; // 新建的线程池名，也可以说是标识符

	public static void initConnection(String url, String user, String password, String minLimit, String maxLimit, String InitialLimit, String connectionWaitTimeout,
			String inactivityTimeout, String abandonedConnectionTimeout, String propertyCheckInterval) {
		try {
			...
			// cacheProperties.setProperty("AbandonedConnectionTimeout",
			// abandonedConnectionTimeout);
			// cacheProperties.setProperty("InactivityTimeout",
			// inactivityTimeout); // Sets the maximum time a physical
			// connection can remain idle in a connection cache.
			cacheProperties.setProperty("PropertyCheckInterval", propertyCheckInterval); // Sets
																							// the
																							// time
																							// interval
																							// at
			...
			logger.error("创建ORACLE连接缓存池出错。", e);
		}
	}
	...
	public static void listCacheInfos() throws SQLException {
		...
		OracleConnectionCacheManager oracleConnectionCacheManager = OracleConnectionCacheManager.getConnectionCacheManagerInstance();
		if (oracleConnectionCacheManager.existsCache(CACHE_NAME)) {
			logger.info(oracleConnectionCacheManager.getNumberOfAvailableConnections(CACHE_NAME) + " connections are available in cache " + CACHE_NAME);
			logger.info(oracleConnectionCacheManager.getNumberOfActiveConnections(CACHE_NAME) + " connections are active");
		}
	}
}
