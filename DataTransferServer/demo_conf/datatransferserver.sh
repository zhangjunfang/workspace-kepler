...
CLASS_HOME=""
SERVICE_CLASS="com.ctfo.datatransferserver.DataTransferMain"
CFG_CLASS="com.lingtu.mcc.traceagent.xconf.SubmitCfg"
SRC_XCONF="${XCONF_HOME}/components/TraceAgent/TraceAgent.xconf"
DST_CONF="${LBS_HOME}/datatransferserver/datatransferserver.xml"
...
unset CLASSPATH
for i in ${LBS_HOME}/datatransferserver/*.jar ; do
  if [ "$CLASSPATH" != "" ]; then
...
case "$1" in
	start)
		if [ `whoami` = "lbs" ]; then
			${JAVACMD} -Xmx1512m -cp $CLASSPATH ${SERVICE_CLASS} -f ${DST_CONF} -start &
		fi
		;;
	stop)
		if [ `whoami` = "lbs" ]; then
