 ...
 * 
 */
public class GradeStatisticTask extends TaskAdapter{
	private static final Logger logger = LoggerFactory.getLogger(GradeStatisticTask.class);

	// -------声明Connection
	private Connection dbConnection;
	
	private String queryLastMonth = "";
...
*/
	...
	public void execute() throws SQLException {
		...
		if(ret == 1){
			logger.info("--------------------【运营考核】任务成功！考核月：【{}】,耗时：【{}】s",queryLastMonth,(System.currentTimeMillis()-start)/1000);
		}else if(ret == 0){
			logger.info("--------------------【运营考核】任务失败！考核月：【{}】,耗时：【{}】s",queryLastMonth,(System.currentTimeMillis()-start)/1000);
		}
	}
	...
	public void init(){
		...
		queryCheckMonthSetSql = ConfigLoader.config.get("sql_queryCheckMonthSetSql");

		// 删除企业统计月的车辆评分结果表TS_GRADE_MONTHSTAT
		delGradeMonthStatSql = ConfigLoader.config.get("sql_delGradeMonthStatSql");

		// 查询车辆日统计信息TS_VEHICLE_DAYSTAT表数据
		queryVehicleDayStatSql = ConfigLoader.config.get("sql_queryVehicleDayStatSql");

		// 插入数据车辆评分结果表TS_GRADE_MONTHSTAT表
		insertGradeMonthStatSql = ConfigLoader.config.get("sql_insertGradeMonthStatSql");
		
		// 统计企业月度考核
		accountGradeMonthCorpSql = ConfigLoader.config.get("accountGradeMonthCorpSql");
		
		 // 统计车队月度考核
		accountGradeMonthTeamSql = ConfigLoader.config.get("accountGradeMonthTeamSql");
	...
	public int executeGradeMonthStat() {
		...
		try {			
			if("restore".equals(this.type)){
				String restoreTime = "";
				System.out.print("--------------------【运营考核】输入您需要补跑的日期(yyyy/mm)：");
				while(true){
					BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
					restoreTime = br.readLine();
					if (!restoreTime.matches("\\d{4}/\\d{2}")) {
						System.out.print("--------------------【运营考核】输入错误,请重新选择输入：");
						continue;
					}else break;
				}
				queryLastMonth = restoreTime.replace("/", "-");
				dependFlag = false; //重跑关闭依赖
			}else if("start".equals(this.type)){
				// 获得上月月份 格式"YYYY-MM"
				queryLastMonth =DateTools.lastMonth();
				dependFlag = true;
				isTimeRun();
			}
			
			logger.info("--------------------【运营考核】任务开始！-------------------------");
			// 获得系统当前日期 格式 "YYYY-MM-DD"
			long sysCurrentDay=DateTools.getYearMonthDayUtc();
			// ///////模拟数据为2011-11
...
//				l_startDate=dbResultSet.getLong(4);
	...
	private boolean deleteGradeMonthCorp(String queryLastMonth){
		PreparedStatement dbPstmt = null;
		try {
			dbPstmt = dbConnection.prepareCall(ConfigLoader.config.get("sql_deleteGradeCorp"));
			dbPstmt.setString(1, queryLastMonth);
	...
	private boolean deleteGradeMonthTeam(String queryLastMonth){
		PreparedStatement dbPstmt = null;
		try {
			dbPstmt = dbConnection.prepareCall(ConfigLoader.config.get("sql_deleteGradeTeam"));
			dbPstmt.setString(1, queryLastMonth);
			dbPstmt.executeUpdate();
			return true;
		}catch(Exception ex){
