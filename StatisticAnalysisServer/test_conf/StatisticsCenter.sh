...
CONSOLE_CONF=" -Djava.rmi.server.hostname=10.8.3.163 -Dcom.sun.management.jmxremote.port=9992 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false "
...
case "$1" in
	start)
		if [ `whoami` = "root" ]; then
			${JAVACMD} ${CONSOLE_CONF} -Xmx3512m -cp $CLASSPATH ${SERVICE_CLASS} -f ${DST_CONF} -start &
		fi
		;;
	stop)
		if [ `whoami` = "root" ]; then
	...
	debug)
		...
		;;
	*)
		echo "Usage: $0 {start|stop|restart|manual|status|verify|convert|frcnv|version|debug}"
esac
