...
if [ "$LBS_HOME" = "" ]; then
	LBS_HOME="/home/kcpt/supp_app/statisticscenter"
fi

if [ "$XCONF_HOME" = "" ]; then
	XCONF_HOME="/home/kcpt/supp_app/tomcat/webapps/Xconf"
fi
...
case "$1" in
	start)
		if [ `whoami` = "lbs" ]; then
			${JAVACMD} ${LOGBACK} -Xmx3024m -Xms3024m -Xmn784m -XX:PermSize=64m -XX:MaxPermSize=256m -XX:+UseConcMarkSweepGC -XX:+UseCMSCompactAtFullCollection -XX:CMSMaxAbortablePrecleanTime=50 -XX:+CMSClassUnloadingEnabled -cp $CLASSPATH ${SERVICE_CLASS} -f ${DST_CONF} -start &
		fi
		;;
	stop)
		if [ `whoami` = "kcpt" ]; then
	...
	debug)
		...
		;;
	*)
		echo "Usage: $0 {start|stop|restart|manual|status|verify|convert|frcnv}"
esac
