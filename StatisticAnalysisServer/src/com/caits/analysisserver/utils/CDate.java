...
public class CDate {
	private static Logger log = LoggerFactory.getLogger(CDate.class);
	...
	public static String  getMainTainDate(String mainTainDate,int intervalDays,int warnDays){
		String r = "";
		try {
			Date date=CDate.changeDateFormat("yyyy-MM-dd", mainTainDate);
			Calendar cal = Calendar.getInstance();
	...
	public static String addDays(String addDate,int days){
		...
		try {
			Date date=CDate.changeDateFormat("yyyy-MM-dd", addDate);
			Calendar cal = Calendar.getInstance();
			cal.setTime(date);	
...
//		Long a1 = stringConvertUtc("20120220/235939");
		...
		Date dt = CDate.strToDateByFormat("20121229", "yyyyMMdd");//转换成当天中午12点时间
		int week = CDate.getDaysWeek(dt);
		System.out.println(week + "--" +CDate.getPreviousWeek());
...
//		System.out.println(CDate.checkSecTime(2*60*60*1000, 5*60*60*1000, 1332359003000l));
		System.out.println("统计数据开始日期"+CDate.utc2Str(a, "yyyyMMdd")+"告警表分区名称："+"TH_VEHICLE_ALARM"+CDate.utc2Str(a + 24 * 60 * 60 * 1000, "yyyyMMdd"));
	}
