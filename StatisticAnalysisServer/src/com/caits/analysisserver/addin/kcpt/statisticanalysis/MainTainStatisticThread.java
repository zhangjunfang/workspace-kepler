...
import com.caits.analysisserver.bean.MainTainStatistic;
...
public class MainTainStatisticThread {
	private static final Logger logger = LoggerFactory.getLogger(MainTainStatisticThread.class);
...
//	private String queryMainTainPlanSql;
	...
	public void initAnalyser() throws Exception {
		...
		// 查询维保计划表
//		queryMainTainPlanSql = SQLPool.getinstance().getSql("sql_queryMainTainPlanSql");
		// 查询维保明细表
		queryMainTainDetailSql = SQLPool.getinstance().getSql("sql_queryMainTainDetailSql");
		
		// 更新MainTain明细表 MILEAGE_MESS字段 TIME_MESS字段
		updateMainTainDetailSql = SQLPool.getinstance().getSql("sql_updateMainTainDetailSql");
		
		// 生成一条记录 插入MainTain明细表
		createNewMainTainDataSql = SQLPool.getinstance().getSql("sql_createNewMainTainDataSql");
		
		//更新MainTain明细表MAINTAIN_ONTIME_STAT字段
		updateMainTainDetailStatSql = SQLPool.getinstance().getSql("sql_updateMainTainDetailStatSql");
		
		//查询车辆已维保次数
		queryVehicleMaintainTimesSql = SQLPool.getinstance().getSql("sql_queryVehicleMaintainTimesSql");
		
		//查询车辆最新维护项目编号
		queryVehicleMaintainNumSql = SQLPool.getinstance().getSql("sql_queryVehicleMaintainNumSql");
		
		//查询维保项目简称
		queryMaintainClassSql = SQLPool.getinstance().getSql("sql_queryMaintainClassSql");

		//查询要扩展的维保明细信息
		queryExtendMaintainDetailSql = SQLPool.getinstance().getSql("sql_queryExtendMaintainDetailSql");
		
		//保存维保明细数据
		saveMaintainDetailSql = SQLPool.getinstance().getSql("sql_saveMaintainDetailSql");
	}
...
//		boolean mesFlag = false;
	...
	private boolean isTimeWarn(MainTainStatistic mainTainStatistic){
		String currentDate = CDate.getStringDateShort();
		return CDate.compareDoubleDate(CDate.addDays(currentDate, mainTainStatistic.getWarnDays()),mainTainStatistic.getPlanMaintainDate());
	}
	
	//根据时间判断是否已延期
	private boolean isTimeMainTainNotOnTime(MainTainStatistic mainTainStatistic){
		return CDate.compareDoubleDate(CDate.getStringDateShort(),mainTainStatistic.getPlanMaintainDate());
	}
	...
	public int createNewMainTainData(MainTainStatistic mainTainStatistic) {
		...
		try {
			...
			try {
				...
				DBAdapter.close(dbPstmt);
				DBAdapter.close(dbPstmt0);
				DBAdapter.close(dbPstmt1);
				DBAdapter.close(dbPstmt3);
				DBAdapter.close(dbPstmt4);
				
			} catch (SQLException ex) {
				// TODO Auto-generated catch block
				logger.error("连接放回连接池过程中出错",ex);
			}
		}
		return flag;
	}
	...
	public String getPlanMainTainTime(MainTainStatistic mainTainStatistic){
		...
		if(mainTainStatistic.getIntervalDays()!=0){
			planMainTainTime = CDate.addDays(mainTainStatistic.getMainTainDate(),mainTainStatistic.getIntervalDays());
		}
	...
	public static void main(String[] args) {
		MainTainStatisticThread mtst = new MainTainStatisticThread();
//		 System.out.println(mtst.getLoopMileageWarnMessage(900,500,100,500,8));
		try {
			mtst.initAnalyser();
		} catch (Exception e) {
			// TODO Auto-generated catch block
