 ...
 * @since JDK1.6
 */
public class StatFactoryMonthsJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(StatFactoryMonthsJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statFactoryMonthsStatSql;// 车厂系统指标月分析
...
//	private long statDate;
	 ...
	 *            当日12点日期时间
	 */
	public StatFactoryMonthsJobdetail(int year,int month) {
		this.year = year;
		this.month = month;
	...
	public void initAnalyser() {
		statFactoryMonthsStatSql = SQLPool.getinstance().getSql(
		"sql_procStatFactoryMonths");
	}
	...
	public int executeStatRecorder() {
		...
		try {
			...
			if (dbConnection != null) {
				...
				// 按企业告警级别统计
				dbPstmt0 = dbConnection.prepareCall(statFactoryMonthsStatSql);
				...
				if (successtag == 1) {
					logger.debug(year+"/"+month + "车厂系统指标月统计成功！");					
				}else{
					logger.error(year+"/"+month + "车厂系统指标月统计出错！");
				}
