 ...
 * @since JDK1.6
 */
public class StatAlarmMonthJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(StatAlarmMonthJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statAlarmMonthsStatSql;// 删除企业日车辆告警情况
...
//	private int count = 0;// 计数器
	 ...
	 *            当日12点日期时间
	 */
	public StatAlarmMonthJobdetail(int year,int month) {
	...
	public void initAnalyser() {
		// 删除企业日车辆运营情况
		statAlarmMonthsStatSql = SQLPool.getinstance().getSql(
				"sql_procStatAlarmMonths");
	...
	public int executeStatRecorder() {
		...
		try {
			...
			if (dbConnection != null) {
				...
				// 按企业告警级别进行月统计
				dbPstmt0 = dbConnection.prepareCall(statAlarmMonthsStatSql);
				...
				if (successtag == 1) {
					logger.debug(year + "年度第"+month+"月 告警按企业告警级别月统计成功！");
				}else{
					logger.error(year+ "年度第"+month+"月 告警按企业告警级别按月统计出错！");
				}
			...
			logger.error("告警按企业告警级别按月统计出错：", e);
			flag = 0;
		} finally {
			try {
				if (dbPstmt0 != null) {
