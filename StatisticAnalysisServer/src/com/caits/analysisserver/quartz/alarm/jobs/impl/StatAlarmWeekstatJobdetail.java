 ...
 * @since JDK1.6
 */
public class StatAlarmWeekstatJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(StatAlarmWeekstatJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statAlarmWeeksStatSql;// 删除企业月车辆运营情况
...
//	private int count = 0;// 计数器
	 ...
	 *            当日12点日期时间
	 */
	public StatAlarmWeekstatJobdetail(int year,int weekOfYear) {
		this.year=year;
	...
	public void initAnalyser() {
		// 删除企业月车辆运营情况
		statAlarmWeeksStatSql = SQLPool.getinstance().getSql(
				"sql_procStatAlarmWeeks");
	...
	public int executeStatRecorder() {
		...
		try {
			...
			if (dbConnection != null) {
				...
				// 按企业告警级别进行周统计
				dbPstmt1 = dbConnection.prepareCall(statAlarmWeeksStatSql);
				...
				if (weeksuccesstag == 1) {
					logger.debug(year + "年度第"+week+"周 告警按企业告警级别周统计成功！");
				}else{
					logger.error(year+ "年度第"+week+"周 告警按企业告警级别按周统计出错！");
				}
