 ...
 * @since JDK1.6
 */
public class VehicleOilWearAnalyseDaystatJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(VehicleOilWearAnalyseDaystatJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String delvehicleoilwearanalysedaystatSql;// 删除车辆日油耗分析情况
	private String addvehicleoilwearanalysedaystatSql;// 统计车辆日油耗分析情况
...
//	private int count = 0;// 计数器
	 ...
	 *            当日12点日期时间
	 */
	public VehicleOilWearAnalyseDaystatJobdetail(Date currDay) {
	...
	public void initAnalyser() {
		// 删除企业日车辆运营情况
		delvehicleoilwearanalysedaystatSql = SQLPool.getinstance().getSql(
				"sql_delvehicleoilwearanalysedaystatSql");
		// 统计企业日车辆运营情况
		addvehicleoilwearanalysedaystatSql = SQLPool.getinstance().getSql(
				"sql_addvehicleoilwearanalysedaystatSql");
		...
		try {
			...
			if (dbConnection != null) {
				...
				dbPstmt1 = dbConnection
						.prepareStatement(delvehicleoilwearanalysedaystatSql);
				...
				dbPstmt2 = dbConnection
						.prepareStatement(addvehicleoilwearanalysedaystatSql);
				...
				logger.debug(CDate.dateToStr(dt) + "企业车辆日油耗统计信息生成成功！");
				flag = 1;
			} else {
				logger.debug("获取数据库链接失败");
			}
		} catch (Exception e) {
			logger.error("生成企业车辆日油耗信息出错：", e);
			flag = 0;
		} finally {
			try {
				if (dbResultSet != null) {
