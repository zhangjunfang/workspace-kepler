 ...
 * @since JDK1.6
 */
public class OrgOperationMonthstatJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(OrgOperationMonthstatJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String delOrgOpterationMonthstatSql;// 删除企业月车辆运营情况
	private String queryOrgOperationMonthstatSql;// 统计企业月车辆运营情况
	private String saveOrgOpterationMonthstatSql; // 保存企业月车辆运营情况
	 ...
	 *            当日12点日期时间
	 */
	public OrgOperationMonthstatJobdetail(int year,int month) {
	...
	public void initAnalyser() {
		// 删除企业月车辆运营情况
		delOrgOpterationMonthstatSql = SQLPool.getinstance().getSql(
				"sql_delOrgOpterationMonthstatSql");
		// 统计企业月车辆运营情况
		queryOrgOperationMonthstatSql = SQLPool.getinstance().getSql(
				"sql_queryOrgOperationMonthstatSql");
		// 保存企业月车辆运营情况
		saveOrgOpterationMonthstatSql = SQLPool.getinstance().getSql(
				"sql_saveOrgOpterationMonthstatSql");
		...
		try {
			...
			if (dbConnection != null) {
				...
				dbPstmt1 = dbConnection
						.prepareStatement(delOrgOpterationMonthstatSql);
				dbPstmt1.setLong(1, beginTime);
				dbPstmt1.executeUpdate();

				// 查询当月企业车辆运营情况
				dbPstmt2 = dbConnection
						.prepareStatement(queryOrgOperationMonthstatSql);
				...
				while (dbResultSet.next()) {
					...
					dbPstmt3 = dbConnection
							.prepareStatement(saveOrgOpterationMonthstatSql);
