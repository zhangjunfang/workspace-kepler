 ...
 * @since JDK1.6
 */
public class OilConsumeYearstatJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(OilConsumeYearstatJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statOilConsumeYearstatSql; // 保存企业日车辆告警情况
...
//	private int count = 0;// 计数器
	 ...
	 *            当日12点日期时间
	 */
	public OilConsumeYearstatJobdetail(int year) {
	...
	public void initAnalyser() {
		// 调用燃油消耗月统计存储过程
		statOilConsumeYearstatSql = SQLPool.getinstance().getSql(
				"sql_procOilConsumeYearStat");
		...
		try {
			...
			if (dbConnection != null) {
				...
				dbPstmt1 = dbConnection
						.prepareCall(statOilConsumeYearstatSql);
				dbPstmt1.setString(1, statYear);
				...
				if (successtag == 1) {
					logger.debug(statYear + "车辆燃油消耗年统计信息生成成功！");
				}else{
					logger.error(statYear+ "车辆燃油消耗年统计统计信息生成出错！");
				}
				flag = 1;
			} else {
			...
			logger.error("生成车辆燃油消耗年统计信息出错：", e);
			flag = 0;
		} finally {
			try {
				/*if (dbResultSet != null) {
