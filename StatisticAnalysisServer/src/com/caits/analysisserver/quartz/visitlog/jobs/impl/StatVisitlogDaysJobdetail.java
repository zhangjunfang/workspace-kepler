 ...
 * @since JDK1.6
 */
public class StatVisitlogDaysJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(StatVisitlogDaysJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statVisitlogDaysStatSql;// 访问记录按小时分析
	 ...
	 *            当日12点日期时间
	 */
	public StatVisitlogDaysJobdetail(Date currDay) {
	...
	public void initAnalyser() {
		// 按小时分析访问记录
		statVisitlogDaysStatSql = SQLPool.getinstance().getSql(
				"sql_procStatVisitlogDaysStat");
	...
	public int executeStatRecorder() {
		...
		try {
			...
			if (dbConnection != null) {
				...
				// 按小时统计访问记录
				dbPstmt0 = dbConnection.prepareCall(statVisitlogDaysStatSql);
				...
				if (successtag == 1) {
					logger.debug(dateStr + "访问记录按日统计成功！");
				}else{
					logger.error(dateStr+ "访问记录按日统计出错！错误代码:" + successtag);
				}
