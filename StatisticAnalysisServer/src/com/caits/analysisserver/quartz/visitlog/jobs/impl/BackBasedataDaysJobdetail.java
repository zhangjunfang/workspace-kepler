 ...
 * @since JDK1.6
 */
public class BackBasedataDaysJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(BackBasedataDaysJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statBackBasedataDaysStatSql;// 访问记录按小时分析
	 ...
	 *            当日12点日期时间
	 */
	public BackBasedataDaysJobdetail(Date currDay) {
	...
	public void initAnalyser() {
		// 按小时分析访问记录
		statBackBasedataDaysStatSql = SQLPool.getinstance().getSql(
				"sql_procBackBasedataDaysStat");
	...
	public int executeStatRecorder() {
		...
		try {
			...
			if (dbConnection != null) {
				...
				// 按小时统计访问记录
				dbPstmt0 = dbConnection.prepareCall(statBackBasedataDaysStatSql);
				...
				if (successtag == 1) {
					logger.debug(dateStr + "基础数据按日备份成功！");
				}else{
					logger.error(dateStr+ "基础数据按日备份出错！错误代码:" + successtag);
				}
