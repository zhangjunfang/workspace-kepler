 ...
 * @since JDK1.6
 */
public class StatBasedataDaysJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(StatBasedataDaysJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statBasedataDaysStatSql;// 访问记录按小时分析
	 ...
	 *            当日12点日期时间
	 */
	public StatBasedataDaysJobdetail(Date currDay) {
	...
	public void initAnalyser() {
		// 按小时分析访问记录
		statBasedataDaysStatSql = SQLPool.getinstance().getSql(
				"sql_procStatBasedataDaysStat");
	...
	public int executeStatRecorder() {
		...
		try {
			...
			if (dbConnection != null) {
				...
				// 按小时统计访问记录
				dbPstmt0 = dbConnection.prepareCall(statBasedataDaysStatSql);
				...
				if (successtag == 1) {
					logger.debug(dateStr + "基础数据按日统计成功！");
				}else{
					logger.error(dateStr+ "基础数据按日统计出错！错误代码:" + successtag);
				}
