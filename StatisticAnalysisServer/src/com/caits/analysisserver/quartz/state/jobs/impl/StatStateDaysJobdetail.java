package com.caits.analysisserver.quartz.state.jobs.impl;
 ...
 * @since JDK1.6
 */
public class StatStateDaysJobdetail {

	private static final Logger logger = LoggerFactory
			.getLogger(StatStateDaysJobdetail.class);

	// ------获得xml拼接的Sql语句
	private String statStateDaysStatSql;// 访问记录按小时分析
	 ...
	 *            当日12点日期时间
	 */
	public StatStateDaysJobdetail(Date currDay) {
	...
	public void initAnalyser() {
		// 企业按告警类别统计
		statStateDaysStatSql = SQLPool.getinstance().getSql(
				"sql_procStatStateDays");
	...
	public int executeStatRecorder() {
		...
		try {
			...
			if (dbConnection != null) {
				...
				// 按企业告警级别统计
				dbPstmt0 = dbConnection.prepareCall(statStateDaysStatSql);
				...
				if (successtag == 1) {
					logger.debug(dateStr + "车辆状态信息按日统计成功！");
				}else{
					logger.error(dateStr+ "车辆状态信息按日统计出错！");
				}
