	...
	public IoClient(MsgProperties properties) throws Exception {
		try {
			if(properties == null || properties.getMsgHost() == null || properties.getMsgPort() == 0){
				throw new Exception("通讯客户端启动异常:启动参数错误!");
			}
			msgProperties = properties;
			
			commandParseThread = new CommandParseThread();
			commandParseThread.start(); 
		} catch (Exception e) {
			throw new Exception("通讯客户端启动异常:" + e.getMessage(), e);
		}
	}
	/*****************************************
	 * <li>描        述：启动IO连接 		</li><br>
...
//		factory.setDecoderMaxLineLength(1064960);// 10k
//		factory.setEncoderMaxLineLength(1064960); // 10k
//		connector.getFilterChain().addLast("codec", new ProtocolCodecFilter(factory));
//		// 设置接收缓冲区的大小
//		connector.getSessionConfig().setReceiveBufferSize(1064960);// 10k
//		// 设置输出缓冲区的大小
//		connector.getSessionConfig().setSendBufferSize(1064960);// 10k
//		// 读写通道10s内无操作,则进入空闲状
...
//		断线重连回调过滤器
		...
		connector.getFilterChain().addLast("mdc", new MdcInjectionFilter());
		TextLineCodecFactory factory = new TextLineCodecFactory(Charset.forName(msgProperties.getMsgEncoding()), LineDelimiter.WINDOWS.getValue(), LineDelimiter.WINDOWS.getValue());
		factory.setDecoderMaxLineLength(1064960);
		factory.setEncoderMaxLineLength(1064960);
		connector.getFilterChain().addLast("codec", new ProtocolCodecFilter(factory));
		// 设置接收缓冲区的大小
		connector.getSessionConfig().setReceiveBufferSize(msgProperties.getReceiveBufferSize());
		// 设置输出缓冲区的大小
		connector.getSessionConfig().setSendBufferSize(msgProperties.getSendBufferSize());
		// 读写通道无操作,则进入空闲状
		...
		handler = new IoHandler(msgProperties, commandParseThread);
